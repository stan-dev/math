...
// NOTE: This constant is made as a macro because some compilers (gcc 4.4 for
// one) don't allow enums or namespace scope constants in alignment attributes
...
enum {
  padding_size = (sizeof(lock_type) <= BOOST_ATOMIC_CACHE_LINE_SIZE
                      ...
                      : (BOOST_ATOMIC_CACHE_LINE_SIZE
                         - sizeof(lock_type) % BOOST_ATOMIC_CACHE_LINE_SIZE))
};
...
// NOTE: This function must NOT be inline. Otherwise MSVC 9 will sometimes
// generate broken code for modulus operation which result in crashes.
BOOST_ATOMIC_DECL lockpool::scoped_lock::scoped_lock(const volatile void* addr)
    BOOST_NOEXCEPT
    : m_lock(&g_lock_pool[reinterpret_cast<std::size_t>(addr)
                          % (sizeof(g_lock_pool) / sizeof(*g_lock_pool))]
                  .lock) {
  while (lock_operations::test_and_set(*static_cast<lock_type*>(m_lock),
                                       memory_order_acquire)) {
    do {
      atomics::detail::pause();
    } while (!!lock_operations::load(*static_cast<lock_type*>(m_lock),
                                     memory_order_relaxed));
  }
}

BOOST_ATOMIC_DECL lockpool::scoped_lock::~scoped_lock() BOOST_NOEXCEPT {
  lock_operations::clear(*static_cast<lock_type*>(m_lock),
...
BOOST_ATOMIC_DECL lockpool::scoped_lock::~scoped_lock() BOOST_NOEXCEPT {
  BOOST_VERIFY(pthread_mutex_unlock(static_cast<pthread_mutex_t*>(m_lock))
               == 0);
}
...
// This function is intentionally non-inline, even if empty. This forces the
// compiler to treat its call as a compiler barrier.
