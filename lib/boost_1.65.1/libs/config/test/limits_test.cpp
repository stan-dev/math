...
template <typename T>
inline T make_char_numeric_for_streaming(T x) {
  return x;
}
namespace fix {
...
inline int make_char_numeric_for_streaming(unsigned char c) { return c; }
    ...
    && defined(BOOST_HAS_LONG_LONG) && !defined(_GLIBCPP_USE_LONG_LONG) \
...
void test_integral_limits(const T&, const char* msg) {
  ...
  std::cout << "Testing " << msg << " (size " << sizeof(T) << ")"
...
void test_float_limits(const T&, const char* msg) {
  ...
  std::cout << "IEEE-compatible: " << lim::is_iec559
            << ", traps: " << lim::traps << ", bounded: " << lim::is_bounded
            << ", exact: " << lim::is_exact << '\n'
