...
// This MSVC-specific cpp file implements non-intrusive cloning of exception
// objects.  Based on an exception_ptr implementation by Anthony Williams.
...
#if defined(BOOST_ENABLE_NON_INTRUSIVE_EXCEPTION_PTR) && defined(_MSC_VER) \
    && (defined(_M_IX86) || defined(_M_X64))

// Non-intrusive cloning support implemented below, only for MSVC versions
// mentioned above.  Thanks Anthony Williams!  Thanks to Martin Weiss for
// implementing 64-bit support!
...
typedef int (dummy_exception_type::*copy_constructor_with_virtual_base_ptr)(
    void *src, void *dst);
...
union cpp_destructor {
  void *address;
  destructor_ptr destructor;
};

enum cpp_type_flags { class_is_simple_type = 1, class_has_virtual_base = 4 };

// ATTENTION: On x86 fields such as type_info and copy_constructor are really
// pointers but on 64bit these are 32bit offsets from HINSTANCE. Hints on the
// 64bit handling from
...
struct exception_object_deleter {
  cpp_exception_type const &et_;
  size_t image_base_;

  exception_object_deleter(cpp_exception_type const &et, size_t image_base)
      : et_(et), image_base_(image_base) {}

  void operator()(void *obj) {
    BOOST_ASSERT(obj != 0);
    dummy_exception_type *dummy_exception_ptr
        = static_cast<dummy_exception_type *>(obj);
    if (et_.destructor) {
      cpp_destructor destructor;
      destructor.address
          = reinterpret_cast<void *>(et_.destructor + image_base_);
...
cpp_type_info const &get_cpp_type_info(cpp_exception_type const &et,
                                       size_t image_base) {
  cpp_type_info_table *const typearray
      = reinterpret_cast<cpp_type_info_table *const>(et.type_info_table
                                                     + image_base);
  cpp_type_info *const ti
      = reinterpret_cast<cpp_type_info *const>(typearray->info + image_base);
  BOOST_ASSERT(ti != 0);
  return *ti;
}

void copy_msvc_exception(void *dst, void *src, cpp_type_info const &ti,
                         size_t image_base) {
  cpp_copy_constructor copy_constructor;
  copy_constructor.address
      = reinterpret_cast<void *>(ti.copy_constructor + image_base);

  if (!(ti.flags & class_is_simple_type)
      && copy_constructor.normal_copy_constructor) {
    dummy_exception_type *dummy_exception_ptr
        = static_cast<dummy_exception_type *>(dst);
    if (ti.flags & class_has_virtual_base)
      (dummy_exception_ptr
           ->*(copy_constructor.copy_constructor_with_virtual_base))(src, dst);
    else
...
boost::shared_ptr<void> clone_msvc_exception(void *src,
                                             cpp_exception_type const &et,
                                             size_t image_base) {
  BOOST_ASSERT(src != 0);
  ...
  if (void *dst = malloc(ti.size)) {
    ...
    return boost::shared_ptr<void>(dst,
                                   exception_object_deleter(et, image_base));
  } else
    throw std::bad_alloc();
}

class cloned_exception : public boost::exception_detail::clone_base {
 ...
 public:
  cloned_exception(EXCEPTION_RECORD const *record)
      : et_(*reinterpret_cast<cpp_exception_type const *>(
            record->ExceptionInformation[2])),
        image_base_((cpp_exception_parameter_count == 4)
                        ? record->ExceptionInformation[3]
                        : 0),
        exc_(clone_msvc_exception(
            reinterpret_cast<void *>(record->ExceptionInformation[1]), et_,
            image_base_)) {}
  ...
  boost::exception_detail::clone_base const *clone() const {
  ...
  void rethrow() const {
    ...
    RaiseException(cpp_exception_code, EXCEPTION_NONCONTINUABLE,
                   cpp_exception_parameter_count, args);
  }
};

bool is_cpp_exception(EXCEPTION_RECORD const *record) {
  return record && (record->ExceptionCode == cpp_exception_code)
         && (record->NumberParameters == cpp_exception_parameter_count)
         && (record->ExceptionInformation[0] == cpp_exception_magic_flag);
}

unsigned long exception_cloning_filter(
    int &result, boost::exception_detail::clone_base const *&ptr, void *info_) {
  BOOST_ASSERT(exception_info_offset >= 0);
  BOOST_ASSERT(info_ != 0);
  EXCEPTION_RECORD *record
      = static_cast<EXCEPTION_POINTERS *>(info_)->ExceptionRecord;
  if (is_cpp_exception(record)) {
    if (!record->ExceptionInformation[2])
      record = *reinterpret_cast<EXCEPTION_RECORD **>(
    ...
    if (is_cpp_exception(record) && record->ExceptionInformation[2])
      try {
        ...
        result = boost::exception_detail::clone_current_exception_result::
            bad_alloc;
      } catch (...) {
        result = boost::exception_detail::clone_current_exception_result::
            bad_exception;
  ...
  if (exception_info_offset >= 0) {
    ...
    __try {
      throw;
        ...
        exception_cloning_filter(result, ptr, GetExceptionInformation())) {
    }
...
// On such platforms, only the intrusive enable_current_exception() cloning will
// work.

namespace boost {
namespace exception_detail {
int clone_current_exception_non_intrusive(clone_base const*&) {
