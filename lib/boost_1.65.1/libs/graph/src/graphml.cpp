...
class graphml_reader {
  ...
  void run(std::istream& in, size_t desired_idx) {
    ...
    BOOST_FOREACH (const ptree* gr, graphs) {
      ...
      BOOST_FOREACH (const ptree::value_type& edge, *gr) {
        ...
        bool is_directed = (local_directed == "" ? default_directed
                                                 : local_directed == "true");
 ...
 private:
  ...
  void handle_edge_property(const std::string& key_id, size_t descriptor,
                            const std::string& value) {
    m_g.set_edge_property(m_key_name[key_id], m_edge[descriptor], value,
                          m_key_type[key_id]);
  }
...
namespace boost {
void BOOST_GRAPH_DECL read_graphml(std::istream& in, mutate_graph& g,
                                   size_t desired_idx) {
