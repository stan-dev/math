...
struct token {
  ...
  token_type type;
  std::string normalized_value;  // May have double-quotes removed and/or some
                                 // escapes replaced
...
bad_graphviz_syntax parse_error(const std::string& errmsg,
...
struct tokenizer {
      ...
      switch (str[0]) {
        ...
        case '-': {
          switch (str[1]) {
            ...
            default:
              BOOST_ASSERT (!"Definition of punctuation_token does not match switch statement");
          }
        }
        default:
          BOOST_ASSERT (!"Definition of punctuation_token does not match switch statement");
  ...
  token get_token() {  // Handle string concatenation
    ...
    return token(
        token::identifier,
        str);  // Note that quoted_string does not get passed to the parser
  }
...
struct parser {
    ...
    switch (peek().type) {
      ...
      case token::identifier: {
        ...
        if (id.type == token::identifier
            && peek().type == token::equal) {  // Graph property
          get();
          ...
          if (peek().type == token::dash_dash
              || peek().type == token::dash_greater) {  // Edge
            parse_edge_stmt(ep);
          } else {
  ...
  edge_endpoint parse_endpoint() {
    switch (peek().type) {
      ...
      default: {
        error(
            "Wanted \"subgraph\", \"{\", or identifier to start node or "
            "subgraph");
  ...
  subgraph_name parse_subgraph(const token& first_token) {
    ...
    if (is_anonymous) {
      name = "___subgraph_" + boost::lexical_cast<std::string>(++sgcounter);
    }
    ...
    switch (peek().type) {
      ...
      case token::colon: {
        ...
        switch (peek().type) {
          ...
          default:
            error(
                "Wanted identifier or left parenthesis as start of port "
                "location");
        }
      }
      default:
  ...
  // Do an edge from the file, the edge may need to be expanded if it connects
  // to a subgraph
  void do_orig_edge(const edge_endpoint& src, const edge_endpoint& tgt,
    ...
    for (std::set<node_and_port>::const_iterator i = sources.begin();
         i != sources.end(); ++i) {
      for (std::set<node_and_port>::const_iterator j = targets.begin();
           j != targets.end(); ++j) {
        do_edge(*i, *j, props);
      }
    }
    ...
    while (!worklist.empty()) {
      ...
      if (ep.is_subgraph) {
        if (done.find(ep.subgraph_ep) == done.end()) {
          ...
          std::map<subgraph_name, subgraph_info>::const_iterator info_i
        ...
        switch (peek().type) {
          ...
          case token::identifier: {
            ...
            if (peek().type == token::equal) {
              get();
              if (peek().type != token::identifier)
                error("Wanted identifier as value of attribute");
...
void parse_graphviz_from_string(const std::string& str, parser_result& result,
                                bool want_directed) {
  parser p(str, result);
...
void translate_results_to_graph(const parser_result& r,
                                ::boost::detail::graph::mutate_graph* mg) {
  typedef boost::detail::graph::edge_t edge;
  for (std::map<node_name, properties>::const_iterator i = r.nodes.begin();
       i != r.nodes.end(); ++i) {
    // std::cerr << i->first << " " << props_to_string(i->second) << std::endl;
    mg->do_add_vertex(i->first);
    for (properties::const_iterator j = i->second.begin(); j != i->second.end();
         ++j) {
  ...
  for (std::vector<edge_info>::const_iterator i = r.edges.begin();
    ...
    for (properties::const_iterator j = ei.props.begin(); j != ei.props.end();
         ++j) {
