...
template <>
void broadcast<const packed_oarchive>(const communicator& comm,
                                      const packed_oarchive& oa, int root) {
...
template <>
void broadcast<packed_oarchive>(const communicator& comm, packed_oarchive& oa,
                                int root) {
  broadcast(comm, const_cast<const packed_oarchive&>(oa), root);
}

template <>
void broadcast<packed_iarchive>(const communicator& comm, packed_iarchive& ia,
                                int root) {
  int size = comm.size();
  if (size < 2)
    return;
  ...
  if (comm.rank() != root) {
    ...
    for (int dest = 0; dest < size; ++dest) {
      if (dest != root) {
        // Build up send requests for each child send.
        num_requests += detail::packed_archive_isend(
            comm, dest, tag, ia, &requests[num_requests], 2);
      }
    }
...
template <>
void broadcast<const packed_skeleton_oarchive>(
    const communicator& comm, const packed_skeleton_oarchive& oa, int root) {
  broadcast(comm, oa.get_skeleton(), root);
}

template <>
void broadcast<packed_skeleton_oarchive>(const communicator& comm,
...
template <>
void broadcast<packed_skeleton_iarchive>(const communicator& comm,
...
void broadcast<const content>(const communicator& comm, const content& c,
  ...
  BOOST_MPI_CHECK_RESULT(MPI_Bcast,
                         (MPI_BOTTOM, 1, c.get_mpi_datatype(), root, comm));
#else
  if (comm.size() < 2)
    return;
    ...
    for (int p = 0; p < comm.size(); ++p) {
      if (p != root) {
        BOOST_MPI_CHECK_RESULT(MPI_Send,
                               (MPI_BOTTOM, 1, c.get_mpi_datatype(), p,
                                environment::collectives_tag(), comm));
      }
    }
  } else {
    BOOST_MPI_CHECK_RESULT(
        MPI_Recv, (MPI_BOTTOM, 1, c.get_mpi_datatype(), root,
                   environment::collectives_tag(), comm, MPI_STATUS_IGNORE));
  }
#endif
}
