...
environment::environment(bool abort_on_exception)
    : i_initialized(false), abort_on_exception(abort_on_exception) {
  if (!initialized()) {
...
int environment::collectives_tag() { return max_tag() + 1; }
...
#else
  BOOST_MPI_CHECK_RESULT(MPI_Attr_get, (MPI_COMM_WORLD, MPI_IO, &io, &found));
#endif
