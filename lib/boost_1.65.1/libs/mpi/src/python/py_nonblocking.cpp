...
std::auto_ptr<request_list> make_request_list_from_py_list(object iterable) {
  ...
  std::copy(stl_input_iterator<python::request_with_value>(iterable),
...
class request_list_indexing_suite
    : public vector_indexing_suite<request_list, false,
                                   request_list_indexing_suite> {
 public:
  ...
  static bool contains(request_list& container, request const& key) {
...
object wrap_wait_any(request_list& requests) {
  ...
  return boost::python::make_tuple(result.second->get_value_or_none(),
                                   result.first,
...
object wrap_test_any(request_list& requests) {
  ...
  if (result)
    return boost::python::make_tuple(
        result->second->get_value_or_none(), result->first,
        distance(requests.begin(), result->second));
  else
    return object();
}
...
int wrap_wait_some(request_list& requests, object py_callable) {
  ...
  if (py_callable != object())
    first_completed
        = wait_some(requests.begin(), requests.end(),
                    status_value_iterator(py_callable, requests.begin()))
              .second;
  else
...
int wrap_test_some(request_list& requests, object py_callable) {
  ...
  if (py_callable != object())
    first_completed
        = test_some(requests.begin(), requests.end(),
                    status_value_iterator(py_callable, requests.begin()))
              .second;
  else
...
void export_nonblocking() {
  ...
  def("wait_any", wrap_wait_any, (arg("requests")),
      nonblocking_wait_any_docstring);
  def("test_any", wrap_test_any, (arg("requests")),
      nonblocking_test_any_docstring);

  def("wait_all", wrap_wait_all, (arg("requests"), arg("callable") = object()),
      nonblocking_wait_all_docstring);
  def("test_all", wrap_test_all, (arg("requests"), arg("callable") = object()),
