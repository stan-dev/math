...
BOOST_PYTHON_DIRECT_SERIALIZATION_ARCHIVE_IMPL(::boost::mpi::packed_iarchive,
...
bool skeleton_and_content_handler_registered(PyTypeObject* type) {
  return skeleton_content_handlers.find(type)
         != skeleton_content_handlers.end();
}

void register_skeleton_and_content_handler(
    PyTypeObject* type, const skeleton_content_handler& handler) {
