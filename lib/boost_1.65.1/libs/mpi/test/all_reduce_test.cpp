...
void all_reduce_one_test(const communicator& comm, Generator generator,
  ...
  value_type expected_result = std::accumulate(
      generated_values.begin(), generated_values.end(), init, op);
...
template <typename Generator, typename Op>
void all_reduce_array_test(const communicator& comm, Generator generator,
                           const char* type_kind, Op op, const char* op_kind,
  ...
  if (comm.rank() == 0) {
    ...
    std::cout << "Reducing (" << place << ") array to " << op_kind << " of "
              << type_kind << "...";
    std::cout.flush();
  }
  ...
  bool got_expected_result
      = (std::equal_range(result.begin(), result.end(), expected_result)
...
template <typename Generator, typename Op>
void all_reduce_test(const communicator& comm, Generator generator,
                     const char* type_kind, Op op, const char* op_kind,
                     typename Generator::result_type init) {
  ...
  all_reduce_one_test(comm, generator, type_kind, op, op_kind, init,
                      out_of_place);
  all_reduce_array_test(comm, generator, type_kind, op, op_kind, init,
                        in_place);
  all_reduce_array_test(comm, generator, type_kind, op, op_kind, init,
                        out_of_place);
}
...
template <>
struct is_commutative<std::plus<wrapped_int>, wrapped_int> : mpl::true_ {};
...
int test_main(int argc, char* argv[]) {
  ...
  all_reduce_test(comm, int_generator(), "integers", std::multiplies<int>(),
                  "product", 1);
  all_reduce_test(comm, int_generator(), "integers", maximum<int>(), "maximum",
                  0);
  all_reduce_test(comm, int_generator(), "integers", minimum<int>(), "minimum",
                  2);
  ...
  all_reduce_test(comm, string_generator(), "strings", std::plus<std::string>(),
                  "concatenation", std::string());
