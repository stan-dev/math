...
static const char* method_kind_names[mk_test_size]
    = {"wait_any",  "test_any",
       "wait_all",  "wait_all (keep results)",
       "test_all",  "test_all (keep results)",
       "wait_some", "wait_some (keep results)",
       "test_some", "test_some (keep results)"};

template <typename T>
void nonblocking_tests(const communicator& comm, const T* values,
                       int num_values, const char* kind, bool composite) {
...
template <typename T>
void nonblocking_test(const communicator& comm, const T* values, int num_values,
  ...
  if (comm.rank() == 0) {
    std::cout << "Testing " << method_kind_names[method] << " with " << kind
              << "...";
    std::cout.flush();
  }
  ...
  reqs.push_back(comm.irecv((comm.rank() + comm.size() - 1) % comm.size(), 0,
                            incoming_value));
  ...
  if (method != mk_wait_any && method != mk_test_any) {
    ...
    reqs.push_back(
        comm.irecv((comm.rank() + comm.size() - 1) % comm.size(), 1));
#endif

    // Send/receive an array
    reqs.push_back(
        comm.isend((comm.rank() + 1) % comm.size(), 2, values, num_values));
    reqs.push_back(comm.irecv((comm.rank() + comm.size() - 1) % comm.size(), 2,
                              &incoming_values.front(), num_values));
  }
  ...
  if (comm.rank() == 0) {
    ...
    if (method != mk_wait_any && method != mk_test_any
        && !std::equal(incoming_values.begin(), incoming_values.end(), values))
      okay = false;
...
int test_main(int argc, char* argv[]) {
  ...
  gps_position gps_array[2]
      = {gps_position(17, 42, .06), gps_position(42, 17, .06)};
