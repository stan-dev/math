...
void reduce_test(const communicator& comm, Generator generator,
                 const char* type_kind, Op op, const char* op_kind,
                 typename Generator::result_type init, int root = -1) {
...
int test_main(int argc, char* argv[]) {
  ...
  reduce_test(comm, int_generator(), "integers", std::multiplies<int>(),
              "product", 1);
  reduce_test(comm, int_generator(), "integers", maximum<int>(), "maximum", 0);
  reduce_test(comm, int_generator(), "integers", minimum<int>(), "minimum", 2);
  ...
  reduce_test(comm, string_generator(), "strings", std::plus<std::string>(),
