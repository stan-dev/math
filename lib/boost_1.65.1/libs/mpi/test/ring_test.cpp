...
template <typename T>
void ring_test(const communicator& comm, const T& pass_value, const char* kind,
               int root = 0) {
  T transferred_value;
  ...
  if (rank == root) {
    ...
    if (transferred_value == pass_value)
...
template <typename T>
void ring_array_test(const communicator& comm, const T* pass_values, int n,
                     const char* kind, int root = 0) {
  T* transferred_values = new T[n];
  int rank = comm.rank();
  int size = comm.size();

  if (rank == root) {
    std::cout << "Passing " << kind << " array around a ring from root " << root
              << "...";
    comm.send((rank + 1) % size, 0, pass_values, n);
    comm.recv((rank + size - 1) % size, 0, transferred_values, n);
    bool okay = std::equal(pass_values, pass_values + n, transferred_values);
    BOOST_CHECK(okay);
    if (okay)
      std::cout << " OK." << std::endl;
  } else {
    ...
    BOOST_CHECK(std::equal(pass_values, pass_values + n, transferred_values));
...
int test_main(int argc, char* argv[]) {
  ...
  gps_position gps_position_array[2]
      = {gps_position(39, 16, 20.2799), gps_position(26, 25, 30.0)};
