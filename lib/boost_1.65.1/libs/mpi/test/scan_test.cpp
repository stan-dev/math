...
void scan_test(const communicator& comm, Generator generator,
...
int test_main(int argc, char* argv[]) {
  ...
  scan_test(comm, int_generator(), "integers", std::multiplies<int>(),
            "product");
  scan_test(comm, int_generator(), "integers", maximum<int>(), "maximum");
  scan_test(comm, int_generator(), "integers", minimum<int>(), "minimum");

  // User-defined MPI datatypes with operations that have the
  // same name as built-in operations.
  scan_test(comm, point_generator(point(0, 0, 0)), "points", std::plus<point>(),
            "sum");
  ...
  scan_test(comm, string_generator(), "strings", std::plus<std::string>(),
            "concatenation");
