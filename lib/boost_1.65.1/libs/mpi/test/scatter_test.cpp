...
void scatter_test(const communicator& comm, Generator generator,
...
template <typename Generator>
void scatterv_test(const communicator& comm, Generator generator,
                   const char* kind, int root = -1) {
  typedef typename Generator::result_type value_type;
  ...
  if (root == -1) {
    ...
    if (comm.rank() == root) {
      ...
      std::cout << "Scatteringv " << kind << " from root " << root << "..."
                << std::endl;
