...
void test_skeleton_and_content(const communicator& comm, int root,
  ...
  if (comm.rank() == root) {
    ...
    std::cout << "Broadcasting reversed integer list content from root " << root
              << "...";
    {
      content c = get_content(original_list);
      for (int p = 0; p < comm.size(); ++p)
...
void test_skeleton_and_content_nonblocking(const communicator& comm, int root) {
  using boost::make_counting_iterator;
  using boost::mpi::broadcast;
  using boost::mpi::content;
  using boost::mpi::get_content;
