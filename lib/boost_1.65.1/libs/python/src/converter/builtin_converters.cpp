...
shared_ptr_deleter::shared_ptr_deleter(handle<> owner) : owner(owner) {}

shared_ptr_deleter::~shared_ptr_deleter() {}

void shared_ptr_deleter::operator()(void const*) { owner.reset(); }
...
struct slot_rvalue_from_python {
 public:
  slot_rvalue_from_python() {
    registry::insert(&slot_rvalue_from_python<T, SlotPolicy>::convertible,
...
struct unsigned_long_long_rvalue_from_python
    : long_long_rvalue_from_python_base {
  static unsigned BOOST_PYTHON_LONG_LONG extract(PyObject* intermediate) {
#if PY_VERSION_HEX < 0x03000000
    if (PyInt_Check(intermediate)) {
      return numeric_cast<unsigned BOOST_PYTHON_LONG_LONG>(
          PyInt_AS_LONG(intermediate));
    } else
#endif
    {
      unsigned BOOST_PYTHON_LONG_LONG result
...
struct float_rvalue_from_python {
  static unaryfunc* get_slot(PyObject* obj) {
    ...
    return (PyLong_Check(obj) || PyFloat_Check(obj)) ? &number_methods->nb_float
                                                     : 0;
  }
...
#else
  static std::string extract(PyObject* intermediate) {
    return std::string(PyString_AsString(intermediate),
...
struct wstring_rvalue_from_python {
  // If the underlying object is "string-able" this will succeed
  static unaryfunc* get_slot(PyObject* obj) {
    return PyUnicode_Check(obj) ? &py_object_identity
#if PY_VERSION_HEX >= 0x03000000
                                : PyBytes_Check(obj)
#else
                                : PyString_Check(obj)
...
struct complex_rvalue_from_python {
  ...
  static std::complex<double> extract(PyObject* intermediate) {
    if (PyComplex_Check(intermediate)) {
      return std::complex<double>(PyComplex_RealAsDouble(intermediate),
...
#define REGISTER_INT_CONVERTERS(signedness, U) \
  slot_rvalue_from_python<                     \
      signedness U, signedness##_int_rvalue_from_python<signedness U> >()
...
// config right all the time.
#ifdef HAVE_LONG_LONG
  slot_rvalue_from_python<signed BOOST_PYTHON_LONG_LONG,
                          long_long_rvalue_from_python>();
  slot_rvalue_from_python<unsigned BOOST_PYTHON_LONG_LONG,
  ...
  slot_rvalue_from_python<std::complex<long double>,
...
#else
  registry::insert(convert_to_cstring, type_id<char>(),
                   &converter::wrap_pytype<&PyUnicode_Type>::get_pytype);
#endif
