...
rvalue_from_python_stage1(PyObject* source, registration const& converters) {
...
BOOST_PYTHON_DECL void* rvalue_result_from_python(
  ...
  registration const& converters
      = *static_cast<registration const*>(converters_);
  ...
  if (!data.convertible) {
    handle<> msg(
        ...
        ("No registered converter was able to produce a C++ rvalue of type %s "
         ...
         converters.target_type.name(), source->ob_type->tp_name));
...
BOOST_PYTHON_DECL void* get_lvalue_from_python(PyObject* source,
                                               registration const& converters) {
...
inline bool visit(rvalue_from_python_chain const* chain) {
  visited_t::iterator const p
      = std::lower_bound(visited.begin(), visited.end(), chain);
...
struct unvisit {
  unvisit(rvalue_from_python_chain const* chain) : chain(chain) {}
...
namespace {
void throw_no_lvalue_from_python(PyObject* source,
                                 registration const& converters,
                                 char const* ref_type) {
  handle<> msg(
#if PY_VERSION_HEX >= 0x03000000
      ...
      ("No registered converter was able to extract a C++ %s to type %s"
       " from this Python object of type %s",
       ref_type, converters.target_type.name(), source->ob_type->tp_name));
...
void* lvalue_result_from_python(PyObject* source,
                                registration const& converters,
                                char const* ref_type) {
  ...
  if (source->ob_refcnt <= 1) {
    handle<> msg(
        ...
        ::PyString_FromFormat
#endif
        ("Attempt to return dangling %s to object of type: %s", ref_type,
...
BOOST_PYTHON_DECL void throw_no_pointer_from_python(
    PyObject* source, registration const& converters) {
  (throw_no_lvalue_from_python)(source, converters, "pointer");
}

BOOST_PYTHON_DECL void throw_no_reference_from_python(
    PyObject* source, registration const& converters) {
