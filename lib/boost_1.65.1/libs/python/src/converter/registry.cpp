...
namespace python {
namespace converter {
BOOST_PYTHON_DECL PyTypeObject const* registration::expected_from_python_type()
    const {
...
BOOST_PYTHON_DECL PyTypeObject const* registration::to_python_target_type()
    const {
  if (this->m_class_object != 0)
    return this->m_class_object;
...
BOOST_PYTHON_DECL PyObject* registration::to_python(
    void const volatile* source) const {
  ...
  return source == 0 ? incref(Py_None)
...
#ifdef BOOST_PYTHON_TRACE_REGISTRY
  std::cout << "registry: ";
  for (registry_t::iterator p = static_registry().begin();
       p != static_registry().end(); ++p) {
    std::cout << p->target_type << "; ";
  }
  std::cout << '\n';
#endif
...
void insert(to_python_function_t f, type_info source_t,
            PyTypeObject const* (*to_python_target_type)()) {
#ifdef BOOST_PYTHON_TRACE_REGISTRY
  ...
  if (slot->m_to_python != 0) {
    std::string msg
        = (std::string("to-Python converter for ") + source_t.name()
...
void insert(convertible_function convertible, constructor_function construct,
            type_info key, PyTypeObject const* (*exp_pytype)()) {
#ifdef BOOST_PYTHON_TRACE_REGISTRY
...
registration const& lookup(type_info key) { return *get(key); }
