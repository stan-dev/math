...
BOOST_PYTHON_DECL char const* gcc_demangle(char const* mangled) {
  ...
  mangling_map::iterator p = std::lower_bound(
  ...
  if (p == demangler.end() || strcmp(p->first, mangled)) {
    int status;
    free_mem keeper(cxxabi::__cxa_demangle(mangled, 0, 0, &status));
    ...
    if (status == -1) {
      ...
      if (cxxabi_cxa_demangle_is_broken() && status == -2
          && strlen(mangled) == 1) {
        // list from
...
BOOST_PYTHON_DECL std::ostream& operator<<(std::ostream& os,
                                           type_info const& x) {
  return os << x.name();
}

namespace detail {
BOOST_PYTHON_DECL std::ostream& operator<<(
    std::ostream& os, detail::decorated_type_info const& x) {
  os << x.m_base_type;
