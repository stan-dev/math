...
detail::new_reference dict_base::call(object const& arg_) {
  return (detail::new_reference)PyObject_CallFunction(
      (PyObject*)&PyDict_Type, const_cast<char*>("(O)"), arg_.ptr());
}

dict_base::dict_base() : object(detail::new_reference(PyDict_New())) {}
...
dict dict_base::copy() {
  if (check_exact(this)) {
    return dict(detail::new_reference(PyDict_Copy(this->ptr())));
  } else {
    return dict(detail::borrowed_reference(this->attr("copy")().ptr()));
  }
}
...
list dict_base::items() const {
  if (check_exact(this)) {
    return list(detail::new_reference(PyDict_Items(this->ptr())));
  } else {
    return assume_list(this->attr("items")());
  }
}

object dict_base::iteritems() const { return this->attr("iteritems")(); }

object dict_base::iterkeys() const { return this->attr("iterkeys")(); }

object dict_base::itervalues() const { return this->attr("itervalues")(); }

list dict_base::keys() const {
  if (check_exact(this)) {
    return list(detail::new_reference(PyDict_Keys(this->ptr())));
  } else {
...
tuple dict_base::popitem() {
  return tuple(detail::borrowed_reference(this->attr("popitem")().ptr()));
}
...
list dict_base::values() const {
  if (check_exact(this)) {
    return list(detail::new_reference(PyDict_Values(this->ptr())));
  } else {
