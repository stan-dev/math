...
#define DTYPE_FROM_CODE(code)          \
  dtype(python::detail::new_reference( \
...
// on Windows. Implement an alternative:
  ...
  if (sizeof(long) == sizeof(int) &&
      ...
      ((a == dtype::get_builtin<long>() || a == dtype::get_builtin<int>())
           && (b == dtype::get_builtin<long>()
               || b == dtype::get_builtin<int>())
       || (a == dtype::get_builtin<unsigned int>()
           || a == dtype::get_builtin<unsigned long>())
              && (b == dtype::get_builtin<unsigned int>()
                  || b == dtype::get_builtin<unsigned long>()))) {
    return true;
  } else {
    return PyArray_EquivTypes(reinterpret_cast<PyArray_Descr*>(a.ptr()),
...
class array_scalar_converter {
 public:
  static PyTypeObject const* get_pytype() {
    // This implementation depends on the fact that get_builtin returns pointers
    // to objects NumPy has declared statically, and that the typeobj member
    // also refers to a static object.  That means we don't need to do any
    // reference counting. In fact, I'm somewhat concerned that increasing the
    // reference count of any of these might cause leaks, because I don't think
    // Boost.Python ever decrements it, but it's probably a moot point if
    // everything is actually static.
    return reinterpret_cast<PyArray_Descr*>(dtype::get_builtin<T>().ptr())
        ->typeobj;
  }
  ...
  static void convert(PyObject* obj,
                      pyconv::rvalue_from_python_stage1_data* data) {
    void* storage
        = reinterpret_cast<pyconv::rvalue_from_python_storage<T>*>(data)
              ->storage.bytes;
    // We assume std::complex is a "standard layout" here and elsewhere; not
    // guaranteed by C++03 standard, but true in every known implementation (and
    // guaranteed by C++11).
