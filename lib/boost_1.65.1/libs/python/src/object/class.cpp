...
instance_holder::instance_holder() : m_next(0) {}
...
static int type_is_gc(PyTypeObject* python_type) {
...
static int property_init(PyObject* self, PyObject* args, PyObject* kwds) {
  ...
  if (!PyArg_ParseTupleAndKeywords(args, kwds, "|OOOO:property",
                                   const_cast<char**>(kwlist), &get, &set, &del,
                                   &doc))
    return -1;
...
static PyObject* static_data_descr_get(PyObject* self, PyObject* /*obj*/,
                                       PyObject* /*type*/) {
...
static int static_data_descr_set(PyObject* self, PyObject* /*obj*/,
                                 PyObject* value) {
  propertyobject* gs = (propertyobject*)self;
  PyObject *func, *res;
  ...
  if (value == NULL)
    ...
    func = gs->prop_set;
  if (func == NULL) {
    PyErr_SetString(PyExc_AttributeError, value == NULL
...
static PyTypeObject static_data_object = {
    PyVarObject_HEAD_INIT(NULL,
                          0) const_cast<char*>("Boost.Python.StaticProperty"),
...
static PyObject* instance_new(PyTypeObject* type_, PyObject* /*args*/,
                              PyObject* /*kw*/) {
  // Attempt to find the __instance_size__ attribute. If not present, no
  // problem.
  PyObject* d = type_->tp_dict;
  PyObject* instance_size_obj
      = PyObject_GetAttrString(d, const_cast<char*>("__instance_size__"));

  ssize_t instance_size = instance_size_obj ?
#if PY_VERSION_HEX >= 0x03000000
                                            PyLong_AsSsize_t(instance_size_obj)
                                            : 0;
#else
                                            PyInt_AsLong(instance_size_obj)
                                            : 0;
#endif
...
static PyMemberDef instance_members[]
    = {{const_cast<char*>("__weakref__"), T_OBJECT,
        offsetof(instance<>, weakrefs), 0, 0},
       {0, 0, 0, 0, 0}};

static PyTypeObject class_type_object = {
    PyVarObject_HEAD_INIT(NULL, 0) const_cast<char*>("Boost.Python.instance"),
...
BOOST_PYTHON_DECL void* find_instance_impl(PyObject* inst, type_info type,
  ...
  for (instance_holder* match = self->objects; match != 0;
       match = match->next()) {
...
inline type_handle query_class(type_info id) {
  ...
  return type_handle(
      python::borrowed(python::allow_null(p ? p->m_class_object : 0)));
}
...
//
inline object new_class(char const* name, std::size_t num_types,
                        type_info const* const types, char const* doc) {
  assert(num_types >= 1);

  // Build a tuple of the base Python type objects. If no bases
  // were declared, we'll use our class_type() as the single base
  // class.
  ssize_t const num_bases
      = (std::max)(num_types - 1, static_cast<std::size_t>(1));
...
class_base::class_base(char const* name, std::size_t num_types,
                       type_info const* const types, char const* doc)
...
BOOST_PYTHON_DECL void copy_class_object(type_info const& src,
                                         type_info const& dst) {
  converter::registration& dst_converters
      = const_cast<converter::registration&>(converter::registry::lookup(dst));

  converter::registration const& src_converters
...
void class_base::add_property(char const* name, object const& fget,
  ...
  object property((python::detail::new_reference)PyObject_CallFunction(
      (PyObject*)&PyProperty_Type, const_cast<char*>("Osss"), fget.ptr(), 0, 0,
      docstr));
...
void class_base::add_property(char const* name, object const& fget,
                              object const& fset, char const* docstr) {
  object property((python::detail::new_reference)PyObject_CallFunction(
      (PyObject*)&PyProperty_Type, const_cast<char*>("OOss"), fget.ptr(),
      fset.ptr(), 0, docstr));
...
void class_base::add_static_property(char const* name, object const& fget) {
  object property((python::detail::new_reference)PyObject_CallFunction(
      static_data(), const_cast<char*>("O"), fget.ptr()));
...
void class_base::add_static_property(char const* name, object const& fget,
                                     object const& fset) {
  object property((python::detail::new_reference)PyObject_CallFunction(
      static_data(), const_cast<char*>("OO"), fget.ptr(), fset.ptr()));
...
PyObject* callable_check(PyObject* callable) {
  ...
  ::PyErr_Format(
      PyExc_TypeError,
      const_cast<char*>("staticmethod expects callable object; got an object "
                        "of type %s, which is not callable"),
...
void class_base::make_method_static(const char* method_name) {
  ...
  this->attr(method_name)
      = object(handle<>(PyStaticMethod_New((callable_check)(method.ptr()))));
}
