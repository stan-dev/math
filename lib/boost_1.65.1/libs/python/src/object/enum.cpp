...
static PyMemberDef enum_members[] = {{const_cast<char*>("name"), T_OBJECT_EX,
                                      offsetof(enum_object, name), READONLY, 0},
                                     {0, 0, 0, 0, 0}};

extern "C" {
static PyObject* enum_repr(PyObject* self_) {
  // XXX(bhy) Potentional memory leak here since PyObject_GetAttrString returns
  // a new reference const char *mod = PyString_AsString(PyObject_GetAttrString(
  // self_, const_cast<char*>("__module__")));
  PyObject* mod = PyObject_GetAttrString(self_, "__module__");
  enum_object* self = downcast<enum_object>(self_);
  if (!self->name) {
    return
#if PY_VERSION_HEX >= 0x03000000
        PyUnicode_FromFormat("%S.%s(%ld)", mod, self_->ob_type->tp_name,
                             PyLong_AsLong(self_));
#else
        PyString_FromFormat("%s.%s(%ld)", PyString_AsString(mod),
                            self_->ob_type->tp_name, PyInt_AS_LONG(self_));
#endif
  } else {
...
#else
        PyString_FromFormat("%s.%s.%s", PyString_AsString(mod),
                            self_->ob_type->tp_name, PyString_AsString(name));
#endif
  }
}
...
#endif
        | Py_TPFLAGS_HAVE_GC | Py_TPFLAGS_BASETYPE, /* tp_flags */
    0,                                              /* tp_doc */
    0,                                              /* tp_traverse */
    0,                                              /* tp_clear */
    0,                                              /* tp_richcompare */
...
enum_base::enum_base(char const* name,
                     converter::to_python_function_t to_python,
                     converter::convertible_function convertible,
                     converter::constructor_function construct, type_info id,
                     char const* doc)
    : object(new_enum_type(name, doc)) {
  converter::registration& converters
      = const_cast<converter::registration&>(converter::registry::lookup(id));
...
PyObject* enum_base::to_python(PyTypeObject* type_, long x) {
  ...
  return incref((v == object() ? type(x) : v).ptr());
}
