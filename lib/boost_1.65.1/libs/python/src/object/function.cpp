...
unsigned py_function_impl_base::max_arity() const { return this->min_arity(); }
...
function::function(py_function const& implementation
    ...
    : m_fn(implementation), m_nkeyword_values(0) {
        ...
        if (f->m_arg_names.is_none()) {
          ...
          if (PyTuple_Size(f->m_arg_names.ptr()) == 0) {
            ...
            for (std::size_t i = 0; i < n_unnamed_actual; ++i)
              PyTuple_SET_ITEM(inner_args.get(), i,
                               incref(PyTuple_GET_ITEM(args, i)));

            // Grab remaining arguments by name from the keyword dictionary
            std::size_t n_actual_processed = n_unnamed_actual;

            for (std::size_t arg_pos = n_unnamed_actual; arg_pos < max_arity;
                 ++arg_pos) {
...
object function::signature(bool show_return_type) const {
  ...
  if (show_return_type)
    return "%s(%s) -> %s"
           % make_tuple(m_name, str(", ").join(formal_params),
                        return_type->basename);
  return "%s(%s)" % make_tuple(m_name, str(", ").join(formal_params));
}
...
void function::argument_error(PyObject* args, PyObject* /*keywords*/) const {
  static handle<> exception(PyErr_NewException(
      const_cast<char*>("Boost.Python.ArgumentError"), PyExc_TypeError, 0));
...
char const* const binary_operator_names[] = {
    "add__",  "and__",     "div__",       "divmod__",  "eq__",   "floordiv__",
    "ge__",   "gt__",      "le__",        "lshift__",  "lt__",   "mod__",
    "mul__",  "ne__",      "or__",        "pow__",     "radd__", "rand__",
    "rdiv__", "rdivmod__", "rfloordiv__", "rlshift__", "rmod__", "rmul__",
    "ror__",  "rpow__",    "rrshift__",   "rshift__",  "rsub__", "rtruediv__",
    "rxor__", "sub__",     "truediv__",   "xor__"};
...
handle<function> not_implemented_function() {
  static object keeper(
      function_object(py_function(&not_implemented, mpl::vector1<void>(), 2),
...
void function::add_to_namespace(object const& name_space, char const* name_,
  ...
  if (attribute.ptr()->ob_type == &function_type) {
    ...
    if (PyClass_Check(ns))
      ...
      dict
          = handle<>(PyObject_GetAttrString(ns, const_cast<char*>("__dict__")));
    ...
    if (existing) {
      if (existing->ob_type == &function_type) {
        new_func->add_overload(
            handle<function>(borrowed(downcast<function>(existing.get()))));
    ...
    handle<> name_space_name(allow_null(::PyObject_GetAttrString(
...
namespace {
struct bind_return {
  bind_return(PyObject*& result, function const* f, PyObject* args,
              PyObject* keywords)
      : m_result(result), m_f(f), m_args(args), m_keywords(keywords) {}

  void operator()() const { m_result = m_f->call(m_args, m_keywords); }
...
static PyObject* function_descr_get(PyObject* func, PyObject* obj,
                                    PyObject* type_) {
#if PY_VERSION_HEX >= 0x03000000
  // The implement is different in Python 3 because of the removal of unbound
  // method
...
static void function_dealloc(PyObject* p) { delete static_cast<function*>(p); }
...
static PyObject* function_get_doc(PyObject* op, void*) {
  ...
  list signatures
      = function_doc_signature_generator::function_doc_signatures(f);
  if (!signatures)
...
static PyObject* function_get_module(PyObject* op, void*) {
  ...
  PyErr_SetString(
      ...
      const_cast<char*>("Boost.Python function __module__ unknown."));
  return 0;
}
}
...
static PyGetSetDef function_getsetlist[] = {
    ...
    {const_cast<char*>("__class__"), (getter)function_get_class, 0, 0,
     0},  // see note above
    {const_cast<char*>("__doc__"), (getter)function_get_doc,
     (setter)function_set_doc, 0, 0},
    {const_cast<char*>("func_doc"), (getter)function_get_doc,
     (setter)function_set_doc, 0, 0},
    {NULL, 0, 0, 0, 0} /* Sentinel */
};

PyTypeObject function_type = {
    PyVarObject_HEAD_INIT(NULL, 0) const_cast<char*>("Boost.Python.function"),
    sizeof(function),
    0,
...
object function_object(py_function const& f,
  ...
  return python::object(python::detail::new_non_null_reference(
      new function(f, keywords.first, keywords.second - keywords.first)));
}
