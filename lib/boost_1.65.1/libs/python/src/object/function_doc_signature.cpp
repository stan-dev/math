...
bool function_doc_signature_generator::arity_cmp(function const *f1,
                                                 function const *f2) {
  return f1->m_fn.max_arity() < f2->m_fn.max_arity();
}

bool function_doc_signature_generator::are_seq_overloads(function const *f1,
                                                         function const *f2,
                                                         bool check_docs) {
  ...
  // if check docs then f1 shold not have docstring or have the same docstring
  // as f2
  ...
  for (unsigned i = 0; i != size; ++i) {
    ...
    if ((f1_has_names && f2_has_names
         && f2->m_arg_names[i - 1] != f1->m_arg_names[i - 1])
        || (f1_has_names && !f2_has_names)
        || (!f1_has_names && f2_has_names
            && f2->m_arg_names[i - 1] != python::object()))
      return false;
  }
  return true;
}

std::vector<function const *> function_doc_signature_generator::flatten(
    function const *f) {
  object name = f->name();
...
std::vector<function const *>
function_doc_signature_generator::split_seq_overloads(
    const std::vector<function const *> &funcs, bool split_on_doc_change) {
  std::vector<function const *> res;
...
str function_doc_signature_generator::raw_function_pretty_signature(
    function const *f, size_t n_overloads, bool cpp_types) {
  str res("object");

  res = str("%s %s(%s)"
            % make_tuple(res, f->m_name, str("tuple args, dict kwds")));

  return res;
}

const char *function_doc_signature_generator::py_type_str(
    const python::detail::signature_element &s) {
...
str function_doc_signature_generator::parameter_string(py_function const &f,
                                                       size_t n,
                                                       object arg_names,
                                                       bool cpp_types) {
  str param;
  ...
  if (cpp_types) {
    ...
    if (n)  // we are processing an argument and trying to come up with a name
            // for it
    {
      object kv;
...
str function_doc_signature_generator::pretty_signature(function const *f,
                                                       size_t n_overloads,
                                                       bool cpp_types) {
  ...
  for (unsigned n = 0; n <= arity; ++n) {
    ...
    formal_params.append(parameter_string(impl, n, f->m_arg_names, cpp_types));

    // find all the arguments with default values preceeding the
    // arity-n_overloads
    if (n && f->m_arg_names) {
  ...
  if (cpp_types) {
    ...
    return str(
        "%s(%s%s%s%s) -> %s"
        % boost::python::make_tuple  // workaround, see top
        (f->m_name, str(",").join(formal_params.slice(0, arity - n_overloads)),
         n_overloads ? (n_overloads != arity ? str(" [,") : str("[ ")) : str(),
         str(" [,").join(formal_params.slice(arity - n_overloads, arity)),
         std::string(n_overloads, ']'), ret_type));
  }

  return str(
      "%s %s(%s%s%s%s) %s"
      % boost::python::make_tuple  // workaround, see top
      (cpp_types ? ret_type : str(""), f->m_name,
       str(",").join(formal_params.slice(0, arity - n_overloads)),
       n_overloads ? (n_overloads != arity ? str(" [,") : str("[ ")) : str(),
       str(" [,").join(formal_params.slice(arity - n_overloads, arity)),
       std::string(n_overloads, ']'), cpp_types ? str("") : ret_type));
}
...
list function_doc_signature_generator::function_doc_signatures(
    function const *f) {
  list signatures;
  ...
  for (fi = funcs.begin(); fi != funcs.end(); ++fi) {
    if (*sfi == *fi) {
      if ((*fi)->doc()) {
        ...
        bool show_py_signature
            = doc_len
                  >= int(sizeof(detail::py_signature_tag) / sizeof(char) - 1)
              && str(detail::py_signature_tag)
                     == func_doc.slice(0, int(sizeof(detail::py_signature_tag)
                                              / sizeof(char))
                                              - 1);
        if (show_py_signature) {
          func_doc = str(func_doc.slice(
              int(sizeof(detail::py_signature_tag) / sizeof(char)) - 1, _));
          doc_len = len(func_doc);
        }

        bool show_cpp_signature
            = doc_len
                  >= int(sizeof(detail::cpp_signature_tag) / sizeof(char) - 1)
              && str(detail::cpp_signature_tag)
                     == func_doc.slice(
                            1
        ...
        if (show_cpp_signature) {
          func_doc = str(func_doc.slice(
              _, 1 - int(sizeof(detail::cpp_signature_tag) / sizeof(char))));
          doc_len = len(func_doc);
        }
        ...
        if (show_cpp_signature) {
          if (len(res) > 1)
            res += "\n" + pad;
          res += detail::cpp_signature_tag + pad + "    "
