...
struct smart_graph {
  ...
  node_distance_map distances_to(vertex_t target) const {
    ...
    std::vector<std::size_t>::iterator to_target
        = m_distances.begin() + n * target;
    ...
    if (to_target[target] != 0) {
      ...
      breadth_first_search(
          ...
          visitor(make_bfs_visitor(record_distances(
              make_iterator_property_map(to_target,
  ...
  smart_graph() : m_known_vertices(0) {}
...
inline type_index_t::iterator demand_type(class_id type) {
  ...
  unused_variable(v2);
  assert(v == v2);
  return type_index().insert(
      p, boost::make_tuple(type, v, dynamic_id_function(0)));
}
...
struct q_elt {
  q_elt(std::size_t distance, void* src_address, vertex_t target,
        cast_function cast)
      : distance(distance),
  ...
  bool operator<(q_elt const& rhs) const { return distance < rhs.distance; }
};
...
inline void* identity_cast(void* p) { return p; }
...
void* search(smart_graph const& g, void* p, vertex_t src, vertex_t dst) {
  ...
  while (!q.empty()) {
    ...
    visited_t::iterator pos
        = std::lower_bound(visited.begin(), visited.end(), s);
    ...
    for (cast_graph::out_edge_iterator p = edges.first, finish = edges.second;
         p != finish; ++p) {
      edge_t e = *p;
      q.push(q_elt(d[target(e, g.topology())], dst_address,
                   target(e, g.topology()), boost::get(casts, e)));
    }
  }
  return 0;
}
...
struct cache_element {
  ...
  BOOST_STATIC_CONSTANT(std::ptrdiff_t,
                        not_found = integer_traits<std::ptrdiff_t>::const_min);

  bool operator<(cache_element const& rhs) const { return this->key < rhs.key; }

  bool unreachable() const { return offset == not_found; }
};
...
inline void* convert_type(void* const p, class_id src_t, class_id dst_t,
  ...
  // info
  boost::python::objects::dynamic_id_t dynamic_id
      = polymorphic ? tuples::get<kdynamic_id>(*src_p)(p)
  ...
  cache_element seek(
      boost::make_tuple(src_t, dst_t, offset, dynamic_id.second));
  cache_t& c = cache();
  ...
  if (cache_pos != c.end() && cache_pos->key == seek.key) {
    return cache_pos->offset == cache_element::not_found
               ? 0
               : (char*)p + cache_pos->offset;
  }

  // If we are starting at the most-derived type, only look in the up graph
  smart_graph const& g
      = polymorphic && dynamic_id.second != src_t ? full_graph() : up_graph();

  void* result = search(g, p, tuples::get<kvertex>(*src_p),
...
BOOST_PYTHON_DECL void* find_dynamic_type(void* p, class_id src_t,
                                          class_id dst_t) {
  return convert_type(p, src_t, dst_t, true);
}

BOOST_PYTHON_DECL void* find_static_type(void* p, class_id src_t,
                                         class_id dst_t) {
...
BOOST_PYTHON_DECL void add_cast(class_id src_t, class_id dst_t,
  ...
  if (c.size() > expected_cache_len) {
    c.erase(
        std::remove_if(c.begin(), c.end(), mem_fn(&cache_element::unreachable)),
        c.end());
