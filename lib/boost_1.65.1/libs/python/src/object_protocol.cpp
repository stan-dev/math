...
BOOST_PYTHON_DECL object getattr(object const& target, object const& key) {
  return object(
      detail::new_reference(PyObject_GetAttr(target.ptr(), key.ptr())));
}
...
BOOST_PYTHON_DECL void setattr(object const& target, object const& key,
                               object const& value) {
...
BOOST_PYTHON_DECL void setattr(object const& target, char const* key,
                               object const& value) {
  if (PyObject_SetAttrString(target.ptr(), const_cast<char*>(key), value.ptr())
      == -1) {
    throw_error_already_set();
  }
}
...
BOOST_PYTHON_DECL void delattr(object const& target, char const* key) {
  if (PyObject_DelAttrString(target.ptr(), const_cast<char*>(key)) == -1) {
    throw_error_already_set();
  }
}
...
BOOST_PYTHON_DECL object getitem(object const& target, object const& key) {
  return object(
      detail::new_reference(PyObject_GetItem(target.ptr(), key.ptr())));
}

BOOST_PYTHON_DECL void setitem(object const& target, object const& key,
                               object const& value) {
...
static PyObject* apply_slice(PyObject* u, PyObject* v,
                             PyObject* w) /* return u[v:w] */
{
...
BOOST_PYTHON_DECL object getslice(object const& target, handle<> const& begin,
                                  handle<> const& end) {
  return object(
      detail::new_reference(apply_slice(target.ptr(), begin.get(), end.get())));
}

BOOST_PYTHON_DECL void setslice(object const& target, handle<> const& begin,
                                handle<> const& end, object const& value) {
  if (assign_slice(target.ptr(), begin.get(), end.get(), value.ptr()) == -1) {
    throw_error_already_set();
  }
}

BOOST_PYTHON_DECL void delslice(object const& target, handle<> const& begin,
                                handle<> const& end) {
  if (assign_slice(target.ptr(), begin.get(), end.get(), 0) == -1) {
    throw_error_already_set();
  }
}
