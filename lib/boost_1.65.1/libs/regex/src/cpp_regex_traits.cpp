 ...
 *   DESCRIPTION: Implements cpp_regex_traits<char> (and associated helper
 * classes).
 */
...
#else
  std::messages<char>::catalog cat
      = reinterpret_cast<std::messages<char>::catalog>(-1);
#endif
  ...
  if ((int)cat >= 0) {
    ...
    try {
      ...
      for (regex_constants::syntax_type i = 1; i < regex_constants::syntax_max;
           ++i) {
        string_type mss
            = this->m_pmessages->get(cat, 0, i, get_default_syntax(i));
...
#endif
    for (regex_constants::syntax_type j = 1; j < regex_constants::syntax_max;
         ++j) {
