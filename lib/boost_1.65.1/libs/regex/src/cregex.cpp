...
class RegExData {
 public:
  enum type { type_pc, type_pf, type_copy };
  regex e;
  cmatch m;
...
void RegExData::update() {
  ...
  if (t == type_pc) {
    for (unsigned int i = 0; i < m.size(); ++i) {
      if (m[i].matched)
        strings[i] = std::string(m[i].first, m[i].second);
...
RegEx::RegEx() { pdata = new BOOST_REGEX_DETAIL_NS::RegExData(); }

RegEx::RegEx(const RegEx& o) {
  pdata = new BOOST_REGEX_DETAIL_NS::RegExData(*(o.pdata));
}

RegEx::~RegEx() { delete pdata; }
...
unsigned int RegEx::error_code() const { return pdata->e.error_code(); }

std::string RegEx::Expression() const { return pdata->e.expression(); }
...
unsigned int RegEx::Grep(GrepCallback cb, const char* p,
  ...
  unsigned int result = regex_grep(BOOST_REGEX_DETAIL_NS::pred1(cb, this), p,
                                   end, pdata->e, flags);
  if (result)
    pdata->update();
  return result;
...
unsigned int RegEx::Grep(std::vector<std::string>& v, const char* p,
                         match_flag_type flags) {
  pdata->t = BOOST_REGEX_DETAIL_NS::RegExData::type_pc;
  pdata->pbase = p;
  const char* end = p;
  ...
  unsigned int result = regex_grep(BOOST_REGEX_DETAIL_NS::pred2(v, this), p,
                                   end, pdata->e, flags);
  if (result)
    pdata->update();
  return result;
...
struct pred3 {
  ...
  const char* base;
  RegEx* pe;
  pred3(std::vector<std::size_t>& o, const char* pb, RegEx* p)
      : v(o), base(pb), pe(p) {}
  bool operator()(const cmatch& m) {
    pe->pdata->m = m;
...
unsigned int RegEx::Grep(std::vector<std::size_t>& v, const char* p,
                         match_flag_type flags) {
  pdata->t = BOOST_REGEX_DETAIL_NS::RegExData::type_pc;
  pdata->pbase = p;
  const char* end = p;
  ...
  unsigned int result = regex_grep(BOOST_REGEX_DETAIL_NS::pred3(v, p, this), p,
                                   end, pdata->e, flags);
  if (result)
    pdata->update();
  return result;
...
struct pred4 {
  ...
  const char* file;
  bool ok;
  pred4(GrepFileCallback c, RegEx* i, const char* f)
      : cb(c), pe(i), file(f), ok(true) {}
...
namespace {
void BuildFileList(std::list<std::string>* pl, const char* files,
                   bool recurse) {
  ...
  if (recurse) {
    ...
    char buf[MAX_PATH];
    BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
        BOOST_REGEX_DETAIL_NS::strcpy_s(buf, MAX_PATH, start.root()));
    if (*buf == 0) {
      BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
          BOOST_REGEX_DETAIL_NS::strcpy_s(buf, MAX_PATH, "."));
      BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
          BOOST_REGEX_DETAIL_NS::strcat_s(buf, MAX_PATH,
    ...
    while (*ptr)
      ++ptr;
    while ((ptr != files) && (*ptr != *directory_iterator::separator())
           && (*ptr != '/'))
      --ptr;
    if (ptr != files)
      ++ptr;
    ...
    while (dstart != dend) {
      ...
      if (std::strlen(dstart.path())
              + std::strlen(directory_iterator::separator()) + std::strlen(ptr)
          >= MAX_PATH) {
    ...
    && !defined(UNDER_CE)
      int r = (::sprintf_s)(buf, sizeof(buf), "%s%s%s", dstart.path(),
                            directory_iterator::separator(), ptr);
#else
      int r = (std::sprintf)(buf, "%s%s%s", dstart.path(),
                             directory_iterator::separator(), ptr);
#endif
      if (r < 0) {
...
unsigned int RegEx::GrepFiles(GrepFileCallback cb, const char* files,
                              bool recurse, match_flag_type flags) {
...
unsigned int RegEx::FindFiles(FindFilesCallback cb, const char* files,
                              bool recurse, match_flag_type flags) {
  unsigned int result = 0;
  std::list<std::string> file_list;
  BuildFileList(&file_list, files, recurse);
...
std::string RegEx::Merge(const char* in, const char* fmt, bool copy,
                         match_flag_type flags) {
  std::string result;
  if (!copy)
    flags |= format_no_copy;
...
std::size_t RegEx::Split(std::vector<std::string>& v, std::string& s,
                         match_flag_type flags, unsigned max_count) {
...
std::size_t RegEx::Position(int i) const {
  switch (pdata->t) {
    case BOOST_REGEX_DETAIL_NS::RegExData::type_pc:
      return pdata->m[i].matched ? pdata->m[i].first - pdata->pbase
                                 : RegEx::npos;
#ifndef BOOST_REGEX_NO_FILEITER
...
std::size_t RegEx::Marks() const { return pdata->e.mark_count(); }

std::size_t RegEx::Length(int i) const {
  switch (pdata->t) {
    case BOOST_REGEX_DETAIL_NS::RegExData::type_pc:
      return pdata->m[i].matched ? pdata->m[i].second - pdata->m[i].first
                                 : RegEx::npos;
#ifndef BOOST_REGEX_NO_FILEITER
    case BOOST_REGEX_DETAIL_NS::RegExData::type_pf:
...
#endif
    case BOOST_REGEX_DETAIL_NS::RegExData::type_copy: {
      std::map<int, std::string, std::less<int> >::iterator pos
          = pdata->strings.find(i);
...
const std::size_t RegEx::npos = ~static_cast<std::size_t>(0);
    ...
    && (__BORLANDC__ <= 0x551) && !defined(_RWSTD_COMPILE_INSTANTIATE)
//
...
template <>
template <>
basic_string<char>& BOOST_REGEX_DECL basic_string<char>::replace<const char*>(
    char* f1, char* f2, const char* i1, const char* i2) {
  ...
  if (insert_len > remove_len) {
    ...
    std::copy_backward(begin() + insert_pos + remove_len, begin() + org_size,
                       end());
    std::copy(i1, i2, begin() + insert_pos);
  } else {
    std::copy(begin() + insert_pos + remove_len, begin() + org_size,
              begin() + insert_pos + insert_len);
    std::copy(i1, i2, begin() + insert_pos);
...
basic_string<wchar_t>::replace<const wchar_t*>(wchar_t* f1, wchar_t* f2,
                                               const wchar_t* i1,
                                               const wchar_t* i2) {
  unsigned insert_pos = f1 - begin();
  unsigned remove_len = f2 - f1;
