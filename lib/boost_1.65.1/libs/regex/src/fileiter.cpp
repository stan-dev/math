 ...
 *   DESCRIPTION: Implements file io primitives + directory searching for class
 * boost::RegEx.
 */
...
#if (defined(__BORLANDC__) || defined(BOOST_REGEX_FI_WIN32_DIR) \
     || defined(BOOST_MSVC))                                    \
    && !defined(BOOST_RE_NO_WIN32)
...
void mapfile::open(const char* file) {
  ...
  if (::MultiByteToWideChar(CP_ACP, 0, file, filename_size, wide_file,
                            filename_size + 1)
      == 0)
    hfile = INVALID_HANDLE_VALUE;
  else
    hfile = CreateFileW(wide_file, GENERIC_READ, FILE_SHARE_READ, 0,
                        OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
#elif defined(__CYGWIN__) || defined(__CYGWIN32__)
  char win32file[MAX_PATH];
  cygwin_conv_to_win32_path(file, win32file);
  hfile = CreateFileA(win32file, GENERIC_READ, FILE_SHARE_READ, 0,
                      OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
#else
  hfile = CreateFileA(file, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING,
  ...
  if (hfile != INVALID_HANDLE_VALUE) {
    ...
    _first
        = static_cast<const char*>(MapViewOfFile(hmap, FILE_MAP_READ, 0, 0, 0));
    if (_first == 0) {
      CloseHandle(hmap);
      CloseHandle(hfile);
      hmap = 0;
...
mapfile::iterator mapfile::begin() const { return mapfile_iterator(this, 0); }

mapfile::iterator mapfile::end() const { return mapfile_iterator(this, _size); }
...
inline _fi_find_handle find_first_file(const char* wild, _fi_find_data& data) {
  ...
  if (::MultiByteToWideChar(CP_ACP, 0, wild, wild_size, wide_wild,
                            wild_size + 1)
      == 0)
...
inline void copy_find_file_result_with_overflow_check(const _fi_find_data& data,
                                                      char* path,
                                                      size_t max_size) {
#ifdef BOOST_NO_ANSI_APIS
  if (::WideCharToMultiByte(CP_ACP, 0, data.cFileName, -1, path, max_size, NULL,
                            NULL)
      == 0)
    BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(1);
#else
  BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
      BOOST_REGEX_DETAIL_NS::strcpy_s(path, max_size, data.cFileName));
#endif
}

inline bool is_not_current_or_parent_path_string(const _fi_find_data& data) {
#ifdef BOOST_NO_ANSI_APIS
  return (std::wcscmp(data.cFileName, L".")
          && std::wcscmp(data.cFileName, L".."));
#else
  return (std::strcmp(data.cFileName, ".")
...
file_iterator::file_iterator(const char* wild) {
  ...
  try {
    ...
    BOOST_REGEX_NOEH_ASSERT(_path)
    BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
        BOOST_REGEX_DETAIL_NS::strcpy_s(_root, MAX_PATH, wild));
    ptr = _root;
    while (*ptr)
      ++ptr;
    while ((ptr > _root) && (*ptr != *_fi_sep) && (*ptr != *_fi_sep_alt))
      --ptr;
    if ((ptr == _root) && ((*ptr == *_fi_sep) || (*ptr == *_fi_sep_alt))) {
      _root[1] = '\0';
      BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
          BOOST_REGEX_DETAIL_NS::strcpy_s(_path, MAX_PATH, _root));
    } else {
      *ptr = 0;
      BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
          BOOST_REGEX_DETAIL_NS::strcpy_s(_path, MAX_PATH, _root));
      if (*_path == 0)
        BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
            BOOST_REGEX_DETAIL_NS::strcpy_s(_path, MAX_PATH, "."));
      BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
          BOOST_REGEX_DETAIL_NS::strcat_s(_path, MAX_PATH, _fi_sep));
    }
    ...
    if (ref->hf == _fi_invalid_handle) {
      *_path = 0;
      ...
      copy_find_file_result_with_overflow_check(ref->_data, ptr,
                                                (MAX_PATH - (ptr - _path)));
...
file_iterator::file_iterator(const file_iterator& other) {
  ...
  try {
    ...
    BOOST_REGEX_NOEH_ASSERT(_path)
    BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
        BOOST_REGEX_DETAIL_NS::strcpy_s(_root, MAX_PATH, other._root));
    BOOST_REGEX_DETAIL_NS::overflow_error_if_not_zero(
        BOOST_REGEX_DETAIL_NS::strcpy_s(_path, MAX_PATH, other._path));
  ...
  try {
    ...
    if (ref->hf == _fi_invalid_handle) {
      *_path = 0;
      ...
      copy_find_file_result_with_overflow_check(ref->_data, ptr,
                                                MAX_PATH - (ptr - _path));
      if (((ref->_data.dwFileAttributes & _fi_dir) == 0)
...
_fi_priv_data::_fi_priv_data(const char* p) {
  ...
  while (*mask)
    ++mask;
  while ((mask > root) && (*mask != *_fi_sep) && (*mask != *_fi_sep_alt))
    --mask;
...
_fi_find_handle _fi_FindFirstFile(const char* lpFileName,
