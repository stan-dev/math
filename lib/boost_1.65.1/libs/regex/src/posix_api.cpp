...
const char* names[] = {
    ...
    "REG_ERPAREN", "REG_EMPTY",     "REG_ECOMPLEXITY", "REG_ESTACK",
    "REG_E_PERL",  "REG_E_UNKNOWN",
};
}  // namespace

typedef boost::basic_regex<char, c_regex_traits<char> > c_regex_type;

BOOST_REGEX_DECL int BOOST_REGEX_CCALL regcompA(regex_tA* expression,
                                                const char* ptr, int f) {
  ...
  boost::uint_fast32_t flags
      = (f & REG_PERLEX)
            ? 0
            : ((f & REG_EXTENDED) ? regex::extended : regex::basic);
  expression->eflags
  ...
  try {
    ...
    static_cast<c_regex_type*>(expression->guts)
        ->set_expression(ptr, p2, flags);
    expression->re_nsub
        = static_cast<c_regex_type*>(expression->guts)->mark_count();
...
BOOST_REGEX_DECL regsize_t BOOST_REGEX_CCALL regerrorA(int code,
                                                       const regex_tA* e,
                                                       char* buf,
                                                       regsize_t buf_size) {
  std::size_t result = 0;
  ...
  if (code <= (int)REG_E_UNKNOWN) {
    ...
    if ((e) && (e->re_magic == magic_value))
      p = static_cast<c_regex_type*>(e->guts)->get_traits().error_string(
...
BOOST_REGEX_DECL int BOOST_REGEX_CCALL regexecA(const regex_tA* expression,
                                                const char* buf, regsize_t n,
...
#pragma warning(disable : 4267)
  ...
  try {
    ...
    if (expression->re_magic == magic_value) {
      result = regex_search(
          start, end, m, *static_cast<c_regex_type*>(expression->guts), flags);
    } else
      return result;
#ifndef BOOST_NO_EXCEPTIONS
  } catch (...) {
