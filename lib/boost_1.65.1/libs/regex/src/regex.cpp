...
#if defined(BOOST_REGEX_HAS_MS_STACK_GUARD) && defined(_MSC_VER) \
    && (_MSC_VER >= 1300)
#include <malloc.h>
#endif
...
regex_error::regex_error(const std::string& s, regex_constants::error_type err,
                         std::ptrdiff_t pos)
    : std::runtime_error(s), m_error_code(err), m_position(pos) {}

regex_error::regex_error(regex_constants::error_type err)
    : std::runtime_error(
          ::boost::BOOST_REGEX_DETAIL_NS::get_default_error_string(err)),
      m_error_code(err),
      m_position(0) {}
...
//
BOOST_REGEX_DECL void BOOST_REGEX_CALL
verify_options(boost::regex::flag_type /*ef*/, match_flag_type mf) {
  ...
  if ((mf & match_extra) && (mf & match_posix)) {
    std::logic_error msg(
        "Usage Error: Can't mix regular expression captures with POSIX "
        "matching rules");
    throw_exception(msg);
  }
#endif
...
BOOST_REGEX_DECL void BOOST_REGEX_CALL reset_stack_guard_page() {
  ...
  // one in use:
  //
  if (!VirtualFree(mi.AllocationBase, (LPBYTE)page - (LPBYTE)mi.AllocationBase,
                   MEM_DECOMMIT)
      || !VirtualProtect(page, si.dwPageSize, PAGE_GUARD | PAGE_READWRITE,
