 ...
 *   DESCRIPTION: Implements w32_regex_traits<char> (and associated helper
 * classes).
 */
...
UINT get_code_page_for_locale_id(lcid_type idx) {
  ...
  if (::GetLocaleInfoW(idx, LOCALE_IDEFAULTANSICODEPAGE, code_page_string, 7)
      == 0)
    return 0;
...
void w32_regex_traits_char_layer<char>::init() {
  ...
  //
  if (cat) {
    for (regex_constants::syntax_type i = 1; i < regex_constants::syntax_max;
         ++i) {
      string_type mss = ::boost::BOOST_REGEX_DETAIL_NS::w32_cat_get(
          cat, this->m_locale, i, get_default_syntax(i));
  ...
  do {
    if (m_char_map[i] == 0) {
      if (::boost::BOOST_REGEX_DETAIL_NS::w32_is(this->m_locale, 0x0002u,
                                                 (char)i))
        m_char_map[i] = regex_constants::escape_type_class;
      else if (::boost::BOOST_REGEX_DETAIL_NS::w32_is(this->m_locale, 0x0001u,
                                                      (char)i))
  ...
  int r = ::LCMapStringA(this->m_locale, LCMAP_LOWERCASE, char_map,
                         1 << CHAR_BIT, this->m_lower_map, 1 << CHAR_BIT);
  BOOST_ASSERT(r != 0);
#else
  ...
  int conv_r = ::MultiByteToWideChar(code_page, 0, char_map, 1 << CHAR_BIT,
                                     wide_char_map, 1 << CHAR_BIT);
  BOOST_ASSERT(conv_r != 0);

  WCHAR wide_lower_map[1 << CHAR_BIT];
  int r = ::LCMapStringW(this->m_locale, LCMAP_LOWERCASE, wide_char_map,
                         1 << CHAR_BIT, wide_lower_map, 1 << CHAR_BIT);
  BOOST_ASSERT(r != 0);

  conv_r = ::WideCharToMultiByte(code_page, 0, wide_lower_map, r,
                                 this->m_lower_map, 1 << CHAR_BIT, NULL, NULL);
  BOOST_ASSERT(conv_r != 0);
#endif
  if (r < (1 << CHAR_BIT)) {
...
#ifndef BOOST_NO_ANSI_APIS
  r = ::GetStringTypeExA(this->m_locale, CT_CTYPE1, char_map, 1 << CHAR_BIT,
                         this->m_type_map);
#else
  r = ::GetStringTypeExW(this->m_locale, CT_CTYPE1, wide_char_map,
                         1 << CHAR_BIT, this->m_type_map);
#endif
  BOOST_ASSERT(0 != r);
}
...
BOOST_REGEX_DECL bool BOOST_REGEX_CALL w32_is_lower(char c, lcid_type idx) {
  ...
  WORD mask;
  if (::GetStringTypeExW(idx, CT_CTYPE1, &wide_c, 1, &mask)
      && (mask & C1_LOWER))
    return true;
  return false;
#endif
...
BOOST_REGEX_DECL bool BOOST_REGEX_CALL w32_is_lower(unsigned short ca,
                                                    lcid_type idx) {
  WORD mask;
  wchar_t c = ca;
  if (::GetStringTypeExW(idx, CT_CTYPE1, &c, 1, &mask) && (mask & C1_LOWER))
...
#ifdef BOOST_REGEX_HAS_OTHER_WCHAR_T
BOOST_REGEX_DECL bool BOOST_REGEX_CALL w32_is_upper(unsigned short ca,
                                                    lcid_type idx) {
  WORD mask;
  wchar_t c = ca;
  if (::GetStringTypeExW(idx, CT_CTYPE1, &c, 1, &mask) && (mask & C1_UPPER))
...
void free_module(void* mod) { ::FreeLibrary(static_cast<HMODULE>(mod)); }

BOOST_REGEX_DECL cat_type BOOST_REGEX_CALL
w32_cat_open(const std::string& name) {
  ...
  if (::MultiByteToWideChar(CP_ACP, 0, name.c_str(), name.size(), wide_name,
                            name.size() + 1)
      == 0)
    return cat_type();
...
BOOST_REGEX_DECL std::string BOOST_REGEX_CALL
w32_cat_get(const cat_type& cat, lcid_type, int i, const std::string& def) {
#ifndef BOOST_NO_ANSI_APIS
  char buf[256];
  if (0 == ::LoadStringA(static_cast<HMODULE>(cat.get()), i, buf, 256)) {
    return def;
  }
#else
  WCHAR wbuf[256];
  int r = ::LoadStringW(static_cast<HMODULE>(cat.get()), i, wbuf, 256);
  if (r == 0)
    return def;

  int buf_size
      = 1 + ::WideCharToMultiByte(CP_ACP, 0, wbuf, r, NULL, 0, NULL, NULL);
...
BOOST_REGEX_DECL std::wstring BOOST_REGEX_CALL
w32_cat_get(const cat_type& cat, lcid_type, int i, const std::wstring& def) {
  wchar_t buf[256];
  if (0 == ::LoadStringW(static_cast<HMODULE>(cat.get()), i, buf, 256)) {
    return def;
  }
  return std::wstring(buf);
}
#ifdef BOOST_REGEX_HAS_OTHER_WCHAR_T
BOOST_REGEX_DECL std::basic_string<unsigned short> BOOST_REGEX_CALL
w32_cat_get(const cat_type& cat, lcid_type, int i,
            const std::basic_string<unsigned short>& def) {
  unsigned short buf[256];
  if (0
      == ::LoadStringW(static_cast<HMODULE>(cat.get()), i, (LPWSTR)buf, 256)) {
    return def;
  }
...
#endif
#endif
BOOST_REGEX_DECL std::string BOOST_REGEX_CALL w32_transform(lcid_type idx,
                                                            const char* p1,
                                                            const char* p2) {
#ifndef BOOST_NO_ANSI_APIS
  ...
  if (::MultiByteToWideChar(code_page, 0, p1, src_len, wide_p1, src_len + 1)
      == 0)
    return std::string(p1, p2);

  int bytes = ::LCMapStringW(idx,            // locale identifier
                             LCMAP_SORTKEY,  // mapping transformation type
  ...
  if (!bytes)
    return std::string(p1, p2);
  std::string result(++bytes, '\0');
  bytes = ::LCMapStringW(idx,            // locale identifier
                         LCMAP_SORTKEY,  // mapping transformation type
                         wide_p1,        // source string
                         src_len,  // number of characters in source string
...
#ifndef BOOST_NO_WREGEX
BOOST_REGEX_DECL std::wstring BOOST_REGEX_CALL
w32_transform(lcid_type idx, const wchar_t* p1, const wchar_t* p2) {
  ...
  bytes = ::LCMapStringW(
      ...
      reinterpret_cast<wchar_t*>(
          &*result.begin()),  // destination buffer *of bytes*
      bytes                   // size of destination buffer
  );
  if (bytes > static_cast<int>(result.size()))
    return std::wstring(p1, p2);
...
#ifdef BOOST_REGEX_HAS_OTHER_WCHAR_T
BOOST_REGEX_DECL std::basic_string<unsigned short> BOOST_REGEX_CALL
w32_transform(lcid_type idx, const unsigned short* p1,
  ...
  for (std::string::size_type i = 0; i < result.size(); ++i)
    r2.append(
        1, static_cast<unsigned short>(static_cast<unsigned char>(result[i])));
  return r2;
}
#endif
#endif
BOOST_REGEX_DECL char BOOST_REGEX_CALL w32_tolower(char c, lcid_type idx) {
  char result[2];
#ifndef BOOST_NO_ANSI_APIS
  int b = ::LCMapStringA(idx,              // locale identifier
  ...
  WCHAR wide_result;
  int b = ::LCMapStringW(idx,              // locale identifier
                         LCMAP_LOWERCASE,  // mapping transformation type
  ...
  if (::WideCharToMultiByte(code_page, 0, &wide_result, 1, result, 2, NULL,
                            NULL)
      == 0)
...
#ifndef BOOST_NO_WREGEX
BOOST_REGEX_DECL wchar_t BOOST_REGEX_CALL w32_tolower(wchar_t c,
                                                      lcid_type idx) {
  wchar_t result[2];
  int b = ::LCMapStringW(idx,              // locale identifier
                         LCMAP_LOWERCASE,  // mapping transformation type
...
#ifdef BOOST_REGEX_HAS_OTHER_WCHAR_T
BOOST_REGEX_DECL unsigned short BOOST_REGEX_CALL w32_tolower(unsigned short c,
                                                             lcid_type idx) {
  wchar_t result[2];
  int b = ::LCMapStringW(idx,                 // locale identifier
                         LCMAP_LOWERCASE,     // mapping transformation type
...
#ifndef BOOST_NO_WREGEX
BOOST_REGEX_DECL wchar_t BOOST_REGEX_CALL w32_toupper(wchar_t c,
                                                      lcid_type idx) {
  wchar_t result[2];
  int b = ::LCMapStringW(idx,              // locale identifier
                         LCMAP_UPPERCASE,  // mapping transformation type
...
#ifdef BOOST_REGEX_HAS_OTHER_WCHAR_T
BOOST_REGEX_DECL unsigned short BOOST_REGEX_CALL w32_toupper(unsigned short c,
                                                             lcid_type idx) {
  wchar_t result[2];
  int b = ::LCMapStringW(idx,                 // locale identifier
                         LCMAP_UPPERCASE,     // mapping transformation type
...
#endif
#endif
BOOST_REGEX_DECL bool BOOST_REGEX_CALL w32_is(lcid_type idx, boost::uint32_t m,
                                              char c) {
  WORD mask;
#ifndef BOOST_NO_ANSI_APIS
  if (::GetStringTypeExA(idx, CT_CTYPE1, &c, 1, &mask)
...
BOOST_REGEX_DECL bool BOOST_REGEX_CALL w32_is(lcid_type idx, boost::uint32_t m,
                                              unsigned short c) {
  WORD mask;
  if (::GetStringTypeExW(idx, CT_CTYPE1, (wchar_t const*)&c, 1, &mask)
      && (mask & m & w32_regex_traits_implementation<wchar_t>::mask_base))
    return true;
  if ((m & w32_regex_traits_implementation<wchar_t>::mask_word) && (c == '_'))
    return true;
