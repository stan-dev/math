...
const wchar_t* wnames[] = {
    L"REG_NOERROR", L"REG_NOMATCH",   L"REG_BADPAT",      L"REG_ECOLLATE",
    L"REG_ECTYPE",  L"REG_EESCAPE",   L"REG_ESUBREG",     L"REG_EBRACK",
    L"REG_EPAREN",  L"REG_EBRACE",    L"REG_BADBR",       L"REG_ERANGE",
    L"REG_ESPACE",  L"REG_BADRPT",    L"REG_EEND",        L"REG_ESIZE",
    L"REG_ERPAREN", L"REG_EMPTY",     L"REG_ECOMPLEXITY", L"REG_ESTACK",
    L"REG_E_PERL",  L"REG_E_UNKNOWN",
};
}  // namespace

typedef boost::basic_regex<wchar_t, c_regex_traits<wchar_t> > wc_regex_type;

BOOST_REGEX_DECL int BOOST_REGEX_CCALL regcompW(regex_tW* expression,
                                                const wchar_t* ptr, int f) {
...
#endif
  // set default flags:
  boost::uint_fast32_t flags
      = (f & REG_PERLEX)
            ? 0
            : ((f & REG_EXTENDED) ? wregex::extended : wregex::basic);
  expression->eflags
      = (f & REG_NEWLINE) ? match_not_dot_newline : match_default;
  ...
  try {
    ...
    static_cast<wc_regex_type*>(expression->guts)
        ->set_expression(ptr, p2, flags);
    expression->re_nsub
        = static_cast<wc_regex_type*>(expression->guts)->mark_count();
...
BOOST_REGEX_DECL regsize_t BOOST_REGEX_CCALL regerrorW(int code,
                                                       const regex_tW* e,
                                                       wchar_t* buf,
                                                       regsize_t buf_size) {
  std::size_t result = 0;
  ...
  if (code <= (int)REG_E_UNKNOWN) {
    ...
    if ((e) && (e->re_magic == wmagic_value))
      p = static_cast<wc_regex_type*>(e->guts)->get_traits().error_string(
          static_cast< ::boost::regex_constants::error_type>(code));
    else {
      p = BOOST_REGEX_DETAIL_NS::get_default_error_string(
          static_cast< ::boost::regex_constants::error_type>(code));
    }
...
BOOST_REGEX_DECL int BOOST_REGEX_CCALL regexecW(const regex_tW* expression,
                                                const wchar_t* buf, regsize_t n,
                                                regmatch_t* array, int eflags) {
#ifdef BOOST_MSVC
#pragma warning(push)
#pragma warning(disable : 4267)
  ...
  try {
    ...
    if (expression->re_magic == wmagic_value) {
      result = regex_search(
          start, end, m, *static_cast<wc_regex_type*>(expression->guts), flags);
    } else
      return result;
#ifndef BOOST_NO_EXCEPTIONS
  } catch (...) {
