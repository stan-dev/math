...
class basic_iarchive_impl {
  ...
  struct aobject {
    ...
    aobject(void *a, class_id_type class_id_)
        : address(a), loaded_as_pointer(false), class_id(class_id_) {}
    aobject() : address(NULL), loaded_as_pointer(false), class_id(-2) {}
  };
  ...
  struct moveable_objects {
    ...
    bool is_pointer;
    moveable_objects() : start(0), end(0), recent(0), is_pointer(false) {}
  } m_moveable_objects;
  ...
  struct cobject_type {
    ...
    cobject_type(std::size_t class_id, const basic_iserializer &bis)
        : m_bis(&bis), m_class_id(class_id) {}
    cobject_type(const cobject_type &rhs)
        : m_bis(rhs.m_bis), m_class_id(rhs.m_class_id) {}
  ...
  struct pending {
    ...
    pending() : object(NULL), bis(NULL), version(0) {}
  } m_pending;

  basic_iarchive_impl(unsigned int flags)
      : m_archive_library_version(BOOST_ARCHIVE_VERSION()), m_flags(flags) {}
  ...
  class_id_type register_type(const basic_iserializer &bis);
  ...
  // public:
  void next_object_pointer(void *t) { m_pending.object = t; }
  void delete_created_pointers();
  class_id_type register_type(const basic_pointer_iserializer &bpis);
  void load_object(basic_iarchive &ar, void *t, const basic_iserializer &bis);
  const basic_pointer_iserializer *load_pointer(
      basic_iarchive &ar, void *&t, const basic_pointer_iserializer *bpis,
      const basic_pointer_iserializer *(*finder)(
          const boost::serialization::extended_type_info &type));
};

inline void basic_iarchive_impl::reset_object_address(
    void const *const new_address, void const *const old_address) {
...
inline void basic_iarchive_impl::delete_created_pointers() {
  object_id_vector_type::iterator i;
  for (i = object_id_vector.begin(); i != object_id_vector.end(); ++i) {
...
inline class_id_type basic_iarchive_impl::register_type(
    const basic_iserializer &bis) {
...
void basic_iarchive_impl::load_preamble(basic_iarchive &ar, cobject_id &co) {
  if (!co.initialized) {
    if (co.bis_ptr->class_info()) {
      ...
      co.file_version = version_type(co.bis_ptr->version());
    }
    co.initialized = true;
  }
}

bool basic_iarchive_impl::track(basic_iarchive &ar, void *&t) {
  object_id_type oid;
  load(ar, oid);
...
inline void basic_iarchive_impl::load_object(basic_iarchive &ar, void *t,
                                             const basic_iserializer &bis) {
  ...
  boost::serialization::state_saver<object_id_type> ss_start(
  ...
  m_moveable_objects.start = this_id = object_id_type(object_id_vector.size());
...
inline const basic_pointer_iserializer *basic_iarchive_impl::load_pointer(
    basic_iarchive &ar, void *&t, const basic_pointer_iserializer *bpis_ptr,
    const basic_pointer_iserializer *(*finder)(
  ...
  if (!tracking) {
    ...
    bpis_ptr->load_object_ptr(ar, t, m_pending.version);
...
BOOST_ARCHIVE_DECL void basic_iarchive::next_object_pointer(void *t) {
...
BOOST_ARCHIVE_DECL void basic_iarchive::set_library_version(
    library_version_type archive_library_version) {
  pimpl->set_library_version(archive_library_version);
}

BOOST_ARCHIVE_DECL void basic_iarchive::reset_object_address(
    const void *new_address, const void *old_address) {
  pimpl->reset_object_address(new_address, old_address);
}

BOOST_ARCHIVE_DECL void basic_iarchive::load_object(
    void *t, const basic_iserializer &bis) {
...
BOOST_ARCHIVE_DECL void basic_iarchive::register_basic_serializer(
    const basic_iserializer &bis) {
  pimpl->register_type(bis);
}

BOOST_ARCHIVE_DECL void basic_iarchive::delete_created_pointers() {
...
BOOST_ARCHIVE_DECL unsigned int basic_iarchive::get_flags() const {
