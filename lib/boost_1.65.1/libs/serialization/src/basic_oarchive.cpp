...
class basic_oarchive_impl {
  ...
  struct aobject {
    ...
    aobject(const void *a, class_id_type class_id_, object_id_type object_id_)
        : address(a), class_id(class_id_), object_id(object_id_) {}
  ...
  struct cobject_type {
    ...
    bool m_initialized;
    cobject_type(std::size_t class_id, const basic_oserializer &bos)
        : m_bos_ptr(&bos), m_class_id(class_id), m_initialized(false) {}
    cobject_type(const basic_oserializer &bos) : m_bos_ptr(&bos) {}
  ...
  basic_oarchive_impl(unsigned int flags)
      : m_flags(flags), pending_object(NULL), pending_bos(NULL) {}

  const cobject_type &find(const basic_oserializer &bos);
  const basic_oserializer *find(
      const serialization::extended_type_info &ti) const;

  // public:
  const cobject_type &register_type(const basic_oserializer &bos);
  void save_object(basic_oarchive &ar, const void *t,
                   const basic_oserializer &bos);
  void save_pointer(basic_oarchive &ar, const void *t,
...
inline const basic_oserializer *basic_oarchive_impl::find(
    const serialization::extended_type_info &ti) const {
#ifdef BOOST_MSVC
#pragma warning(push)
#pragma warning(disable : 4511 4512)
  ...
  class bosarg : public basic_oserializer {
    ...
    void save_object_data(basic_oarchive & /*ar*/, const void * /*x*/
                          ) const {
      BOOST_ASSERT(false);
    }
...
inline const basic_oarchive_impl::cobject_type &basic_oarchive_impl::find(
    const basic_oserializer &bos) {
...
basic_oarchive_impl::register_type(const basic_oserializer &bos) {
...
inline void basic_oarchive_impl::save_object(basic_oarchive &ar, const void *t,
                                             const basic_oserializer &bos) {
  ...
  std::pair<basic_oarchive_impl::object_set_type::const_iterator, bool> aresult
      = object_set.insert(ao);
...
inline void basic_oarchive_impl::save_pointer(
    basic_oarchive &ar, const void *t,
...
BOOST_ARCHIVE_DECL void basic_oarchive::save_object(
    const void *x, const basic_oserializer &bos) {
  pimpl->save_object(*this, x, bos);
}

BOOST_ARCHIVE_DECL void basic_oarchive::save_pointer(
    const void *t, const basic_pointer_oserializer *bpos_ptr) {
  pimpl->save_pointer(*this, t, bpos_ptr);
}

BOOST_ARCHIVE_DECL void basic_oarchive::register_basic_serializer(
    const basic_oserializer &bos) {
...
BOOST_ARCHIVE_DECL unsigned int basic_oarchive::get_flags() const {
  return pimpl->m_flags;
}

BOOST_ARCHIVE_DECL void basic_oarchive::end_preamble() {}

BOOST_ARCHIVE_DECL helper_collection &basic_oarchive::get_helper_collection() {
  return *this;
}
