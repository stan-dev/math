...
struct key_compare {
  bool operator()(const extended_type_info *lhs,
                  const extended_type_info *rhs) const {
...
class extended_type_info_arg : public extended_type_info {
  ...
  virtual const char *get_debug_info() const { return get_key(); }
  virtual void *construct(unsigned int /*count*/, ...) const {
    BOOST_ASSERT(false);
    return NULL;
  }
  virtual void destroy(void const *const /*p*/) const { BOOST_ASSERT(false); }

 public:
  extended_type_info_arg(const char *key) : extended_type_info(0, key) {}
...
BOOST_SERIALIZATION_DECL void extended_type_info::key_register() const {
  if (NULL == get_key())
    return;
  singleton<detail::ktmap>::get_mutable_instance().insert(this);
}

BOOST_SERIALIZATION_DECL void extended_type_info::key_unregister() const {
  if (NULL == get_key())
    return;
...
BOOST_SERIALIZATION_DECL const extended_type_info *extended_type_info::find(
    const char *key) {
...
extended_type_info::extended_type_info(const unsigned int type_info_key,
                                       const char *key)
    : m_type_info_key(type_info_key), m_key(key) {}

BOOST_SERIALIZATION_DECL
extended_type_info::~extended_type_info() {}

BOOST_SERIALIZATION_DECL bool extended_type_info::operator<(
    const extended_type_info &rhs) const {
