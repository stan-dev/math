...
class void_caster_shortcut : public void_caster {
  ...
  virtual bool is_shortcut() const { return true; }
  virtual bool has_virtual_base() const { return m_includes_virtual_base; }

 public:
  void_caster_shortcut(extended_type_info const *derived,
                       extended_type_info const *base,
                       std::ptrdiff_t difference, bool includes_virtual_base,
...
void const *void_caster_shortcut::vbc_downcast(void const *const t) const {
...
void const *void_caster_shortcut::vbc_upcast(void const *const t) const {
  // try to find a chain that gives us what we want
  const void_cast_detail::set_type &s
      = void_cast_detail::void_caster_registry::get_const_instance();
  void_cast_detail::set_type::const_iterator it;
...
class void_caster_argument : public void_caster {
 ...
 public:
  void_caster_argument(extended_type_info const *derived,
                       extended_type_info const *base)
...
BOOST_SERIALIZATION_DECL void void_caster::recursive_register(
    bool includes_virtual_base) const {
  ...
  for (it = s.begin(); it != s.end(); ++it) {
    if (*m_derived == *(*it)->m_base) {
      const void_caster_argument vca((*it)->m_derived, m_base);
      void_cast_detail::set_type::const_iterator i;
      i = s.find(&vca);
      if (i == s.end()) {
        new void_caster_shortcut(
            (*it)->m_derived, m_base, m_difference + (*it)->m_difference,
            (*it)->has_virtual_base() || includes_virtual_base, this);
      }
    }
    if (*(*it)->m_derived == *m_base) {
      const void_caster_argument vca(m_derived, (*it)->m_base);
      void_cast_detail::set_type::const_iterator i;
      i = s.find(&vca);
      if (i == s.end()) {
        new void_caster_shortcut(
            m_derived, (*it)->m_base, m_difference + (*it)->m_difference,
            (*it)->has_virtual_base() || includes_virtual_base, this);
      }
    }
  }
...
BOOST_SERIALIZATION_DECL void void_caster::recursive_unregister() const {
  ...
  void_cast_detail::set_type &s = void_caster_registry::get_mutable_instance();
...
BOOST_SYMBOL_VISIBLE void const *void_upcast(extended_type_info const &derived,
                                             extended_type_info const &base,
                                             void const *const t);
...
BOOST_SERIALIZATION_DECL void const *void_upcast(
    extended_type_info const &derived, extended_type_info const &base,
    void const *const t) {
...
BOOST_SYMBOL_VISIBLE void const *void_downcast(
    extended_type_info const &derived, extended_type_info const &base,
    void const *const t);

BOOST_SERIALIZATION_DECL void const *void_downcast(
    extended_type_info const &derived, extended_type_info const &base,
    void const *const t) {
  // same types - trivial case
  if (derived == base)
