...
struct ThreadProxyData {
  ...
  func start_address_;
  void* arglist_;
  ThreadProxyData(func start_address, void* arglist)
      : start_address_(start_address), arglist_(arglist) {}
};

DWORD WINAPI ThreadProxy(LPVOID args) {
  boost::csbl::unique_ptr<ThreadProxyData> data(
      reinterpret_cast<ThreadProxyData*>(args));
...
inline uintptr_t _beginthreadex(void* security, unsigned stack_size,
  ...
  HANDLE hthread
      = CreateThread(static_cast<LPSECURITY_ATTRIBUTES>(security), stack_size,
                     ThreadProxy, data, initflag, &threadID);
  if (hthread == 0) {
    delete data;
    return 0;
  }
...
bool win32::scoped_winrt_thread::start(thread_func address, void* parameter,
                                       unsigned int* thrdId) {
  ...
  *thrdId = ++threadCount;
  ...
  Microsoft::WRL::ComPtr<ABI::Windows::System::Threading::IWorkItemHandler>
      workItem = Microsoft::WRL::Callback<Microsoft::WRL::Implements<
          Microsoft::WRL::RuntimeClassFlags<Microsoft::WRL::ClassicCom>,
          ABI::Windows::System::Threading::IWorkItemHandler,
          Microsoft::WRL::FtmBase>>(
          [address, parameter,
           completionHandle](ABI::Windows::Foundation::IAsyncAction*) {
            // Add a reference since we need to access the completionHandle
            // after the thread_start_function. This is to handle cases where
            // detach() was called and run_thread_exit_callbacks() would end up
            // closing the handle.
            ::boost::detail::thread_data_base* const thread_info(
                reinterpret_cast<::boost::detail::thread_data_base*>(
                    parameter));
  ...
  hr = threadPoolFactory->RunWithPriorityAndOptionsAsync(
      workItem.Get(), ABI::Windows::System::Threading::WorkItemPriority_Normal,
...
void run_thread_exit_callbacks() {
  detail::thread_data_ptr current_thread_data(detail::get_current_thread_data(),
                                              false);
...
unsigned __stdcall thread_start_function(void* param) {
  detail::thread_data_base* const thread_info(
      reinterpret_cast<detail::thread_data_base*>(param));
...
bool thread::start_thread_noexcept() {
  ...
  if (!thread_info->thread_handle.start(&thread_start_function,
                                        thread_info.get(), &thread_info->id)) {
  ...
  return true;
#else
  uintptr_t const new_thread
      = _beginthreadex(0, 0, &thread_start_function, thread_info.get(),
...
bool thread::start_thread_noexcept(const attributes& attr) {
  ...
  uintptr_t const new_thread
      = _beginthreadex(0, static_cast<unsigned int>(attr.get_stack_size()),
                       &thread_start_function, thread_info.get(),
                       CREATE_SUSPENDED, &thread_info->id);
  if (!new_thread) {
    return false;
    //          boost::throw_exception(thread_resource_error());
...
thread::thread(detail::thread_data_ptr data) : thread_info(data) {}
...
void make_external_thread_data() {
  externally_launched_thread* me
      = detail::heap_new<externally_launched_thread>();
  BOOST_TRY { set_current_thread_data(me); }
  BOOST_CATCH(...) {
...
detail::thread_data_base* get_or_make_current_thread_data() {
  detail::thread_data_base* current_thread_data(
...
bool thread::join_noexcept() {
  detail::thread_data_ptr local_thread_info = (get_thread_info)();
  if (local_thread_info) {
    this_thread::interruptible_wait(this->native_handle(),
                                    detail::timeout::sentinel());
    release_handle();
    return true;
  } else {
    return false;
...
void thread::detach() { release_handle(); }

void thread::release_handle() { thread_info = 0; }
...
bool thread::interruption_requested() const BOOST_NOEXCEPT {
  ...
  return local_thread_info.get()
         && (detail::win32::WaitForSingleObjectEx(
                 local_thread_info->interruption_handle, 0, 0)
             == 0);
}
...
#if BOOST_PLAT_WINDOWS_RUNTIME
  // There is no 'real' Win32 handle so we return a handle that at least can be
  // waited on.
...
LARGE_INTEGER get_due_time(detail::timeout const& target_time) {
  ...
  if (target_time.relative) {
    detail::win32::ticks_type const elapsed_milliseconds
        = detail::win32::GetTickCount64_()() - target_time.start;
    LONGLONG const remaining_milliseconds
        = (target_time.milliseconds - elapsed_milliseconds);
    LONGLONG const hundred_nanoseconds_in_one_millisecond = 10000;

    if (remaining_milliseconds > 0) {
      due_time.QuadPart
          = -(remaining_milliseconds * hundred_nanoseconds_in_one_millisecond);
    }
  } else {
    ...
    target_system_time.wMinute
        = (WORD)target_time.abs_time.time_of_day().minutes();
    target_system_time.wSecond
        = (WORD)target_time.abs_time.time_of_day().seconds();
    ...
    if (!SystemTimeToFileTime(&target_system_time, ((FILETIME*)&due_time))) {
      ...
      if (ticks_per_second > hundred_nanoseconds_in_one_second) {
        posix_time::time_duration::tick_type const ticks_per_hundred_nanoseconds
...
// static REASON_CONTEXT
// default_reason_context={0/*POWER_REQUEST_CONTEXT_VERSION*/,
// 0x00000001/*POWER_REQUEST_CONTEXT_SIMPLE_STRING*/, (LPWSTR)L"generic"};
typedef BOOL(WINAPI* setwaitabletimerex_t)(HANDLE, const LARGE_INTEGER*, LONG,
                                           PTIMERAPCROUTINE, LPVOID,
                                           PREASON_CONTEXT, ULONG);
static inline BOOL WINAPI SetWaitableTimerEx_emulation(
...
#ifdef _MSC_VER
#pragma warning(push)
#pragma warning(disable : 6387)  // MSVC sanitiser warns that GetModuleHandleA()
                                 // might fail
#endif
...
#endif
#endif
bool interruptible_wait(detail::win32::handle handle_to_wait_for,
  ...
  if (detail::get_current_thread_data()
      && detail::get_current_thread_data()->interruption_enabled) {
...
#ifndef UNDER_CE
#if !BOOST_PLAT_WINDOWS_RUNTIME
  // Preferentially use coalescing timers for better power consumption and timer
  // accuracy
  if (!target_time.is_sentinel()) {
    detail::timeout::remaining_time const time_left
        = target_time.remaining_milliseconds();
    ...
    if (timer_handle != 0) {
      ...
      // bool const
      // set_time_succeeded=detail_::SetWaitableTimerEx()(timer_handle,&due_time,0,0,0,&detail_::default_reason_context,tolerable)!=0;
      bool const set_time_succeeded
          = detail_::SetWaitableTimerEx()(timer_handle, &due_time, 0, 0, 0,
                                          NULL, tolerable)
            != 0;
  ...
  do {
    ...
    if (handle_count) {
      unsigned long const notified_index
          = detail::win32::WaitForMultipleObjectsEx(
              handle_count, handles, false,
              using_timer ? INFINITE : time_left.milliseconds, 0);
...
bool non_interruptible_wait(detail::win32::handle handle_to_wait_for,
                            detail::timeout target_time) {
...
bool interruption_requested() BOOST_NOEXCEPT {
  return detail::get_current_thread_data()
         && (detail::win32::WaitForSingleObjectEx(
                 detail::get_current_thread_data()->interruption_handle, 0, 0)
             == 0);
}
#endif
...
BOOST_THREAD_DECL void __cdecl on_process_exit() { boost::cleanup_tls_key(); }
