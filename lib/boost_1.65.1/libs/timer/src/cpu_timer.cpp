...
void show_time(const cpu_times& times, std::ostream& os, const std::string& fmt,
               short places)
...
void get_cpu_times(boost::timer::cpu_times& current) {
  boost::chrono::duration<boost::int64_t, boost::nano> x(
  ...
  if (c == static_cast<clock_t>(-1))  // error
  {
    ...
    current.system
        = boost::timer::nanosecond_type(tm.tms_stime + tm.tms_cstime);
