...
void Arrow_widget::draw_arrow(int x1, int y1, int x2, int y2,
                              QPainter& painter) {
  ...
  double start_angle = straight_angle > 0 ? straight_angle - angle_delta
  ...
  double end_angle = straight_angle > 0 ? (straight_angle + 180 + angle_delta)
...
void Arrow_widget::paintEvent(QPaintEvent*) {
  ...
  for (int x_step = 0; x_step < 6; ++x_step) {
    for (int y_step = 1; y_step <= 3; ++y_step) {
      draw_arrow(base_x, base_y, base_x + x_step * 100, base_y - y_step * 50,
                 p);
