...
To load an error object is to move it into an active <<context>> object, usually contained within a <<try_handle_some>>, a <<try_handle_all>> or a <<try_catch>> scope in the calling thread, where it becomes uniquely associated with a specific <<error_id>> -- or discarded if storage is not available.
...
<1> `parse_info` parses `f`, communicating errors using `leaf::result`.
<2> Using `on_error` ensures that the file name is included with any error reported out of `parse_file`. When the `load` object expires, if an error is being reported, the passed `e_file_name` value will be automatically associated with it.
...
Finally, if `on_error` is passed a function that takes a single error object by mutable reference, the behavior is similar to how such functions are used by `load`; see <<tutorial-loading>>.
...
That works, but it may be better to bind the error handlers in a `std::tuple`:
...
Like exceptions, error objects are local to a thread. When using concurrency, sometimes we need to collect error objects in one thread, then use them to handle the error in another thread.

LEAF offers two interfaces for this purpose, one using `result<T>`, and another using exception handling.
...
namespace boost { namespace leaf {
  ...
  template <class CharT, class Traits>
  friend std::ostream & context<E...>::operator<<( std::basic_ostream<CharT, Traits> & os, context const & ctx )
  {
      ctx.print(os);
      return os;
  }
