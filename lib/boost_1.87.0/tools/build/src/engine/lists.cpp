...
LIST * list_append(LIST * l, LIST * nl)
{
	...
	if (!list_empty(nl))
	{
		...
		l->impl.size = size;
		memcpy(
			list_begin(l) + l_size, list_begin(nl), nl_size * sizeof(OBJECT *));
		list_dealloc(nl);
	}
	return l;
}
...
LIST * list_copy_range(LIST * l, LISTITER first, LISTITER last)
{
	...
	else
	{
		...
		result->impl.size = size;
		for (; first != last; ++first, ++dest) *dest = object_copy(*first);
		return result;
	}
}
...
int32_t list_is_sublist(LIST * sub, LIST * l)
{
	...
	for (; iter != end; iter = list_next(iter))
		if (!list_in(l, list_item(iter))) return 0;
	return 1;
}
...
int32_t list_in(LIST * l, OBJECT * value)
{
	...
	for (; iter != end; iter = list_next(iter))
		if (object_equal(list_item(iter), value)) return 1;
	return 0;
}
