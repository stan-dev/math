...
void ARKStepPrintMem(void* arkode_mem, FILE* outfile)
{
  ...
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: q = %i\n", step_mem->q);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: p = %i\n", step_mem->p);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: istage = %i\n", step_mem->istage);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: stages = %i\n", step_mem->stages);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: mnewt = %i\n", step_mem->mnewt);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: maxcor = %i\n", step_mem->maxcor);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: maxnef = %i\n", step_mem->maxnef);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: maxncf = %i\n", step_mem->maxncf);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: msbp = %i\n", step_mem->msbp);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: predictor = %i\n", step_mem->predictor);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: lsolve_type = %i\n", step_mem->lsolve_type);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: msolve_type = %i\n", step_mem->msolve_type);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: convfail = %i\n", step_mem->convfail);

  /* output long integer quantities */
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: nst_attempts = %li\n", step_mem->nst_attempts);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: nfe = %li\n", step_mem->nfe);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: nfi = %li\n", step_mem->nfi);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: ncfn = %li\n", step_mem->ncfn);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: netf = %li\n", step_mem->netf);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: nsetups = %li\n", step_mem->nsetups);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: nstlp = %li\n", step_mem->nstlp);

  /* output boolean quantities */
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: user_linear = %i\n", step_mem->linear);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: user_linear_timedep = %i\n", step_mem->linear_timedep);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: user_explicit = %i\n", step_mem->explicit);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: user_implicit = %i\n", step_mem->implicit);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: hadapt_pq = %i\n", step_mem->hadapt_pq);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: jcur = %i\n", step_mem->jcur);

  /* output realtype quantities */
  if (step_mem->Be != NULL) {
    STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: explicit Butcher table:\n");
    ARKodeButcherTable_Write(step_mem->Be, outfile);
  }
  if (step_mem->Bi != NULL) {
    STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: implicit Butcher table:\n");
    ARKodeButcherTable_Write(step_mem->Bi, outfile);
  }
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: gamma = %"RSYM"\n", step_mem->gamma);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: gammap = %"RSYM"\n", step_mem->gammap);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: gamrat = %"RSYM"\n", step_mem->gamrat);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: crate = %"RSYM"\n", step_mem->crate);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: eRNrm = %"RSYM"\n", step_mem->eRNrm);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: nlscoef = %"RSYM"\n", step_mem->nlscoef);
  if (step_mem->hadapt_mem != NULL) {
    STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: timestep adaptivity structure:\n");
    arkPrintAdaptMem(step_mem->hadapt_mem, outfile);
  }
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: crdown = %"RSYM"\n", step_mem->crdown);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: rdiv = %"RSYM"\n", step_mem->rdiv);
  STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: dgmax = %"RSYM"\n", step_mem->dgmax);

#ifdef DEBUG_OUTPUT
  /* output vector quantities */
  if (step_mem->sdata != NULL) {
    STAN_SUNDIALS_FPRINTF(outfile, "ARKStep: sdata:\n");
    N_VPrint_Serial(step_mem->sdata);
  }
  if (step_mem->zpred != NULL) {
    STAN_SUNDIALS_FPRINTF(outfile, "ARKStep: zpred:\n");
    N_VPrint_Serial(step_mem->zpred);
  }
  if (step_mem->zcor != NULL) {
    STAN_SUNDIALS_FPRINTF(outfile, "ARKStep: zcor:\n");
    N_VPrint_Serial(step_mem->zcor);
  }
  if (step_mem->Fe != NULL)
    for (i=0; i<step_mem->stages; i++) {
      STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: Fe[%i]:\n", i);
      N_VPrint_Serial(step_mem->Fe[i]);
    }
  if (step_mem->Fi != NULL)
    for (i=0; i<step_mem->stages; i++) {
      STAN_SUNDIALS_FPRINTF(outfile,"ARKStep: Fi[%i]:\n", i);
...
#ifdef DEBUG_OUTPUT
STAN_SUNDIALS_PRINTF("step %li,  stage %i,  h = %"RSYM",  t_n = %"RSYM"\n",
...
#ifdef DEBUG_OUTPUT
STAN_SUNDIALS_PRINTF("predictor:\n");
 N_VPrint_Serial(step_mem->zpred);
#endif
...
#ifdef DEBUG_OUTPUT
STAN_SUNDIALS_PRINTF("rhs data:\n");
 N_VPrint_Serial(step_mem->sdata);
#endif

      /* Solver diagnostics reporting */
      if (ark_mem->report)
        STAN_SUNDIALS_FPRINTF(ark_mem->diagfp, "ARKStep  step  %li  %"RSYM"  %i  %"RSYM"\n",
...
#ifdef DEBUG_OUTPUT
STAN_SUNDIALS_PRINTF("nonlinear solution:\n");
...
#ifdef DEBUG_OUTPUT
STAN_SUNDIALS_PRINTF("explicit solution:\n");
 N_VPrint_Serial(ark_mem->ycur);
#endif
...
#ifdef DEBUG_OUTPUT
STAN_SUNDIALS_PRINTF("error estimate = %"RSYM"\n", dsm);
#endif

    /* Solver diagnostics reporting */
    if (ark_mem->report)
      STAN_SUNDIALS_FPRINTF(ark_mem->diagfp, "ARKStep  etest  %li  %"RSYM"  %"RSYM"\n",
...
#ifdef DEBUG_OUTPUT
STAN_SUNDIALS_PRINTF("error test flag = %i\n", eflag);
#endif
