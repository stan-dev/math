...
int ARKStepWriteParameters(void *arkode_mem, FILE *fp)
{
  ...
  STAN_SUNDIALS_FPRINTF(fp, "ARKStep time step module parameters:\n");
  STAN_SUNDIALS_FPRINTF(fp, "  Method order %i\n",step_mem->q);
  if (step_mem->linear) {
    STAN_SUNDIALS_FPRINTF(fp, "  Linear implicit problem");
    if (step_mem->linear_timedep) {
      STAN_SUNDIALS_FPRINTF(fp, " (time-dependent Jacobian)\n");
    } else {
      STAN_SUNDIALS_FPRINTF(fp, " (time-independent Jacobian)\n");
    }
  }
  if (step_mem->explicit && step_mem->implicit) {
    STAN_SUNDIALS_FPRINTF(fp, "  ImEx integrator\n");
  } else if (step_mem->implicit) {
    STAN_SUNDIALS_FPRINTF(fp, "  Implicit integrator\n");
  } else {
    STAN_SUNDIALS_FPRINTF(fp, "  Explicit integrator\n");
  }
  if (step_mem->hadapt_mem != NULL) {
    STAN_SUNDIALS_FPRINTF(fp, "  Maximum step increase (first step) = %"RSYM"\n",
            step_mem->hadapt_mem->etamx1);
    STAN_SUNDIALS_FPRINTF(fp, "  Step reduction factor on multiple error fails = %"RSYM"\n",
            step_mem->hadapt_mem->etamxf);
    STAN_SUNDIALS_FPRINTF(fp, "  Minimum error fails before above factor is used = %i\n",
            step_mem->hadapt_mem->small_nef);
    STAN_SUNDIALS_FPRINTF(fp, "  Step reduction factor on nonlinear convergence failure = %"RSYM"\n",
            step_mem->hadapt_mem->etacf);
    if (step_mem->explicit)
      STAN_SUNDIALS_FPRINTF(fp, "  Explicit safety factor = %"RSYM"\n",
              step_mem->hadapt_mem->cfl);
    if (step_mem->hadapt_mem->HAdapt == NULL) {
      STAN_SUNDIALS_FPRINTF(fp, "  Time step adaptivity method %i\n", step_mem->hadapt_mem->imethod);
      STAN_SUNDIALS_FPRINTF(fp, "     Safety factor = %"RSYM"\n", step_mem->hadapt_mem->safety);
      STAN_SUNDIALS_FPRINTF(fp, "     Bias factor = %"RSYM"\n", step_mem->hadapt_mem->bias);
      STAN_SUNDIALS_FPRINTF(fp, "     Growth factor = %"RSYM"\n", step_mem->hadapt_mem->growth);
      STAN_SUNDIALS_FPRINTF(fp, "     Step growth lower bound = %"RSYM"\n", step_mem->hadapt_mem->lbound);
      STAN_SUNDIALS_FPRINTF(fp, "     Step growth upper bound = %"RSYM"\n", step_mem->hadapt_mem->ubound);
      STAN_SUNDIALS_FPRINTF(fp, "     k1 = %"RSYM"\n", step_mem->hadapt_mem->k1);
      STAN_SUNDIALS_FPRINTF(fp, "     k2 = %"RSYM"\n", step_mem->hadapt_mem->k2);
      STAN_SUNDIALS_FPRINTF(fp, "     k3 = %"RSYM"\n", step_mem->hadapt_mem->k3);
      if (step_mem->hadapt_mem->expstab == arkExpStab) {
        STAN_SUNDIALS_FPRINTF(fp, "  Default explicit stability function\n");
      } else {
        STAN_SUNDIALS_FPRINTF(fp, "  User provided explicit stability function\n");
      }
    } else {
      STAN_SUNDIALS_FPRINTF(fp, "  User provided time step adaptivity function\n");
    }
  }
  ...
  if (step_mem->implicit) {
    STAN_SUNDIALS_FPRINTF(fp, "  Maximum number of convergence test failures = %i\n",step_mem->maxncf);
    STAN_SUNDIALS_FPRINTF(fp, "  Implicit predictor method = %i\n",step_mem->predictor);
    STAN_SUNDIALS_FPRINTF(fp, "  Implicit solver tolerance coefficient = %"RSYM"\n",step_mem->nlscoef);
    STAN_SUNDIALS_FPRINTF(fp, "  Maximum number of nonlinear corrections = %i\n",step_mem->maxcor);
    STAN_SUNDIALS_FPRINTF(fp, "  Nonlinear convergence rate constant = %"RSYM"\n",step_mem->crdown);
    STAN_SUNDIALS_FPRINTF(fp, "  Nonlinear divergence tolerance = %"RSYM"\n",step_mem->rdiv);
    STAN_SUNDIALS_FPRINTF(fp, "  Gamma factor LSetup tolerance = %"RSYM"\n",step_mem->dgmax);
    STAN_SUNDIALS_FPRINTF(fp, "  Number of steps between LSetup calls = %i\n",step_mem->msbp);
  }
...
int ARKStepWriteButcher(void *arkode_mem, FILE *fp)
{
  ...
  if (step_mem->explicit && (step_mem->Be != NULL)) {
    STAN_SUNDIALS_FPRINTF(fp, "  Explicit Butcher table:\n");
    ARKodeButcherTable_Write(step_mem->Be, fp);
  }
  STAN_SUNDIALS_FPRINTF(fp, "\n");
  if (step_mem->implicit && (step_mem->Bi != NULL)) {
    STAN_SUNDIALS_FPRINTF(fp, "  Implicit Butcher table:\n");
