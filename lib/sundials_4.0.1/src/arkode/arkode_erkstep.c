...
void ERKStepPrintMem(void* arkode_mem, FILE* outfile)
{
  ...
  STAN_SUNDIALS_FPRINTF(outfile,"ERKStep: q = %i\n", step_mem->q);
  STAN_SUNDIALS_FPRINTF(outfile,"ERKStep: p = %i\n", step_mem->p);
  STAN_SUNDIALS_FPRINTF(outfile,"ERKStep: stages = %i\n", step_mem->stages);
  STAN_SUNDIALS_FPRINTF(outfile,"ERKStep: maxnef = %i\n", step_mem->maxnef);

  /* output long integer quantities */
  STAN_SUNDIALS_FPRINTF(outfile,"ERKStep: nst_attempts = %li\n", step_mem->nst_attempts);
  STAN_SUNDIALS_FPRINTF(outfile,"ERKStep: nfe = %li\n", step_mem->nfe);
  STAN_SUNDIALS_FPRINTF(outfile,"ERKStep: netf = %li\n", step_mem->netf);

  /* output boolean quantities */
  STAN_SUNDIALS_FPRINTF(outfile,"ERKStep: hadapt_pq = %i\n", step_mem->hadapt_pq);

  /* output realtype quantities */
  STAN_SUNDIALS_FPRINTF(outfile,"ERKStep: Butcher table:\n");
  ARKodeButcherTable_Write(step_mem->B, outfile);
  if (step_mem->hadapt_mem != NULL) {
    STAN_SUNDIALS_FPRINTF(outfile,"ERKStep: timestep adaptivity structure:\n");
  ...
  for (i=0; i<step_mem->stages; i++) {
    STAN_SUNDIALS_FPRINTF(outfile,"ERKStep: F[%i]:\n", i);
...
#ifdef DEBUG_OUTPUT
STAN_SUNDIALS_PRINTF("stage 0 RHS:\n");
...
STAN_SUNDIALS_PRINTF("step %li,  stage %i,  h = %"RSYM",  t_n = %"RSYM"\n",
      ...
      if (ark_mem->report)
        STAN_SUNDIALS_FPRINTF(ark_mem->diagfp, "ERKStep  step  %li  %"RSYM"  %i  %"RSYM"\n",
...
STAN_SUNDIALS_PRINTF("updated solution:\n");
 N_VPrint_Serial(ark_mem->ycur);
#endif

    /* Solver diagnostics reporting */
    if (ark_mem->report)
      STAN_SUNDIALS_FPRINTF(ark_mem->diagfp, "ERKStep  etest  %li  %"RSYM"  %"RSYM"\n",
