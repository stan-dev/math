...
{
  if (interp_mem != NULL) {
    STAN_SUNDIALS_FPRINTF(outfile, "ark_interp: order = %d\n", interp_mem->order);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_interp: told = %"RSYM"\n", interp_mem->told);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_interp: tnew = %"RSYM"\n", interp_mem->tnew);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_interp: t_fa = %"RSYM"\n", interp_mem->t_fa);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_interp: t_fb = %"RSYM"\n", interp_mem->t_fb);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_interp: h = %"RSYM"\n", interp_mem->h);
#ifdef DEBUG_OUTPUT
    if (interp_mem->fold != NULL) {
      STAN_SUNDIALS_FPRINTF(outfile, "ark_interp: fold:\n");
      N_VPrint_Serial(interp_mem->fold);
    }
    if (interp_mem->fnew != NULL) {
      STAN_SUNDIALS_FPRINTF(outfile, "ark_interp: fnew:\n");
      N_VPrint_Serial(interp_mem->fnew);
    }
    if (interp_mem->yold != NULL) {
      STAN_SUNDIALS_FPRINTF(outfile, "ark_interp: yold:\n");
      N_VPrint_Serial(interp_mem->yold);
    }
    if (interp_mem->ynew != NULL) {
      STAN_SUNDIALS_FPRINTF(outfile, "ark_interp: ynew:\n");
      N_VPrint_Serial(interp_mem->ynew);
    }
    if (interp_mem->fa != NULL) {
      STAN_SUNDIALS_FPRINTF(outfile, "ark_interp: fa:\n");
      N_VPrint_Serial(interp_mem->fa);
    }
    if (interp_mem->fb != NULL) {
      STAN_SUNDIALS_FPRINTF(outfile, "ark_interp: fb:\n");
