...
void MRIStepPrintMem(void* arkode_mem, FILE* outfile)
{
  ...
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: nfs = %li\n", step_mem->nfs);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: nff = %li\n", step_mem->nff);

  /* output realtype quantities */
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: Butcher table:\n");
  ARKodeButcherTable_Write(step_mem->B, outfile);

#ifdef DEBUG_OUTPUT
  /* output vector quantities */
  for (i=0; i<step_mem->stages; i++) {
    STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: F[%i]:\n", i);
    N_VPrint_Serial(step_mem->F[i]);
  }
#endif

  /* Print inner stepper memory */
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep Fast Stepper Mem:\n");
