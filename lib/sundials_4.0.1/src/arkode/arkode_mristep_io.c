...
int MRIStepWriteParameters(void *arkode_mem, FILE *fp)
{
  ...
  STAN_SUNDIALS_FPRINTF(fp, "MRIStep time step module parameters:\n");
...
int MRIStepWriteButcher(void *arkode_mem, FILE *fp)
{
  ...
  if (step_mem->B != NULL) {
    STAN_SUNDIALS_FPRINTF(fp, "  Slow Butcher table (stages = %i):\n", step_mem->stages);
  ...
  if (Bfe != NULL) {
    STAN_SUNDIALS_FPRINTF(fp, "  Fast Butcher table (stages = %i):\n", Bfe->stages);
