...
int arkPrintRootMem(void* arkode_mem, FILE *outfile)
{
  ...
  ark_mem = (ARKodeMem) arkode_mem;
  if (ark_mem->root_mem != NULL) {
    STAN_SUNDIALS_FPRINTF(outfile, "ark_nrtfn = %i\n", ark_mem->root_mem->nrtfn);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_nge = %li\n", ark_mem->root_mem->nge);
    if (ark_mem->root_mem->iroots != NULL)
      for (i=0; i<ark_mem->root_mem->nrtfn; i++)
        STAN_SUNDIALS_FPRINTF(outfile, "ark_iroots[%i] = %i\n", i, ark_mem->root_mem->iroots[i]);
    if (ark_mem->root_mem->rootdir != NULL)
      for (i=0; i<ark_mem->root_mem->nrtfn; i++)
        STAN_SUNDIALS_FPRINTF(outfile, "ark_rootdir[%i] = %i\n", i, ark_mem->root_mem->rootdir[i]);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_taskc = %i\n", ark_mem->root_mem->taskc);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_irfnd = %i\n", ark_mem->root_mem->irfnd);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_mxgnull = %i\n", ark_mem->root_mem->mxgnull);
    if (ark_mem->root_mem->gactive != NULL)
      for (i=0; i<ark_mem->root_mem->nrtfn; i++)
        STAN_SUNDIALS_FPRINTF(outfile, "ark_gactive[%i] = %i\n", i, ark_mem->root_mem->gactive[i]);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_tlo = %"RSYM"\n", ark_mem->root_mem->tlo);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_thi = %"RSYM"\n", ark_mem->root_mem->thi);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_trout = %"RSYM"\n", ark_mem->root_mem->trout);
    if (ark_mem->root_mem->glo != NULL)
      for (i=0; i<ark_mem->root_mem->nrtfn; i++)
        STAN_SUNDIALS_FPRINTF(outfile, "ark_glo[%i] = %"RSYM"\n", i, ark_mem->root_mem->glo[i]);
    if (ark_mem->root_mem->ghi != NULL)
      for (i=0; i<ark_mem->root_mem->nrtfn; i++)
        STAN_SUNDIALS_FPRINTF(outfile, "ark_ghi[%i] = %"RSYM"\n", i, ark_mem->root_mem->ghi[i]);
    if (ark_mem->root_mem->grout != NULL)
      for (i=0; i<ark_mem->root_mem->nrtfn; i++)
        STAN_SUNDIALS_FPRINTF(outfile, "ark_grout[%i] = %"RSYM"\n", i, ark_mem->root_mem->grout[i]);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_toutc = %"RSYM"\n", ark_mem->root_mem->toutc);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_ttol = %"RSYM"\n", ark_mem->root_mem->ttol);
  }
  return(ARK_SUCCESS);
}
