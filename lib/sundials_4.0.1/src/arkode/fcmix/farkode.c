...
void FARK_MALLOC(realtype *t0, realtype *y0, int *imex,
  ...
  *ier = 0;
  ...
  if(F2C_ARKODE_vec->ops->nvgetarraypointer == NULL) {
    *ier = -1;
    STAN_SUNDIALS_FPRINTF(stderr, "Error: getarraypointer vector operation is not implemented.\n\n");
    return;
  }
  if(F2C_ARKODE_vec->ops->nvsetarraypointer == NULL) {
    *ier = -1;
    STAN_SUNDIALS_FPRINTF(stderr, "Error: setarraypointer vector operation is not implemented.\n\n");
    return;
  }
  if(F2C_ARKODE_vec->ops->nvcloneempty == NULL) {
    *ier = -1;
    STAN_SUNDIALS_FPRINTF(stderr, "Error: cloneempty vector operation is not implemented.\n\n");
    return;
  }
...
void FARK_SETIIN(char key_name[], long int *ival, int *ier) {
  if (!strncmp(key_name, "ORDER", 5))
    ...
    *ier = -99;
    STAN_SUNDIALS_FPRINTF(stderr, "FARKSETIIN: Unrecognized key.\n\n");
  }
  return;
}
...
void FARK_SETRIN(char key_name[], realtype *rval, int *ier) {
  if (!strncmp(key_name, "INIT_STEP", 9))
    ...
    *ier = -99;
    STAN_SUNDIALS_FPRINTF(stderr, "FARKSETRIN: Unrecognized key: %s\n\n",key_name);
  }
  return;
}
