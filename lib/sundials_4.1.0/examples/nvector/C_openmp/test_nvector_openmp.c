...
int main(int argc, char *argv[])
{
  ...
  /* Create new vectors */
  W = N_VNewEmpty_OpenMP(length, nthreads);
  if (W == NULL) {
    printf("FAIL: Unable to create a new empty vector \n\n");
    return(1);
  }

  X = N_VNew_OpenMP(length, nthreads);
  if (X == NULL) {
    N_VDestroy(W);
  ...
  /* Check vector ID */
  fails += Test_N_VGetVectorID(X, SUNDIALS_NVEC_OPENMP, 0);
  ...
  U = N_VNew_OpenMP(length, nthreads);
  ...
  V = N_VNew_OpenMP(length, nthreads);
