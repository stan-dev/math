 ...
 * -----------------------------------------------------------------
 * This is the testing routine to check the NVECTOR hip module
 * implementation.
 * -----------------------------------------------------------------*/
...
int main(int argc, char *argv[])
{
  ...
  for (policy=DEFAULT_POL; policy<=LDS_REDUCTIONS; ++policy) {
    ...
    for (memtype=UNMANAGED; memtype<=SUNMEMORY; ++memtype) {
      ...
      realtype* xdata = N_VGetHostArrayPointer_Hip(X);
      ...
      realtype* ydata = N_VGetHostArrayPointer_Hip(Y);
      realtype* zdata = N_VGetHostArrayPointer_Hip(Z);
      for (sunindextype j=0; j<length; j++) {
      ...
      /* Check vector ID */
      fails += Test_N_VGetVectorID(X, SUNDIALS_NVEC_HIP, 0);
...
realtype get_element(N_Vector X, sunindextype i)
{
  ...
  N_VCopyFromDevice_Hip(X);
  return (N_VGetHostArrayPointer_Hip(X))[i];
}
