...
! SPFGMR SUNLinearSolver implementation. We run three tests to
! exercise the solver:
...
contains
  ...
  integer(C_INT) function unit_tests() result(fails)
    ...
    fails = fails + FSUNLinSol_SPFGMRSetGSType(LS, gstype)

    if (fails /= 0) then
      print *, 'FAIL: FSUNLinSol_SPFGMR module, initialization'
    else
      print *, 'SUCCESS: FSUNLinSol_SPFGMR module, initialization tests passed'
      print *, ''
    end if
    ...
    if (fails /= 0) then
      ...
      print *, 'SUCCESS: FSUNLinSol_SPFGMR module, problem 1, all tests passed'
      print *, ''
    end if
    ...
    if (fails /= 0) then
      ...
      print *, 'SUCCESS: FSUNLinSol_SPFGMR module, problem 2, all tests passed'
      print *, ''
    end if
    ...
    if (fails /= 0) then
      ...
      print *, 'SUCCESS: FSUNLinSol_SPFGMR module, problem 3, all tests passed'
      print *, ''
    end if
...
integer(C_INT) function check_vector(X, Y, tol) result(failure)
  use, intrinsic :: iso_c_binding
  use test_fsunlinsol_spfgmr_serial
  use test_utilities
