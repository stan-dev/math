...
! SPTFQMR SUNLinearSolver implementation. We run three tests to
! exercise the solver:
...
module test_fsunlinsol_sptfqmr_serial
...
contains
  ...
  integer(C_INT) function unit_tests() result(fails)
    ...
    LS => FSUNLinSol_SPTFQMR(x, pretype, maxl, sunctx)
    ...
    if (fails /= 0) then
      ...
      print *, 'SUCCESS: FSUNLinSol_SPTFQMR module, initialization tests passed'
      print *, ''
    end if
    ...
    if (fails /= 0) then
      print *, 'FAIL: FSUNLinSol_SPTFQMR module, problem 1'
    else
      print *, 'SUCCESS: FSUNLinSol_SPTFQMR module, problem 1, all tests passed'
      print *, ''
    end if
    ...
    ! Run tests with this setup
    fails = fails + FSUNLinSol_SPTFQMRSetPrecType(LS, pretype);
    fails = fails + Test_FSUNLinSolSetup(LS, A, 0);
    fails = fails + Test_FSUNLinSolSolve(LS, A, x, b, tol, 0);
    fails = fails + Test_FSUNLinSolLastFlag(LS, 0);
    fails = fails + Test_FSUNLinSolNumIters(LS, 0);
    ...
    if (fails /= 0) then
      print *, 'FAIL: FSUNLinSol_SPTFQMR module, problem 2'
    else
      print *, 'SUCCESS: FSUNLinSol_SPTFQMR module, problem 2, all tests passed'
      print *, ''
    end if
    ...
    if (fails /= 0) then
      print *, 'FAIL: FSUNLinSol_SPTFQMR module, problem 3'
    else
      print *, 'SUCCESS: FSUNLinSol_SPTFQMR module, problem 3, all tests passed'
      print *, ''
    end if
...
integer(C_INT) function check_vector(X, Y, tol) result(failure)
  use, intrinsic :: iso_c_binding
  use test_fsunlinsol_sptfqmr_serial
  use test_utilities
...
program main
  ...
  !============== Introduction =============
  print *, 'SPTFQMR SUNLinearSolver Fortran 2003 interface test'
  print *, ''
