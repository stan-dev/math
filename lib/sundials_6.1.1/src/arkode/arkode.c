              ...
              realtype *tret, int itask)
{
  ...
  for(;;) {
    ...
    for(;;) {
     ...
     STAN_SUNDIALS_PRINTF("ARKODE start step %li,  attempt %i,  h = %"RSYM",  t_n = %"RSYM"\n",
                   ...
                   const char *function, char *msg, void *data)
{
  ...
  if (ark_mem->errfp!=NULL) {
    STAN_SUNDIALS_FPRINTF(ark_mem->errfp,"\n[%s %s]  %s\n",module,err_type,function);
    STAN_SUNDIALS_FPRINTF(ark_mem->errfp,"  %s\n\n",msg);
  }
#endif
...
void arkPrintMem(ARKodeMem ark_mem, FILE *outfile)
{
  ...
  STAN_SUNDIALS_FPRINTF(outfile, "itol = %i\n", ark_mem->itol);
  STAN_SUNDIALS_FPRINTF(outfile, "ritol = %i\n", ark_mem->ritol);
  STAN_SUNDIALS_FPRINTF(outfile, "mxhnil = %i\n", ark_mem->mxhnil);
  STAN_SUNDIALS_FPRINTF(outfile, "mxstep = %li\n", ark_mem->mxstep);
  STAN_SUNDIALS_FPRINTF(outfile, "lrw1 = %li\n", (long int) ark_mem->lrw1);
  STAN_SUNDIALS_FPRINTF(outfile, "liw1 = %li\n", (long int) ark_mem->liw1);
  STAN_SUNDIALS_FPRINTF(outfile, "lrw = %li\n", (long int) ark_mem->lrw);
  STAN_SUNDIALS_FPRINTF(outfile, "liw = %li\n", (long int) ark_mem->liw);
  STAN_SUNDIALS_FPRINTF(outfile, "user_efun = %i\n", ark_mem->user_efun);
  STAN_SUNDIALS_FPRINTF(outfile, "tstopset = %i\n", ark_mem->tstopset);
  STAN_SUNDIALS_FPRINTF(outfile, "tstop = %" RSYM"\n", ark_mem->tstop);
  STAN_SUNDIALS_FPRINTF(outfile, "report = %i\n", ark_mem->report);
  STAN_SUNDIALS_FPRINTF(outfile, "VabstolMallocDone = %i\n", ark_mem->VabstolMallocDone);
  STAN_SUNDIALS_FPRINTF(outfile, "MallocDone = %i\n", ark_mem->MallocDone);
  STAN_SUNDIALS_FPRINTF(outfile, "initsetup = %i\n", ark_mem->initsetup);
  STAN_SUNDIALS_FPRINTF(outfile, "init_type = %i\n", ark_mem->init_type);
  STAN_SUNDIALS_FPRINTF(outfile, "firststage = %i\n", ark_mem->firststage);
  STAN_SUNDIALS_FPRINTF(outfile, "uround = %" RSYM"\n", ark_mem->uround);
  STAN_SUNDIALS_FPRINTF(outfile, "reltol = %" RSYM"\n", ark_mem->reltol);
  STAN_SUNDIALS_FPRINTF(outfile, "Sabstol = %" RSYM"\n", ark_mem->Sabstol);
  STAN_SUNDIALS_FPRINTF(outfile, "fixedstep = %i\n", ark_mem->fixedstep);
  STAN_SUNDIALS_FPRINTF(outfile, "tolsf = %" RSYM"\n", ark_mem->tolsf);
  STAN_SUNDIALS_FPRINTF(outfile, "call_fullrhs = %i\n", ark_mem->call_fullrhs);

  /* output counters */
  STAN_SUNDIALS_FPRINTF(outfile, "nhnil = %i\n", ark_mem->nhnil);
  STAN_SUNDIALS_FPRINTF(outfile, "nst_attempts = %li\n", ark_mem->nst_attempts);
  STAN_SUNDIALS_FPRINTF(outfile, "nst = %li\n", ark_mem->nst);
  STAN_SUNDIALS_FPRINTF(outfile, "ncfn = %li\n", ark_mem->ncfn);
  STAN_SUNDIALS_FPRINTF(outfile, "netf = %li\n", ark_mem->netf);

  /* output time-stepping values */
  STAN_SUNDIALS_FPRINTF(outfile, "hin = %" RSYM"\n", ark_mem->hin);
  STAN_SUNDIALS_FPRINTF(outfile, "h = %" RSYM"\n", ark_mem->h);
  STAN_SUNDIALS_FPRINTF(outfile, "hprime = %" RSYM"\n", ark_mem->hprime);
  STAN_SUNDIALS_FPRINTF(outfile, "next_h = %" RSYM"\n", ark_mem->next_h);
  STAN_SUNDIALS_FPRINTF(outfile, "eta = %" RSYM"\n", ark_mem->eta);
  STAN_SUNDIALS_FPRINTF(outfile, "tcur = %" RSYM"\n", ark_mem->tcur);
  STAN_SUNDIALS_FPRINTF(outfile, "tretlast = %" RSYM"\n", ark_mem->tretlast);
  STAN_SUNDIALS_FPRINTF(outfile, "hmin = %" RSYM"\n", ark_mem->hmin);
  STAN_SUNDIALS_FPRINTF(outfile, "hmax_inv = %" RSYM"\n", ark_mem->hmax_inv);
  STAN_SUNDIALS_FPRINTF(outfile, "h0u = %" RSYM"\n", ark_mem->h0u);
  STAN_SUNDIALS_FPRINTF(outfile, "tn = %" RSYM"\n", ark_mem->tn);
  STAN_SUNDIALS_FPRINTF(outfile, "hold = %" RSYM"\n", ark_mem->hold);
  STAN_SUNDIALS_FPRINTF(outfile, "maxnef = %i\n", ark_mem->maxnef);
  STAN_SUNDIALS_FPRINTF(outfile, "maxncf = %i\n", ark_mem->maxncf);

  /* output time-stepping adaptivity structure */
  STAN_SUNDIALS_FPRINTF(outfile, "timestep adaptivity structure:\n");
  arkPrintAdaptMem(ark_mem->hadapt_mem, outfile);

  /* output inequality constraints quantities */
  STAN_SUNDIALS_FPRINTF(outfile, "constraintsSet = %i\n", ark_mem->constraintsSet);
  STAN_SUNDIALS_FPRINTF(outfile, "maxconstrfails = %i\n", ark_mem->maxconstrfails);
  ...
  STAN_SUNDIALS_FPRINTF(outfile, "Vapbsol:\n");
  N_VPrintFile(ark_mem->Vabstol, outfile);
  STAN_SUNDIALS_FPRINTF(outfile, "ewt:\n");
  N_VPrintFile(ark_mem->ewt, outfile);
  if (!ark_mem->rwt_is_ewt) {
    STAN_SUNDIALS_FPRINTF(outfile, "rwt:\n");
    N_VPrintFile(ark_mem->rwt, outfile);
  }
  STAN_SUNDIALS_FPRINTF(outfile, "ycur:\n");
  N_VPrintFile(ark_mem->ycur, outfile);
  STAN_SUNDIALS_FPRINTF(outfile, "yn:\n");
  N_VPrintFile(ark_mem->yn, outfile);
  STAN_SUNDIALS_FPRINTF(outfile, "fn:\n");
  N_VPrintFile(ark_mem->fn, outfile);
  STAN_SUNDIALS_FPRINTF(outfile, "tempv1:\n");
  N_VPrintFile(ark_mem->tempv1, outfile);
  STAN_SUNDIALS_FPRINTF(outfile, "tempv2:\n");
  N_VPrintFile(ark_mem->tempv2, outfile);
  STAN_SUNDIALS_FPRINTF(outfile, "tempv3:\n");
  N_VPrintFile(ark_mem->tempv3, outfile);
  STAN_SUNDIALS_FPRINTF(outfile, "tempv4:\n");
  N_VPrintFile(ark_mem->tempv4, outfile);
  STAN_SUNDIALS_FPRINTF(outfile, "constraints:\n");
...
#ifdef SUNDIALS_DEBUG
 STAN_SUNDIALS_PRINTF("ARKODE end step %li,  h = %"RSYM",  t_n = %"RSYM"\n",
