...
{
  if (hadapt_mem != NULL) {
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: etamax = %"RSYM"\n", hadapt_mem->etamax);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: etamx1 = %"RSYM"\n", hadapt_mem->etamx1);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: etamxf = %"RSYM"\n", hadapt_mem->etamxf);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: etamin = %"RSYM"\n", hadapt_mem->etamin);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: small_nef = %i\n", hadapt_mem->small_nef);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: etacf = %"RSYM"\n", hadapt_mem->etacf);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: imethod = %i\n", hadapt_mem->imethod);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: ehist =  %"RSYM"  %"RSYM"\n",
            hadapt_mem->ehist[0],
            hadapt_mem->ehist[1]);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: hhist =  %"RSYM"  %"RSYM"\n",
            hadapt_mem->hhist[0],
            hadapt_mem->hhist[1]);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: cfl = %"RSYM"\n", hadapt_mem->cfl);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: safety = %"RSYM"\n", hadapt_mem->safety);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: bias = %"RSYM"\n", hadapt_mem->bias);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: growth = %"RSYM"\n", hadapt_mem->growth);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: lbound = %"RSYM"\n", hadapt_mem->lbound);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: ubound = %"RSYM"\n", hadapt_mem->ubound);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: k1 = %"RSYM"\n", hadapt_mem->k1);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: k2 = %"RSYM"\n", hadapt_mem->k2);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: k3 = %"RSYM"\n", hadapt_mem->k3);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: q = %i\n", hadapt_mem->q);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: p = %i\n", hadapt_mem->p);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: pq = %i\n", hadapt_mem->pq);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: nst_acc = %li\n", hadapt_mem->nst_acc);
    STAN_SUNDIALS_FPRINTF(outfile, "ark_hadapt: nst_exp = %li\n", hadapt_mem->nst_exp);
    if (hadapt_mem->expstab == arkExpStab) {
      STAN_SUNDIALS_FPRINTF(outfile, "  ark_hadapt: Default explicit stability function\n");
    } else {
      STAN_SUNDIALS_FPRINTF(outfile, "  ark_hadapt: User provided explicit stability function\n");
             ...
             realtype ecur, long int nst)
{
  ...
  if (ark_mem->report)
    STAN_SUNDIALS_FPRINTF(ark_mem->diagfp, "ARKadapt  adapt  %"RSYM"  %"RSYM"  %"RSYM"  %"RSYM"  %"RSYM"  %"RSYM"  %"RSYM"  %"RSYM"  ",
  ...
  /* Solver diagnostics reporting */
  if (ark_mem->report)
    STAN_SUNDIALS_FPRINTF(ark_mem->diagfp, "%"RSYM"  %"RSYM"  ", h_acc, h_cfl);
  ...
  /* Solver diagnostics reporting */
  if (ark_mem->report)
    STAN_SUNDIALS_FPRINTF(ark_mem->diagfp, "%"RSYM"\n", ark_mem->eta);
