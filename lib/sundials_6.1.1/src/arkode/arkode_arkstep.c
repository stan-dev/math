...
int arkStep_TakeStep_Z(void* arkode_mem, realtype *dsmPtr, int *nflagPtr)
{
  ...
  for (is=0; is<step_mem->stages; is++) {
   ...
   STAN_SUNDIALS_PRINTF("    ARKStep step %li,  stage %i,  h = %"RSYM",  t_n = %"RSYM"\n",
...
#ifdef SUNDIALS_DEBUG
    if (implicit_stage)
     STAN_SUNDIALS_PRINTF("implicit stage\n");
    else
     STAN_SUNDIALS_PRINTF("explicit stage\n");
#endif
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
   STAN_SUNDIALS_PRINTF("    ARKStep predictor:\n");
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
   STAN_SUNDIALS_PRINTF("    ARKStep rhs data:\n");
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
     STAN_SUNDIALS_PRINTF("    ARKStep implicit stage %i solution:\n",is);
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
     STAN_SUNDIALS_PRINTF("    ARKStep explicit stage %i solution:\n",is);
      N_VPrint(ark_mem->ycur);
#endif
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
     STAN_SUNDIALS_PRINTF("    ARKStep implicit stage RHS Fi[%i]:\n",is);
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
       STAN_SUNDIALS_PRINTF("    ARKStep explicit stage RHS Fe[%i]:\n",is);
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
   STAN_SUNDIALS_PRINTF("    ARKStep updated solution:\n");
    N_VPrint(ark_mem->ycur);
#endif

#ifdef SUNDIALS_DEBUG
 STAN_SUNDIALS_PRINTF("    ARKStep error estimate = %"RSYM"\n", *dsmPtr);
#endif
