...
#ifdef SUNDIALS_DEBUG_PRINTVEC
 STAN_SUNDIALS_PRINTF("    ERKStep step %li,  stage 0,  h = %"RSYM",  t_n = %"RSYM"\n",
         ark_mem->nst, ark_mem->h, ark_mem->tcur);
#endif

#ifdef SUNDIALS_DEBUG_PRINTVEC
 STAN_SUNDIALS_PRINTF("    ERKStep stage 0 solution:\n");
  N_VPrint(ark_mem->ycur);
 STAN_SUNDIALS_PRINTF("    ERKStep stage RHS F[0]:\n");
  ...
  for (is=1; is<step_mem->stages; is++) {
   ...
   STAN_SUNDIALS_PRINTF("    ERKStep step %li,  stage %i,  h = %"RSYM",  t_n = %"RSYM"\n",
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
   STAN_SUNDIALS_PRINTF("    ERKStep stage RHS F[%i]:\n",is);
...
#ifdef SUNDIALS_DEBUG
 STAN_SUNDIALS_PRINTF("    ERKStep error estimate = %"RSYM"\n", *dsmPtr);
#endif
#ifdef SUNDIALS_DEBUG_PRINTVEC
 STAN_SUNDIALS_PRINTF("    ERKStep updated solution:\n");
  N_VPrint(ark_mem->ycur);
#endif
