...
{
  if (interp != NULL) {
    STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Hermite): degree = %d\n", HINT_DEGREE(interp));
    STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Hermite): told = %"RSYM"\n", HINT_TOLD(interp));
    STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Hermite): tnew = %"RSYM"\n", HINT_TNEW(interp));
    STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Hermite): h = %"RSYM"\n", HINT_H(interp));
#ifdef SUNDIALS_DEBUG_PRINTVEC
    STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Hermite): fold:\n");
    N_VPrintFile(HINT_FOLD(interp), outfile);
    STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Hermite): fnew:\n");
    N_VPrintFile(HINT_FNEW(interp), outfile);
    STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Hermite): yold:\n");
    N_VPrintFile(HINT_YOLD(interp), outfile);
    STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Hermite): ynew:\n");
    N_VPrintFile(HINT_YNEW(interp), outfile);
    STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Hermite): fa:\n");
    N_VPrintFile(HINT_FA(interp), outfile);
    STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Hermite): fb:\n");
...
void arkInterpPrintMem_Lagrange(ARKInterp I, FILE *outfile)
{
  int i;
  if (I != NULL) {
    STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Lagrange): nmax = %i\n", LINT_NMAX(I));
    STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Lagrange): nhist = %i\n", LINT_NHIST(I));
    if (LINT_THIST(I) != NULL) {
      STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Lagrange): thist =");
      for (i=0; i<LINT_NMAX(I); i++)
        STAN_SUNDIALS_FPRINTF(outfile, "  %"RSYM,LINT_TJ(I,i));
      STAN_SUNDIALS_FPRINTF(outfile, "\n");
    }
    if (LINT_YHIST(I) != NULL) {
      STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Lagrange): yhist ptrs =");
      for (i=0; i<LINT_NMAX(I); i++)
        STAN_SUNDIALS_FPRINTF(outfile, "  %p",(void*) LINT_YJ(I,i));
      STAN_SUNDIALS_FPRINTF(outfile, "\n");
    }
#ifdef SUNDIALS_DEBUG_PRINTVEC
    if (LINT_YHIST(I) != NULL) {
      for (i=0; i<LINT_NMAX(I); i++) {
        STAN_SUNDIALS_FPRINTF(outfile, "arkode_interp (Lagrange): yhist[%i]:\n",i);
