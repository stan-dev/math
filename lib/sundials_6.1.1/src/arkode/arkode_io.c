...
int arkWriteParameters(ARKodeMem ark_mem, FILE *fp)
{
  ...
  STAN_SUNDIALS_FPRINTF(fp, "ARKode solver parameters:\n");
  if (ark_mem->hmin != ZERO)
    STAN_SUNDIALS_FPRINTF(fp, "  Minimum step size = %" RSYM"\n",ark_mem->hmin);
  if (ark_mem->hmax_inv != ZERO)
    STAN_SUNDIALS_FPRINTF(fp, "  Maximum step size = %" RSYM"\n",ONE/ark_mem->hmax_inv);
  if (ark_mem->fixedstep)
    STAN_SUNDIALS_FPRINTF(fp, "  Fixed time-stepping enabled\n");
  if (ark_mem->itol == ARK_WF) {
    STAN_SUNDIALS_FPRINTF(fp, "  User provided error weight function\n");
  } else {
    STAN_SUNDIALS_FPRINTF(fp, "  Solver relative tolerance = %" RSYM"\n", ark_mem->reltol);
    if (ark_mem->itol == ARK_SS) {
      STAN_SUNDIALS_FPRINTF(fp, "  Solver absolute tolerance = %" RSYM"\n", ark_mem->Sabstol);
    } else {
      STAN_SUNDIALS_FPRINTF(fp, "  Vector-valued solver absolute tolerance\n");
    }
  }
  if (!ark_mem->rwt_is_ewt) {
    if (ark_mem->ritol == ARK_WF) {
      STAN_SUNDIALS_FPRINTF(fp, "  User provided residual weight function\n");
    } else {
      if (ark_mem->ritol == ARK_SS) {
        STAN_SUNDIALS_FPRINTF(fp, "  Absolute residual tolerance = %" RSYM"\n", ark_mem->SRabstol);
      } else {
        STAN_SUNDIALS_FPRINTF(fp, "  Vector-valued residual absolute tolerance\n");
      }
    }
  }
  if (ark_mem->hin != ZERO)
    STAN_SUNDIALS_FPRINTF(fp, "  Initial step size = %" RSYM"\n",ark_mem->hin);
