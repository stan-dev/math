               ...
               N_Vector vtemp1, N_Vector vtemp2, N_Vector vtemp3)
{
  ...
  /* access ARKLsMem structure */
  retval = arkLs_AccessLMem(arkode_mem, "arkLsSetup",
                            &ark_mem, &arkls_mem);
  if (retval != ARK_SUCCESS)  return(retval);
               ...
               N_Vector ynow, N_Vector fnow, realtype eRNrm, int mnewt)
{
  ...
  if (ark_mem->report)
    STAN_SUNDIALS_FPRINTF(ark_mem->diagfp, "ARKLS  kry  %"RSYM"  %"RSYM"  %i  %i\n",
...
int arkLsMassSolve(void *arkode_mem, N_Vector b, realtype nlscoef)
{
  ...
  /* Log solver statistics to diagnostics file (if requested) */
  if (ark_mem->report)
    STAN_SUNDIALS_FPRINTF(ark_mem->diagfp, "ARKLS  mass  %"RSYM"  %i  %i\n",
