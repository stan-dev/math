...
void MRIStepCoupling_Write(MRIStepCoupling MRIC, FILE *outfile)
{
  ...
  STAN_SUNDIALS_FPRINTF(outfile, "  nmat = %i\n", MRIC->nmat);
  STAN_SUNDIALS_FPRINTF(outfile, "  stages = %i\n", MRIC->stages);
  STAN_SUNDIALS_FPRINTF(outfile, "  method order (q) = %i\n", MRIC->q);
  STAN_SUNDIALS_FPRINTF(outfile, "  embedding order (p) = %i\n", MRIC->p);

  STAN_SUNDIALS_FPRINTF(outfile, "  c = ");
  for (i = 0; i < MRIC->stages; i++)
    STAN_SUNDIALS_FPRINTF(outfile, "%"RSYM"  ", MRIC->c[i]);
  STAN_SUNDIALS_FPRINTF(outfile, "\n");

  if (MRIC->W) {
    for (k = 0; k < MRIC->nmat; k++) {
      STAN_SUNDIALS_FPRINTF(outfile, "  W[%i] = \n", k);
      for (i = 0; i < MRIC->stages; i++){
        STAN_SUNDIALS_FPRINTF(outfile, "      ");
        for (j = 0; j < MRIC->stages; j++)
          STAN_SUNDIALS_FPRINTF(outfile, "%"RSYMW"  ", MRIC->W[k][i][j]);
        STAN_SUNDIALS_FPRINTF(outfile, "\n");
      }
      STAN_SUNDIALS_FPRINTF(outfile, "\n");
    }
  ...
  if (MRIC->G) {
    for (k = 0; k < MRIC->nmat; k++) {
      STAN_SUNDIALS_FPRINTF(outfile, "  G[%i] = \n", k);
      for (i = 0; i < MRIC->stages; i++) {
        STAN_SUNDIALS_FPRINTF(outfile, "      ");
        for (j = 0; j < MRIC->stages; j++)
          STAN_SUNDIALS_FPRINTF(outfile, "%"RSYMW"  ", MRIC->G[k][i][j]);
        STAN_SUNDIALS_FPRINTF(outfile, "\n");
      }
      STAN_SUNDIALS_FPRINTF(outfile, "\n");
    }
