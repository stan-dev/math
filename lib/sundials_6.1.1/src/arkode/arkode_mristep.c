...
void MRIStepPrintMem(void* arkode_mem, FILE* outfile)
{
  ...
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: q = %i\n", step_mem->q);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: p = %i\n", step_mem->p);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: istage = %i\n", step_mem->istage);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: stages = %i\n", step_mem->stages);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: maxcor = %i\n", step_mem->maxcor);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: msbp = %i\n", step_mem->msbp);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: predictor = %i\n", step_mem->predictor);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: convfail = %i\n", step_mem->convfail);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: stagetypes =");
  for (i=0; i<step_mem->stages; i++)
    STAN_SUNDIALS_FPRINTF(outfile," %i",step_mem->stagetypes[i]);
  STAN_SUNDIALS_FPRINTF(outfile,"\n");

  /* output long integer quantities */
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: nfse = %li\n", step_mem->nfse);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: nfsi = %li\n", step_mem->nfsi);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: nsetups = %li\n", step_mem->nsetups);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: nstlp = %li\n", step_mem->nstlp);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: nls_iters = %li\n", step_mem->nls_iters);

  /* output boolean quantities */
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: user_linear = %i\n", step_mem->linear);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: user_linear_timedep = %i\n", step_mem->linear_timedep);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: user_explicit = %i\n", step_mem->explicit_rhs);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: user_implicit = %i\n", step_mem->implicit_rhs);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: jcur = %i\n", step_mem->jcur);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: ownNLS = %i\n", step_mem->ownNLS);

  /* output realtype quantities */
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: Coupling structure:\n");
  MRIStepCoupling_Write(step_mem->MRIC, outfile);

  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: gamma = %"RSYM"\n", step_mem->gamma);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: gammap = %"RSYM"\n", step_mem->gammap);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: gamrat = %"RSYM"\n", step_mem->gamrat);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: crate = %"RSYM"\n", step_mem->crate);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: delp = %"RSYM"\n", step_mem->delp);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: eRNrm = %"RSYM"\n", step_mem->eRNrm);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: nlscoef = %"RSYM"\n", step_mem->nlscoef);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: crdown = %"RSYM"\n", step_mem->crdown);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: rdiv = %"RSYM"\n", step_mem->rdiv);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: dgmax = %"RSYM"\n", step_mem->dgmax);
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: Ae_row =");
  for (i=0; i<step_mem->nstages_stored; i++)
    STAN_SUNDIALS_FPRINTF(outfile," %"RSYM,step_mem->Ae_row[i]);
  STAN_SUNDIALS_FPRINTF(outfile,"\n");
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: Ai_row =");
  for (i=0; i<step_mem->nstages_stored; i++)
    STAN_SUNDIALS_FPRINTF(outfile," %"RSYM,step_mem->Ai_row[i]);
  STAN_SUNDIALS_FPRINTF(outfile,"\n");

#ifdef SUNDIALS_DEBUG_PRINTVEC
  /* output vector quantities */
  STAN_SUNDIALS_FPRINTF(outfile, "MRIStep: sdata:\n");
  N_VPrintFile(step_mem->sdata, outfile);
  STAN_SUNDIALS_FPRINTF(outfile, "MRIStep: zpred:\n");
  N_VPrintFile(step_mem->zpred, outfile);
  STAN_SUNDIALS_FPRINTF(outfile, "MRIStep: zcor:\n");
  N_VPrintFile(step_mem->zcor, outfile);
  if (step_mem->Fse)
    for (i=0; i<step_mem->nstages_stored; i++) {
      STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: Fse[%i]:\n", i);
      N_VPrintFile(step_mem->Fse[i], outfile);
    }
  if (step_mem->Fsi)
    for (i=0; i<step_mem->nstages_stored; i++) {
      STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: Fsi[%i]:\n", i);
...
int mriStep_TakeStep(void* arkode_mem, realtype *dsmPtr, int *nflagPtr)
{
 ...
 STAN_SUNDIALS_PRINTF("    MRIStep step %li,  stage 0,  h = %"RSYM",  t_n = %"RSYM"\n",
         ark_mem->nst, ark_mem->h, ark_mem->tcur);
#endif

#ifdef SUNDIALS_DEBUG_PRINTVEC
   STAN_SUNDIALS_PRINTF("    MRIStep slow stage 0 solution:\n");
    N_VPrint(ark_mem->ycur);
    if (step_mem->explicit_rhs)
    {
     STAN_SUNDIALS_PRINTF("    MRIStep slow stage RHS Fe[0]:\n");
      N_VPrint(step_mem->Fse[0]);
    }
    if (step_mem->implicit_rhs)
    {
     STAN_SUNDIALS_PRINTF("    MRIStep slow stage RHS Fi[0]:\n");
  ...
  for (is = 1; is < step_mem->stages; is++) {
   ...
   STAN_SUNDIALS_PRINTF("    MRIStep step %li,  stage %i,  h = %"RSYM",  t_n = %"RSYM"\n",
    ...
    if (ark_mem->report)
      STAN_SUNDIALS_FPRINTF(ark_mem->diagfp, "MRIStep  step  %li  %"RSYM"  %i  %"RSYM"\n",
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
   STAN_SUNDIALS_PRINTF("    MRIStep slow stage %i solution:\n",is);
    N_VPrint(ark_mem->ycur);
#endif
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
       STAN_SUNDIALS_PRINTF("    MRIStep slow stage RHS Fe[%i]:\n",is);
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
       STAN_SUNDIALS_PRINTF("    MRIStep slow stage RHS Fi[%i]:\n",is);
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
   STAN_SUNDIALS_PRINTF("    MRIStep updated solution:\n");
    N_VPrint(ark_mem->ycur);
#endif

  /* Solver diagnostics reporting */
  if (ark_mem->report)
    STAN_SUNDIALS_FPRINTF(ark_mem->diagfp, "MRIStep  etest  %li  %"RSYM"  %"RSYM"\n",
...
#ifdef SUNDIALS_DEBUG
 STAN_SUNDIALS_PRINTF("    MRIStep ERK fast stage\n");
#endif
...
#ifdef SUNDIALS_DEBUG
 STAN_SUNDIALS_PRINTF("    MRIStep ERK stage\n");
#endif
...
{
#ifdef SUNDIALS_DEBUG
 STAN_SUNDIALS_PRINTF("    MRIStep DIRK fast stage\n");
#endif
...
#ifdef SUNDIALS_DEBUG
 STAN_SUNDIALS_PRINTF("    MRIStep DIRK stage\n");
#endif
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
 STAN_SUNDIALS_PRINTF("    MRIStep predictor:\n");
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
 STAN_SUNDIALS_PRINTF("    MRIStep rhs data:\n");
...
#ifdef SUNDIALS_DEBUG_PRINTVEC
  for (k = 0; k < nmat; k++) {
   STAN_SUNDIALS_PRINTF("    MRIStep forcing[%i]:\n", k);
                                  ...
                                  FILE* outfile)
{
  ...
  STAN_SUNDIALS_FPRINTF(outfile,"MRIStepInnerStepper Mem:\n");
  ...
  if (stepper->forcing != NULL) {
    for (i = 0; i < stepper->nforcing; i++) {
      STAN_SUNDIALS_FPRINTF(outfile,"MRIStep: inner_forcing[%i]:\n", i);
