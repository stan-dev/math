...
template<class T> struct rem_cvref {
...
///as a tag for the free functions below. Without this class, the free functions
///would have to be written to extend std::complex, which would change its
///semantics, or would have to be specialized to only stan types.
template<class T> struct complex: std::complex<T>{
	using std::complex<T>::complex;///< inherit all std::complex ctors
 complex(const std::complex<T>&t):std::complex<T>(t){};///< allow promotion
 /**without this converting ctor, copy initialization of std::complex<z_(f)var>
  * from an (f)var fails, because the default implementation in the std::complex
  * template requires the argument type to match the template type. This is
  * the same reason why std::complex<double> can't be multiplied by an int
  * tparam R type of real argument
...
//The free functions that follow extend stan::math::internal::complex beyond the
//interface of std::complex. This is necessary because the std::complex
...
inline complex<T> operator+(complex<T> t,U u){return t+=u;}

template<class T,class U>
inline complex<T> operator+(U u,complex<T> t){return t+=u;}
...
template<class T,class U>
inline complex<T> operator-(U u, complex<T> t){return (t-=u)*(-1);}

template<class T,class U>
inline complex<T>	operator/(complex<T> t, U u){return t/=u;}

template<class T,class U>
inline complex<T> operator/(U u, complex<T> t){return complex<T>(u)/=t;}
...
template<class T,class U>
inline bool	operator==(std::complex<U> u,complex<T> t){return complex<T>(u)==t;}

template<class T,class U>
inline bool	operator==(complex<T> t,std::complex<U> u){return complex<T>(u)==t;}

template<class T,class U>
inline bool	operator==(complex<T> t, U u){return t.real()==u && t.imag()==T();}

template<class T,class U>
inline bool	operator==(U u, complex<T> t){return t.real()==u && t.imag()==T();}

template<class T,class U>
inline bool	operator!=(complex<T> t, U u){return !(t==u);}

template<class T,class U>
inline bool	operator!=(U u, complex<T> t){return !(t==u);}
