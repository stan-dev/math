...
template <typename Mat1, typename Mat2,
          ...
          typename = require_not_eigen_row_and_col_t<Mat1, Mat2>, unsigned int = 0>
...
inline auto multiply(const Mat1& m1, const Mat2& m2) {
  check_multiplicable("multiply", "m1", m1, "m2", m2);
  Eigen::Matrix<value_type_t<Mat1>, Mat1::RowsAtCompileTime, Mat2::ColsAtCompileTime>
      result(m1.rows(), m2.cols());
  for (size_type i = 0; i < m1.rows(); i++) {
    Eigen::Matrix<value_type_t<Mat1>, 1, Mat1::ColsAtCompileTime> crow = m1.row(i);
...
inline auto multiply(const Mat1& m1, const Mat2& m2) {
  check_multiplicable("multiply", "m1", m1, "m2", m2);
  Eigen::Matrix<value_type_t<Mat2>, Mat1::RowsAtCompileTime, Mat2::ColsAtCompileTime>
      result(m1.rows(), m2.cols());
  for (size_type i = 0; i < m1.rows(); i++) {
