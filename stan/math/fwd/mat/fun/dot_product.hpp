...
inline fvar<T> dot_product(const Eigen::Matrix<fvar<T>, R1, C1>& v1,
  ...
  fvar<T> ret(0, 0);
  for (size_type i = 0; i < v1.size(); i++) ret += v1(i) * v2(i);
  return ret;
}
...
inline fvar<T> dot_product(const Eigen::Matrix<fvar<T>, R1, C1>& v1,
  ...
  fvar<T> ret(0, 0);
  for (size_type i = 0; i < length; i++) ret += v1(i) * v2(i);
  return ret;
}
...
inline fvar<T> dot_product(const std::vector<fvar<T> >& v1,
  ...
  check_matching_sizes("dot_product", "v1", v1, "v2", v2);
  fvar<T> ret(0, 0);
  for (size_t i = 0; i < v1.size(); i++) ret += v1.at(i) * v2.at(i);
  return ret;
}
...
inline fvar<T> dot_product(const std::vector<fvar<T> >& v1,
  ...
  fvar<T> ret(0, 0);
  for (size_type i = 0; i < length; i++) ret += v1.at(i) * v2.at(i);
  return ret;
}
