...
namespace math {
   ...
   * @return A Matrix with automatic differentiation variables
   */
  template <typename T1, int R1, int C1, typename T2, int R2, int C2, enable_if_fvar<T2>* = nullptr>
  inline Eigen::Matrix<T2, R1, C1> to_fvar(const Eigen::Matrix<T1, R1, C1>& m1, const Eigen::Matrix<T2, R2, C2>& m2) {
   ...
   * @return A Matrix with automatic differentiation variables
   */
  template <typename T1, typename T2, int R2, int C2, enable_if_fvar<T2>* = nullptr>
  inline T1 to_fvar(const T1& m1, const Eigen::Matrix<T2, R2, C2>& m2) {
    T2 m_v(m1);
    return m_v;
  }

  template <typename T1, int R1, int C1, typename T2, typename = std::enable_if_t<ad_promotable<T1, T2>::value>>
  inline Eigen::Matrix<T2, R1, C1> to_fvar(const Eigen::Matrix<T1, R1, C1>& m1, const T2& m2) {
    Eigen::Matrix<T2, R1, C1> m_v = m1;
    return m_v;
  }
