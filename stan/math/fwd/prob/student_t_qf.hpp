...
FvarT student_t_qf(const T_p& p, const T_mu& mu,
  ...
  FvarT rtn = mu_val + p_sign * sigma_val * sqrt(nu_val)
                * math::sqrt(-1.0 + 1.0 / ibeta_arg);

  if constexpr (!is_constant<T_p>::value) {
    rtn.d_ += p.d_ * exp(log(p_val) - student_t_lpdf(rtn, mu_val, sigma_val, nu_val));
  }
  ...
  if constexpr (!is_constant<T_nu>::value) {
    ...
    const T_partials hyper_arg = hypergeometric_3F2({0.5, half_nu, half_nu}, {1 + half_nu, 1 + half_nu}, ibeta_arg);
    ...
    const T_partials num1 = p_sign * (-2 + (2 - arg_1 + arg_2) / ibeta_arg);
    const T_partials den1 = 4 * sqrt(nu_val) * sqrt(-1 + 1 / ibeta_arg);
    nu.d_ += rtn.d_ * p_sign * num1 / den1;
  }
