...
inline fvar<T> owens_t(const fvar<T> &x1, const fvar<T> &x2) {
  ...
  return fvar<T>(owens_t(x1.val_, x2.val_),
                 -x1.d_ * (erf(x2.val_ * x1.val_ * INV_SQRT_2) *
 ...
 * @return Owen's T function applied to the specified arguments.
 */
template <typename T> inline fvar<T> owens_t(double x1, const fvar<T> &x2) {
  using std::exp;

  T neg_x1_sq_div_2 = -square(x1) * 0.5;
  T one_p_x2_sq = 1.0 + square(x2.val_);
  return fvar<T>(owens_t(x1, x2.val_), x2.d_ *
 ...
 * @return Owen's T function applied to the specified arguments.
 */
template <typename T> inline fvar<T> owens_t(const fvar<T> &x1, double x2) {
  using std::exp;
