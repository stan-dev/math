...
namespace stan {
  namespace math {
    ...
    inline fvar<T> pow(const fvar<T>& x1, const fvar<T>& x2) {
      ...
      return fvar<T>(
          pow_x1_x2,
          (x2.d_ * log(x1.val_) + x2.val_ * x1.d_ / x1.val_) * pow_x1_x2);
    }
    ...
    inline fvar<T> pow(const fvar<T>& x1, double x2) {
      ...
      if (x2 == -2) return inv_square(x1);
      if (x2 == -1) return inv(x1);
      if (x2 == -0.5) return inv_sqrt(x1);
      if (x2 == 0.5) return sqrt(x1);
      if (x2 == 1.0) return x1;
      if (x2 == 2.0) return square(x1);
