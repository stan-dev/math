...
const char *cholesky_decompose_kernel_code = STRINGIFY(
    ...
    __kernel void cholesky_decompose(__global double *A, __global double *B,
      ...
      // The following code is the sequential version of the non-inplace cholesky
      // decomposition. Only the innermost loops are parallelized.
      // The rows are processed sequentially. This loop process all the rows:
      for (int j = 0; j < rows; j++) {
        //one thread calculates the 		
        if (index == 0) {
            double sum = 0;
            for (int k = 0; k < j; k++) {
            sum += B(j, k) * B(j, k);
        ...
        if (index >= (j + 1) && index < rows) {
          double s = 0;
          for (int k = 0; k < j; k++) {
            s += B(index, k) * B(j, k);
          }
          B(index, j) = (1.0 / B(j, j) * (A(index, j) - s));
        }
        barrier(CLK_LOCAL_MEM_FENCE);
      }      
    }
...
/**
 * See the docs for \link kernels/cholesky_decompose.hpp cholesky_decompose() \endlink
 */
