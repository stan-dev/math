...
__kernel void multiply_transpose(const __global double* A,
  ...
  // global thread index
  const int i
      = WORKGROUP_SIZE_MULT_SELF_TRANS * get_group_id(0) + workgroup_row;
  const int j
      = WORKGROUP_SIZE_MULT_SELF_TRANS * get_group_id(1) + workgroup_col;
  ...
  const int iMax
      = WORKGROUP_SIZE_MULT_SELF_TRANS * get_group_id(0) + get_local_size(0);

  // local memory
  __local double A_local[WORKGROUP_SIZE_MULT_SELF_TRANS]
                        [WORKGROUP_SIZE_MULT_SELF_TRANS];
  __local double B_local[WORKGROUP_SIZE_MULT_SELF_TRANS]
                        [WORKGROUP_SIZE_MULT_SELF_TRANS];
  ...
  const int numTiles = (N + WORKGROUP_SIZE_MULT_SELF_TRANS - 1)
                       / WORKGROUP_SIZE_MULT_SELF_TRANS;
  ...
  for (int t = 0; t < numTiles; t++) {
    ...
    if (jMin <= iMax) {
      // each thread copies WORK_PER_THREAD_MULT_SELF_TRANS values to the local
      // memory
      for (int w = 0; w < WORK_PER_THREAD_MULT_SELF_TRANS; w++) {
        A_local[workgroup_col + w * WORKGROUP_SIZE_MULT_SELF_TRANS_COL]
               [workgroup_row]
            = A[i + (tiled_j + w * WORKGROUP_SIZE_MULT_SELF_TRANS_COL) * M];
        B_local[workgroup_col + w * WORKGROUP_SIZE_MULT_SELF_TRANS_COL]
               [workgroup_row]
            = A[(j + w * WORKGROUP_SIZE_MULT_SELF_TRANS_COL) + tiled_i * M];
      }
    }
          ...
          if ((j + w * WORKGROUP_SIZE_MULT_SELF_TRANS_COL) <= i) {
            acc[w] += A_local[k][workgroup_row]
                      * B_local[workgroup_col
                                + w * WORKGROUP_SIZE_MULT_SELF_TRANS_COL][k];
          }
        }
      }
    }
