...
namespace stan {
  namespace math {
    ...
    inline matrix_gpu lower_triangular_inverse(const matrix_gpu & A) {
      ...
      int A_rows_padded = ((A.rows()+thread_block_2D_dim-1)/thread_block_2D_dim)*thread_block_2D_dim;
      ...
      int wpt = opencl_kernels::lower_tri_inverse_step2.make_functor.get_opts().at("WORK_PER_THREAD");
      int parts = (inv_padded.rows()+thread_block_size_1D-1)/thread_block_size_1D;
      try {
        opencl_kernels::lower_tri_inverse_step1(
            cl::NDRange(parts*thread_block_size_1D),
            cl::NDRange(thread_block_size_1D),
            inv.buffer(), temp.buffer(), inv.rows());
      } catch (cl::Error& e) {
      ...
      while (parts > 1) {
        ...
        opencl_kernels::lower_tri_inverse_step2(
          ...
          inv_padded.buffer(), temp.buffer(), inv_padded.rows(), result_matrix_dim, result_matrix_dim );
      ...
      if (parts>0) {
        opencl_kernels::lower_tri_inverse_step2(
          cl::NDRange(inv.rows()-result_matrix_dim, result_matrix_dim/wpt, parts),
          cl::NDRange(thread_block_2D_dim, thread_block_2D_dim/wpt, 1),
          inv_padded.buffer(), temp.buffer(), inv_padded.rows(), inv.rows()-result_matrix_dim, result_matrix_dim );
        opencl_kernels::lower_tri_inverse_step3(
          cl::NDRange(inv.rows()-result_matrix_dim, result_matrix_dim/wpt, parts),
          cl::NDRange(thread_block_2D_dim, thread_block_2D_dim/wpt, 1),
          inv_padded.buffer(), temp.buffer(), inv_padded.rows(), result_matrix_dim, result_matrix_dim );
