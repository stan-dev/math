...
namespace math {
   ...
   * 
   * FIX ME - address case where the covariance matrix has a spatial
   * structure.
   * FIX ME - split the global parameter into phi and rho.
   */
  template <typename T0, typename T1>
  Eigen::Matrix<stan::return_type<T0, T1>::type,
                Eigen::Dynamic, Eigen::Dynamic>
  covariance (T0 phi, T1 rho, int M) {
    Eigen::Matrix<stan::return_type<T0, T1>::type,
                  Eigen::Dynamic, Eigen::Dynamic> Sigma(M, M);
  ...
  struct lgp_dense_system {
    ...
    struct deriv_objective {
      ...
      operator ()(const T& phi) const {
        Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>
          Sigma = covariance(phi, rho_, M_);
    ...
    solver_gradient(const Eigen::Matrix<T1, Eigen::Dynamic, 1>& theta) const {
      ...
      Eigen::Matrix<T0, Eigen::Dynamic, 1> phi_vec;
      phi_vec(0) = phi_;
      jacobian(f, phi_vec, dummy, phi_sensitivities);
