 ...
 */
template <typename LFun, typename EtaVec, typename DeltaVec, typename CovarFun, typename ThetaVec,
          typename RNG, typename TrainTuple, typename PredTuple, typename... Args>
inline Eigen::VectorXd laplace_marginal_tol_rng(
    LFun&& L_f, const EtaVec& eta,
    const DeltaVec& delta_L, const std::vector<int>& delta_int_L,
  ...
  return laplace_base_rng(diff_likelihood<LFun>(std::forward<LFun>(L_f),
                          ...
                          K_f, eta, theta_0, rng, msgs, tolerance,
                          max_num_steps, hessian_block_size, solver,
                          max_steps_line_search, std::forward<TrainTuple>(train_tuple), std::forward<PredTuple>(pred_tuple),
...
inline Eigen::VectorXd laplace_marginal_rng(
  ...
  return laplace_base_rng(diff_likelihood<LFun>(std::forward<LFun>(L_f),
                          ...
                          std::forward<TrainTuple>(train_tuple), std::forward<PredTuple>(pred_tuple),
                          std::forward<Args>(args)...);
}
