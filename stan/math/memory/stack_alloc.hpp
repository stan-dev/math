...
class stack_alloc {
 private:
  ...
  char* move_to_next_block(size_t len) {
    ...
    if (unlikely(cur_block_ >= blocks_.size())) {
      ...
      if (!blocks_.back()) throw std::bad_alloc();
 ...
 public:
  ...
  inline void* alloc(size_t len) {
    ...
    if (unlikely(next_loc_ >= cur_block_end_)) result = move_to_next_block(len);
  ...
  inline void recover_nested() {
    if (unlikely(nested_cur_blocks_.empty())) recover_all();
  ...
  inline bool in_stack(const void* ptr) const {
    for (size_t i = 0; i < cur_block_; ++i)
      if (ptr >= blocks_[i] && ptr < blocks_[i] + sizes_[i]) return true;
    if (ptr >= blocks_[cur_block_] && ptr < next_loc_) return true;
    return false;
  }
};
