...
inline auto copy_and_promote(Args&&... args) {
  return filter_map<is_any_var_scalar>([](auto&& arg){
    if constexpr (is_tuple<std::decay_t<decltype(arg)>>::value) {
      return copy_and_promote(std::forward<decltype(arg)>(arg));
    } else {
      return promote_scalar<PromotedType>(value_of(std::forward<decltype(arg)>(arg)));
    }
  }, args...);
}
...
inline auto diff(F&& f, const Theta& theta,
  ...
  Eigen::Matrix<double, Theta::RowsAtCompileTime, Theta::ColsAtCompileTime>
      theta_gradient;
  const auto eta_size = count_vars(args...);
  constexpr bool contains_var = is_any_var_scalar<Args...>::value;
  Eigen::Matrix<double, contains_var ? -1 : 0, 1> eta_gradient{eta_size};
  {
    nested_rev_autodiff nested;
    Matrix<var, Dynamic, 1> theta_var = theta;
    auto hard_copy_args = copy_and_promote<var>(args...);
    ...
    serialize_adjoints(eta_gradient, hard_copy_args);
  }
...
inline auto compute_s2(F&& f, const Theta& theta,
  ...
  auto copy_vargs = copy_and_promote<var>(args...);
  for (Eigen::Index i = 0; i < hessian_block_size; ++i) {
    ...
    auto hard_copy_args = stan::math::filter_map<is_any_var_scalar>([](auto&& arg){
      return copy_and_promote<fvar<fvar<var>>>(arg);
    }, copy_vargs);
  ...
  serialize_adjoints(eta_gradient, copy_vargs);
...
inline auto diff_eta_implicit(F&& f, const V_t& v,
  ...
  nested_rev_autodiff nested;
  const Eigen::Index eta_size = count_vars(args...);
  auto copy_vargs = copy_and_promote<var>(args...);
  ...
  auto hard_copy_args = stan::math::filter_map<is_any_var_scalar>([](auto&& arg){
      return copy_and_promote<fvar<var>>(arg);
  }, copy_vargs);
  ...
  grad(f_fvar.d_.vi_);
  Eigen::Matrix<double, -1, 1> eta_gradient{eta_size};
  serialize_adjoints(eta_gradient, copy_vargs);
  return eta_gradient;
}
