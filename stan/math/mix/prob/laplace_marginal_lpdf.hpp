 ...
 * @param[in] args A parameter pack of arguments to be send to the covariance function
 */
...
inline auto laplace_marginal_tol_lpdf(
    LFun&& L_f, LArgs&& l_args, const EtaVec& eta,
 ...
 */
template <bool propto = false, typename LFun, typename LArgs, typename EtaVec,
          typename CovarFun, typename Theta0,
          typename... Args>
inline auto laplace_marginal_tol_lpmf(
  ...
  return laplace_marginal_tol_lpdf<propto>(
      std::forward<LFun>(L_f), std::forward<LArgs>(l_args), eta,
      tolerance, max_num_steps, hessian_block_size, solver,
      max_steps_line_search, theta_0, std::forward<CovarFun>(K_f),
...
inline auto laplace_marginal_tol_lpmf(
    LFun&& L_f, LArgs&& l_args, const Theta0& theta_0,
    CovarFun&& K_f, const double tolerance,
    const long int max_num_steps, const int hessian_block_size,
    const int solver, const int max_steps_line_search,
    std::ostream* msgs, Args&&... args) {
  return laplace_marginal_tol_lpdf<propto>(
      std::forward<LFun>(L_f), std::forward<LArgs>(l_args),
      theta_0, std::forward<CovarFun>(K_f),
      tolerance, max_num_steps, hessian_block_size, solver,
