...
inline auto laplace_marginal_tol_lpdf(
  ...
  return laplace_marginal_density(std::forward<LFun>(L_f),
      std::forward_as_tuple(y, delta_int_L),
      std::forward<CovarFun>(K_f), eta, theta_0, msgs, ops,
      std::forward<Args>(args)...);
}

template <bool propto = false, typename YVec, typename LFun, typename CovarFun,
          typename Theta0, typename... Args,
          require_all_eigen_vector_t<YVec, Theta0>* = nullptr>
inline auto laplace_marginal_tol_lpdf(
    const YVec& y, LFun&& L_f, const std::vector<int>& delta_int_L,
 ...
 * of double is passed as data.
 */
template <bool propto = false, typename LFun, typename CovarFun,
          typename DeltaLVec, typename Theta0, typename... Args>
inline auto laplace_marginal_tol_lpmf(
    const std::vector<int>& y, LFun&& L_f, const DeltaLVec& delta_L,
