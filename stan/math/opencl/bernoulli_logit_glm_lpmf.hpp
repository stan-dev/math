 ...
 * @param y_cl binary scalar or vector parameter on OpenCL device. If it is a scalar it will be
...
return_type_t<T_alpha, T_beta> bernoulli_logit_glm_lpmf(
  ...
  try {
    opencl_kernels::bernoulli_logit_glm(
        ...
        beta_cl, N, M, y_cl.size() != 1, length(alpha) != 1, need_theta_derivative,
