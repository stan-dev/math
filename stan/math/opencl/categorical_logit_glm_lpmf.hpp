 ...
 * @param x design matrix or row vector. This overload does not support broadcasting of a row vector x!
...
return_type_t<T_alpha_scalar, T_beta_scalar>
categorical_logit_glm_lpmf(
  ...
  = partials_return_t<T_alpha_scalar, T_beta_scalar>;
  ...
  const int local_size = opencl_kernels::categorical_logit_glm.get_option("LOCAL_SIZE_");
  ...
  matrix_cl<double> neg_softmax_lin_cl(need_alpha_derivative || need_beta_derivative ? N_instances : 0, N_classes);
  matrix_cl<double> alpha_derivative_cl(need_alpha_derivative ? wgs : 0, N_classes);

  try{
    opencl_kernels::categorical_logit_glm(cl::NDRange(local_size * wgs), cl::NDRange(local_size),
                                          logp_cl, exp_lin_cl, inv_sum_exp_lin_cl, neg_softmax_lin_cl,
                                          alpha_derivative_cl,
                                          y_cl, x_beta_cl, alpha_cl,
                                          N_instances, N_attributes, N_classes,
  ...
  if (!std::isfinite(logp)) {
    check_bounded(function, "categorical outcome out of support", from_matrix_cl(y_cl), 1,
                  N_classes);
  ...
  if (!is_constant_all<T_alpha_scalar>::value) {
    ops_partials.edge1_.partials_ = from_matrix_cl(alpha_derivative_cl).colwise().sum();
  }
  if (!is_constant_all<T_beta_scalar>::value) {
    ...
    try {
      opencl_kernels::categorical_logit_glm_beta_derivative(cl::NDRange(local_size * N_attributes), cl::NDRange(local_size),
                                                            beta_derivative_cl, temp, y_cl, x_cl,
                                                            N_instances, N_attributes, N_classes, y_cl.size() != 1);
    }
    catch (const cl::Error& e) {
      check_opencl_error(function, e);
    }
