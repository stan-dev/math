 ...
 * <p>\f$A = L \times L^T\f$.
 * The Cholesky decomposition is computed with OpenCL. This algorithm is
 * recursive. The matrix is subset into a matrix of size
 *  <code>A.rows() / 4</code>, and if the submatrix size is less than
 * 50 or <code>min_block</code> then the OpenCL cholesky decomposition
 ...
 * @param A Symmetric OpenCL matrix.
 * @return Square root of the OpenCL matrix.
...
inline matrix_cl cholesky_decompose(matrix_cl& A) {
  ...
  check_nan("cholesky_decompose_cl", "Matrix m", A);
  check_diagonal_zeros("cholesky_decompose_cl", "Matrix m", A);
  return A;
}
