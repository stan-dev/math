 ...
 */
inline matrix_cl diagonal_multiply(matrix_cl& A, const double scalar) {
  matrix_cl B(A);
  if (B.size() == 0)
    return B;
  ...
  if (B.cols() < min_dim)
    min_dim = B.cols();
  try {
    cl::Event mul_event = opencl_kernels::scalar_mul_diagonal(
        cl::NDRange(min_dim), B, scalar, B.rows(), min_dim);
    B.add_event(mul_event);
