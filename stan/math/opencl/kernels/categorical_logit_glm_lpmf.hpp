...
// \endcond
         ...
         * @param[out] logp_global partially summed log probabiltiy (1 value per work group)
         * @param[out] exp_lin_global exponentiation of sum of alpha and matrix product of x and beta
         * @param[out] inv_sum_exp_lin_global inverse of rowwise sum of \c exp_lin_global
         * @param[out] neg_softmax_lin_global negated softmax of sum of alpha and matrix product of x and beta
         ...
         * @param is_y_vector 0 or 1 - whether y is a vector (alternatively it is a scalar)
         * @param need_alpha_derivative interpreted as boolean - whether alpha_derivative needs to be computed
         * @param need_neg_softmax_lin_global interpreted as boolean - whether neg_softmax_lin_global needs to be computed
         */
        __kernel void categorical_logit_glm(__global double* logp_global, __global double* exp_lin_global, __global double* inv_sum_exp_lin_global,
                                            __global double* neg_softmax_lin_global, __global double* alpha_derivative,
                                            const __global int* y_global, const __global double* x_beta_global, const __global double* alpha_global,
                                          const int N_instances, const int N_attributes, const int N_classes, const int is_y_vector,
                                          const int need_alpha_derivative, const int need_neg_softmax_lin_global) {
...
/**
 * See the docs for \link kernels/categorical_logit_glm.hpp categorical_logit_glm() \endlink
 */
const kernel_cl<out_buffer, out_buffer, out_buffer, out_buffer, out_buffer, in_buffer, in_buffer, in_buffer, int, int, int, int, int, int>
        categorical_logit_glm("categorical_logit_glm", {categorical_logit_glm_kernel_code}, {{"REDUCTION_STEP_SIZE", 4}, {"LOCAL_SIZE_", 64}});

// \cond
static const std::string categorical_logit_glm_beta_derivative_kernel_code = STRINGIFY(
// \endcond
/**
 ...
 * @param is_y_vector 0 or 1 - whether y is a vector (alternatively it is a scalar)
 */
        __kernel void categorical_logit_glm_beta_derivative(__global double* beta_derivative, __global double* temp, const __global int* y, const __global double* x,
                                                            const int N_instances, const int N_attributes, const int N_classes, const int is_y_vector) {
          const int gid = get_global_id(0);
          const int lid = get_local_id(0);
          const int lsize = get_local_size(0);
...
/**
 * See the docs for \link kernels/categorical_logit_glm.hpp categorical_logit_glm_beta_derivative() \endlink
 */
const kernel_cl<in_out_buffer, in_out_buffer, in_buffer, in_buffer, int, int, int, int>
        categorical_logit_glm_beta_derivative("categorical_logit_glm_beta_derivative", {categorical_logit_glm_beta_derivative_kernel_code});
