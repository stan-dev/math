#ifndef STAN_MATH_OPENCL_KERNELS_MULTIPLY_RECT_HPP
#define STAN_MATH_OPENCL_KERNELS_MULTIPLY_RECT_HPP
#ifdef STAN_OPENCL
...
// \cond
static const char* multiply_rect_kernel_code = STRINGIFY(
    // \endcond
    /**
    ...
    __kernel void multiply_rect(
      ...
      const int Kpad = ((K + THREAD_BLOCK_SIZE - 1) / THREAD_BLOCK_SIZE)
                                                         * THREAD_BLOCK_SIZE;
      const int num_tiles = Kpad / THREAD_BLOCK_SIZE;
...
/**
 * See the docs for \link kernels/multiply_rect.hpp multiply_rect() \endlink
 */
const local_range_kernel<cl::Buffer, cl::Buffer, cl::Buffer, int, int, int,
                         TriangularViewCL, TriangularViewCL>
    multiply_rect("multiply_rect", multiply_rect_kernel_code,
