...
static const char* ordered_logistic_kernel_code = STRINGIFY(
     ...
     * LOCAL_SIZE_.
     * @param[out] lambda_sum partially summed lambda (1 value per work
     * group)
     ...
     * scalar)
     * @param need_lambda_derivative interpreted as boolean - whether
     * lambda_derivative needs to be computed
    ...
    __kernel void ordered_logistic(
        __global double* lambda_sum, __global double* logp_global,
      ...
      if (lid == 0) {
        lambda_sum[wg_id] = local_storage[0];
      }
    }
    // \cond
);
