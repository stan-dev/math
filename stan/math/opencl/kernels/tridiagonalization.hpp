  ...
  for (int i = start; i < end; i += 1) {
    ...
    static constexpr const char* tridiagonalization_v_step_1_kernel_code
        = STRINGIFY(
            // \endcond
            /**
             * Calculates first part of constructing the vector v: Uu = Pb * u
             * and Vu = Vl * u. Pb is a block of packed matrix, Vl is left part
             * of matrix V and u is householder vector. Must be run with number
             * of work groups equal to size of resulting vectors and 64 threads
             * per work group.
             * @param P Packed matrix being constructed.
             * @param V Matrix V.
             * @param[out] Uu First resulting vector.
             ...
             * @param P_rows Number of rows of the packed matrix
             * @param V_rows Number of rows of the matrix V
             * @param k Index of the householder vector in the block we use as
             * input
             */
            __kernel void tridiagonalization_v_step_1(
              ...
              const __global double* M1
                  = P + P_rows * (k + wgid) + k + ngroups + 1;
              const __global double* M2 = V + V_rows * wgid + ngroups;
              for (int i = lid; i < P_rows - k - ngroups - 1;
                   i += LOCAL_SIZE_) {  // go over column of the matrix in steps
                                        // of 64
                double v = vec[i];
                acc1 += M1[i] * v;
                acc2 += M2[i] * v;
...
<<<<<<< HEAD
    ...
    static constexpr const char* tridiagonalization_v_step_2_kernel_code
        = STRINGIFY(
            // \endcond
            /**
             * Second part in constructing vector v: v = Pb * u + V * Uu + U *
             * Vu. Pb is a block of packed matrix and U is householder vector.
             * Pb is symmetric with only lower triangle having values. That is
             * why two columns of V are written that must be added to obtain the
             * vector v. Must be run with 64 threads per work group and total
             * number of threads equal or greater than size of result vector.
             * @param P Packed matrix being constructed.
             * @param V Matrix V.
             * @param Uu Uu from previous kernel.
             * @param Vu Vu from previous kernel.
...
>>>>>>> develop
    ...
    const kernel_cl<in_out_buffer, in_out_buffer, out_buffer, int, int, int,
                    int>
