...
inline matrix_cl lower_triangular_inverse(const matrix_cl& A) {
  ...
  zero_mat.zeros<stan::math::TriangularViewOpencl::Entire>();
  temp.zeros<stan::math::TriangularViewOpencl::Entire>();
  inv_padded.zeros<stan::math::TriangularViewOpencl::Entire>();
  ...
  inv_padded.sub_block(zero_mat, 0, 0, inv_mat.rows(), 0, zero_mat.rows(),
                       zero_mat.cols());
  inv_padded.zeros<stan::math::TriangularViewOpencl::Upper>();
  if (parts == 1) {
