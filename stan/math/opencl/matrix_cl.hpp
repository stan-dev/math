 ...
 public:
  ...
  explicit matrix_cl(Vec&& A) try : rows_(A.empty() ? 0 : A[0].size()),
    ...
    for (int i = 0, offset_size = 0; i < cols_; i++, offset_size += rows_) {
      ...
      queue.enqueueWriteBuffer(buffer_cl_, opencl_context.in_order(),
                               sizeof(double) * offset_size, sizeof(T) * rows_,
                               A[i].data(), nullptr, &write_event);
  ...
  explicit matrix_cl(const T&& A,
    ...
    try {
      ...
      cl::Event transfer_event;
      queue.enqueueWriteBuffer(buffer_cl_, std::is_rvalue_reference<T&&>::value,
                               0, sizeof(std::decay_t<T>), &A, nullptr,
                               &transfer_event);
      this->add_write_event(transfer_event);
    } catch (const cl::Error& e) {
      check_opencl_error("matrix constructor", e);
