 ...
 */
template <typename T_n_cl, typename T_prob_cl,
...
return_type_t<T_prob_cl> bernoulli_cdf(const T_n_cl& n,
  ...
  auto Pi = select(n >= 1, 0, 1.0 - theta_val);
  auto P_expr = colwise_prod(Pi);

  matrix_cl<double> Pi_cl;
  matrix_cl<double> Pi_sum_cl;
  matrix_cl<double> P_cl;

  results(check_theta_bounded, Pi_cl, Pi_sum_cl, P_cl) = expressions(
      theta_bounded_expr,
      calc_if<(!is_constant_all<T_prob_cl>::value && is_theta_vector)>(Pi),
      calc_if<(!is_constant_all<T_prob_cl>::value && !is_theta_vector)>(
          Pi_sum_cl),
      P_expr);
  ...
  if (!is_constant_all<T_prob_cl>::value) {
    if (is_theta_vector) {
      ops_partials.edge1_.partials_ = -P / Pi;
    } else {
      ops_partials.edge1_.partials_ = -P / sum(from_matrix_cl(Pi_sum_cl));
    }
  }
