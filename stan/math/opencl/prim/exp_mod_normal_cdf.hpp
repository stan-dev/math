...
inline opencl::scalar_cl_return_t<T_y_cl, T_loc_cl, T_scale_cl, T_inv_scale_cl>
exp_mod_normal_cdf(const T_y_cl& y, const T_loc_cl& mu, const T_scale_cl& sigma,
  ...
  auto ops_partials
      = opencl::make_partials_propagator(y_col, mu_col, sigma_col, lambda_col);
  if constexpr (is_any_autodiff_v<T_y_cl, T_loc_cl, T_scale_cl,
    ...
    auto mu_deriv = elt_multiply(
        ...
        opencl::internal::as_operand(cdf));
    auto y_deriv = -mu_deriv;
    auto sigma_deriv = elt_multiply(
        static_select<is_constant_v<T_scale_cl>>(0, sigma_deriv_cl),
        opencl::internal::as_operand(cdf));
    auto lambda_deriv = elt_multiply(
        static_select<is_constant_v<T_inv_scale_cl>>(0, lambda_deriv_cl),
        opencl::internal::as_operand(cdf));
