...
exp_mod_normal_lpdf(const T_y_cl& y, const T_loc_cl& mu,
  ...
  auto q_expr = 0.5 * elt_multiply(a_expr, a_expr)
                - elt_multiply(a_expr, z_expr);
  ...
  auto log_exp_cdf_expr = select(
      use_erfcx,
      -0.5 * elt_multiply(z_expr, z_expr) + LOG_HALF + log(erfcx),
  ...
  auto mills_excess_expr
      = select(erfc_arg >= 20.0, mills_excess_asymptotic,
               select(use_erfcx, mills_excess_erfcx,
                      mills_u_expr - (a_expr - z_expr)));
  ...
  auto deriv_sigma_expr
      = elt_multiply(-elt_multiply(z_expr, dz_expr)
                         + elt_multiply(a_expr, da_expr),
                     inv_sigma_expr);
  ...
  results(logp_cl, y_deriv_cl, mu_deriv_cl, sigma_deriv_cl, lambda_deriv_cl)
      = expressions(logp_expr, calc_if<is_autodiff_v<T_y_cl>>(
