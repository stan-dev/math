 ...
 * @return The log of the product of the densities, as a device scalar.
...
inline opencl::scalar_cl_return_t<T_y_cl, T_loc_cl, T_scale_cl> normal_lpdf(
  ...
  results(check_y_not_nan, check_mu_finite, check_sigma_positive, logp_cl,
      ...
      = expressions(y_not_nan, mu_finite, sigma_positive, logp_expr,
                    calc_if<is_autodiff_v<T_y_cl>>(-scaled_diff),
                    calc_if<is_autodiff_v<T_loc_cl>>(scaled_diff),
                    calc_if<is_autodiff_v<T_scale_cl>>(sigma_deriv));

  opencl::ScalarCl<double> logp(matrix_cl<double>(1, 1));
  opencl::internal::sum_into(
      logp, logp_cl, false,
      include_summand<propto>::value ? NEG_LOG_SQRT_TWO_PI * N : 0.0);
