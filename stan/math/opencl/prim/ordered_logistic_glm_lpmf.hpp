...
return_type_t<T_x, T_beta, T_cuts> ordered_logistic_glm_lpmf(
  ...
  if (!is_constant_all<T_beta>::value) {
    ...
    stan::math::edge<1>(ops_partials).partials_ = matrix_cl<double>(
        edge2_partials_transpose.buffer(), edge2_partials_transpose.cols(),
        edge2_partials_transpose.rows());
    if (beta.rows() != 0) {
      stan::math::edge<1>(ops_partials).partials_.add_write_event(
          edge2_partials_transpose.write_events().back());
    }
  }
  if (!is_constant_all<T_cuts>::value) {
    if (wgs == 1) {
      stan::math::edge<2>(ops_partials).partials_ = std::move(cuts_derivative_cl);
    } else {
      stan::math::edge<2>(ops_partials).partials_ = rowwise_sum(cuts_derivative_cl);
    }
  }
