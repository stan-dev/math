#ifndef STAN_MATH_OPENCL_PRIM_DOUBLE_SKEW_DOUBLE_EXPONENTIAL_CDF_HPP
#define STAN_MATH_OPENCL_PRIM_DOUBLE_SKEW_DOUBLE_EXPONENTIAL_CDF_HPP
#ifdef STAN_OPENCL
...
skew_double_exponential_cdf(const T_y_cl& y, const T_loc_cl& mu,
  ...
  auto cdf_expr = colwise_prod(
      select(y_val <= mu_val, elt_multiply(tau_val, exp(-2.0 * expo)),
             1.0 + elt_multiply(tau_minus_1, exp(-2.0 * expo))));
  ...
  if constexpr (!is_constant_all<T_y_cl, T_loc_cl, T_scale_cl, T_skewness_cl>::value) {
    ...
    if constexpr (!is_constant<T_skewness_cl>::value) {
