...
inline return_type_t<T_y_cl, T_dof_cl, T_loc_cl, T_scale_cl> student_t_lpdf(
  ...
  auto ops_partials = partials_propagator(y_col, nu_col, mu_col, sigma_col);
  ...
  if (!is_constant<T_loc_cl>::value) {
    stan::math::edge<2>(ops_partials).partials_ = std::move(mu_deriv_cl);
  }
  if (!is_constant<T_scale_cl>::value) {
    stan::math::edge<3>(ops_partials).partials_ = std::move(sigma_deriv_cl);
  }
