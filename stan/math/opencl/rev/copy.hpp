...
namespace math {
  ...
  inline Eigen::Matrix<var, Eigen::Dynamic, Eigen::Dynamic> from_matrix_cl(
    ...
    try {
      ...
      queue.enqueueReadBuffer(src.val().buffer(), CL_FALSE, 0,
                              sizeof(double) * dst.size(), dst.val().eval().data(),
                              &src.write_events(), &copy_event);
      queue.enqueueReadBuffer(src.adj().buffer(), CL_FALSE, 0,
                              sizeof(double) * dst.size(), dst.adj().eval().data(),
                              &src.write_events(), &copy_event);
      copy_event.wait();
  ...
  inline std::vector<var> packed_copy(const matrix_cl<var>& src) {
    ...
    try {
      const cl::CommandQueue queue = opencl_context.queue();
      matrix_cl<var> packed(packed_size, 1);
      // TODO(Steve):Finish this
      stan::math::opencl_kernels::pack(cl::NDRange(src.rows(), src.rows()),
                                       packed.val(), src.val(), src.rows(), src.rows(),
                                       triangular_view);
...
inline Eigen::Matrix<var, Eigen::Dynamic, Eigen::Dynamic> from_matrix_cl(
  ...
  Eigen::Matrix<var, Eigen::Dynamic, Eigen::Dynamic> dst(src.rows(),
                                                            src.cols());
  dst.val() = from_matrix_cl(src.val());
  dst.adj() = from_matrix_cl(src.adj());
  return dst;
} catch (const cl::Error& e) {
  Eigen::Matrix<var, Eigen::Dynamic, Eigen::Dynamic> dst(src.rows(),
                                                            src.cols());
