...
/**
 * Elementwise multiplication of a reverse mode matrices and a scalar.
...
inline var_value<matrix_cl<double>> elt_multiply(T_a&& a, T_b&& b) {
  ...
  matrix_cl<double> res_val = elt_multiply(value_of(a_arena), value_of(b_arena));
  ...
  return make_callback_var(
      ...
      [a_arena, b_arena](const vari_value<matrix_cl<double>>& res) mutable {
        if (!is_constant<T_a>::value) {
          ...
          a_adj = a_adj + elt_multiply(res.adj(), value_of(b_arena));
        }
        if (!is_constant<T_b>::value) {
          auto& b_adj = forward_as<var_value<double>>(b_arena).adj();
          b_adj = b_adj + sum(elt_multiply(res.adj(), value_of(a_arena)));
        }
      });
}

/**
 * Elementwise multiplication of a scalar and a reverse mode matrix.
  ...
  return make_callback_var(
      ...
      [a_arena, b_arena](const vari_value<matrix_cl<double>>& res) mutable {
        ...
        if (!is_constant<T_b>::value) {
          ...
          b_adj = b_adj + elt_multiply(res.adj(), value_of(a_arena));
        }
      });
}
