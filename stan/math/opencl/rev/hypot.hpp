  ...
  return make_callback_var(
      ...
      [a_arena, b_arena](const vari_value<matrix_cl<double>>& res) mutable {
        if (!is_constant<T_a>::value) {
          ...
          a_adj = a_adj + elt_multiply(res.adj(), elt_divide(value_of(a_arena), res.val()));
        }
        if (!is_constant<T_b>::value) {
          auto& b_adj = forward_as<var_value<double>>(b_arena).adj();
          b_adj = b_adj + sum(elt_multiply(res.adj(), elt_divide(value_of(b_arena), res.val())));
        }
      });
}
  ...
  return make_callback_var(
      ...
      [a_arena, b_arena](const vari_value<matrix_cl<double>>& res) mutable {
        if (!is_constant<T_a>::value) {
          ...
          a_adj = a_adj + sum(elt_multiply(res.adj(), elt_divide(value_of(a_arena), res.val())));
        }
        if (!is_constant<T_b>::value) {
          auto& b_adj = forward_as<var_value<matrix_cl<double>>>(b_arena).adj();
          b_adj = b_adj + elt_multiply(res.adj(), elt_divide(value_of(b_arena), res.val()));
        }
      });
}
