...
inline opencl::ScalarCl<var> variance(const var_value<T>& A) {
  if (A.size() == 1) {
  ...
  return opencl::make_callback_scalar_cl(
      sum(sq_norm) / (A.size() - 1.0),
      [A, diff](const auto& res_adj, const auto&) mutable {
        A.adj() += (2.0 / (A.size() - 1.0)) * (res_adj * diff);
      });
}
