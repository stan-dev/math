...
namespace stan {
  namespace math {
    ...
    integrate_ode_rk45(const F& f, const std::vector<T1> y0, double t0,
      ...
      coupled_ode_system<F, T1, T2> coupled_system(f, y0, theta, x, x_int,
                                                   msgs);

      // first time in the vector must be time of initial state
      std::vector<double> ts_vec(ts.size() + 1);
      ts_vec[0] = t0;
      for (size_t n = 0; n < ts.size(); n++) ts_vec[n + 1] = ts[n];
      ...
      integrate_times(
          ...
          boost::ref(coupled_system), initial_coupled_state,
          boost::begin(ts_vec), boost::end(ts_vec), step_size, observer,
