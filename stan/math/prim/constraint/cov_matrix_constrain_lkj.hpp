...
inline auto cov_matrix_constrain_lkj(T&& x, size_t k,
                                     return_type_t<T>& lp) {
...
inline auto cov_matrix_constrain_lkj(T&& x, size_t k,
                                     return_type_t<T>& lp) {
  return apply_vector_unary<std::decay_t<T>>::apply(std::forward<T>(x), [&lp, k](auto&& v) {
    return cov_matrix_constrain_lkj<Jacobian>(std::forward<decltype(v)>(v), k, lp);
  });
}
