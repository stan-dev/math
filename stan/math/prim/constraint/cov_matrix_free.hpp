#ifndef STAN_MATH_PRIM_CONSTRAINT_COV_MATRIX_FREE_HPP
...
Eigen::Matrix<value_type_t<T>, Eigen::Dynamic, 1> cov_matrix_free(T&& y) {
...
auto cov_matrix_free(T&& x) {
  return apply_vector_unary<T>::apply(
    std::forward<T>(x), [](auto&& v) { return cov_matrix_free(std::forward<decltype(v)>(v)); });
}
