 ...
 */
template <typename T, typename Lp, require_not_std_vector_t<T>* = nullptr>
...
inline auto sum_to_zero_constrain(T&& y, Lp& lp) {
  return apply_vector_unary<std::decay_t<T>>::apply(
      std::forward<T>(y), [&lp](auto&& v) { return sum_to_zero_constrain(std::forward<decltype(v)>(v), lp); });
}
...
inline plain_type_t<T> sum_to_zero_constrain(T&& y, Lp& lp) {
  return sum_to_zero_constrain(std::forward<T>(y), lp);
}
