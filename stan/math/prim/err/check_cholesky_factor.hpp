...
/**
 * Throw an exception if the specified matrix is a valid Cholesky factor. A Cholesky factor is a lower triangular matrix whose diagonal elements are all positive.  Note that Cholesky factors need not be square, but require at least as many rows M as columns N (i.e., `M >= N`).
 * @tparam Mat Type inheriting from `MatrixBase` with neither rows or columns defined at compile time to be equal to 1 or a `var_value` with the var's inner type inheriting from `Eigen::MatrixBase` with neither rows or columns defined at compile time to be equal to 1.
...
/**
 * Throw an exception if the specified matrix is a valid Cholesky factor. A Cholesky factor is a lower triangular matrix whose diagonal elements are all positive.  Note that Cholesky factors need not be square, but require at least as many rows M as columns N (i.e., `M >= N`).
 * @tparam StdVec A standard vector with inner type either inheriting from `MatrixBase` with neither rows or columns defined at compile time to be equal to 1 or a `var_value` with the var's inner type inheriting from `Eigen::MatrixBase` with neither rows or columns defined at compile time to be equal to 1.
 * @param function Function name (for error messages)
 * @param name Variable name (for error messages)
