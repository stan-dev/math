...
template <typename ScalarY, typename ScalarLow,
          require_all_stan_scalar_t<ScalarY, ScalarLow>* = nullptr,
          typename... Idxs>
inline void check_greater(const char* function, const char* name,
                          const ScalarY& y, const ScalarLow& low,
                          Idxs... idxs) {
  if (unlikely(!(y > low))) {
 ...
 */
template <
    typename ScalarY, typename VecLow,
    ...
    typename... Idxs>
inline void check_greater(const char* function, const char* name,
                          const ScalarY& y, const VecLow& low, Idxs... idxs) {
 ...
 * @tparam Idxs A parameter pack of Integral types
 * @tparam ScalarY A scalar type
 * @tparam DenseLow Type inheriting from `Eigen::DenseBase` or a \ref
 ...
 * element of y or low is `NaN`
 */
template <typename ScalarY, typename DenseLow,
          require_stan_scalar_t<ScalarY>* = nullptr,
          require_dense_dynamic_t<DenseLow>* = nullptr, typename... Idxs>
inline void check_greater(const char* function, const char* name,
                          const ScalarY& y, const DenseLow& low, Idxs... idxs) {
 ...
 */
template <
    typename VecY, typename ScalarLow, require_vector_t<VecY>* = nullptr,
 ...
 * element of y or low is `NaN`
 */
template <typename DenseY, typename ScalarLow,
          require_dense_dynamic_t<DenseY>* = nullptr,
          require_stan_scalar_t<ScalarLow>* = nullptr, typename... Idxs>
inline void check_greater(const char* function, const char* name,
                          const DenseY& y, const ScalarLow& low, Idxs... idxs) {
...
template <typename DenseY, typename DenseLow,
          require_all_dense_dynamic_t<DenseY, DenseLow>* = nullptr,
          typename... Idxs>
inline void check_greater(const char* function, const char* name,
                          const DenseY& y, const DenseLow& low, Idxs... idxs) {
  auto&& y_arr = value_of_rec(to_ref(y));
  auto&& low_arr = value_of_rec(to_ref(low));
  for (Eigen::Index j = 0; j < low_arr.cols(); ++j) {
    for (Eigen::Index i = 0; i < low_arr.rows(); ++i) {
 ...
 * @tparam T_low A scalar type or the same type as the underlying type in
 * `ContainerY`
 * @param function Function name (for error messages)
 * @param name Variable name (for error messages)
 * @param y Variable to check
 ...
 * element of `low`.
 * @tparam Idxs A parameter pack of Integral types
 * @tparam ScalarY A scalar type or the same type as the inner type of
 * `ContainerLow`
 * @tparam ContainerLow A standard vector type with a \ref stan::value_type of a
 * standard vector or type inheriting from `Eigen::DenseBase`
 * @param function Function name (for error messages)
 * @param name Variable name (for error messages)
 * @param y Variable to check
 ...
 * @tparam StdVecLow A standard vector type whose \ref stan::value_type is a
 * scalar, type inheriting from `Eigen::EigenBase`, or another standard vector
 * @param function Function name (for error messages)
 * @param name Variable name (for error messages)
 * @param y Variable to check
...
template <typename StdVecY, typename StdVecLow,
          require_any_std_vector_vt<is_container_or_var_matrix, StdVecY,
                                    StdVecLow>* = nullptr,
          require_all_std_vector_t<StdVecY, StdVecLow>* = nullptr,
          typename... Idxs>
inline void check_greater(const char* function, const char* name,
                          const StdVecY& y, const StdVecLow& low,
                          Idxs... idxs) {
  for (size_t i = 0; i < y.size(); ++i) {
