...
struct MatrixExponentialScalingOp {
  ...
  using ComplexScalar = stan::math::complex<RealScalar>;
  ...
  inline const ComplexScalar operator()(const ComplexScalar &x) const {
    using std::ldexp;
    return ComplexScalar(ldexp(x.real(), -m_squarings),
                         ldexp(x.imag(), -m_squarings));
  }
...
void matrix_exp_pade3(const MatrixType &A, MatrixType &U, MatrixType &V) {
  ...
  const MatrixType tmp
      = b[3] * A2 + b[1] * MatrixType::Identity(A.rows(), A.cols());
  U.noalias() = A * tmp;
...
void matrix_exp_pade5(const MatrixType &A, MatrixType &U, MatrixType &V) {
  ...
  const MatrixType tmp
      = b[5] * A4 + b[3] * A2 + b[1] * MatrixType::Identity(A.rows(), A.cols());
  U.noalias() = A * tmp;
...
template <typename MatrixType,
          typename RealScalar = typename Eigen::NumTraits<
              typename Eigen::internal::traits<MatrixType>::Scalar>::Real>
struct matrix_exp_computeUV {
  /** \brief Compute Pad&eacute; approximant to the exponential.
   *
   * Computes \c U, \c V and \c squarings such that \f$ (V+U)(V-U)^{-1} \f$ is a
   * Pad&eacute; approximant of \f$ \exp(2^{-\mbox{squarings}}M) \f$ around \f$
   * M = 0 \f$, where \f$ M \f$ denotes the matrix \c arg. The degree of the
   * Pad&eacute; approximant and the value of squarings are chosen such that the
   * approximation error is no more than the round-off error.
   *
   ...
   * matrix.
   */
  static void run(const MatrixType &arg, MatrixType &U, MatrixType &V,
                  int &squarings);
};
