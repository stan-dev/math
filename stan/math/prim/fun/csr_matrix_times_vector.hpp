...
csr_matrix_times_vector(int m, int n, const T1& w, const std::vector<int>& v,
  ...
  check_size_match("csr_matrix_times_vector", "v + 1", v.size() + 1, "u", u.size());
  ...
  std::transform(u.begin(), u.end(), u_zeroed.begin(), [](auto&& x) {return x - 1;});
  std::vector<int> v_zeroed(u.size());
  std::transform(v.begin(), v.end(), v_zeroed.begin(), [](auto&& x) {return x - 1;});
  auto&& w_ref = to_ref(w);
  Eigen::Map<const Eigen::SparseMatrix<scalar_type_t<T1>>> w_mat(m, n, w_ref.size(), v_zeroed.data(), u_zeroed.data(), w_ref.data());
  auto&& b_ref = to_ref(b);
  Eigen::Map<const Eigen::Matrix<scalar_type_t<T2>, Eigen::Dynamic, 1>> b_vec(b_ref.data(), b_ref.size());
  return w_mat * b_vec;
}
