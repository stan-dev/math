#ifndef STAN_MATH_PRIM_FUN_RFFT_HPP
#define STAN_MATH_PRIM_FUN_RFFT_HPP
...
template <typename V, require_eigen_vector_t<V>* = nullptr,
          require_not_complex_t<value_type_t<V>>* = nullptr,
...
template <typename M, require_eigen_dense_dynamic_t<M>* = nullptr,
          require_not_complex_t<value_type_t<M>>* = nullptr,
          require_not_var_t<base_type_t<M>>* = nullptr>
inline Eigen::Matrix<complex_return_t<value_type_t<M>>, -1, -1> rfft2(
    const M& x) {
  Eigen::Matrix<complex_return_t<value_type_t<M>>, -1, -1> y(x.rows(),
                                                             x.cols());
  for (int i = 0; i < y.rows(); ++i)
    y.row(i) = rfft(x.row(i));
...
inline Eigen::Matrix<base_type_t<M>, -1, -1> inv_rfft2(const M& y) {
  ...
  Eigen::Matrix<base_type_t<M>, -1, -1> x_r(y.rows(), y.cols());
  for (int i = 0; i < x.rows(); ++i)
    x_r.row(i) = inv_rfft(x.row(i));
  return x_r;
}
