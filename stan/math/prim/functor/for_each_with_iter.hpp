#ifndef STAN_MATH_PRIM_FUNCTOR_FOR_EACH_WITH_ITER_HPP
#define STAN_MATH_PRIM_FUNCTOR_FOR_EACH_WITH_ITER_HPP
...
/**
 * Apply a function to each element of a tuple where the functor `F` also
 * expects the last argument to be the index in the tuple.
...
/**
 * Apply a function to each element of two tuples where the functor `F` also
 * expects the last argument to be the index in the tuple.
 * @tparam F type with a valid `operator()`
 * @tparam T1 Tuple
