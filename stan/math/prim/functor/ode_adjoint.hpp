 ...
 */
template <typename Enable, typename ReturnType, typename F, typename T_y0,
          typename T_t0, typename T_ts, typename... T_Args>
class cvodes_integrator_adjoint_impl;

template <typename Enable, typename ReturnType, typename F, typename T_y0_t0,
          typename T_t0, typename T_ts, typename... T_Args>
struct cvodes_integrator_adjoint_data;

template <typename ReturnType, typename T_y0, typename T_t0, typename T_ts,
          typename... T_Args>
struct cvodes_integrator_adjoint_data<require_arithmetic_t<ReturnType>,
                                      ReturnType, T_y0, T_t0, T_ts, T_Args...> {
  ...
  template <typename FF, typename T_y0,
...
template <typename ReturnType, typename F, typename T_y0, typename T_t0,
          typename T_ts, typename... T_Args>
class cvodes_integrator_adjoint_impl<require_arithmetic_t<ReturnType>, F, T_y0,
                                     T_t0, T_ts, T_Args...> : {
  ...
  cvodes_solver<T_Return> solver_;
  struct cvodes_integrator_adjoint_data<void, T_Return, T_y0, T_t0, T_ts,
                                        T_Args...>
      cvode_data;
 ...
 public:
  ...
  cvodes_integrator_adjoint_impl(
      ...
      : cvode_data_(/* Pass members */),
        solver_(function_name, f, N_, num_args_vars_, ts_.size(),
                solver_forward_, state_forward_, state_backward_, quad_,
                absolute_tolerance_forward_, absolute_tolerance_backward_) {
    check_flag_sundials(
    ...
    for (size_t n = 0; n < ts_dbl.size(); ++n) {
      ...
      if (t_final != t_init) {
        int error_code = CVode(solver_->cvodes_mem_, t_final,
                               solver_->nv_state_forward_, &t_init, CV_NORMAL);
...
std::vector<Eigen::VectorXd> ode_adjoint_impl(
  ...
  using cvode_integrator
      = cvodes_integrator_adjoint_impl<void, T_Return, F, plain_type_t<T_y0>,
                                       T_t0, T_ts, plain_type_t<T_Args>...>;
  ...
  return cvode_integrator(
             ...
             interpolation_polynomial, solver_forward, solver_backward, msgs,
             args...)
      .solution();
}
