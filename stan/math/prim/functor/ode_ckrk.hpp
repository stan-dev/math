 ...
 * times, { t1, t2, t3, ... } using SUNDIALS ARKODE's ERKStep Cash-Karp54
 * solver.
 *
...
/**
 * Solve the ODE initial value problem y' = f(t, y), y(t0) = y0 at a set of
 * times, { t1, t2, t3, ... } using SUNDIALS ARKODE's ERKStep Cash-Karp
 * solver.
 *
 * If the system of equations is stiff, <code>ode_bdf</code> will likely be
 * faster.
 ...
 * Solve the ODE initial value problem y' = f(t, y), y(t0) = y0 at a set of
 * times, { t1, t2, t3, ... } using SUNDIALS ARKODE's ERKStep Cash-Karp
 * Runge-Kutta solver with defaults for relative_tolerance,
