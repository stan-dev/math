  ...
  #include <viennacl/linalg/ilu_operations.hpp>
        ...
        "__kernel void inverse_step1(__global double* ap,__global double* vv, int remain_der,int part_size_fixed, int M, int Mpad){ \n"
        "   \n"
        "  int indeks=get_global_id(0); \n"
        "  int i=indeks*part_size_fixed; \n"
        "  int part_size; \n"
        "  double faktor; \n"
        "  if(indeks<remain_der){ \n"
        "    i+=indeks; \n"
        ...
        "#define RTS  8 \n"
        "#define TS2 32 \n"
        "__kernel void inverse_step2(__global double* ap,__global int* sizes,__global double* MM, int repeat, int remain_der,int part_size_fixed, int M, int Mpad){ \n"
        " \n"
        ...
        "  if(((n+1)*repeat)<=remain_der) \n"
        " offset_i+=(n+1)*repeat; \n"
        "  else \n"
        "    offset_i+=remain_der; \n"
        "     \n"
        "  if((n*repeat)<=remain_der) \n"
        "    offset_j+=n*repeat; \n"
        "  else \n"
        "    offset_j+=remain_der; \n"
        "   \n"
        ...
        "} \n"
        " \n"
        "__kernel void inverse_step3(__global double* ap,__global int* sizes,__global double* MM, int repeat, int remain_der,int part_size_fixed, int M, int Mpad){ \n"
        " \n"
        "  int n=get_global_id(2)*2; \n"
        "  double sum=0; \n"
        "  int part_size1=0,part_size2=0; \n"
