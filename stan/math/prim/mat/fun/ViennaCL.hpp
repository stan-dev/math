  ...
  #include <viennacl/linalg/ilu_operations.hpp>
    ...
    "   int remainder,int part_size_fixed, int M, int Mpad){ \n"
    "  \n"
    ...
    "            vv[offset + j * (part_size_fixed + 1) + k] = vv[offset + j * \n"
    "              (part_size_fixed + 1) + k] - \n"
    ...
    "#define RTS	8 \n"
    "#define TS2 32 \n"
    "__kernel void inverse_step2(__global double* ap,__global int* sizes,__global double* MM, \n"
    "   int repeat, int remainder,int part_size_fixed, int M, int Mpad){ \n"
    " \n"
    ...
    "				Bsub[col+w*RTS][row] = ap[(j + offset_j + w * RTS) * \n"
    "         Mpad+(tiledRow+offset_i-part_size1)]; \n"
    "			}else{		 \n"
    "			 Bsub[col+w*RTS][row] = 0.0; \n"
    "			} \n"
    "		} \n"
