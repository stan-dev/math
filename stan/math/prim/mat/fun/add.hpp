...
namespace stan {
  namespace math {
    ...
    inline Eigen::Matrix<
        typename boost::math::tools::promote_args<T1, T2>::type, R, C>
    add(const Eigen::Matrix<T1, R, C>& m1, const Eigen::Matrix<T2, R, C>& m2) {
      check_matching_dims("add", "m1", m1, "m2", m2);
      Eigen::Matrix<typename boost::math::tools::promote_args<T1, T2>::type, R,
                    C>
          result(m1.rows(), m1.cols());
      for (int i = 0; i < result.size(); ++i) result(i) = m1(i) + m2(i);
      return result;
    }
    ...
    add(const Eigen::Matrix<T1, R, C>& m, const T2& c) {
      ...
      for (int i = 0; i < result.size(); ++i) result(i) = m(i) + c;
      return result;
    }
    ...
    add(const T1& c, const Eigen::Matrix<T2, R, C>& m) {
      Eigen::Matrix<typename boost::math::tools::promote_args<T1, T2>::type, R,
                    C>
          result(m.rows(), m.cols());
      for (int i = 0; i < result.size(); ++i) result(i) = c + m(i);
      return result;
    }
