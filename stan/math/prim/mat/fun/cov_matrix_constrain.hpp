...
namespace stan {
  ...
  namespace math {
    ...
    Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> cov_matrix_constrain(
        ...
        typename math::index_type<Eigen::Matrix<T, Eigen::Dynamic, 1> >::type
            K) {
      using Eigen::Dynamic;
      using Eigen::Matrix;
      using std::exp;
      ...
      for (index_t m = 0; m < K; ++m) {
        for (int n = 0; n < m; ++n) L(m, n) = x(i++);
        L(m, m) = exp(x(i++));
        for (index_t n = m + 1; n < K; ++n) L(m, n) = 0.0;
      }
    ...
    Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> cov_matrix_constrain(
      ...
      int i = 0;
      for (index_t m = 0; m < K; ++m) {
        for (index_t n = 0; n < m; ++n) L(m, n) = x(i++);
        L(m, m) = exp(x(i++));
        for (index_t n = m + 1; n < K; ++n) L(m, n) = 0.0;
      }
