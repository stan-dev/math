...
namespace stan {
  namespace math {
     ...
     */
    template <typename T>
    inline T& get_base1_lhs(std::vector<T>& x, size_t i,
     ...
     */
    template <typename T>
    inline T& get_base1_lhs(std::vector<std::vector<T> >& x, size_t i1,
    ...
    inline T& get_base1_lhs(
        ...
        size_t i2, size_t i3, size_t i4, const std::string& error_msg,
        size_t idx) {
      check_range("[]", "x", x.size(), i1, idx, error_msg);
    ...
    inline T& get_base1_lhs(Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>& x,
                            size_t m, size_t n, const std::string& error_msg,
                            size_t idx) {
      check_range("[]", "rows of x", x.rows(), m, idx, error_msg);
