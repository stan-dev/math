#ifndef STANMATH_MATRIX_INVERSE_HPP_
#define STANMATH_MATRIX_INVERSE_HPP_
...
#include "stanmathcl/ocl.hpp"
#include "stanmathcl/matrix.hpp"

/** @file stanmathcl/matrix_inverse.hpp
    @brief matrix_inverse - functions for matrix inversion: lower triangular, upper triangular, regular, ...
*/

//CURRENTLY ONLY SUPPORTS LOWER TRIANGULAR
namespace stanmathcl{
...
namespace matrix_inverse{
  ...
  void lower_triangular_inverse(stanmathcl::Matrix & A){
    ...
    try{
      ...
      cmdQueue.enqueueWriteBuffer(sizes,CL_TRUE,0,sizeof(int)*parts,stl_sizes);

      kernel_step1.setArg(0,A.buffer()); kernel_step1.setArg(1,temp.buffer()); kernel_step1.setArg(2,remainder);kernel_step1.setArg(3,part_size_fixed);
      ...
      for(int pp=parts;pp>1;pp/=2){
        ...
        kernel_step2.setArg(0,A.buffer());kernel_step2.setArg(1,sizes);kernel_step2.setArg(2,temp.buffer());
        kernel_step2.setArg(3,repeat);kernel_step2.setArg(4,remainder);kernel_step2.setArg(5,part_size_fixed);
        kernel_step2.setArg(6,A.M);

        kernel_step3.setArg(0,A.buffer());kernel_step3.setArg(1,sizes);kernel_step3.setArg(2,temp.buffer());
        kernel_step3.setArg(3,repeat);kernel_step3.setArg(4,remainder);kernel_step3.setArg(5,part_size_fixed);
