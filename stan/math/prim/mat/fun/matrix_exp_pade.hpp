...
MatrixType matrix_exp_pade(const MatrixType& arg) {
  ...
  int squarings;
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> tmp(arg.rows(),
                                                            arg.cols());
  for (int i = 0; i < arg.size(); ++i) {
    tmp(i) = value_of(arg(i));
  }
  Eigen::matrix_exp_computeUV<
      Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic>>::run(tmp, U, V,
                                                                  squarings,
                                                                  tmp(0, 0));
  // Pade approximant is
  // (U+V) / (-U+V)
  ...
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> pade_approximation
      = denom.partialPivLu().solve(numer);
