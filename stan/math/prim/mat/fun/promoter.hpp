...
namespace stan {
  namespace math {
    ...
    /**
     * Struct which holds static function for element-wise type promotion.
     * This specialization promotes a vector of Eigen matrix elements of different types.
     *
     * @tparam F type of input element
     * @tparam T type of output element
     * @tparam R number of rows
     ...
     */
    template <typename F, typename T, int R, int C>
    struct promoter<std::vector<Eigen::Matrix<F, R, C> >, std::vector<Eigen::Matrix<T, R, C> > > {
      inline static void promote(const std::vector<Eigen::Matrix<F, R, C> >& u,
                                 std::vector<Eigen::Matrix<T, R, C> >& t) {
        t.resize(u.size());
        for (size_t i = 0; i < u.size(); ++i)
          promoter<Eigen::Matrix<F, R, C>,
                   Eigen::Matrix<T, R, C> >::promote(u[i], t[i]);
      }
      inline static std::vector<Eigen::Matrix<T, R, C> >
      promote_to(const std::vector<Eigen::Matrix<F, R, C> >& u) {
        std::vector<Eigen::Matrix<T, R, C> >t;
        promoter<Eigen::Matrix<F, R, C>,
                 Eigen::Matrix<T, R, C> >::promote(u, t);
        return t;
     ...
     */
    template <typename T, int R, int C>
    struct promoter<std::vector<Eigen::Matrix<T, R, C> >, std::vector<Eigen::Matrix<T, R, C> > > {
      inline static void promote(const std::vector<Eigen::Matrix<T, R, C> >& u,
                                 std::vector<Eigen::Matrix<T, R, C> >& t) {
        t = u;
      }
      inline static std::vector<Eigen::Matrix<T, R, C> >
      promote_to(const std::vector<Eigen::Matrix<T, R, C> >& u) {
        return u;
      }
    };
