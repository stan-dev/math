...
namespace stan {
  namespace math {
    ...
    inline Eigen::Matrix<T, Eigen::Dynamic, 1> tail(
      ...
      if (n != 0) check_row_index("tail", "n", v, n);
      return v.tail(n);
    }
    ...
    inline Eigen::Matrix<T, 1, Eigen::Dynamic> tail(
      ...
      if (n != 0) check_column_index("tail", "n", rv, n);
      return rv.tail(n);
    }
    ...
    std::vector<T> tail(const std::vector<T>& sv, size_t n) {
      ...
      if (n != 0) check_std_vector_index("tail", "n", sv, n);
      std::vector<T> s;
      for (idx_t i = sv.size() - n; i < sv.size(); ++i) s.push_back(sv[i]);
      return s;
    }
