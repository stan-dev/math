...
inline Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> to_matrix(
  ...
  int rows = x.size();
  if (rows == 0) return Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic>(0, 0);
  int cols = x[0].size();
  Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> result(rows, cols);
  for (int i = 0, ij = 0; i < cols; i++)
    for (int j = 0; j < rows; j++, ij++) result(ij) = x[j][i];
  return result;
}
...
to_matrix(const std::vector<std::vector<T> >& x) {
  ...
  for (size_t i = 0, ij = 0; i < cols; i++)
    for (size_t j = 0; j < rows; j++, ij++) result(ij) = x[j][i];
  return result;
}
...
inline Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> to_matrix(
  ...
  for (int i = 0; i < size; i++) result(i) = x[i];
  return result;
}
...
inline Eigen::Matrix<T, Eigen::Dynamic, Eigen::Dynamic> to_matrix(
  ...
  if (col_major) return to_matrix(x, m, n);
  ...
  for (int i = 0, ij = 0; i < m; i++)
    for (int j = 0; j < n; j++, ij++) result(i, j) = x(ij);
  return result;
}
