 ...
 */
void block_householder_tridiag(const Eigen::MatrixXd& A,
  ...
  for (size_t k = 0; k < packed.rows() - 2; k += r) {
    ...
    for (size_t j = 0; j < actual_r; j++) {
      ...
      if (j != 0) {
        ...
        householder_whole -= packed.block(j + k, k, householder_whole.size(), j)
                                 * V.block(j - 1, 0, 1, j).transpose()
                             + V.block(j - 1, 0, householder_whole.size(), j)
                                   * packed.block(j + k, k, 1, j).transpose();
      }
    ...
    Eigen::MatrixXd partial_update
        = packed.block(k + actual_r, k, packed.rows() - k - actual_r, actual_r)
          * V.bottomRows(V.rows() - actual_r + 1).transpose();
    packed
        .block(k + actual_r, k + actual_r, packed.rows() - k - actual_r,
               packed.cols() - k - actual_r)
        .triangularView<Eigen::Lower>()
        -= partial_update + partial_update.transpose();
  }
 ...
 * larger cache sizes a larger value is optimal.
 */
void block_apply_packed_Q(const Eigen::MatrixXd& packed, Eigen::MatrixXd& A,
                          const int r = 100) {
