...
template <class T>
struct is_eigen_col_vector_impl<T, std::enable_if_t<is_eigen<std::decay_t<T>>::value>>
    : std::integral_constant<bool, T::ColsAtCompileTime == 1> {};

// Template that checks Eigen matrix for one row
template <class T>
struct is_eigen_row_vector_impl<T, std::enable_if_t<is_eigen<std::decay_t<T>>::value>>
...
template <typename T>
struct is_eigen_col_vector<T, std::enable_if_t<internal::is_eigen_col_vector_impl<T>::value>> : std::true_type {};


// Check whether type is an eigen row vector
template <typename T>
struct is_eigen_row_vector<T, std::enable_if_t<internal::is_eigen_row_vector_impl<T>::value>> : std::true_type {};

// Checks whether decayed type is an eigen vector
template <typename T>
struct is_eigen_vector<T, std::enable_if_t<is_eigen_col_vector<T>::value || is_eigen_row_vector<T>::value>> : std::true_type {};
