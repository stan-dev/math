...
typename boost::math::tools::promote_args<T_prob>::type categorical_lpmf(
  ...
  if (include_summand<propto, T_prob>::value) return log(theta(n - 1));
  return 0.0;
}
...
typename boost::math::tools::promote_args<T_prob>::type categorical_lpmf(
  ...
  for (int i = 0; i < theta.size(); ++i) log_theta(i) = log(theta(i));

  Eigen::Matrix<typename boost::math::tools::promote_args<T_prob>::type,
                Eigen::Dynamic, 1>
      log_theta_ns(ns.size());
  for (size_t i = 0; i < ns.size(); ++i) log_theta_ns(i) = log_theta(ns[i] - 1);
