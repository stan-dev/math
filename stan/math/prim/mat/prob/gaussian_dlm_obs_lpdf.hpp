...
namespace stan {
  namespace math {
    ...
    gaussian_dlm_obs_lpdf(
      ...
      check_size_match(function, "columns of F", F.cols(), "rows of y",
                       y.rows());
      ...
      check_size_match(function, "size of m0", m0.size(), "rows of G",
                       G.rows());
      check_finite(function, "m0", m0);
      check_size_match(function, "rows of C0", C0.rows(), "rows of G",
                       G.rows());
      check_cov_matrix(function, "C0", C0);
      check_finite(function, "C0", C0);

      if (y.cols() == 0 || y.rows() == 0) return lp;
    ...
    gaussian_dlm_obs_lpdf(
      ...
      check_size_match(function, "rows of G", G.rows(), "columns of G",
                       G.cols());
      check_finite(function, "G", G);
