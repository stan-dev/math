...
namespace stan {
  namespace math {
     ...
     * w.  y is a dxN matrix, where each column is a different observation and
     * each row is a different output dimension.  The Gaussian Process is
     * assumed to have a scaled kernel matrix with a different scale for each
     * output dimension. This distribution is equivalent to: for (i in 1:d)
     * row(y, i) ~ multi_normal(0, (1/w[i])*Sigma).
     *
     * @param y A dxN matrix
     * @param Sigma The NxN kernel matrix
     * @param w A d-dimensional vector of positve inverse scale parameters for
     * each output.
    ...
    multi_gp_lpdf(
      ...
      if (include_summand<propto, T_y, T_w, T_covar>::value) {
        Eigen::Matrix<T_w, Eigen::Dynamic, Eigen::Dynamic> w_mat(
            w.asDiagonal());
