...
#pragma omp parallel for if (M > OMP_TRIGGER * omp_get_max_threads()) default( \
    none) shared(beta_dbl, beta_vec)
...
#pragma omp parallel for if (N > OMP_TRIGGER * omp_get_max_threads()) \
    reduction(+ : logp) default(none) shared(N, theta_dbl, alpha_vec, \
    y_seq_view, theta_derivative_sum)
#endif
