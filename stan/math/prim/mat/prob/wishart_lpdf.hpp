...
namespace stan {
  namespace math {
     ...
     * \f{eqnarray*}{
     ...
     \log (p (W \, |\, \nu, S) ) &=& \log \left( \left(2^{\nu k/2} \pi^{k (k-1)
     /4} \prod_{i=1}^k{\Gamma (\frac{\nu + 1 - i}{2})} \right)^{-1} \times
     \left| S \right|^{-\nu/2} \left| W \right|^{(\nu - k - 1) / 2}
     \times \exp (-\frac{1}{2} \mbox{tr} (S^{-1} W)) \right) \\
     &=& -\frac{\nu k}{2}\log(2) - \frac{k (k-1)}{4} \log(\pi) -
     \sum_{i=1}^{k}{\log
     (\Gamma (\frac{\nu+1-i}{2}))}
     -\frac{\nu}{2} \log(\det(S)) + \frac{\nu-k-1}{2}\log (\det(W)) -
     \frac{1}{2} \mbox{tr} (S^{-1}W) \f}
     *
    ...
    wishart_lpdf(
      ...
      if (include_summand<propto, T_dof>::value) lp -= lmgamma(k, 0.5 * nu);
      ...
      if (include_summand<propto, T_scale, T_y>::value) {
        Matrix<typename promote_args<T_y, T_scale>::type, Dynamic, Dynamic>
            Sinv_W(mdivide_left_ldlt(
                ldlt_S, static_cast<Matrix<T_y, Dynamic, Dynamic> >(
