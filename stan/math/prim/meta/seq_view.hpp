#ifndef STAN_MATH_PRIM_META_SEQ_VIEW_HPP
#define STAN_MATH_PRIM_META_SEQ_VIEW_HPP
...
class seq_view {
 ...
 public:
  ...
  Eigen::Index size() const { return 1; }
};
...
class seq_view<T, Eigen::Matrix<S, Eigen::Dynamic, 1> > {
 ...
 public:
  ...
  Eigen::Index size() const { return x_.size(); }
};
...
class seq_view<T, std::vector<S> > {
 ...
 public:
  ...
  Eigen::Index size() const { return x_.size() * elt_size_; }
};
...
class seq_view<T, std::vector<std::vector<T> > > {
 ...
 public:
  ...
  Eigen::Index size() const { return x_.size() * cols_; }
};
