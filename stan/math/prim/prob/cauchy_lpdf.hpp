    ...
    if (!is_constant_all<T_y, T_loc>::value) {
      ...
      if (!is_constant_all<T_y>::value) {
        if (is_vector<T_y>::value) {
          stan::math::edge<0>(ops_partials).partials_ = -mu_deriv;
        } else {
          stan::math::edge<0>(ops_partials).partials_[0] = -sum(mu_deriv);
        }
      }
      if (!is_constant_all<T_loc>::value) {
        stan::math::edge<1>(ops_partials).partials_ = std::move(mu_deriv);
      }
    }
    if (!is_constant_all<T_scale>::value) {
      stan::math::edge<2>(ops_partials).partials_ = (y_minus_mu_squared - sigma_squared)
                                      * inv_sigma
