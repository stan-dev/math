...
inline exp_mod_normal_log_phi_terms<T> exp_mod_normal_exp_log_cdf_terms(
  ...
  if (value_of_rec(erfc_arg) >= 5.0) {
    ...
    const T mills_excess
        = value_of_rec(erfc_arg) >= 20.0
              ? exp_mod_normal_mills_excess(a - z)
              : SQRT_TWO_OVER_SQRT_PI / erfcx - (a - z);
    return {-0.5 * z * z + LOG_HALF + log(erfcx),
            a - z + mills_excess, mills_excess};
  }
  const auto normal_terms = std_normal_lcdf_and_mills(z - a);
  return {0.5 * a * a - a * z + normal_terms.log_cdf,
          normal_terms.mills_ratio, normal_terms.mills_ratio - (a - z)};
}
...
inline exp_mod_normal_log_cdf_terms<T> exp_mod_normal_cdf_terms(const T& z,
  ...
  if (value_of_rec(a) < 1e-8) {
    const T m0_factor = value_of_rec(z) < -4.0
                            ? exp_cdf_terms.mills_excess
    ...
    if (value_of_rec(m0_factor) > 0.0
      ...
      const T dz_log_cdf
          = 1.0 / m0_factor - a * dm1_over_m0 / remainder;
  ...
  const T dz_log_cdf
      = cdf_weight * z_terms.mills_ratio
  ...
  const T dz_log_ccdf
      = -ccdf_weight * neg_z_terms.mills_ratio
        + exp_ccdf_weight * (-z + exp_cdf_terms.mills_excess);
  const T da_log_ccdf
      = -exp_ccdf_weight * exp_cdf_terms.mills_excess;

  return {log_cdf,       log_ccdf,       dz_log_cdf,
          da_log_cdf,    a * da_log_cdf, dz_log_ccdf,
          da_log_ccdf};
}
