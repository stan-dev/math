...
return_type_t<T_y, T_inv_scale> exponential_lccdf(const T_y& y,
  ...
  if (!is_constant_all<T_y>::value) {
    ...
    if (is_vector<T_y>::value && !is_vector<T_inv_scale>::value) {
      ...
      stan::math::edge<0>(ops_partials).partials_[0] = -sum(beta_val);
    }
  }
  if (!is_constant_all<T_inv_scale>::value) {
    ...
    if (is_vector<T_inv_scale>::value && !is_vector<T_y>::value) {
      ...
      stan::math::edge<0>(ops_partials).partials_ = -forward_as<y_val_array>(y_val);
    } else {
      stan::math::edge<0>(ops_partials).partials_[0] = -sum(y_val);
    }
  }
