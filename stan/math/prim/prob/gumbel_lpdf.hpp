...
return_type_t<T_y, T_loc, T_scale> gumbel_lpdf(const T_y& y, const T_loc& mu,
  ...
  for (size_t n = 0; n < N; n++) {
    ...
    if (!is_constant_all<T_y>::value) {
      ops_partials.template edge<1>().partials_[n] -= inv_beta[n] - scaled_diff;
    }
    if (!is_constant_all<T_loc>::value) {
      ops_partials.template edge<2>().partials_[n] += inv_beta[n] - scaled_diff;
    }
    if (!is_constant_all<T_scale>::value) {
      ops_partials.template edge<3>().partials_[n] += -inv_beta[n]
