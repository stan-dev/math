...
if (include_summand<propto, T_y, T_dof, T_loc, T_covar_elem>::value) {
  ...
  for (size_t i = 0; i < size_vec; i++) {
    ...
    T_partials_return scale_val = nu_plus_dims / (dot_half + nu);
     ...
     if (!is_constant_all<T_y>::value) {
      ...
      if (!is_constant_all<T_covar>::value) {
        ...
        for (size_t i = 0; i < num_dims; i++) {
          L_deriv.col(i).segment(i, num_dims - i) = scaled_diff.block(0, 0, num_dims - i, size_vec) * half.row(i);
          }

        L_deriv.diagonal() += -inv(L_val.diagonal());

       // L_deriv = L_deriv.template triangularView<Eigen::Lower>() * scale_val;

        ops_partials.edge4_.partials_ += L_deriv * scale_val;
      }
...
//   if (include_summand<propto, T_covar>::value) {
//       lp += -sum(log(L.diagonal())) * size_vec;
//    }

//   if (include_summand<propto, T_y, T_dof, T_loc, T_covar_elem>::value) {
  

//   Eigen::Matrix<lp_type, Eigen::Dynamic, Eigen::Dynamic> y_val_minus_mu_val(num_dims, size_vec);
...
//   Eigen::Matrix<lp_type, Eigen::Dynamic, Eigen::Dynamic> half = mdivide_left_tri<Eigen::Lower>(L, y_val_minus_mu_val).transpose();

//   lp_type sum_lp_vec(0.0);

//   for (size_t i = 0; i < size_vec; i++) {
//     const auto& dot_half = dot_self(half.row(i));
//       sum_lp_vec += log1p(dot_half / nu);
//   }
...
if (include_summand<propto, T_y, T_dof, T_loc, T_covar_elem>::value) {
  ...
  if (!is_constant_all<T_y, T_loc, T_covar_elem>::value) {
    ...
    if (!is_constant_all<T_covar_elem>::value) {
      ...
      for (size_t i = 0; i < num_dims; i++) {
        L_deriv.col(i).segment(i, num_dims - i) = scaled_diff.segment(i, num_dims - i) * half;
