...
namespace stan {
  ...
  return_type_t<T_x, T_beta, T_cuts> ordered_logistic_glm_lpmf(
    ...
    check_bounded(function, "Vector of dependent variables", y_ref, 1,
                  N_classes);
    ...
    Array<T_cuts_partials, Dynamic, 1> cuts_y1(N_instances),
        cuts_y2(N_instances);
    ...
    if (is_vector<T_y>::value) {
      Eigen::Map<const Eigen::Matrix<int, Eigen::Dynamic, 1>> y_vec(
    ...
    if (!is_constant_all<T_x, T_beta, T_cuts>::value) {
      ...
      Array<T_partials_return, Dynamic, 1> exp_cuts_diff
          = exp(cuts_y2 - cuts_y1);
      Array<T_partials_return, Dynamic, 1> d1
          = (cut2 > 0).select(exp_m_cut2 / (1 + exp_m_cut2),
                              1 / (1 + exp(cut2)))
