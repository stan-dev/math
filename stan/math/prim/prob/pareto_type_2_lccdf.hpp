...
return_type_t<T_y, T_loc, T_scale, T_shape> pareto_type_2_lccdf(
  ...
  if (!is_constant_all<T_y, T_loc, T_scale>::value) {
    ...
    if (!is_constant_all<T_scale>::value) {
      stan::math::edge<2>(ops_partials).partials_ = rep_deriv * (y_val - mu_val) / lambda_val;
    }
    if (!is_constant_all<T_loc>::value) {
      stan::math::edge<1>(ops_partials).partials_ = std::move(rep_deriv);
    }
  }
  ...
  if (!is_constant_all<T_shape>::value) {
    if (is_vector<T_shape>::value) {
      ...
      if (is_vector<T_y>::value || is_vector<T_loc>::value
        ...
        stan::math::edge<3>(ops_partials).partials_ = -forward_as<Log_temp_array>(log_temp);
      } else {
        stan::math::edge<3>(ops_partials).partials_ = Log_temp_array::Constant(
