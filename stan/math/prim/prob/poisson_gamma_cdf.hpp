#ifndef STAN_MATH_PRIM_PROB_POISSON_GAMMA_CDF_HPP
#define STAN_MATH_PRIM_PROB_POISSON_GAMMA_CDF_HPP
...
return_type_t<T_shape, T_inv_scale> poisson_gamma_cdf(const T_n& n,
                                                      const T_shape& alpha,
                                                      const T_inv_scale& beta) {
  static const char* function = "poisson_gamma_cdf";
  ...
  return neg_binomial_2_cdf<propto>(n_ref,
                                     elt_divide(alpha_ref, beta_ref),
                                     alpha_ref);
}

template <typename T_n, typename T_shape, typename T_inv_scale>
inline return_type_t<T_shape, T_inv_scale> poisson_gamma_cdf(
    const T_n& n, const T_shape& alpha, const T_inv_scale& beta) {
  return poisson_gamma_cdf<false>(n, alpha, beta);
}
