...
inline return_type_t<T_rate> poisson_lcdf(const T_n& n, const T_rate& lambda) {
  ...
  static constexpr const char* function = "poisson_lcdf";
  ...
  const auto& log_Pi
      = to_ref_if<is_autodiff_v<T_rate>>(log(gamma_q(n_val + 1.0, lambda_val)));
