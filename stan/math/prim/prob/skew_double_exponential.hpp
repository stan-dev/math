...
return_type_t<T_y, T_loc, T_scale> skew_double_exponential_lpdf(
  ...
  check_finite(function, "Skewness parameter", tau);
...
//  }
  ...
  operands_and_partials<T_y, T_loc, T_scale, T_skewness> ops_partials(y, mu, sigma, tau);

  scalar_seq_view<T_y> y_vec(y);
  scalar_seq_view<T_loc> mu_vec(mu);
  scalar_seq_view<T_scale> sigma_vec(sigma);
  scalar_seq_view<T_skewness> tau_vec(tau);
  size_t size_sigma = stan::math::size(sigma);
  size_t size_tau = stan::math::size(tau);
  size_t N = max_size(y, mu, sigma, tau);

  VectorBuilder<true, T_partials_return, T_scale> inv_sigma(size_sigma);
  VectorBuilder<true, T_partials_return, T_scale> inv_sigma(size_tau);
  VectorBuilder<include_summand<propto, T_scale>::value, T_partials_return,
                                                         T_scale> log_sigma(size_sigma);
  VectorBuilder<include_summand<propto, T_scale>::value, T_partials_return,
    T_scale> log_sigma(size_sigma);
