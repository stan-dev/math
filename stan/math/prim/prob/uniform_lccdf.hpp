...
return_type_t<T_y, T_low, T_high> uniform_lccdf(const T_y& y,
  ...
  if (!is_constant_all<T_y>::value) {
    stan::math::edge<0>(ops_partials).partials_ = inv(-b_minus_a * ccdf_log_n);
  }
  if (!is_constant_all<T_low, T_high>::value) {
    ...
    if (!is_constant_all<T_low>::value) {
      stan::math::edge<1>(ops_partials).partials_ = (beta_val - y_val) * rep_deriv;
    }
    if (!is_constant_all<T_high>::value) {
      stan::math::edge<2>(ops_partials).partials_ = (y_val - alpha_val) * rep_deriv;
    }
  }
