...
inline return_type_t<T_x, T_k> von_mises_cdf_series(const T_x& x, const T_k& k) {
 ...
 */
template <typename T_x, typename T_k>
inline return_type_t<T_x, T_k> von_mises_cdf_normalapprox(const T_x& x, const T_k& k) {
  using std::exp;
  using std::sqrt;
 ...
 */
template <typename T_x, typename T_k>
inline return_type_t<T_x, T_k> von_mises_cdf_centered(const T_x& x, const T_k& k) {
  using return_t = return_type_t<T_x, T_k>;
  return_t f;
  if (k < 49) {
