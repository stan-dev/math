 ...
 */
template <typename Scalar, typename T_a, typename T_w, typename T_v>
inline Scalar wiener_prob(const T_a& a, const T_v& v_value, const T_w& w_value) noexcept {
	const Scalar v = -v_value;
	const Scalar w = 1 - w_value;
    if (fabs(v) == 0.0) {
		return log1p(-w);
	}

    const Scalar exponent = -2.0 * v * a * (1.0 - w);
    if (exponent < 0) {
 ...
 */
template <typename Scalar, typename T_a, typename T_w, typename T_v>
inline Scalar wiener_prob_derivative_term(const T_a& a, const T_v& v_value, const T_w& w_value) noexcept {
     const Scalar exponent_m1 = log1p(-1.1 * 1.0e-8);
     Scalar ans;
	 const Scalar v = -v_value;
	 const Scalar w = 1 - w_value;
     ...
     if (v < 0) {
         const Scalar exponent_with_1mw = 2.0 * v * a * (1.0 - w);
		 const Scalar exponent_with_w = 2 * a * v * w;
		 const Scalar exponent = 2 * a * v;

         if (((exponent_with_1mw >= exponent_m1) || (exponent_with_w >= exponent_m1)) || (exponent >= exponent_m1)) {
			 return(-w);
		 }
         ans = LOG_TWO + exponent_with_1mw - log1m_exp(exponent_with_1mw);
         const Scalar diff_term = log1m_exp(exponent_with_w) - log1m_exp(exponent);
         const Scalar log_w = log(w);
         if (log_w > diff_term) {
         ...
         const Scalar diff_term = log_diff_exp(exponent_with_1mw, exponent) - log1m_exp(exponent);
		 const Scalar log_w = log(w);
         if (log_w > diff_term) {
             ans += log_diff_exp(log_w, diff_term);
             ans = -exp(ans);
 ...
 */
template <typename Scalar, typename T_a, typename T_w, typename T_v>
inline Scalar wiener_prob_grad_a(const T_a& a, const T_v& v, const T_w& w) noexcept{
     if (fabs(v) == 0.0) {
		 return 0.0;
	 }
	 const Scalar deriv_term = wiener_prob_derivative_term<Scalar>(a, v, w);
     const Scalar ans = -deriv_term * v;
     if (fabs(ans) < INFTY) {
		 return ans;
	 } else {
         return(NEGATIVE_INFTY);
 ...
 */
template <typename Scalar, typename T_a, typename T_w, typename T_v>
inline Scalar wiener_prob_grad_v(const T_a& a, const T_v& v, const T_w& w) noexcept{
	 const Scalar deriv_term = wiener_prob_derivative_term<Scalar>(a, v, w);
     const Scalar ans = -1 * deriv_term * a;
     if (fabs(ans) < INFTY) {
		 return(ans);
	 } else {
         return(NEGATIVE_INFTY);
 ...
 */
template <typename Scalar, typename T_a, typename T_w, typename T_v>
inline Scalar wiener_prob_grad_w(const T_a& a, const T_v& v_value, const T_w& w_value) noexcept{
	 const Scalar v = - v_value;
	 const Scalar w = 1 - w_value;
     if (fabs(v) == 0.0) {
		 return 1 / (1.0 - w);
     ...
     if (v < 0) {
         const Scalar exponent = 2.0 * v * a * (1.0 - w);
         return exp(LOG_TWO + exponent + log(fabs(v)) + log(a) - log1m_exp(exponent));
     }
     else
     {
         const Scalar exponent = -2.0 * v * a * (1.0 - w);
...
inline Scalar wiener4_ccdf(const T_y& y, const T_a& a, const T_v& v, const T_w& w, Scalar wildcard = 0, Scalar err=log(1e-12)) noexcept {
 ...
 */
template <typename Scalar, typename T_y, typename T_a, typename T_w, typename T_v>
inline Scalar wiener4_ccdf_grad_a(const T_y& y, const T_a& a, const T_v& v, const T_w& w, Scalar cdf, Scalar err=log(1e-12)) noexcept {
	const Scalar prob = wiener_prob<Scalar>(a, v, w); // maybe hand over to this function, but then wiener7_integrate_cdf has problems
	const Scalar prob_grad_a = wiener_prob_grad_a<Scalar>(a, v, w);
	const Scalar cdf_grad_a = wiener4_cdf_grad_a<Scalar>(y, a, v, w, cdf, err);
 ...
 */
template <typename Scalar, typename T_y, typename T_a, typename T_w, typename T_v>
inline Scalar wiener4_ccdf_grad_v(const T_y& y, const T_a& a, const T_v& v, const T_w& w, Scalar cdf, Scalar err=log(1e-12)) noexcept {
	const Scalar prob = wiener_prob<Scalar>(a, v, w); // maybe hand over to this function, but then wiener7_integrate_cdf has problems
	const Scalar prob_grad_v = wiener_prob_grad_v<Scalar>(a, v, w);
	const Scalar cdf_grad_v = wiener4_cdf_grad_v<Scalar>(y, a, v, w, cdf, err);
 ...
 */
template <typename Scalar, typename T_y, typename T_a, typename T_w, typename T_v>
inline Scalar wiener4_ccdf_grad_w(const T_y& y, const T_a& a, const T_v& v, const T_w& w, Scalar cdf, Scalar err=log(1e-12)) noexcept {
	const Scalar prob = wiener_prob<Scalar>(a, v, w); // maybe hand over to this function, but then wiener7_integrate_cdf has problems
	const Scalar prob_grad_w = wiener_prob_grad_w<Scalar>(a, v, w);
	const Scalar cdf_grad_w = wiener4_cdf_grad_w<Scalar>(y, a, v, w, cdf, err);
...
inline return_type_t<T_y, T_a, T_t0, T_w, T_v> wiener4_lccdf(const T_y& y, const T_a& a, const T_t0& t0, const T_w& w, const T_v& v, const ReturnT& precision_derivatives) {
  ...
  auto ops_partials = make_partials_propagator(y_ref, a_ref, t0_ref, w_ref,
    ...
    const T_partials_return v_value = v_vec.val(i);
		...
		const T_partials_return cdf = internal::estimate_with_err_check<T_partials_return, 5, false, 0, false>(
			[&](auto&&... args) { return internal::wiener4_distribution<true, T_partials_return>(args...); },
			log_error_cdf - LOG_TWO, y_value - t0_value, a_value, v_value, w_value, 0, 
			log_error_absolute);
			
		const T_partials_return prob =  exp(internal::wiener_prob<T_partials_return>(a_value, v_value, w_value));
		const T_partials_return ccdf = prob - cdf;
		
		lccdf += log(ccdf);
		
		const T_partials_return new_est_err = log(ccdf) + log_error_derivative - LOG_FOUR;
		...
		if (!is_constant_all<T_a>::value) {
			...
			[&](auto&&... args) { return internal::wiener4_ccdf_grad_a<T_partials_return>(args...); },
			new_est_err, y_value - t0_value, a_value, v_value, w_value, cdf,
			log_error_absolute) / ccdf;
		}
		...
		if (!is_constant_all<T_w>::value) {
			...
			[&](auto&&... args) { return internal::wiener4_ccdf_grad_w<T_partials_return>(args...); },
			new_est_err, y_value - t0_value, a_value, v_value, w_value, cdf,
			log_error_absolute) / ccdf;
		}
		if(!is_constant_all<T_v>::value) {
			partials<4>(ops_partials)[i] = internal::wiener4_ccdf_grad_v<T_partials_return>(y_value - t0_value, a_value, v_value, w_value, cdf) / ccdf;
		}
	} // for loop
