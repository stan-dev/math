 ...
 * @return 'P' term
 */ 
template <typename Scalar, bool Distribution = false, bool GradAV = false, bool GradW = false, typename T_a, typename T_w, typename T_v>
 ...
 * @return 'K_small' term
 */ 
template <typename Scalar, bool Distribution = false, bool GradA = false, bool GradV = false, bool GradW = false, typename T_y, typename T_a, typename T_w, typename T_v>
inline Scalar K_Small(const T_y& y, const T_a& a, const T_v& v, const T_w& w, Scalar precision) noexcept {
	if (Distribution) {
		const Scalar K1 = 0.5 * (fabs(v) / a * y - w);
		const Scalar arg = fmax(0, fmin(1, exp(v*a*w + square(v)*y / 2 + (precision)) / 2));
		const Scalar K2 = (arg==0) ? INFTY : (arg==1) ? NEGATIVE_INFTY : -sqrt(y) / 2 / a * inv_Phi(arg);
	...
	if (GradW) {
		...
		K_small = (arg == 0) ? INFTY : (arg == 1) ? NEGATIVE_INFTY : -sqrt_y / a * inv_Phi(arg) - wdash;
	}
 ...
 * @return 'K_large' term
 */
template <typename Scalar, bool Distribution = false, bool GradA = false, bool GradV = false, typename T_y, typename T_a, typename T_w, typename T_v>
inline Scalar K_Large(const T_y& y, const T_a& a, const T_v& v, const T_w& w, Scalar precision) noexcept {
	if (Distribution) {
		const Scalar api = a / pi();
		const Scalar v_square = square(v);
		const Scalar sqrtL1 = sqrt(1 / y) * api;
		const Scalar sqrtL2 = sqrt(fmax(1.0, -2 / y * square(api) * (precision + log(pi() * y / 2 * (v_square + square(pi() / a))) + v * a * w + v_square * y / 2)));
	...
	if (GradA) {
		...
		const Scalar alphK = fmin(factor + LOG_PI + log_y + log_a - LOG_TWO - C1, 0.0);
	...
	if (GradV && v == 0) {
		...
		if (GradV) {
			...
			alphK = rexp<Scalar>(factor + 0.5 * (7*LOG_PI + log_y) - 2.5 * LOG_TWO - 3 * log_a - log_v);
			alphK = fmax(0.0, fmin(1.0, alphK));
		} else {
			alphK = rexp<Scalar>(factor + 0.5 * (LOG_PI + log_y) - 1.5 * LOG_TWO - log_a);
			alphK = fmax(0.0, fmin(1.0, alphK)); 
		}
		return fmax(ceil((alphK == 0) ? INFTY : (alphK == 1) ? NEGATIVE_INFTY : temp * inv_Phi(alphK)), 1.0);
	}
}
 ...
 */	
template <typename Scalar, bool Distribution = false, bool GradA = false, bool GradV = false, bool GradW = false, typename T_y, typename T_a, typename T_w, typename T_v>
inline Scalar logFs(const T_y& y, const T_a& a, const T_v& v, const T_w& w, Scalar K) noexcept {
	const Scalar sqrt_y = sqrt(y);
	const Scalar vy = v * y;
	Scalar ans = 0;
	if (Distribution) {
 ...
 */	 
template <typename Scalar, bool Distribution = false, bool GradA = false, bool GradV = false, bool GradW = false, typename T_y, typename T_a, typename T_w, typename T_v>
inline Scalar logFl(const T_y& y, const T_a& a, const T_v& v, const T_w& w, Scalar K, Scalar cdf = 0) noexcept {
...
//			Scalar summand_2 = temp0 - log_sum_exp(2 * log_v, 2 * (temp0 + LOG_PI - log_a)) - 0.5 * square(temp1 / a) * y + log(fabs(check));
//			std::forward_as_tuple(ans, ans_sign)
//			= stan::math::log_sum_exp_signed(ans, ans_sign, summand_2, check_sign);
			if (check > 0) {
				fplus = log_sum_exp(fplus, temp0 - log_sum_exp(2 * log_v, 2 * (temp0 + LOG_PI - log_a)) - 0.5 * square(temp1 / a) * y + log(check));
			} else if (check < 0) {
				fminus = log_sum_exp(fminus, temp0 - log_sum_exp(2 * log_v, 2 * (temp0 + LOG_PI - log_a)) - 0.5 * square(temp1 / a) * y + log(-check));
			}
		}
		if (fplus > fminus) {
			ans = log_diff_exp(fplus, fminus);
	...
	for (Scalar k = K; k >= 1; k--) {
		...
		Scalar last = GradA ? square(kpi) / pow(a, 3) * (y + 2.0 * denom) : GradV ? denom : kpi;
		last *= denomk * ekpia2y;
		ans += - last * factor;
	}
	...
	Scalar temp_deriv = GradA ? ((fabs(v) == 0) ? 0 : is_inf(dav * v) ? NEGATIVE_INFTY : dav * v) : GradV ? is_inf(dav * a)? NEGATIVE_INFTY : dav * a : dav;
	temp_deriv *= temp;
	ans = GradA ? (-2 / a - v * w) * (cdf-temp) + ans * pia2 * evaw : GradV ? (-w * a - v * y) * (cdf - temp) + ans * (-2 * v) * pia2 * evaw : -v * a * (cdf-temp) + ans * pia2 * evaw;
 ...
 *
 * @tparam NaturalScale Whether to return the distribution on natural or log-scale
 *
 * @param y A scalar variable; the reaction time in seconds
 * @param a The boundary separation
 ...
 * @return distribution
 */
template <bool NaturalScale = false, typename Scalar, typename T_y, typename T_a, typename T_w, typename T_v>
inline Scalar wiener4_distribution(const T_y& y, const T_a& a, const T_v& vn, const T_w& wn, Scalar wildcard=0, Scalar err=log(1e-12)) noexcept {
	Scalar ans;
	const Scalar v = -vn;
	const Scalar w = 1-wn;
 ...
 */
template <typename Scalar, typename T_y, typename T_a, typename T_w, typename T_v>
inline Scalar wiener4_cdf_grad_a(const T_y& y, const T_a& a, const T_v& vn, const T_w& wn, Scalar cdf, Scalar err=log(1e-12)) noexcept {
	const Scalar v = -vn;
	const Scalar w = 1-wn;
 ...
 */
template <typename Scalar, typename T_y, typename T_a, typename T_w, typename T_v>
inline Scalar wiener4_cdf_grad_v(const T_y& y, const T_a& a, const T_v& vn, const T_w& wn, Scalar cdf, Scalar err=log(1e-12)) noexcept {
	const Scalar v = -vn;
	const Scalar w = 1-wn;
	...
	if (K_large > 4 * K_small) {
		return -1 * ((-w * a - v * y) * cdf + logFs<Scalar, false, false, true>(y, a, v, w, (K_small)));
	} else {
 ...
 */
template <typename Scalar, typename T_y, typename T_a, typename T_w, typename T_v>
inline Scalar wiener4_cdf_grad_w(const T_y& y, const T_a& a, const T_v& vn, const T_w& wn, Scalar cdf, Scalar err=log(1e-12)) noexcept {	
    const Scalar v = -vn;
	const Scalar w = 1 - wn;
	const Scalar K_large = K_Large<Scalar>(y, a, v, w, err);
	const Scalar K_small = K_Small<Scalar, false, false, false, true>(y, a, v, w, err);
	if (K_large > 4 * K_small) {
		return  -1 * (-v * a * cdf + logFs<Scalar, false, false, false, true>(y, a, v, w, (K_small)));
	} else {
		return -1 * logFl<Scalar, false, false, false, true>(y, a, v, w, (K_large), cdf);
	}
}
}  // namespace internal
 ...
 */
template <bool propto = false, typename T_y, typename T_a, typename T_t0, typename T_w, typename T_v, typename ReturnT
          = return_type_t<T_y, T_a, T_t0, T_w, T_v>>
inline return_type_t<T_y, T_a, T_t0, T_w, T_v> wiener4_lcdf(const T_y& y, const T_a& a, const T_t0& t0, const T_w& w, const T_v& v, const ReturnT& precision_derivatives) {
  ...
  auto ops_partials = make_partials_propagator(y_ref, a_ref, t0_ref, w_ref,
    ...
    const T_partials_return v_value = v_vec.val(i);
		...
		const T_partials_return log_cdf = internal::estimate_with_err_check<T_partials_return, 5, false, 0, false>(
			[&](auto&&... args) { return internal::wiener4_distribution<false, T_partials_return>(args...); },
			log_error_cdf - LOG_TWO, y_value - t0_value, a_value, v_value, w_value, 0, 
			log_error_absolute);
		...
		const T_partials_return new_est_err = log_cdf + log_error_derivative - LOG_FOUR;

		const T_partials_return deriv_y = internal::estimate_with_err_check<T_partials_return, 5>(
			[&](auto&&... args) { return internal::wiener5_density<true, T_partials_return>(args...); },
		...
		if (!is_constant_all<T_a>::value) {
			partials<1>(ops_partials)[i] = internal::estimate_with_err_check<T_partials_return, 5>(
			[&](auto&&... args) { return internal::wiener4_cdf_grad_a<T_partials_return>(args...); },
			new_est_err, y_value - t0_value, a_value, v_value, w_value, cdf,
			log_error_absolute) / cdf;
		}
		if(!is_constant_all<T_t0>::value) {
			partials<2>(ops_partials)[i] = -deriv_y / cdf;
		}
		if (!is_constant_all<T_w>::value) {
			partials<3>(ops_partials)[i] = internal::estimate_with_err_check<T_partials_return, 5>(
			[&](auto&&... args) { return internal::wiener4_cdf_grad_w<T_partials_return>(args...); },
			new_est_err, y_value - t0_value, a_value, v_value, w_value, cdf,
			log_error_absolute) / cdf;
		}
		if(!is_constant_all<T_v>::value) {
			partials<4>(ops_partials)[i] = internal::wiener4_cdf_grad_v<T_partials_return>(y_value-t0_value, a_value, v_value, w_value, cdf) / cdf;
		}
	} // for loop
