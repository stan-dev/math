...
namespace stan {
  namespace math {
    ...
    inline void check_3F2_converges(const std::string& function, const T_a1& a1,
                                    const T_a2& a2, const T_a3& a3,
                                    const T_b1& b1, const T_b2& b2,
                                    const T_z& z) {
      using std::floor;
      using std::fabs;
      ...
      bool is_undefined =
          (is_nonpositive_integer(b1) && fabs(b1) <= num_terms) ||
