...
namespace stan {
  namespace math {
     ...
     *   - <code>|z|</code> is equal to one and <code>b1 + b2 < a1 + a2 +
     * a3</code> This function is a rational polynomial if
    ...
    T F32(const T& a1, const T& a2, const T& a3, const T& b1, const T& b2,
      ...
      for (int k = 0; k <= max_steps; ++k) {
        ...
        if (fabs(t_new) <= precision) return t_acc;
