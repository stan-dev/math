...
namespace stan {
  namespace math {
     ...
     *
     * <p>\f$\Gamma_k(x) = \pi^{k(k-1)/4} \, \prod_{j=1}^k \Gamma(x + (1 -
     j)/2)\f$,
     *
     ...
     *
     ...
     \{\dots, -3, -2, -1, 0\}\\[6pt] \textrm{NaN} & \mbox{if } x = \textrm{NaN}
     \end{cases} \f]
       ...
       \f[
       \frac{\partial \, \ln\Gamma_n(x)}{\partial x} = \sum_{j=1}^n \Psi(x + (1
     - j) / 2) \f]
     *
     ...
     */
    template <typename T>
    inline typename boost::math::tools::promote_args<T>::type lmgamma(int k,
                                                                      T x) {
      using boost::math::lgamma;
      typename boost::math::tools::promote_args<T>::type result =
          k * (k - 1) * LOG_PI_OVER_FOUR;

      for (int j = 1; j <= k; ++j) result += lgamma(x + (1.0 - j) / 2.0);
      return result;
    }
