  ...
  namespace math {
    ...
    struct lower_reg_inc_gamma<2> {
      ...
      grad(T1 a, T2 z, double precision = 1e-10,
        ...
        return -stan::math::grad_reg_inc_gamma(a, z, tg, dig,
          max_steps, precision);
      }
    };
    ...
    struct lower_reg_inc_gamma<0> {
      ...
      grad(T1 a, T2 z, double precision = 1e-10,
        ...
        if ((a < 0.8 && z > 15.0) || (a < 12.0 && z > 30.0) ||
            a < sqrt(-756 - z * z + 60 * z)) {
          result = stan::math::lower_reg_inc_gamma<2>::grad<1>(a, z,
            precision, max_steps);
        } else if (a < 0.8 && z <= 15.0) {
          result = stan::math::lower_reg_inc_gamma<3>::grad<1>(a, z,
            precision, max_steps);
        } else {
          result = stan::math::lower_reg_inc_gamma<1>::grad<1>(a, z,
            precision, max_steps);
        }
        return result;
      }
