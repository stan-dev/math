 ...
 *  during compile time. You can think of these as "legacy concepts" aka I think the closest thing you can get to C++20 concepts without having the requires keyword.
 *
 * Legacy concepts are done through enable_if_t aliases which are called require_*. There are 5 basic types of requires.
 *
 *    requires_*_t: This means the function requires a certain type to turn on. For instance
 *~~~~~{.cpp}
