...
namespace stan {
  namespace math {
    ...
    /**
     * Returns the log PMF of the Beta-Binomial distribution with given
    ...
    typename return_type<T_size1, T_size2>::type beta_binomial_lpmf(
      ...
      if (!include_summand<propto, T_size1, T_size2>::value) return 0.0;
      ...
      for (size_t i = 0; i < size; i++)
        if (include_summand<propto, T_size1, T_size2>::value)
          lbeta_numerator[i] =
              lbeta(n_vec[i] + value_of(alpha_vec[i]),
      ...
      for (size_t i = 0; i < max_size(N, alpha, beta); i++)
        if (contains_nonconstant_struct<T_size1, T_size2>::value)
          digamma_N_plus_alpha_plus_beta[i] = digamma(
              N_vec[i] + value_of(alpha_vec[i]) + value_of(beta_vec[i]));
      ...
      VectorBuilder<!is_constant_struct<T_size1>::value, T_partials_return,
                    T_size1>
      ...
      VectorBuilder<!is_constant_struct<T_size2>::value, T_partials_return,
                    T_size2>
      ...
      for (size_t i = 0; i < size; i++) {
        if (include_summand<propto>::value) logp += normalizing_constant[i];
        if (include_summand<propto, T_size1, T_size2>::value)
