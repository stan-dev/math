...
namespace stan {
  namespace math {
    ...
    /**
     * The log of the beta density for the specified scalar(s) given the
     * specified sample size(s). y, alpha, or beta can each either be scalar or
     * a vector. Any vector inputs must be the same length.
     *
    ...
    typename return_type<T_y, T_scale_succ, T_scale_fail>::type beta_lpdf(
      ...
      typedef typename stan::partials_return_type<
          T_y, T_scale_succ, T_scale_fail>::type T_partials_return;
      ...
      for (size_t n = 0; n < N; n++) {
        ...
        if (y_dbl < 0 || y_dbl > 1) return LOG_ZERO;
      }
      ...
      VectorBuilder<
          contains_nonconstant_struct<T_scale_succ, T_scale_fail>::value,
          T_partials_return, T_scale_succ, T_scale_fail>
