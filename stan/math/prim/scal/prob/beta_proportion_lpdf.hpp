...
typename return_type<T_y, T_loc, T_prec>::type
beta_proportion_lpdf(const T_y &y, const T_loc &mu, const T_prec &kappa) {
  ...
  for (size_t n = 0; n < N; n++) {
    ...
    if (!is_constant_struct<T_loc>::value)
      ops_partials.edge2_.partials_[n] +=
          kappa_dbl * (digamma_kappa_mukappa[n] - digamma_mukappa[n] +
                       log_y[n] - log1m_y[n]);
    if (!is_constant_struct<T_prec>::value)
...
inline typename return_type<T_y, T_loc, T_prec>::type
beta_proportion_lpdf(const T_y &y, const T_loc &mu, const T_prec &kappa) {
