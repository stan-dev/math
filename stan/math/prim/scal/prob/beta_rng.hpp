      ...
      for (size_t n = 0; n < N; ++n) {
        ...
        if (alpha_vec[n] > 1.0 && beta_vec[n] > 1.0) {
          variate_generator<RNG&, gamma_distribution<> >
            rng_gamma_alpha(rng, gamma_distribution<>(alpha_vec[n], 1.0));
          variate_generator<RNG&, gamma_distribution<> >
            rng_gamma_beta(rng, gamma_distribution<>(beta_vec[n], 1.0));
          double a = rng_gamma_alpha();
          double b = rng_gamma_beta();
          output[n] = a / (a + b);
        } else {
          variate_generator<RNG&, gamma_distribution<> >
            rng_gamma_alpha(rng, gamma_distribution<>(alpha_vec[n] + 1, 1.0));
          variate_generator<RNG&, gamma_distribution<> >
            rng_gamma_beta(rng, gamma_distribution<>(beta_vec[n] + 1, 1.0));
          double log_a = std::log(uniform_rng()) / alpha_vec[n]
            + std::log(rng_gamma_alpha());
          double log_b = std::log(uniform_rng()) / beta_vec[n]
            + std::log(rng_gamma_beta());
