...
namespace stan {
  namespace math {
    ...
    /**
     * Binomial log PMF in logit parametrization. Binomial(n|n,
     * inv_logit(alpha))
     *
     ...
     *
     * @throw std::domain_error if N is negative or probability parameter is
     * invalid
     * @throw std::invalid_argument if vector sizes do not match
     */
    template <bool propto, typename T_n, typename T_N, typename T_prob>
    typename return_type<T_prob>::type binomial_logit_lpmf(
      ...
      if (!include_summand<propto, T_prob>::value) return 0.0;
