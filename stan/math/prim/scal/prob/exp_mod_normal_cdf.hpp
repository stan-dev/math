...
inline auto exp_mod_normal_cdf(const T_y& y, const T_loc& mu,
                               const T_scale& sigma,
  ...
  for (size_t n = 0; n < N; n++) {
    ...
    const T_partials erf_calc = 0.5 * (1 + erf(-v / SQRT_2 + scaled_diff));
    const T_partials deriv_1 = lambda_dbl * exp(0.5 * v_sq - u) * erf_calc;
