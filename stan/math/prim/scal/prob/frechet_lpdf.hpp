...
namespace stan {
  namespace math {
    ...
    typename return_type<T_y, T_shape, T_scale>::type frechet_lpdf(
      ...
      if (!include_summand<propto, T_y, T_shape, T_scale>::value) return 0.0;
      ...
      operands_and_partials<T_y, T_shape, T_scale> ops_partials(y, alpha,
                                                                sigma);
      for (size_t n = 0; n < N; n++) {
        ...
        if (!is_constant_struct<T_scale>::value)
          ops_partials.edge3_.partials_[n] += alpha_dbl /
