...
namespace stan {
  namespace math {
    /**
     * Return a pseudorandom Frechet variate with given shape and scale using
     * the specified random number generator.
     *
     * alpha and sigma can each be either scalars or vectors. Any vector inputs
     * must be the same length.
     *
     * @tparam T_shape Type of scale parameter
     * @tparam T_scale Type of shape parameter
    ...
    frechet_rng(const T_shape& alpha,
                const T_scale& sigma,
                RNG& rng) {
      ...
      for (size_t n = 0; n < N; n++) {
        variate_generator<RNG&, weibull_distribution<> >
          weibull_rng(rng,
                      weibull_distribution<>(alpha_vec[n], 1.0 / sigma_vec[n]));
        output[n] = 1.0 / weibull_rng();
      }
