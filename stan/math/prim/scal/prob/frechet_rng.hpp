...
namespace stan {
  namespace math {
     ...
     *
     * alpha and sigma can each be a scalar, a std::vector, an Eigen::Vector, or
    ...
    frechet_rng(const T_shape& alpha, const T_scale& sigma, RNG& rng) {
      ...
      for (size_t n = 0; n < N; ++n) {
        variate_generator<RNG&, weibull_distribution<> >
          weibull_rng(rng, weibull_distribution<>(alpha_vec[n],
                                                  1.0 / sigma_vec[n]));
