...
namespace stan {
  namespace math {
     ...
     *
     ...
     \\
     &=& \alpha \log(\beta) - \log(\Gamma(\alpha)) + (\alpha - 1) \log(y) -
     \beta y\\ & & \mathrm{where} \; y > 0 \f}
    ...
    typename return_type<T_y, T_shape, T_inv_scale>::type gamma_lpdf(
      ...
      typedef
          typename stan::partials_return_type<T_y, T_shape, T_inv_scale>::type
              T_partials_return;
      ...
      for (size_t n = 0; n < length(y); n++) {
        ...
        if (y_dbl < 0) return LOG_ZERO;
      }

      size_t N = max_size(y, alpha, beta);
      operands_and_partials<T_y, T_shape, T_inv_scale> ops_partials(y, alpha,
                                                                    beta);
      ...
      if (include_summand<propto, T_y, T_shape>::value) {
        for (size_t n = 0; n < length(y); n++) {
          if (value_of(y_vec[n]) > 0) log_y[n] = log(value_of(y_vec[n]));
        }
      }
      ...
      for (size_t n = 0; n < N; n++) {
        ...
        if (include_summand<propto, T_shape>::value) logp -= lgamma_alpha[n];
        if (include_summand<propto, T_shape, T_inv_scale>::value)
