...
namespace stan {
  namespace math {
     ...
     * @throw std::domain_error if y is nan, mu is infinite, or beta is
     * nonpositive
    ...
    typename return_type<T_y, T_loc, T_scale>::type gumbel_lpdf(
      ...
      check_consistent_sizes(function, "Random variable", y,
                             "Location parameter", mu, "Scale parameter", beta);
      ...
      for (size_t n = 0; n < N; n++) {
        ...
        if (include_summand<propto, T_scale>::value) logp -= log_beta[n];
        if (include_summand<propto, T_y, T_loc, T_scale>::value)
          logp += -y_minus_mu_over_beta - exp(-y_minus_mu_over_beta);

        T_partials_return scaled_diff =
            inv_beta[n] * exp(-y_minus_mu_over_beta);
