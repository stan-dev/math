...
typename return_type<T_y, T_loc, T_scale>::type lognormal_lpdf(
  ...
  VectorBuilder<include_summand<propto, T_y, T_loc, T_scale>::value,
      ...
      log_y(length(y));
  if (include_summand<propto, T_y, T_loc, T_scale>::value) {
    for (size_t n = 0; n < length(y); n++) log_y[n] = log(value_of(y_vec[n]));
  }

  VectorBuilder<!is_constant_struct<T_y>::value, T_partials_return, T_y> inv_y(
      length(y));
  if (!is_constant_struct<T_y>::value) {
    for (size_t n = 0; n < length(y); n++) inv_y[n] = 1 / value_of(y_vec[n]);
  }

  if (include_summand<propto>::value) logp += N * NEG_LOG_SQRT_TWO_PI;
  ...
  for (size_t n = 0; n < N; n++) {
    ...
    if (include_summand<propto, T_scale>::value) logp -= log_sigma[n];
    if (include_summand<propto, T_y>::value) logp -= log_y[n];
    if (include_summand<propto, T_y, T_loc, T_scale>::value)
