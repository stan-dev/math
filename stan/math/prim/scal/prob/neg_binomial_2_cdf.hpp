...
namespace stan {
  namespace math {
    ...
    typename return_type<T_location, T_precision>::type neg_binomial_2_cdf(
      ...
      if (!(stan::length(n) && stan::length(mu) && stan::length(phi))) return P;
      ...
      for (size_t i = 0; i < stan::length(n); i++) {
        if (value_of(n_vec[i]) < 0) return ops_partials.build(0.0);
      }
      ...
      for (size_t i = 0; i < size; i++) {
        ...
        if (!is_constant_struct<T_location>::value)
          ops_partials.edge1_.partials_[i] +=
              -inc_beta_ddz(phi_dbl, n_dbl + 1.0, p_dbl) * phi_dbl * d_dbl /
              P_i;
