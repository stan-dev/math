...
namespace stan {
  namespace math {
    ...
    template <bool propto, typename T_n, typename T_location,
    ...
    typename return_type<T_location, T_precision>::type neg_binomial_2_lpmf(
      ...
      typedef typename stan::partials_return_type<
          T_n, T_location, T_precision>::type T_partials_return;
      ...
      check_consistent_sizes(function, "Failures variable", n,
                             "Location parameter", mu, "Precision parameter",
                             phi);

      if (!include_summand<propto, T_location, T_precision>::value) return 0.0;
      ...
      VectorBuilder<true, T_partials_return, T_n, T_precision> n_plus_phi(
          len_np);
      for (size_t i = 0; i < len_np; ++i) n_plus_phi[i] = n_vec[i] + phi__[i];

      for (size_t i = 0; i < size; i++) {
        if (include_summand<propto>::value) logp -= lgamma(n_vec[i] + 1.0);
    ...
    inline typename return_type<T_location, T_precision>::type
    neg_binomial_2_lpmf(const T_n& n, const T_location& mu,
