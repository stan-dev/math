...
namespace stan {
  namespace math {
    ...
    typename return_type<T_shape, T_inv_scale>::type neg_binomial_lpmf(
      ...
      typedef
          typename stan::partials_return_type<T_n, T_shape, T_inv_scale>::type
              T_partials_return;
      ...
      if (!include_summand<propto, T_shape, T_inv_scale>::value) return 0.0;
      ...
      VectorBuilder<true, T_partials_return, T_inv_scale> log1p_beta(
          length(beta));
      ...
      for (size_t i = 0; i < size; i++) {
        if (alpha_vec[i] > 1e10) {  // reduces numerically to Poisson
          ...
          if (include_summand<propto, T_shape, T_inv_scale>::value)
            logp +=
                alpha_times_log_beta_over_1p_beta[i] - n_vec[i] * log1p_beta[i];
