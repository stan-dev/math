...
namespace stan {
  namespace math {
    ...
    typename return_type<T_y, T_s, T_loc, T_scale>::type normal_sufficient_lpdf(
        const T_y& y_bar, const T_s& s_squared, const T_n& n_obs,
        const T_loc& mu, const T_scale& sigma) {
      static const std::string function = "normal_sufficient_lpdf";
      typedef
          typename stan::partials_return_type<T_y, T_s, T_n, T_loc,
      ...
      check_consistent_sizes(
          function, "Location parameter sufficient statistic", y_bar,
          "Scale parameter sufficient statistic", s_squared,
          "Number of observations", n_obs, "Location parameter", mu,
          "Scale parameter", sigma);
      // check if no variables are involved and prop-to
      if (!include_summand<propto, T_y, T_s, T_loc, T_scale>::value) return 0.0;

      // set up template expressions wrapping scalars into vector views
      operands_and_partials<T_y, T_s, T_loc, T_scale> ops_partials(
          y_bar, s_squared, mu, sigma);
