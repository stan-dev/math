...
typename return_type<T_y, T_scale, T_shape>::type pareto_lpdf(
  ...
  if (!include_summand<propto, T_y, T_scale, T_shape>::value) return 0.0;
  ...
  for (size_t n = 0; n < N; n++) {
    if (y_vec[n] < y_min_vec[n]) return LOG_ZERO;
  }
  ...
  for (size_t n = 0; n < N; n++) {
    ...
    if (include_summand<propto, T_shape>::value) logp += log_alpha[n];
    if (include_summand<propto, T_scale, T_shape>::value)
