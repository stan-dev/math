...
namespace stan {
  namespace math {
    ...
    typename return_type<T_rate>::type poisson_lpmf(const T_n& n,
      ...
      if (!(stan::length(n) && stan::length(lambda))) return 0.0;
      ...
      if (!include_summand<propto, T_rate>::value) return 0.0;
      ...
      for (size_t i = 0; i < size; i++)
        if (is_inf(lambda_vec[i])) return LOG_ZERO;
      for (size_t i = 0; i < size; i++)
    ...
    inline typename return_type<T_rate>::type poisson_lpmf(
        const T_n& n, const T_rate& lambda) {
