...
namespace stan {
  namespace math {
    ...
    typename return_type<T_y, T_scale>::type rayleigh_cdf(
      ...
      if (!(stan::length(y) && stan::length(sigma))) return cdf;
      ...
      for (size_t n = 0; n < N; n++) {
        ...
        if (include_summand<false, T_y, T_scale>::value) cdf *= (1.0 - exp_val);
      }
