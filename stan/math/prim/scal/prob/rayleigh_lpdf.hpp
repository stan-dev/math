...
namespace stan {
  namespace math {
    ...
    typename return_type<T_y, T_scale>::type rayleigh_lpdf(
      ...
      if (!(stan::length(y) && stan::length(sigma))) return 0.0;
      ...
      if (!include_summand<propto, T_y, T_scale>::value) return 0.0;
      ...
      for (size_t n = 0; n < N; n++) {
        ...
        if (include_summand<propto, T_scale>::value) logp -= 2.0 * log_sigma[n];
        if (include_summand<propto, T_y>::value) logp += log(y_dbl);
