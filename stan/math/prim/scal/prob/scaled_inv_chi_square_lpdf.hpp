...
namespace stan {
  namespace math {
    ...
    typename return_type<T_y, T_dof, T_scale>::type scaled_inv_chi_square_lpdf(
      ...
      if (!(stan::length(y) && stan::length(nu) && stan::length(s))) return 0.0;
      ...
      if (!include_summand<propto, T_y, T_dof, T_scale>::value) return 0.0;
      ...
      for (size_t n = 0; n < N; n++) {
        if (value_of(y_vec[n]) <= 0) return LOG_ZERO;
      }
      ...
      VectorBuilder<include_summand<propto, T_dof>::value, T_partials_return,
                    T_dof>
      ...
      for (size_t n = 0; n < N; n++) {
        ...
        if (!is_constant_struct<T_dof>::value) {
          ops_partials.edge2_.partials_[n] +=
              0.5 * log_half_nu[n] + 0.5 - digamma_half_nu_over_two[n] +
              log_s[n] - 0.5 * log_y[n] - 0.5 * s_dbl * s_dbl * inv_y[n];
        }
