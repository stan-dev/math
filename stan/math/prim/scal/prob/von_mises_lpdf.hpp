...
typename return_type<T_y, T_loc, T_scale>::type von_mises_lpdf(
  ...
  if (!(stan::length(y) && stan::length(mu) && stan::length(kappa))) return 0.0;
  ...
  for (size_t n = 0; n < N; n++) {
    ...
    if (compute_bessel0) bessel0 = modified_bessel_first_kind(0, kappa_dbl[n]);
    T_partials_return bessel1 = 0;
    if (compute_bessel1) bessel1 = modified_bessel_first_kind(-1, kappa_dbl[n]);
    ...
    if (include_summand<propto, T_scale>::value) logp -= log_bessel0[n];
    if (include_summand<propto, T_y, T_loc, T_scale>::value) logp += kappa_cos;

    if (!y_const) ops_partials.edge1_.partials_[n] += kappa_sin;
    if (!mu_const) ops_partials.edge2_.partials_[n] -= kappa_sin;
    if (!kappa_const)
