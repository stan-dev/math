...
inline auto weibull_log(T_y&& y, T_shape&& alpha, T_scale&& sigma) {
  return weibull_lpdf<propto>(std::forward<T_y>(y),
...
inline auto weibull_log(T_y&& y, T_shape&& alpha, T_scale&& sigma) {
  return weibull_lpdf(std::forward<T_y>(y), std::forward<T_shape>(alpha),
                      std::forward<T_scale>(sigma));
}
