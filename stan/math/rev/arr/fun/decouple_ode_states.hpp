...
namespace stan {
  namespace math {
    ...
    decouple_ode_states(const std::vector<std::vector<double> >& y,
      ...
      const size_t S =
          (initial_var::value ? N : 0) + (param_var::value ? M : 0);

      vars.reserve(S);
      if (initial_var::value) vars.insert(vars.end(), y0.begin(), y0.end());
      if (param_var::value) vars.insert(vars.end(), theta.begin(), theta.end());
     ...
     */
    template <>
    inline std::vector<std::vector<double> > decouple_ode_states(
        const std::vector<std::vector<double> >& y,
        const std::vector<double>& y0, const std::vector<double>& theta) {
      return y;
    }
