  ...
  namespace math {
    ...
    inline void add_initial_values(const std::vector<var>& y0,
      ...
      for (size_t n = 0; n < y.size(); n++)
        for (size_t m = 0; m < y0.size(); m++) y[n][m] += y0[m];
    }
    ...
    struct coupled_ode_system<F, double, var> {
      ...
      std::vector<double> initial_state() const {
        ...
        for (size_t n = 0; n < N_; n++) state[n] = y0_dbl_[n];
        return state;
      }
      ...
      std::vector<std::vector<var> > decouple_states(
        ...
        for (size_t i = 0; i < y.size(); i++) {
          ...
          for (size_t j = 0; j < N_; j++) {
            ...
            temp_vars[j] =
                precomputed_gradients(y[i][j], theta_, temp_gradients);
          }
    ...
    struct coupled_ode_system<F, var, double> {
      ...
      void operator()(const std::vector<double>& z, std::vector<double>& dz_dt,
        ...
        for (size_t n = 0; n < N_; n++) y[n] += y0_dbl_[n];
