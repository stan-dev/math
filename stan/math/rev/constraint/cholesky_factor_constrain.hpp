...
inline var_value<Eigen::MatrixXd> cholesky_factor_constrain(const T& x, int M,
                                                            int N) {
  using std::exp;
 ...
 */
template <typename T, require_var_vector_t<T>* = nullptr>
inline var_value<Eigen::MatrixXd> cholesky_factor_constrain(
    const T& x, int M, int N, scalar_type_t<T>& lp) {
  check_size_match("cholesky_factor_constrain", "x.size()", x.size(),
                   "((N * (N + 1)) / 2 + (M - N) * N)",
                   ((N * (N + 1)) / 2 + (M - N) * N));
  int pos = 0;
