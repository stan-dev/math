#ifndef STAN_MATH_REV_CORE_COPY_VARS_HPP
#define STAN_MATH_REV_CORE_COPY_VARS_HPP
...
inline decltype(auto) copy_vars(Arith&& arg) {
  return std::forward<Arith>(arg);
}

/**
 * Copy the value of a var but reallocate a new vari on the chainable stack
 *
 * @param arg A var
 * @return A new var
 */
inline auto copy_vars(const var& arg) {
  return var(new internal::add_vd_vari(arg.vi_, 0.0));
}
...
inline auto copy_vars(VarVec&& arg) {
  std::vector<var> copy_vec(arg.size());
  for (size_t i = 0; i < arg.size(); ++i) {
    // copy_vec[i] = new vari(arg[i].val(), false);
    copy_vec[i] = var(new internal::add_vd_vari(arg[i].vi_, 0.0));
  }
  return copy_vec;
}
...
inline auto copy_vars(VecContainer&& arg) {
  std::vector<value_type_t<VecContainer>> copy_vec(arg.size());
  for (size_t i = 0; i < arg.size(); ++i) {
    copy_vec[i] = copy_vars(arg[i]);
  }
  return copy_vec;
}
...
inline auto copy_vars(EigT&& arg) {
  // return arg.unaryExpr([](auto&& x) { return var(new vari(x.val(), false));
  // })
  //    .eval();
  return arg
      .unaryExpr(
          [&](auto&& x) { return var(new internal::add_vd_vari(x.vi_, 0.0)); })
      .eval();
}
