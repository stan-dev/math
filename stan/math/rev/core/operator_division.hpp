...
inline auto divide(const Mat& m, Scalar c) {
  if constexpr (!is_constant<Mat>::value && !is_constant<Scalar>::value) {
  ...
  } else if constexpr (!is_constant<Mat>::value) {
    arena_t<promote_scalar_t<var, Mat>> arena_m = m;
...
inline auto divide(Scalar c, const Mat& m) {
  if constexpr (!is_constant<Scalar>::value && !is_constant<Mat>::value) {
    arena_t<promote_scalar_t<var, Mat>> arena_m = m;
