...
inline var_value<T>& var_value<T, require_floating_point_t<T>>::operator-=(const var_value<S>& b) {
...
inline var_value<T>& var_value<T, require_floating_point_t<T>>::operator-=(Arith b) {
  if (b == 0.0) {
    return *this;
  }
