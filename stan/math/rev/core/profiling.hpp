...
inline auto profile_start(std::string name, profile_map& profiles) {
    ...
    if (profiles[name].meta.fwd_pass_active || profiles[name].meta.rev_pass_active) {
    ...
    reverse_pass_callback([name, &profiles]() mutable {
        profiles[name].rev_pass += std::chrono::duration<double>(std::chrono::steady_clock::now() - profiles[name].meta.rev_pass_start).count();
...
inline auto profile_stop(std::string name, profile_map& profiles) {
    ...
    profiles[name].fwd_pass += std::chrono::duration<double>(std::chrono::steady_clock::now() - profiles[name].meta.fwd_pass_start).count();
...
class profile {
  ...
  public:
  profile(std::string name, profile_map& profiles) : name_(name), profiles_(&profiles) {
    internal::profile_start(name_, *profiles_);
  }
  ~profile() {
    internal::profile_stop(name_, *profiles_);
  }
};
