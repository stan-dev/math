...
class dynamic_to_static_vari : public vari_base {
 ...
 public:
  dynamic_to_static_vari(vari_value<T>* input, vari_value<double>* output,
                         Eigen::Index N)
      : input_(input), N_(N), output_(output) {}
  void chain() {
...
class static_to_dynamic_vari : public vari_base {
 ...
 public:
  static_to_dynamic_vari(vari_value<double>* input, vari_value<T>* output,
                         Eigen::Index N)
      : input_(input), N_(N), output_(output) {}
  void chain() {
    for (size_t n = 0; n < N_; ++n) {
