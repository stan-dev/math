...
inline auto append_col(const T1& A, const T2& B) {
  check_size_match("append_col", "columns of A", A.rows(), "columns of B", B.rows());
  if (!is_constant<T1>::value && !is_constant<T2>::value) {
    arena_t<promote_scalar_t<var, T1>> arena_A = A;
    arena_t<promote_scalar_t<var, T2>> arena_B = B;
    return make_callback_var(append_col(value_of(arena_A), value_of(arena_B)), [arena_A, arena_B](auto& vi) mutable {
    ...
    return make_callback_var(append_col(value_of(arena_A), arena_B), [arena_A](auto& vi) mutable {
      arena_A.adj() += vi.adj().leftCols(arena_A.cols());
    });
  } else {
    arena_t<promote_scalar_t<double, T1>> arena_A = value_of(A);
    arena_t<promote_scalar_t<var, T2>> arena_B = B;
    return make_callback_var(append_col(arena_A, value_of(arena_B)), [arena_B](auto& vi) mutable {
      arena_B.adj() += vi.adj().rightCols(arena_B.cols());
    });
  }
}
