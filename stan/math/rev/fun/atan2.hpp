...
inline auto atan2(const Mat1& a, const Mat2& b) {
  if constexpr (!is_constant<Mat1>::value && !is_constant<Mat2>::value) {
  ...
  } else if constexpr (!is_constant<Mat1>::value) {
    arena_t<promote_scalar_t<var, Mat1>> arena_a = a;
  ...
  } else if constexpr (!is_constant<Mat2>::value) {
...
inline auto atan2(const Scalar& a, const VarMat& b) {
  if constexpr (!is_constant<Scalar>::value && !is_constant<VarMat>::value) {
    var arena_a = a;
  ...
  } else if constexpr (!is_constant<Scalar>::value) {
    var arena_a = a;
  ...
  } else if constexpr (!is_constant<VarMat>::value) {
...
inline auto atan2(const VarMat& a, const Scalar& b) {
  if constexpr (!is_constant<VarMat>::value && !is_constant<Scalar>::value) {
