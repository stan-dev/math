...
template <typename T>
class ceil_matrix_vari : public vari {
 public:
  int A_rows_;
  int A_cols_;
  int A_size_;
   ...
   * @param A matrix
   */
  explicit ceil_matrix_vari(const T& A)
      : vari(0.0),
        A_rows_(A.rows()),
        A_cols_(A.cols()),
        A_size_(A.size()),
        ...
        variRefCeil_(ChainableStack::instance_->memalloc_.alloc_array<vari*>(
            A_size_)) {
    using Eigen::Map;
    Map<matrix_vi>(variRefA_, A_rows_, A_cols_) = A.vi();
    Map<matrix_d> Ad(Ad_, A_rows_, A_cols_);
    Ad = A.val();
    Map<matrix_vi>(variRefCeil_, A_rows_, A_cols_).array()
        = Ad.array().ceil().unaryExpr([](double x) { return new vari(x, false); });
  }
...
inline auto ceil(const Container& x) {
  return apply_vector_unary<Container>::apply(
      x, [](const auto& v) {
        ...
        const T_ref& v_ref = v;
        auto* baseVari = new internal::ceil_matrix_vari<T_ref>(v_ref);
        T_plain AB_v(v_ref.rows(), v_ref.cols());
        AB_v.vi() = Eigen::Map<matrix_vi>(baseVari->variRefCeil_, v_ref.rows(), v_ref.cols());
