...
class cosh_matrix_vari : public vari {
 public:
   ...
   * @param A matrix
   */
  explicit cosh_matrix_vari(const T& A)
      : vari(0.0),
        A_rows_(A.rows()),
        A_cols_(A.cols()),
        A_size_(A.size()),
    ...
    Map<matrix_vi>(variRefCosh_, A_rows_, A_cols_).array()
        = Ad.array().cosh().unaryExpr([](double x) { return new vari(x, false); });
  }
...
inline auto cosh(const Container& x) {
  return apply_vector_unary<Container>::apply(
      x, [](const auto& v) {
        ...
        AB_v.vi() = Eigen::Map<matrix_vi>(baseVari->variRefCosh_, v_ref.rows(), v_ref.cols());
