...
inline const T& deep_copy(const T& arg) {
  return arg;
}

inline var deep_copy(const var& arg) { return var(new vari(arg.val(), false)); }

inline std::vector<var> deep_copy(const std::vector<var>& arg) {
...
inline std::vector<T> deep_copy(const std::vector<T>& arg) {
...
inline Eigen::Matrix<var, RowType, ColType> deep_copy(
