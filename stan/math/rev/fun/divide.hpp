...
namespace math {
   ...
   *
   * @tparam Mat A type inheriting from `EigenBase` with an `Arithmetic` scalar type.
   ...
   * Return matrix divided by scalar.
   *
   * @tparam Mat Either a type inheriting from `EigenBase` with a scalar type of `var` or a `var_value<T>`
