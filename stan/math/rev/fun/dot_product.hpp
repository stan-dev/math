...
namespace stan {
  ...
  class dot_product_vari : public vari {
   protected:
    ...
    inline static double var_dot(const Eigen::DenseBase<Derived1>& v1,
      ...
      vector_d vd1
          = Eigen::Ref<const Eigen::Matrix<typename Derived1::Scalar, -1, 1>>(
                v1)
                .val();
      vector_d vd2
          = Eigen::Ref<const Eigen::Matrix<typename Derived2::Scalar, -1, 1>>(
                v2)
                .val();
      return vd1.dot(vd2);
    }
    ...
    inline void initialize(vari**& mem_v, const var* inv,
      ...
      if (shared == nullptr) {
        mem_v = reinterpret_cast<vari**>(
            ChainableStack::instance_->memalloc_.alloc(length_
                                                       * sizeof(vari*)));
  ...
  inline return_type_t<T1, T2> dot_product(
  ...
  inline return_type_t<T1, T2> dot_product(const std::vector<T1>& v1,
    ...
    check_matching_sizes("dot_product", "v1", v1, "v2", v2);
    return var(
        new internal::dot_product_vari<T1, T2>(&v1[0], &v2[0], v1.size()));
  }
