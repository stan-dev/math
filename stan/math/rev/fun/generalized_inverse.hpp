...
inline auto generalized_inverse(const VarMat& G) {
  ...
  if (G.rows() < G.cols()) {
    ...
    arena_t<VarMat> inv_G(mdivide_left_spd(A_spd.val_op(), G_arena.val_op()))
        .transpose();
    ...
    arena_t<VarMat> inv_G(mdivide_right_spd(G_arena.val_op(), A_spd.val_op()))
        .transpose();
...
inline auto generalized_inverse(const VarMat& G, const double a) {
  ...
  if (G.rows() < G.cols()) {
    ...
    arena_t<VarMat> inv_G(mdivide_left_spd(A_spd, G_arena.val_op()))
        .transpose();
    ...
    auto A_spd = crossprod(G_arena.val_op());
    A_spd.diagonal().array() += a;
    arena_t<VarMat> inv_G(mdivide_right_spd(G_arena.val_op(), A_spd))
        .transpose();
