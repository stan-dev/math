...
namespace internal {
   ...
   * @param[out] inv_logit_x Returns the value of underflow corrected inverse logit.
...
inline auto lub_constrain(const T& x, const L& lb, const U& ub) {
  ...
  if (unlikely(is_ub_inf)) {
    ...
    reverse_pass_callback(
        [arena_x, ub, arena_lb, ret, diff, inv_logit_x, exp_neg_x, is_lb_inf]() mutable {
          ...
          if (!is_constant<U>::value) {
            forward_as<var>(ub).adj() += (is_lb_inf).select(
              ret.adj().array(),
              ret.adj().array() * inv_logit_x).sum();
          }
          if (!is_constant<L>::value) {
            forward_as<L_var>(arena_lb).adj().array() += (is_lb_inf).select(0,
...
inline auto lub_constrain(const T& x, const L& lb, const U& ub) {
  ...
  if (unlikely(is_lb_inf)) {
    ...
    auto diff = to_arena(ub_val - lb_val);
    arena_t<plain_type_t<decltype(arena_x_val.array())>> exp_neg_x(x.rows(), x.cols());
    arena_t<plain_type_t<decltype(arena_x_val.array())>> inv_logit_x(x.rows(), x.cols());
    internal::setup_inv_logit_and_exp_x(exp_neg_x, inv_logit_x, arena_x_val.array());
    arena_t<ret_type> ret = (is_ub_inf).select(
        arena_x_val.array().exp() + lb_val,
      diff * inv_logit_x + lb_val);
    reverse_pass_callback(
        [arena_x, arena_x_val, arena_ub, lb, ret, is_ub_inf, exp_neg_x, inv_logit_x, diff]() mutable {
          using T_var = arena_t<promote_scalar_t<var, T>>;
          ...
          if (!is_constant<L>::value) {
            forward_as<var>(lb).adj() += (is_ub_inf).select(
              ret.adj().array(),
              ret.adj().array() * (1.0 - inv_logit_x)).sum();
          }
          if (!is_constant<U>::value) {
...
inline auto lub_constrain(const T& x, const L& lb, const U& ub, std::decay_t<return_type_t<T, L, U>>& lp) {
  ...
  if (unlikely(is_lb_inf)) {
    ...
    arena_t<ret_type> ret = (is_ub_inf).select(
        arena_x_val.array().exp() + lb_val,
      diff * inv_logit_x + lb_val);
    reverse_pass_callback([arena_x, neg_abs_x, arena_x_val, diff, exp_neg_x, inv_logit_x, arena_ub, lb, ret, lp,
                           is_ub_inf]() mutable {
     using T_var = arena_t<promote_scalar_t<var, T>>;
     using U_var = arena_t<promote_scalar_t<var, U>>;
      const auto lp_adj = lp.adj();
      ...
      if (!is_constant<L>::value) {
        forward_as<var>(lb).adj() += (is_ub_inf).select(
          ret.adj().array(),
          ret.adj().array() * (1.0 - inv_logit_x) + -(1.0 / diff) * lp_adj).sum();
      }
      if (!is_constant<U>::value) {
        forward_as<U_var>(arena_ub).adj().array() += (is_ub_inf).select(
          0,
...
inline auto lub_constrain(const T& x, const L& lb, const U& ub) {
  ...
  reverse_pass_callback([arena_x, arena_x_val, exp_neg_x, inv_logit_x,
    arena_ub, arena_lb, diff, ret, is_ub_inf, is_lb_inf, is_lb_ub_inf]() mutable {
    using T_var = arena_t<promote_scalar_t<var, T>>;
    using L_var = arena_t<promote_scalar_t<var, L>>;
    using U_var = arena_t<promote_scalar_t<var, U>>;
    ...
    if (is_none_inf) {
      ...
      if (!is_constant<L>::value) {
        forward_as<L_var>(arena_lb).adj().array() += ret.adj().array() * (1.0 - inv_logit_x);
      }
      if (!is_constant<U>::value) {
        forward_as<U_var>(arena_ub).adj().array() += ret.adj().array() * inv_logit_x;
      }
    } else {
      if (!is_constant<T>::value) {
        forward_as<T_var>(arena_x).adj().array() += (is_lb_ub_inf).select(
          ret.adj().array(),
          (is_lb_inf).select(
            ret.adj().array() * -arena_x_val.array().exp(),
            (is_ub_inf).select(
 ...
 */
 ...
 inline auto lub_constrain(const T& x, const L& lb, const U& ub, return_type_t<T, L, U>& lp) {
   ...
   auto neg_abs_x = to_arena(-(arena_x_val.array()).abs());
   lp += (is_lb_ub_inf).select(
     0,
     (is_lb_inf || is_ub_inf).select(
       arena_x_val.array(),
       log(diff) + (neg_abs_x - (2.0 * log1p_exp(neg_abs_x))))).sum();
   reverse_pass_callback([arena_x, arena_x_val, exp_neg_x, inv_logit_x, neg_abs_x,
     arena_ub, arena_lb, diff, ret, is_ub_inf, is_lb_inf, is_lb_ub_inf, lp]() mutable {
     using T_var = arena_t<promote_scalar_t<var, T>>;
     using L_var = arena_t<promote_scalar_t<var, L>>;
     using U_var = arena_t<promote_scalar_t<var, U>>;
     const auto lp_adj = lp.adj();
