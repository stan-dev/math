...
inline auto multiply(const T1& A, const T2& B) {
  ...
  if (!is_constant<T2>::value && !is_constant<T1>::value) {
    arena_t<plain_type_t<promote_scalar_t<var, T1>>> arena_A = A;
    arena_t<plain_type_t<promote_scalar_t<var, T2>>> arena_B = B;
    using return_t
        = return_var_matrix_t<decltype(arena_A.val() * arena_B.val()), T1, T2>;
    arena_t<plain_type_t<return_t>> res = arena_A.val() * arena_B.val();

    reverse_pass_callback(
        [arena_A, arena_B, res]() mutable {
          arena_A.adj() += res.adj() * arena_B.val().transpose();
          arena_B.adj() += arena_A.val().transpose() * res.adj();
        });
    return return_t(res);
  } else if (!is_constant<T2>::value) {
    arena_t<plain_type_t<promote_scalar_t<double, T1>>> arena_A = value_of(A);
    arena_t<plain_type_t<promote_scalar_t<var, T2>>> arena_B = B;
    using return_t
        = return_var_matrix_t<decltype(arena_A * arena_B.val().eval()), T1, T2>;
    arena_t<plain_type_t<return_t>> res = arena_A * arena_B.val();
    reverse_pass_callback([arena_B, arena_A, res]() mutable {
      arena_B.adj() += arena_A.transpose() * res.adj();
    });
    return return_t(res);
  } else {
    arena_t<plain_type_t<promote_scalar_t<var, T1>>> arena_A = A;
    arena_t<plain_type_t<promote_scalar_t<double, T2>>> arena_B = value_of(B);
    using return_t
        = return_var_matrix_t<decltype(arena_A.val().eval() * arena_B), T1, T2>;
    arena_t<plain_type_t<return_t>> res = arena_A.val() * arena_B;
