...
/**
 * Impl of rep_matrix returning an `var_value<Eigen::Matrix>` with a double scalar type.
 ...
 */
template <typename Ret, typename T, require_var_matrix_t<Ret>* = nullptr, require_arithmetic_t<T>* = nullptr>
 ...
 * @param n Number of columns.
 */
template <typename Ret, typename T, require_var_matrix_t<Ret>* = nullptr, require_var_t<T>* = nullptr>
inline auto rep_matrix(const T& x, int m, int n) {
  check_nonnegative("rep_matrix", "rows", m);
  check_nonnegative("rep_matrix", "cols", n);
  return make_callback_var(value_type_t<Ret>::Constant(m, n, x.val()),
      [x](auto& rep) mutable {
        x.adj() += rep.adj().sum();
      });
}
...
/**
 * Impl of rep_matrix returning an `var_value<Eigen::Matrix>` from an Eigen matrix with Arithmetic scalar.
 ...
 * @param x A `var_value` with inner Eigen vector type. For Row vectors the values are replacated rowwise and for column vectors the values are repliacated colwise.
 * @param n Number of rows or columns.
 */
template <typename Ret, typename Vec, require_var_matrix_t<Ret>* = nullptr,
...
inline auto rep_matrix(const Vec& x, int n) {
  if (is_row_vector<Vec>::value) {
    check_nonnegative("rep_matrix", "rows", n);
    return make_callback_var(x.val().replicate(n, 1),
        [x](auto& rep) mutable {
            x.adj() += rep.adj().rowwise().sum();
        });
  } else {
    check_nonnegative("rep_matrix", "cols", n);
    return make_callback_var(x.val().replicate(1, n),
        [x](auto& rep) mutable {
