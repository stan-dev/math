...
inline Eigen::Matrix<var, 1, Mat::RowsAtCompileTime> rows_dot_self(
    const Mat& x) {
  Eigen::Matrix<var, 1, Mat::RowsAtCompileTime> ret(1, x.rows());
...
inline auto rows_dot_self(const Mat& x) {
  using ret_type
      = return_var_matrix_t<decltype(x.val().rowwise().squaredNorm()), Mat>;
