...
inline decltype(auto) to_var(T&& m) { return std::forward<T>(m); }
 ...
 */
inline auto to_var(const matrix_d& m) {
  matrix_v m_v = m;
  return m_v;
}
 ...
 *   values of v
 */
inline auto to_var(const vector_d& v) {
  vector_v v_v = v;
  return v_v;
}
 ...
 *   values of rv.
 */
inline auto to_var(const row_vector_d& rv) {
