...
inline var trace_gen_inv_quad_form_ldlt(const Td& D,
  ...
  if (!is_constant<Ta>::value && !is_constant<Tb>::value && !is_constant<Td>::value) {
    ...
    reverse_pass_callback([A, BTAsolveB, AsolveB, arena_B, arena_D, res]() mutable {
      double C_adj = res.adj();

      forward_as<promote_scalar_t<var, Ta>>(A.matrix()).adj()
	-= C_adj * AsolveB * arena_D.val_op().transpose() * AsolveB.transpose();
      arena_B.adj()
	+= C_adj * AsolveB * (arena_D.val_op() + arena_D.val_op().transpose());
	...
	-= C_adj * AsolveB * arena_D.transpose() * AsolveB.transpose();
      arena_B.adj()
	+= C_adj * AsolveB * (arena_D + arena_D.transpose());
    });
    ...
    reverse_pass_callback([BTAsolveB, AsolveB, arena_B, arena_D, res]() mutable {
      double C_adj = res.adj();
    ...
    reverse_pass_callback([AsolveB, arena_B, arena_D, res]() mutable {
      arena_B.adj()
	+= res.adj() * AsolveB * (arena_D + arena_D.transpose());
    });
    ...
    const auto& B_ref = to_ref(B);
    arena_t<promote_scalar_t<var, Td>> arena_D = D;
    auto BTAsolveB = to_arena(value_of(B_ref).transpose() * A.ldlt().solve(value_of(B_ref)));
