...
namespace math {
  ...
  inline auto ub_constrain(const T& x, const L& lb) {
    using std::exp;
  ...
  template <typename T, typename L, require_all_stan_scalar_t<T, L>* = nullptr,
   require_any_var_t<T, L>* = nullptr>
  inline auto ub_constrain(const T& x, const L& lb, return_type_t<T, L>& lp) {
    using std::exp;
    const auto lb_val = value_of(lb);
...
inline auto ub_constrain(const T& x, const L& lb) {
  ...
  if(!is_constant<T>::value && !is_constant<L>::value) {
    arena_t<promote_scalar_t<var, T>> arena_x = x;
    arena_t<promote_scalar_t<var, L>> arena_lb = lb;
    arena_t<ret_type> ret = ub_constrain(arena_x.val(), arena_lb.val());
    ...
    arena_t<promote_scalar_t<var, L>> arena_lb = lb;
    arena_t<ret_type> ret = ub_constrain(value_of(x), arena_lb.val());
...
inline auto ub_constrain(const T& x, const L& lb, return_type_t<T, L>& lp) {
  ...
  if(!is_constant<T>::value && !is_constant<L>::value) {
    ...
    arena_t<promote_scalar_t<var, L>> arena_lb = lb;
    double lp_val = 0.0;
    arena_t<ret_type> ret = ub_constrain(arena_x.val(), arena_lb.val(), lp_val);
    lp += lp_val;
    reverse_pass_callback([arena_x, arena_lb, ret, lp]() mutable {
    ...
    arena_t<promote_scalar_t<var, L>> arena_lb = lb;
    double lp_val = 0.0;
    arena_t<ret_type> ret = ub_constrain(value_of(x), arena_lb.val(), lp_val);
    lp += lp_val;
    reverse_pass_callback([arena_lb, ret]() mutable {
      for (Eigen::Index j = 0; j < arena_lb.cols(); ++j) {
        for (Eigen::Index i = 0; i < arena_lb.rows(); ++i) {
