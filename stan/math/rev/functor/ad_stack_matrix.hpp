...
public:
    ...
    /**
     * Constructs `AD_stack_matrix` with given size. This only works if `MatrixType` is row or col vector.
    ...
    /**
     * Copy constructor. Copies values, not the whole object (same as Eigen types).
    ...
    /**
     * Copy assignment operator. Copies values, not the whole object (same as Eigen types).
     * @param other matrix to copy from
     * @return `*this`
     */
    ...
    void resize(Eigen::Index rows, Eigen::Index cols){
        //placement new changes what data map points to - this new is not an allocation
        new (this) Eigen::Map<MatrixType>(ChainableStack::instance_->memalloc_.alloc_array<Scalar>(rows*cols), rows, cols);
    }

    /**
     * Resizes this object to given size. This only works if `MatrixType` is row or col vector.
     * @param size number of elements
     */
