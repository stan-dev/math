...
class cvodes_integrator_adjoint_vari : public vari {
  ...
  inline void rhs(double t, const double y[], double dy_dt[]) {
    ...
    const Eigen::VectorXd dy_dt_vec
        = rhs(t, y_vec, value_of_args_tuple_);

    check_size_match(function_name_, "dy_dt", dy_dt_vec.size(),
                     "states", N_);
  ...
  void rhs_adj_sens(double t, N_Vector y, N_Vector yB, N_Vector yBdot) {
    ...
    check_size_match(function_name_, "dy_dt", f_y_t_vars.size(),
                     "states", N_);
  ...
  inline void jacobian_states(double t, N_Vector y, SUNMatrix J) {
    ...
    check_size_match(function_name_, "dy_dt", fy_var.size(), "states",
                     N_);
 ...
 public:
  ...
  : vari(NOT_A_NUMBER),
    ...
    t0_varis_(
        ChainableStack::instance_->memalloc_.alloc_array<vari*>(num_t0_vars_)),
    ts_varis_(
        ChainableStack::instance_->memalloc_.alloc_array<vari*>(num_ts_vars_)),
    y0_varis_(
        ChainableStack::instance_->memalloc_.alloc_array<vari*>(num_y0_vars_)),
    ...
    apply(
        [&](auto&&... args) {
          std::vector<int> unused_temp{
              0, (check_finite(function_name_, "ode parameters and data", args), 0)...};
        },
        local_args_tuple_);
    ...
    for (size_t n = 0; n < ts_dbl.size(); ++n) {
      ...
      if (t_final != t_init) {
        if (num_vars_ != 0) {
          ...
          int error_code
              = CVodeF(cvodes_mem_, t_final, nv_state_forward_, &t_init,
                       CV_NORMAL, &ncheck);
          ...
          int error_code = CVode(cvodes_mem_, t_final,
    ...
    */
    ...
    for (int i = ts_.size() - 1; i >= 0; --i) {
      ...
      if (num_ts_vars_ > 0 && i >= 0) {
        ts_varis_[i]->adj_ += step_sens.dot(rhs(
            t_init, y_[i], value_of_args_tuple_));
        /*
          apply(
          [&](auto&&... args) {
      ...
      if (t_final != t_init) {
        if (!backward_is_initialized_) {
          ...
          check_flag_sundials(
              CVodeSetUserDataB(cvodes_mem_, index_backward_,
          ...
          check_flag_sundials(
              CVodeSetMaxNumStepsB(cvodes_mem_, index_backward_,
                                   max_num_steps_),
              "CVodeSetMaxNumStepsB");

          check_flag_sundials(
              CVodeSetLinearSolverB(cvodes_mem_, index_backward_,
                                    LS_backward_, A_backward_),
          ...
          check_flag_sundials(
              CVodeReInitB(cvodes_mem_, index_backward_, t_init,
                           nv_state_backward_),
              "CVodeReInitB");

          if (num_args_vars_ > 0) {
            check_flag_sundials(
                CVodeQuadReInitB(cvodes_mem_, index_backward_,
                                 nv_quad_),
                "CVodeQuadReInitB");
          }
        }
        ...
        // reached of t_final
        check_flag_sundials(CVodeGetB(cvodes_mem_, index_backward_,
                                      &t_init, nv_state_backward_),
                            "CVodeGetB");

        if (num_args_vars_ > 0) {
          check_flag_sundials(
              CVodeGetQuadB(cvodes_mem_, index_backward_, &t_init,
                            nv_quad_),
              "CVodeGetQuadB");
        }
      }
    ...
    if (num_t0_vars_ > 0) {
      ...
      t0_varis_[0]->adj_ += -state_backward_.dot(
          rhs(t_init, y0d, value_of_args_tuple_));
      /*
        apply(
        [&](auto&&... args) {
