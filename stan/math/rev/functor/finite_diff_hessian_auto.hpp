...
void finite_diff_hessian_auto(const F& f, const VectorT& x, double& fx,
                              VectorT& grad_fx, Eigen::MatrixXd& hess_fx) {
  int d = x.size();
  ...
  std::vector<VectorT> g_plus(d);
  std::vector<VectorT> g_minus(d);
