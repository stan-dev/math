...
integrate_ode_adams(const F& f, const std::vector<T_y0>& y0, const T_t0& t0,
  ...
  const Eigen::VectorXd absolute_tolerance_state
      = Eigen::VectorXd::Constant(y0.size(), absolute_tolerance);
  auto y
      = ode_adams_tol_impl("integrate_ode_adams", f_adapted, to_vector(y0), t0,
                           ts, relative_tolerance, absolute_tolerance_state,
