...
double ode_time_gradient_var(double t, double grad) {
  invalid_argument("ode_time_gradient_var", "t", t, "t must be of type var, not double");
  return 0.0;
}
...
template <typename F, typename T_initial,
	  typename T_out, typename... T_Args>
auto ode_add_time_gradients(const F& f,
...
template <typename F, typename T_initial, typename T_t0, typename T_ts,
	  typename T_out, typename... T_Args>
std::vector<std::vector<var>> ode_add_time_gradients(const F& f,
						     const std::vector<T_initial>& y0,
						     const T_t0& t0,
  ...
  if(is_var<T_t0>::value) {
    ...
    check_size_match("coupled_ode_observer", "dy_dt", f_y0_t0.size(), "states", y0.size());
  }
