...
std::vector<Eigen::Matrix<stan::return_type_t<T_initial, T_t0, T_ts, T_Args...>,
                          Eigen::Dynamic, 1>>
ode_bdf_adjoint_tol(const F& f, const Eigen::Matrix<T_initial, Eigen::Dynamic, 1>& y0,
            const T_t0& t0, const std::vector<T_ts>& ts,
            double relative_tolerance, double absolute_tolerance,
            long int max_num_steps, std::ostream* msgs, const T_Args&... args) {
  auto integrator
      = new stan::math::cvodes_integrator_adjoint_vari<CV_BDF, F, T_initial, T_t0, T_ts,
                                               T_Args...>(
...
std::vector<Eigen::Matrix<stan::return_type_t<T_initial, T_t0, T_ts, T_Args...>,
                          Eigen::Dynamic, 1>>
ode_bdf_adjoint(const F& f, const Eigen::Matrix<T_initial, Eigen::Dynamic, 1>& y0,
