...
class operands_and_partials_impl<ReturnType, require_var_t<ReturnType>,
                                 Ops...> {
 public:
  std::tuple<
...
class ops_partials_edge<double, std::vector<Eigen::Matrix<var, R, C>>> {
 public:
  ...
  inline auto& partial() noexcept { return partials_vec_; }
  inline auto& operand() noexcept { return operands_; }
 ...
 public:
  ...
  explicit ops_partials_edge(const std::vector<std::vector<var>>& ops)
    ...
    for (size_t i = 0; i < stan::math::size(ops); ++i) {
      partials_vec_[i]
          = to_arena(Eigen::Matrix<double, -1, 1>(stan::math::size(ops[i])));
    }
  }
  inline auto& partial() noexcept { return partials_vec_; }
