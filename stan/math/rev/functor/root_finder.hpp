...
auto root_finder_tol(F&& f, FDiv&& f_div, FDivDiv&& f_div_div,
  ...
  auto f_wrt_x = [&](auto&& x) {
    return apply([&](auto&&... args) { return f(x, args...); },
                 args_vals_tuple);
  };
  ...
  //jacobian(f_wrt_x, theta_dbl, f_x, Jf_x);
  {
    ...
    stan::math::var fx_var = f_wrt_x(x_var);
    f_x = fx_var.val();
    fx_var.grad();
    Jf_x = x_var.adj();
  }
  stan::math::var ret = theta_dbl;
  //auto Jf_x_T_lu_ptr = make_unsafe_chainable_ptr(Jf_x.transpose().partialPivLu());  // Lu
  auto Jf_x_T_lu_ptr = 1. / Jf_x;  // Lu

  reverse_pass_callback(
      [f, ret, arena_args_tuple, Jf_x_T_lu_ptr]() mutable {
        // Eigen::VectorXd eta = -Jf_x_T_lu_ptr->solve(ret.adj().eval());
        double eta = -(ret.adj() * Jf_x_T_lu_ptr);
