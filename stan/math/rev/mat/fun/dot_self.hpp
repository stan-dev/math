...
class dot_self_vari : public vari {
 ...
 public:
  ...
  explicit dot_self_vari(const Eigen::DenseBase<Derived>& v)
    ...
    for (size_t i = 0; i < size_; i++) v_[i] = v[i].vi_;
  }
  ...
  inline static double var_dot_self(vari** v, size_t size) {
    double sum = 0.0;
    for (size_t i = 0; i < size; ++i) sum += square(v[i]->val_);
    return sum;
  }
  template <typename Derived>
  double var_dot_self(const Eigen::DenseBase<Derived>& v) {
    double sum = 0.0;
    for (int i = 0; i < v.size(); ++i) sum += square(v(i).vi_->val_);
    return sum;
  }
  template <int R, int C>
  ...
  virtual void chain() {
    for (size_t i = 0; i < size_; ++i) v_[i]->adj_ += adj_ * 2.0 * v_[i]->val_;
  }
};
}  // namespace
/**
