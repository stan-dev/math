...
class gp_exp_quad_cov_vari<T_x, double, T_l> : public vari {
 public:
   ...
   * @param l length scale
   */
  gp_exp_quad_cov_vari(const std::vector<T_x>& x, double sigma, const T_l& l)
      : vari(0.0),
        size_(x.size()),
        size_ltri_(size_ * (size_ - 1) / 2),
        l_d_(value_of(l)),
...
gp_exp_quad_cov(const std::vector<T_x>& x, const var& sigma, const var& l) {
  check_positive("gp_exp_quad_cov", "sigma", sigma);
  check_positive("gp_exp_quad_cov", "l", l);
  ...
  gp_exp_quad_cov_vari<T_x, var, var>* baseVari
      = new gp_exp_quad_cov_vari<T_x, var, var>(x, sigma, l);
...
gp_exp_quad_cov(const std::vector<T_x>& x, double sigma, const var& l) {
  ...
  check_positive("gp_exp_quad_cov", "length-scale", l);
  size_t x_size = x.size();
  for (size_t i = 0; i < x_size; ++i)
    check_not_nan("gp_exp_quad_cov", "x", x[i]);
  ...
  gp_exp_quad_cov_vari<T_x, double, var>* baseVari
      = new gp_exp_quad_cov_vari<T_x, double, var>(x, sigma, l);
