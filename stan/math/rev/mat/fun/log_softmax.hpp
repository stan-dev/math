...
namespace stan {
  namespace math {
    ...
    namespace {
      ...
      public:
        log_softmax_elt_vari(double val, vari** alpha,
                             const double* softmax_alpha, int size, int idx)
            : vari(val),
              alpha_(alpha),
    ...
    inline Eigen::Matrix<var, Eigen::Dynamic, 1> log_softmax(
      ...
      for (int i = 0; i < alpha.size(); ++i) alpha_vi_array[i] = alpha(i).vi_;

      Matrix<double, Dynamic, 1> alpha_d(alpha.size());
      for (int i = 0; i < alpha_d.size(); ++i) alpha_d(i) = alpha(i).val();
      ...
      for (int i = 0; i < alpha_d.size(); ++i) softmax_alpha_d(i) /= sum;
      ...
      for (int k = 0; k < log_softmax_alpha.size(); ++k)
        log_softmax_alpha(k) = var(
            new log_softmax_elt_vari(log_softmax_alpha_d[k], alpha_vi_array,
