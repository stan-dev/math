...
namespace stan {
    namespace math {
        ...
        struct matrix_exp_computeUV<MatrixType, var>
        {
            static void run(const MatrixType& arg, MatrixType& U, MatrixType& V, int& squarings)
            {
                using std::frexp;
                using std::pow;
                const var l1norm = arg.cwiseAbs().colwise().sum().maxCoeff();
                squarings = 0;
                if (l1norm < 1.495585217958292e-002) {
                    ...
                    MatrixType A = arg.unaryExpr(MatrixExponentialScalingOp<var>(squarings));
       ...
       expm(const Eigen::Matrix<var, Eigen::Dynamic, Eigen::Dynamic>& A) {
