...
class mdivide_left_ldlt_vv_vari : public vari {
 public:
  ...
  virtual void chain() {
    ...
    int pos = 0;
    for (int j = 0; j < N_; j++)
      for (int i = 0; i < M_; i++) adjB(i, j) = variRefC_[pos++]->adj_;
    ...
    pos = 0;
    for (int j = 0; j < N_; j++)
      for (int i = 0; i < M_; i++) variRefB_[pos++]->adj_ += adjB(i, j);
  }
};
 ...
 public:
  ...
  virtual void chain() {
    ...
    for (int j = 0; j < adjB.cols(); j++)
      for (int i = 0; i < adjB.rows(); i++) adjB(i, j) = variRefC_[pos++]->adj_;
 ...
 public:
  ...
  virtual void chain() {
    ...
    for (int j = 0; j < adjC.cols(); j++)
      for (int i = 0; i < adjC.rows(); i++) adjC(i, j) = variRefC_[pos++]->adj_;
