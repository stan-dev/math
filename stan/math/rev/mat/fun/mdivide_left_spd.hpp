...
class mdivide_left_spd_vv_vari : public vari {
 public:
  ...
  virtual void chain() {
    ...
    size_t pos = 0;
    for (size_type j = 0; j < N_; j++)
      for (size_type i = 0; i < M_; i++) adjB(i, j) = variRefC_[pos++]->adj_;
    ...
    pos = 0;
    for (size_type j = 0; j < M_; j++)
      for (size_type i = 0; i < M_; i++) variRefA_[pos++]->adj_ += adjA(i, j);

    pos = 0;
    for (size_type j = 0; j < N_; j++)
      for (size_type i = 0; i < M_; i++) variRefB_[pos++]->adj_ += adjB(i, j);
  }
};
