  ...
  namespace math {
    ...
    public:
      ...
      virtual void chain() {
        ...
        for (size_type i = 0; i < A_size_; ++i) variRefA_[i]->adj_ += adjA(i);
        for (size_type i = 0; i < B_size_; ++i) variRefB_[i]->adj_ += adjB(i);
      }
    };
    ...
    public:
      ...
      multiply_mat_vari(const Eigen::Matrix<TA, 1, CA>& A,
        ...
        double AB =
            Map<RowVectorXd>(Ad_, 1, size_) * Map<VectorXd>(Bd_, size_, 1);
      ...
      virtual void chain() {
        ...
        for (size_type i = 0; i < size_; ++i) variRefA_[i]->adj_ += adjA(i);
        for (size_type i = 0; i < size_; ++i) variRefB_[i]->adj_ += adjB(i);
      }
    };
    ...
    public:
      ...
      multiply_mat_vari(const Eigen::Matrix<double, RA, CA>& A,
        ...
        using Eigen::MatrixXd;
        using Eigen::Map;
        for (size_type i = 0; i < A.size(); ++i) Ad_[i] = A.coeffRef(i);
        for (size_type i = 0; i < B.size(); ++i) {
          variRefB_[i] = B.coeffRef(i).vi_;
          Bd_[i] = B.coeffRef(i).val();
        }
    ...
    public:
      ...
      multiply_mat_vari(const Eigen::Matrix<double, 1, CA>& A,
        ...
        using Eigen::VectorXd;
        using Eigen::RowVectorXd;
        for (size_type i = 0; i < size_; ++i) Ad_[i] = A.coeffRef(i);
        for (size_type i = 0; i < size_; ++i) {
          variRefB_[i] = B.coeffRef(i).vi_;
          Bd_[i] = B.coeffRef(i).val();
        }
    ...
    public:
      ...
      multiply_mat_vari(const Eigen::Matrix<TA, 1, CA>& A,
        ...
        for (size_type i = 0; i < size_; ++i) Bd_[i] = B.coeffRef(i);
        double AB =
            Map<RowVectorXd>(Ad_, 1, size_) * Map<VectorXd>(Bd_, size_, 1);
        variRefAB_ = new vari(AB, false);
      }
    ...
    inline Eigen::Matrix<var, R2, C2> multiply(
        const T1& c, const Eigen::Matrix<T2, R2, C2>& m) {
    ...
    inline Eigen::Matrix<var, R1, C1> multiply(
        const Eigen::Matrix<T1, R1, C1>& m, const T2& c) {
      // TODO(trangucci) pull out to eliminate overpromotion of one side
      // move to matrix.hpp w. promotion?
      return to_var(m) * to_var(c);
    }
