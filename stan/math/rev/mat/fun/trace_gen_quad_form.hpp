...
namespace stan {
  namespace math {
    namespace {
      ...
      protected:
        static inline void computeAdjoints(
            double adj, const Eigen::Matrix<double, RD, CD>& D,
            const Eigen::Matrix<double, RA, CA>& A,
            const Eigen::Matrix<double, RB, CB>& B,
            Eigen::Matrix<var, RD, CD>* varD, Eigen::Matrix<var, RA, CA>* varA,
            Eigen::Matrix<var, RB, CB>* varB) {
          Eigen::Matrix<double, CA, CB> AtB;
          Eigen::Matrix<double, RA, CB> BD;
          if (varB || varA) BD.noalias() = B * D;
          if (varB || varD) AtB.noalias() = A.transpose() * B;

          if (varB) {
            Eigen::Matrix<double, RB, CB> adjB(adj *
                                               (A * BD + AtB * D.transpose()));
      ...
      public:
        ...
        virtual void chain() {
          computeAdjoints(
              adj_, value_of(impl_->D_), value_of(impl_->A_),
              value_of(impl_->B_),
    ...
    trace_gen_quad_form(const Eigen::Matrix<TD, RD, CD>& D,
      ...
      trace_gen_quad_form_vari_alloc<TD, RD, CD, TA, RA, CA, TB, RB, CB>*
          baseVari = new trace_gen_quad_form_vari_alloc<TD, RD, CD, TA, RA, CA,
                                                        TB, RB, CB>(D, A, B);

      return var(
          new trace_gen_quad_form_vari<TD, RD, CD, TA, RA, CA, TB, RB, CB>(
              baseVari));
    }
