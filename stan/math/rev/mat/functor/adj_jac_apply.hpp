 ...
 */
template <typename F, typename... Targs> struct adj_jac_vari : public vari {
   ...
   */
  std::vector<var>
  build_return_varis_and_vars(const std::vector<double> &val_y) {
    M_[0] = val_y.size();
  ...
  build_return_varis_and_vars(const Eigen::Matrix<double, R, C> &val_y) {
    M_[0] = val_y.rows();
    M_[1] = val_y.cols();
