...
namespace stan {
  namespace math {
    ...
    struct algebra_solver_vari : public vari {
      ...
      algebra_solver_vari(const FS& fs, const F& f, const Eigen::VectorXd& x,
          ...
          : vari(theta_dbl(0)),
            ...
            Jx_y_(ChainableStack::memalloc_.alloc_array<double>(x_size_ *
                                                                y_size_)) {
        using Eigen::MatrixXd;
        using Eigen::Map;
        for (int i = 0; i < y.size(); ++i) y_[i] = y(i).vi_;
     ...
     * elements.
     * @throw <code>std::invalid_argument</code> if relative_tolerance is
     * strictly negative.
     * @throw <code>std::invalid_argument</code> if function_tolerance is
     * strictly negative.
     * @throw <code>std::invalid_argument</code> if max_num_steps is not
     * positive.
    ...
    Eigen::Matrix<T, Eigen::Dynamic, 1> algebra_solver(
      ...
      for (int i = 1; i < x.size(); ++i) theta(i) = var(vi0->theta_[i]);
