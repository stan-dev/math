...
class idas_integrator {
 ...
 public:
  ...
  typename Dae::return_type integrate(Dae& dae, double t0,
    ...
    if (rtol_ <= 0)
      invalid_argument(caller, "relative_tolerance,", rtol_, "",
                       ", must be greater than 0");
    if (atol_ <= 0)
      invalid_argument(caller, "absolute_tolerance,", atol_, "",
                       ", must be greater than 0");
    if (max_num_steps_ <= 0)
      invalid_argument(caller, "max_num_steps,", max_num_steps_, "",
                       ", must be greater than 0");
