...
/**
 * Return the numerical integral of the specified function and
 * gradient function, within the specified integration limits, given
 * parameters, data, and tolerances for integration.
 *
 * This function uses the algorithm of <code>integrate_1d_tscg</code>
 ...
 * <code>double(double, C_param, data C_x_r, data C_x_i, ostream&)</code>
 * where `C_param` is `T_param` or `vector<T_param>`, `C_x_r` is
 * `T_x_r` or `vector<T_x_r>`, and `C_x_i` is `T_x_i` or
 * `vector<T_x_i>`, with the first argument being integrated, the
 * second being parameters, the next two being fixed data, and the
 * last being a stream for messages.
