...
integrate_ode_bdf(const F& f, const std::vector<T_initial>& y0, double t0,
  ...
  if (relative_tolerance <= 0)
    invalid_argument("integrate_ode_bdf", "relative_tolerance,",
                     relative_tolerance, "", ", must be greater than 0");
  if (absolute_tolerance <= 0)
    invalid_argument("integrate_ode_bdf", "absolute_tolerance,",
                     absolute_tolerance, "", ", must be greater than 0");
  if (max_num_steps <= 0)
    invalid_argument("integrate_ode_bdf", "max_num_steps,", max_num_steps, "",
  ...
  try {
    ...
    cvodes_check_flag(CVDlsSetDenseJacFn(cvodes_mem, &ode_data::dense_jacobian),
                      "CVDlsSetDenseJacFn");
    ...
    if (S > 0) {
      ...
      if (initial_var::value) {
        for (size_t n = 0; n < N; n++) NV_Ith_S(cvodes_state_sens[n], n) = 1.0;
      }
      cvodes_check_flag(
    ...
    for (size_t n = 0; n < ts.size(); ++n) {
      ...
      if (S > 0) {
        cvodes_check_flag(CVodeGetSens(cvodes_mem, &t_init, cvodes_state_sens),
                          "CVodeGetSens");
        for (size_t s = 0; s < S; s++)
