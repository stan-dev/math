...
namespace stan {
   ...
   *  index sequence.
   */
  template <template <typename...> class Checker, typename T, size_t J, size_t... I>
  constexpr auto conditional_sequence(Checker<T> x, std::index_sequence<J, I...> /* ignore */) {
   ...
   */
  template <template <typename...> class Checker, typename T1, size_t J, size_t... I, typename T, typename... Types>
  constexpr auto conditional_sequence(Checker<T1> toss, std::index_sequence<J, I...> /* ignore */,
                                    T&& x, Types&&... args) {
    constexpr auto iter = J + Checker<std::decay_t<T>>::value;
    return conditional_sequence(toss, std::index_sequence<iter, I..., J>{}, args...);
  }
}
#endif
