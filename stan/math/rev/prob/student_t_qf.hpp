...
var student_t_qf(const T_p& p, const T_nu& nu, const T_mu& mu,
  ...
  return make_callback_var(ret_val,
    [p, mu, sigma, nu, ibeta_arg](auto& vi) mutable {
    const double p_val = value_of(p);
    const double mu_val = value_of(mu);
    const double sigma_val = value_of(sigma);
    const double nu_val = value_of(nu);
    ...
    if constexpr (!is_constant<T_nu>::value) {
      ...
      const double hyper_arg = hypergeometric_pFq(hyper_arg_a, hyper_arg_b,
                                                  ibeta_arg);

      const double hyper2f1 = hypergeometric_2F1(1.0, nu_p1_div_2,
                                                (2.0 + nu_val) / 2.0,
                                                ibeta_arg);
