...
class fma_vvv_vari : public op_vvv_vari {
 public:
  ...
  void chain() {
    if (unlikely(is_nan(avi_->val_, bvi_->val_, cvi_->val_))) {
...
class fma_vvd_vari : public op_vvd_vari {
 public:
  ...
  void chain() {
    if (unlikely(is_nan(avi_->val_, bvi_->val_, cd_))) {
      avi_->adj_ = std::numeric_limits<double>::quiet_NaN();
      bvi_->adj_ = std::numeric_limits<double>::quiet_NaN();
    } else {
      avi_->adj_ += adj_ * bvi_->val_;
...
class fma_vdv_vari : public op_vdv_vari {
 public:
  ...
  void chain() {
    if (unlikely(is_nan(avi_->val_, cvi_->val_, bd_))) {
      avi_->adj_ = std::numeric_limits<double>::quiet_NaN();
      cvi_->adj_ = std::numeric_limits<double>::quiet_NaN();
    } else {
      avi_->adj_ += adj_ * bd_;
...
class fma_vdd_vari : public op_vdd_vari {
 public:
  ...
  void chain() {
    if (unlikely(is_nan(avi_->val_, bd_, cd_)))
      avi_->adj_ = std::numeric_limits<double>::quiet_NaN();
    else
      avi_->adj_ += adj_ * bd_;
  }
...
class fma_ddv_vari : public op_ddv_vari {
 public:
  ...
  void chain() {
    if (unlikely(is_nan(cvi_->val_, ad_, bd_)))
      cvi_->adj_ = std::numeric_limits<double>::quiet_NaN();
    else
      cvi_->adj_ += adj_;
  }
