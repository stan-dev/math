...
namespace stan {
  namespace math {
     ...
     * @param[out] one_m_t_prod_exp_lam2_m_lam1 shared term in deriv
     * calculation.
     * @param[out] one_d_t_plus_one_m_t_prod_exp_lam2_m_lam1 shared term in
     * deriv calculation.
     */
     ...
     * = \log \left( \theta \exp(\lambda_1) + (1 - \theta) \exp(\lambda_2)
     * \right). \f]
     *
     * \f[
    ...
    inline typename return_type<T_theta, T_lambda1, T_lambda2>::type log_mix(
        const T_theta& theta, const T_lambda1& lambda1,
      ...
      if (lambda1 > lambda2) {
        ...
        log_mix_partial_helper(1.0 - theta_double, lambda2_double,
                               lambda1_double, one_m_exp_lam2_m_lam1,
                               one_m_t_prod_exp_lam2_m_lam1,
                               one_d_t_plus_one_m_t_prod_exp_lam2_m_lam1);
