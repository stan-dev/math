...
namespace {
  ...
  struct integrate_func {
    ...
    inline typename stan::return_type<T1, T2, T3>::type operator()(
      ...
      T_Ret uB = pow(u, beta);
      T_Ret first = beta * exp(-uB) * pow(2 * z + uB, v_mhalf) * 
                      boost::math::pow<7>(u);
      T_Ret second = exp(-1.0 / u);
      if (second > 0) second = second * pow(u, neg2v_m1) * 
                      pow(2 * z * u + 1, v_mhalf);
      return first + second;      
      }
   };
...
std::ostream &msgs) {
  ...
  T_Ret Q = integrate_1d(integrate_func{},
    0.0, 1.0, theta, std::vector<double>(), std::vector<int>(), msgs, 1e-4);
