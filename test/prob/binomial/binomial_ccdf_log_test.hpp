...
public:
  void valid_values(vector<vector<double>> &parameters,
    ...
    ccdf_log.push_back(
        std::log(1.0 - 0.06757822542283530020679)); // expected ccdf_log
  }
  ...
  ccdf_log(const T_n &n, const T_N &N, const T_prob &theta, const T3 &,
           const T4 &, const T5 &) {
  ...
  template <typename T_n, typename T_N, typename T_prob, typename T3,
            typename T4, typename T5>
  typename stan::return_type<T_prob>::type
  ccdf_log_function(const T_n &n, const T_N &N, const T_prob &theta, const T3 &,
                    const T4 &, const T5 &) {
