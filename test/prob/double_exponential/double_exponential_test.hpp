...
public:
  void valid_values(vector<vector<double>> &parameters,
    ...
    log_prob.push_back(
        -0.6931471805599453094172321214581765680755001343602552541206800094933936219696947156058633269964186875); // expected log_prob
    ...
    parameters.push_back(param);
    log_prob.push_back(
        -1.693147180559945309417232121458176568075500134360255254120680009493393621969694715605863326996418688); // expected log_prob
    ...
    parameters.push_back(param);
    log_prob.push_back(
        -5.693147180559945309417232121458176568075500134360255254120680009493393621969694715605863326996418688); // expected log_prob
    ...
    parameters.push_back(param);
    log_prob.push_back(
        -1.8862943611198906188344642429163531361510002687205105082413600189867872439393894312117266539928373751); // expected log_prob
  ...
  log_prob(const T_y &y, const T_loc &mu, const T_scale &sigma, const T3 &,
           const T4 &, const T5 &) {
...
TEST(ProbDistributionsDoubleExponential, Cumulative) {
  ...
  EXPECT_FLOAT_EQ(0.003368973,
                  stan::math::double_exponential_cdf(-3.0, 2.0, 1.0));
