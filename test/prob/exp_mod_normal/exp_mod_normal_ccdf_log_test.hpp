...
public:
  void valid_values(vector<vector<double> >& parameters,
    ...
    ccdf_log.push_back(
        std::log(1.0 - 0.2384217081348766198445));  // expected ccdf_log
    ...
    parameters.push_back(param);
    ccdf_log.push_back(
        std::log(1.0 - 0.538079416212226213645));  // expected ccdf_log
  }
  ...
  ccdf_log_function(const T_y& y, const T_loc& mu, const T_scale& sigma,
    ...
    return log(
        ...
        (0.5 * (1 + erf((y - mu) / (sqrt(2.0) * sigma))) -
         exp(-lambda * (y - mu) + lambda * sigma * lambda * sigma / 2.0) * 0.5 *
             (1 +
              erf(((y - mu) - sigma * lambda * sigma) / (sqrt(2.0) * sigma)))));
  }
};
