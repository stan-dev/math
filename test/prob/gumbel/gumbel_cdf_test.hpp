...
public:
  void valid_values(vector<vector<double>> &parameters, vector<double> &cdf) {
    ...
    cdf.push_back(
        0.3678794411714423215955237701614608674458111310317678); // expected
                                                                 // cdf
    ...
    parameters.push_back(param);
    cdf.push_back(
        0.6922006275553463538654219971827897614906780292975447); // expected
                                                                 // cdf
  ...
  cdf(const T_y &y, const T_loc &mu, const T_scale &beta, const T3 &,
      const T4 &, const T5 &) {
  ...
  template <typename T_y, typename T_loc, typename T_scale, typename T3,
            typename T4, typename T5>
  typename stan::return_type<T_y, T_loc, T_scale>::type
  cdf_function(const T_y &y, const T_loc &mu, const T_scale &beta, const T3 &,
               const T4 &, const T5 &) {
