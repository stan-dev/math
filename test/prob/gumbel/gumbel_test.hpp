...
public:
  void valid_values(vector<vector<double>> &parameters,
    ...
    parameters.push_back(param);
    log_prob.push_back(
        -1.36787944117144232159552377016146086744581113103176783450783); // expected
    // log_prob
    ...
    parameters.push_back(param);
    log_prob.push_back(
        -5.38905609893065022723042746057500781318031557055184732408712); // expected
    // log_prob
    ...
    parameters.push_back(param);
    log_prob.push_back(
        -3.34108104263468429556956520337231251605487161479989511157723); // expected
    // log_prob
  }
  ...
  log_prob(const T_y &y, const T_loc &mu, const T_scale &beta, const T3 &,
           const T4 &, const T5 &) {
