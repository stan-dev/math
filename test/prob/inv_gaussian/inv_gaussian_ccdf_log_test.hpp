...
class AgradCcdfLogInvGaussian : public AgradCcdfLogTest {
 public:
  void valid_values(vector<vector<double> >& parameters,
    ...
    ccdf_log.push_back(-3.9949836760335225255);  // expected ccdf_log

    param[0] = 0.3;  // y
    param[1] = 1.0;  // mu
    param[2] = 5.0;  // lambda
    parameters.push_back(param);
    ccdf_log.push_back(-0.003364845660995599504277);  // expected ccdf_log
  }
  ...
  stan::return_type_t<T_y, T_loc, T_shape> ccdf_log(const T_y& y,
                                                    const T_loc& mu,
  ...
  template <typename T_y, typename T_loc, typename T_shape, typename T3,
            typename T4, typename T5>
  stan::return_type_t<T_y, T_loc, T_shape> ccdf_log_function(
