...
class AgradCdfLogInvGaussian : public AgradCdfLogTest {
 public:
  void valid_values(vector<vector<double> >& parameters,
    ...
    cdf_log.push_back(-0.01857927772712011847827);  // expected cdf_log

    param[0] = 0.3;  // y
    param[1] = 1.0;  // mu
    param[2] = 5.0;  // lambda
    parameters.push_back(param);
    cdf_log.push_back(-5.696055133984662592139);  // expected cdf_log
  }
  ...
  stan::return_type_t<T_y, T_loc, T_shape> cdf_log(const T_y& y,
                                                   const T_loc& mu,
  ...
  template <typename T_y, typename T_loc, typename T_shape, typename T3,
            typename T4, typename T5>
  stan::return_type_t<T_y, T_loc, T_shape> cdf_log_function(
    ...
    return log(0.5 * erfc(-sqrt(lambda / y) * (y / mu - 1.0) * INV_SQRT_TWO)
