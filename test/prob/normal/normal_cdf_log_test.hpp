...
class AgradCdfLogNormal : public AgradCdfLogTest {
 public:
  ...
  typename stan::return_type<T_y, T_loc, T_scale>::type cdf_log_function(
    ...
    typename stan::return_type<T_y, T_loc, T_scale>::type p[6]
    ...
    typename stan::return_type<T_y, T_loc, T_scale>::type q[6]
    ...
    if (scaled_diff > 0.0) {
      ...
      if (10.0 * log(-scaled_diff) < log(std::numeric_limits<double>::max())) {
        ...
        typename stan::return_type<T_y, T_loc, T_scale>::type x2
            = square(scaled_diff);
