...
class AgradCdfLogNormal : public AgradCdfLogTest {
 public:
  ...
  typename stan::return_type<T_y, T_loc, T_scale>::type cdf_log_function(
    ...
    typename stan::return_type<T_y, T_loc, T_scale>::type cdf_log(0.0);

    // define constants used in numerical approximation of log(CDF) for x<-20
    typename stan::return_type<T_y, T_loc, T_scale>::type p[6] = {-0.000658749161529837803157, -0.0160837851487422766278,
	-0.125781726111229246204, -0.360344899949804439429, -0.305326634961232344035, -0.0163153871373020978498};
    typename stan::return_type<T_y, T_loc, T_scale>::type q[6] = {0.00233520497626869185443, 0.0605183413124413191178,
	0.527905102951428412248, 1.87295284992346047209, 2.56852019228982242072, 1.0};
    typename stan::return_type<T_y, T_loc, T_scale>::type scaled_diff(0.0);
    scaled_diff += (y - mu) / (sigma * SQRT_2);
    typename stan::return_type<T_y, T_loc, T_scale>::type sigma2pi(0.0);
    sigma2pi += sigma * SQRT_2;

    // use erfc() instead of erf() in order to retain precision since for x>0 erfc()->0
    if (scaled_diff>0.0) {
	...
	typename stan::return_type<T_y, T_loc, T_scale>::type temp_q=0.0;
	typename stan::return_type<T_y, T_loc, T_scale>::type x2=square(scaled_diff);
	typename stan::return_type<T_y, T_loc, T_scale>::type pow_x=0.0;
	for (int j=0;j<6;j++) {
	  pow_x=pow(scaled_diff,-2*j);
	  temp_p=temp_p-p[j]*pow_x;
	  temp_q=temp_q-q[j]*pow_x;
	}
	cdf_log += log(0.5)+log(1.0/SQRT_PI+(temp_p/temp_q)/x2)-log(-scaled_diff)-x2;
      }
      else {
