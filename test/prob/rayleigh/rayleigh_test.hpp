...
public:
  void valid_values(vector<vector<double>> &parameters,
    ...
    log_prob.push_back(
        -6.613705638880109381165535757083646863848999731279489491758639981013212756060610568788273346007162625); // expected log_prob
    ...
    parameters.push_back(param);
    log_prob.push_back(
        -1.306852819440054690582767878541823431924499865639744745879319990506606378030305284394136673003581312); // expected log_prob
  ...
  log_prob(const T_y &y, const T_scale &sigma, const T2 &, const T3 &,
           const T4 &, const T5 &) {
  ...
  template <typename T_y, typename T_scale, typename T2, typename T3,
            typename T4, typename T5>
  typename stan::return_type<T_y, T_scale>::type
  log_prob_function(const T_y &y, const T_scale &sigma, const T2 &, const T3 &,
                    const T4 &, const T5 &) {
    using stan::math::pi;
