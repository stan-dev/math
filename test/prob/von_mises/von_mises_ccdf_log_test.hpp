...
class AgradCcdfLogVonMises : public AgradCcdfLogTest {
 public:
  void valid_values(vector<vector<double> >& parameters,
    ...
    log_ccdf.push_back(
        std::log(1.0 - 0.1097601896880533));  // expected Log_CCDF

    param[0] = -1.5707463267948965;     // y
    param[1] = 0;                       // mu
    param[2] = 0.01;                    // kappa
    parameters.push_back(param);
    log_ccdf.push_back(std::log(1.0 - 0.2484164302237636));  // expected Log_CCDF

    param[0] = 0.0;                     // y
    param[1] = 6.283185307179586;       // mu
    param[2] = 0.1;                     // kappa
    parameters.push_back(param);
    log_ccdf.push_back(std::log(0.5));                 // expected Log_CCDF
  }
  ...
  stan::return_type_t<T_y, T_mu, T_scale> ccdf_log(const T_y& y, const T_mu& mu,
                                                   const T_scale& scale,
                                                   const T3&, const T4&,
                                                   const T5&) {
    return stan::math::von_mises_lccdf(y, mu, scale);
  }

  template <typename T_x, typename T_mu, typename T_k, typename T3, typename T4,
            typename T5>
  stan::return_type_t<T_x, T_mu, T_k> ccdf_log_function(const T_x& x,
                                                        const T_mu& mu,
    ...
    if (x <= cut) {
      return stan::math::log1m(von_mises_cdf_centered(x2, k) - f_bound_val);
    } else {
      return stan::math::log1m(von_mises_cdf_centered(x2, k) + 1 - f_bound_val);
    }
  }
};
