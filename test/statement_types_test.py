...
def test_int(int_var):
    ...
    assert int_var.is_eigen_compatible() == False
...
def test_real(real_var):
    ...
    assert real_var.is_eigen_compatible() == False
...
def test_matrix(matrix_var):
    ...
    assert matrix_var.is_eigen_compatible() == True
...
def test_simplex():
    ...
    assert simplex_var.is_eigen_compatible() == True
...
def test_positive_definite_matrix():
    ...
    assert positive_definite_matrix_var.is_eigen_compatible() == True
...
def test_array_variable(matrix_var):
    array_variable = ArrayVariable("Rev", "myarray", 3, "real", 2, matrix_var)
    ...
    assert array_variable.is_eigen_compatible() == False
...
def test_array_variable_list_initializer():
    ...
    assert array_variable.cpp() == "std::vector<stan::math::var> myarray = {1,2};"

def test_function_call_assign(int_var, real_var):
    function_call_assign = FunctionCall("stan::math::add", "result", int_var, real_var)
    assert function_call_assign.cpp() == "auto result = stan::math::eval(stan::math::add(myint,myreal));"

def test_function_call(real_var):
    function_call = FunctionCall("stan::math::subtract", None, real_var, real_var)
...
auto myexpr = mymatrix.block(0,0,2,2).unaryExpr(myexpr_counter_op);"""
    ...
    assert expression_variable.is_eigen_compatible() == True
