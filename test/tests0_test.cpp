...
TEST(ExpressionTestFwd, generalized_inverse23215) {
  ...
  stan::test::counterOp<stan::math::fvar<double>> matrix0_expr1_counter_op(
      &matrix0_expr1_counter);
  auto matrix0_expr1
      = matrix0.block(0, 0, 1, 1).unaryExpr(matrix0_expr1_counter_op);
  auto matrix2 = stan::test::make_arg<
      Eigen::Matrix<stan::math::fvar<double>, Eigen::Dynamic, Eigen::Dynamic>>(
      0.4, 1);
  auto result3
      = stan::math::eval(stan::math::generalized_inverse(matrix0_expr1));
...
TEST(ExpressionTestPrim, generalized_inverse23215) {
  ...
  int matrix0_expr1_counter = 0;
  stan::test::counterOp<double> matrix0_expr1_counter_op(
      &matrix0_expr1_counter);
  auto matrix0_expr1
      = matrix0.block(0, 0, 1, 1).unaryExpr(matrix0_expr1_counter_op);
  auto matrix2 = stan::test::make_arg<
