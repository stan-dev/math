...
TEST(MathMatrix, value_of_rec) {
...

  {
    ...
    for (int i = 0; i < 5; ++i) EXPECT_FLOAT_EQ(b[i].val_, d_b[i]);

    for (int i = 0; i < 10; ++i) EXPECT_FLOAT_EQ(a[i].val_, d_a[i]);
  }

  {
    ...
    for (int i = 0; i < 5; ++i) EXPECT_FLOAT_EQ(b[i].val_.val_, d_b[i]);

    for (int i = 0; i < 10; ++i) EXPECT_FLOAT_EQ(a[i].val_.val_, d_a[i]);
  }
}
