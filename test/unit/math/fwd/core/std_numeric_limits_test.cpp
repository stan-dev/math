...
TEST(AgradFwdNumericLimits, All_Fvar) {
  ...
  EXPECT_FLOAT_EQ(2.22507e-308, std::numeric_limits<fvar<double>>::min().val_);
  EXPECT_FLOAT_EQ(2.22507e-308,
                  std::numeric_limits<fvar<fvar<double>>>::min().val_.val_);

  EXPECT_FLOAT_EQ(1.79769e+308, std::numeric_limits<fvar<double>>::max().val_);
  ...
  EXPECT_FLOAT_EQ(0.5, std::numeric_limits<fvar<double>>::round_error().val_);
  EXPECT_FLOAT_EQ(
      0.5, std::numeric_limits<fvar<fvar<double>>>::round_error().val_.val_);

  EXPECT_FLOAT_EQ(-1021, std::numeric_limits<fvar<double>>::min_exponent);
  EXPECT_FLOAT_EQ(-1021, std::numeric_limits<fvar<fvar<double>>>::min_exponent);
  ...
  EXPECT_FLOAT_EQ(1024, std::numeric_limits<fvar<fvar<double>>>::max_exponent);

  EXPECT_FLOAT_EQ(308, std::numeric_limits<fvar<double>>::max_exponent10);
  EXPECT_FLOAT_EQ(308, std::numeric_limits<fvar<fvar<double>>>::max_exponent10);
  ...
  EXPECT_FLOAT_EQ(
      INFTY, std::numeric_limits<fvar<fvar<double>>>::infinity().val_.val_);
