...
TEST(ProbDistributionsOrdLog, fd_fd_stvec) {
  ...
  vector_fd c2_fd(3);
  c2_fd << -0.95, -0.10, 0.95;
  for (int i = 0; i < 3; i++)
    c2_fd[i].d_ = 1.0;

  vector_fd c3_fd(3);
  c3_fd << -0.95, -0.10, 0.95;
  for (int i = 0; i < 3; i++)
    c3_fd[i].d_ = 1.0;

  vector_fd c4_fd(3);
  c4_fd << -0.95, -0.10, 0.95;
  for (int i = 0; i < 3; i++)
    c4_fd[i].d_ = 1.0;
...
TEST(ProbDistributionsOrdLog, fd_d_stvec) {
  ...
  EXPECT_FLOAT_EQ(out_fd.val_, -3.9442226297351447171906);
  EXPECT_FLOAT_EQ(out_fd.d_, -0.340621916056822);
  ...
  EXPECT_FLOAT_EQ(out_ffd.val_.val_, -3.9442226297351447171906);
  EXPECT_FLOAT_EQ(out_ffd.d_.val_, -0.340621916056822);
