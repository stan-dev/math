...
TEST(RevMatVectorize,applyScalarUnary) {
  ...
  expect_match_return_t<std::vector<fvar<double> >, std::vector<fvar<double> > >();
}
...
TEST(MathPrimMatVectorize, applyScalarUnaryMatrix) {
  using stan::math::fvar;
  using stan::math::foo;
  typedef Eigen::Matrix<fvar<double> , Eigen::Dynamic, Eigen::Dynamic> MatrixXvar;
