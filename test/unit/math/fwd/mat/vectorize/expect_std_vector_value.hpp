#ifndef TEST_UNIT_MATH_FWD_MAT_VECTORIZE_EXPECT_STD_VECTOR_VALUE_HPP
#define TEST_UNIT_MATH_FWD_MAT_VECTORIZE_EXPECT_STD_VECTOR_VALUE_HPP
  ...
  for (size_t i = 0; i < vector_vector_size; ++i) {
    for (size_t j = 0; j < num_inputs; ++j) {
      ...
      for (size_t k = 0; k < num_inputs; ++k) {
        if (i == k)
          z.push_back(build_vector<F>(vector<T>(), j));
        else
          z.push_back(build_vector<F>(vector<T>()));
      }
      vector<vector<T> > fz = 
