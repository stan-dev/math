...
TEST(ProbDistributionsMultiGP, fvar_double) {
  ...
  EXPECT_FLOAT_EQ(lp_ref.val_, stan::math::multi_gp_lpdf(y, Sigma, w).val_);
  EXPECT_FLOAT_EQ(-74.572952, stan::math::multi_gp_lpdf(y, Sigma, w).d_);
}
...
TEST(ProbDistributionsMultiGP, fvar_fvar_double) {
  ...
  EXPECT_FLOAT_EQ(lp_ref.val_.val_,
                  stan::math::multi_gp_lpdf(y, Sigma, w).val_.val_);
  EXPECT_FLOAT_EQ(-74.572952, stan::math::multi_gp_lpdf(y, Sigma, w).d_.val_);
}
