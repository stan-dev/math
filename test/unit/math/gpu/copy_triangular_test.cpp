...
TEST(MathMatrixGPU, copy_triangular_m_exception_pass) {
  ...
  EXPECT_NO_THROW(stan::math::copy_triangular<stan::math::Upper>(m0));
  EXPECT_NO_THROW(stan::math::copy_triangular<stan::math::Lower>(m0));
...
TEST(MathMatrixGPU, copy_triangular_m_pass) {
  ...
  EXPECT_NO_THROW(m00_dst
                  = stan::math::copy_triangular<stan::math::Upper>(m00));
  ...
  EXPECT_NO_THROW(m00_dst
                  = stan::math::copy_triangular<stan::math::Lower>(m00));
  EXPECT_NO_THROW(stan::math::copy(m0_dst, m00_dst));
  EXPECT_EQ(1, m0_dst(0, 0));
