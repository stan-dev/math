...
TEST(MathMatrixGPU, lower_tri_inverse) {
  ...
  std::vector<int> sizes = {32,48,64,80,96,112,128,160,192,224,256,288,320,352,384,416,448,480,512,576,640,704,768,832,896,960,1024,1152,1280,1408,1536,1664,1798,1920,2048,2304,2560,2816,3072,3328,3584,3840,4096,4608,5120,5632,6144,6656,7168,7680,8192,9216,10240};
  for(auto s : sizes){
    std::cout << s << "\t";
    auto m1 = stan::math::matrix_d(s, s);
    auto m5 = stan::math::matrix_d(s, s);
    populate_lower_triangular_inverse_input(m1);
    std::vector<size_t> temp_gpu;
    stan::math::matrix_gpu m2(s, s);
    while(!are_timings_steady(temp_gpu)) {
        std::chrono::steady_clock::time_point begin = std::chrono::steady_clock::now();
        stan::math::copy(m2, m1);
        auto m3 = stan::math::lower_triangular_inverse(m2);
        stan::math::copy(m5, m3);
        std::chrono::steady_clock::time_point end= std::chrono::steady_clock::now();   
        size_t current = std::chrono::duration_cast<std::chrono::nanoseconds> (end - begin).count();
        temp_gpu.push_back(current);
    }    
  }
}
#endif