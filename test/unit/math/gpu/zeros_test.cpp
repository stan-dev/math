...
TEST(MathMatrixGPU, zero_m_exception_pass) {
  ...
  EXPECT_NO_THROW(m.zeros<stan::math::Entire>());
  EXPECT_NO_THROW(m.zeros<stan::math::Lower>());
  EXPECT_NO_THROW(m.zeros<stan::math::Upper>());

  stan::math::matrix_gpu m0;
  EXPECT_NO_THROW(m0.zeros<stan::math::Entire>());
  EXPECT_NO_THROW(m0.zeros<stan::math::Lower>());
  EXPECT_NO_THROW(m0.zeros<stan::math::Upper>());
}
...
TEST(MathMatrixGPU, zero_m_value_check) {
  ...
  EXPECT_NO_THROW(m.zeros<stan::math::Entire>());
  EXPECT_NO_THROW(m_lower.zeros<stan::math::Lower>());
  EXPECT_NO_THROW(m_upper.zeros<stan::math::Upper>());
