...
TEST_F(laplace_bernoulli_dim500_test, laplace_marginal_diag_lpmf) {
  using stan::math::laplace_marginal_lpmf;
  using stan::math::to_vector;
  using stan::math::var;

  stan::math::test::sqr_exp_kernel_diag_functor K;
  bernoulli_logit_likelihood L;
  var target_dense
    = laplace_marginal_lpmf<false>(y, L, eta_dummy, delta_lk,
                                   theta_0, K,
  ...
  // QUESTION -- why do I get a warning message if I use <false> before
  // arguments?
  var target =
    laplace_marginal_tol_lpmf(y, L, eta_dummy, delta_lk,
                              tolerance,
                              max_num_steps,
                              hessian_block_size,
                              covariance_block_size,
  ...
  double target_1u = laplace_marginal_lpmf<false>(y, L, eta_dummy_dbl, delta_lk,
                                 theta_0, K,
                                 nullptr, x, phi_1u(0), phi_1u(1), n_observations);
  double target_1l = laplace_marginal_lpmf<false>(y, L, eta_dummy_dbl, delta_lk,
                                        theta_0, K,
                                        nullptr, x, phi_1l(0), phi_1l(1),
                                        n_observations);
  double target_2u = laplace_marginal_lpmf<false>(y, L, eta_dummy_dbl, delta_lk,
                                        theta_0, K,
                                        nullptr, x, phi_2u(0), phi_2u(1),
                                        n_observations);
  double target_2l = laplace_marginal_lpmf<false>(y, L, eta_dummy_dbl, delta_lk,
                                        theta_0, K,
                                        nullptr, x, phi_2l(0), phi_2l(1),
                                        n_observations);
...
TEST_F(laplace_bernoulli_dim500_test, laplace_marginal_lpmf) {
  using stan::math::laplace_marginal_lpmf;
  using stan::math::laplace_marginal_tol_lpmf;
  using stan::math::var;
...
struct covariance_motorcycle_functor {
  template <typename TX, typename LengthF, typename LengthG, typename SigmaF, typename SigmaG>
  auto operator()(
      const TX& x, const LengthF& length_scale_f, const LengthG& length_scale_g,
      const SigmaF& sigma_f, const SigmaG& sigma_g, const int n_obs,
    ...
    double jitter = 1e-6;
    Matrix<scalar_t, -1, -1> kernel_f = gp_exp_quad_cov(x, sigma_f, length_scale_f);
    Matrix<scalar_t, -1, -1> kernel_g = gp_exp_quad_cov(x, sigma_g, length_scale_g);

    Matrix<scalar_t, -1, -1> kernel_all = Eigen::MatrixXd::Zero(2 * n_obs, 2 * n_obs);
...
struct normal_likelihood {
  template <typename Theta, typename Eta>
  auto operator()(
      const Theta& theta, const Eta& eta, const Eigen::VectorXd& y,
