...
TEST(CubicOrBisect, ReturnsInteriorMaximiser) {
  using stan::math::internal::cubic_interpolation;
  double a = 0.0;
  double b = 1.0;
  double fa = 0.0;
  double fb = -1.0;
  double fpa = 1.0;
  double fpb = -0.5;
  double alpha = cubic_interpolation(a, fa, fpa, b, fb, fpb);
  EXPECT_GT(alpha, a);
  EXPECT_LT(alpha, b);
}
...
TEST(CubicOrBisect, MovesRightWhenRightImproves) {
  ...
  double fb1 = -1.0;
  double fpb = -0.5;
  double alpha1 = cubic_interpolation(a, fa, fpa, b1, fb1, fpb);
  double b2 = 1.0;
  double fb2 = -0.1;  // improved right endpoint
  double alpha2 = cubic_interpolation(a, fa, fpa, b2, fb2, fpb);
