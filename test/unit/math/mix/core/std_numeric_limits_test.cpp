...
TEST(AgradMixNumericLimits, All_Fvar) {
  ...
  EXPECT_FLOAT_EQ(
      0.5, std::numeric_limits<fvar<fvar<var>>>::round_error().val_.val_.val());
  ...
  EXPECT_FLOAT_EQ(INFTY, std::numeric_limits<fvar<var>>::infinity().val_.val());
  EXPECT_FLOAT_EQ(
      INFTY, std::numeric_limits<fvar<fvar<var>>>::infinity().val_.val_.val());
