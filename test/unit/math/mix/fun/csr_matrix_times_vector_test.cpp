...
TEST(MathMixMatFun, csr_matrix_times_vector_err) {
  ...
  EXPECT_THROW(stan::math::csr_matrix_times_vector(8, 8, w, v_too_long, u, b),
               std::invalid_argument);
  std::vector<int> v_zero{0, 3, 4, 2, 8, 3, 4};
  EXPECT_THROW(stan::math::csr_matrix_times_vector(8, 8, w, v_zero, u, b),
               std::domain_error);
  std::vector<int> u_zero{0, 3, 5, 7, 7, 9};
  EXPECT_THROW(
      stan::math::csr_matrix_times_vector(8, 8, w, v_too_long, u_zero, b),
      std::domain_error);
}
