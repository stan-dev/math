...
T g1(const T& x) {
  ...
  return stan::math::simplex_column_constrain<false>(x, lp);
}
template <typename T>
T g2(const T& x) {
  stan::scalar_type_t<T> lp = 0;
  return stan::math::simplex_column_constrain<true>(x, lp);
}
template <typename T>
...
TEST(MathMixMatFun, simplexTransform) {
  ...
  v1 << 1, 2, 3;
  simplex_column_constrain_test::expect_simplex_transform(v1);

  Eigen::MatrixXd v2(2, 3);
  v2 << 3, -1, 3, -1, 3, -1;
  simplex_column_constrain_test::expect_simplex_transform(v2);

  Eigen::MatrixXd v3(3, 3);
  v3 << 2, 3, -1, 2, 3, -1, 2, 3, -1;
  simplex_column_constrain_test::expect_simplex_transform(v3);

  Eigen::MatrixXd v4(4, 3);
  v4 << 2, -1, 0, -1.1, 2, -1, 0, -1.1, 2, -1, 0, -1.1;
  simplex_column_constrain_test::expect_simplex_transform(v4);

  Eigen::MatrixXd v5(5, 3);
  v5 << 1, -3, 2, 0, -1, 1, -3, 2, 0, -1, 1, -3, 2, 0, -1;
  simplex_column_constrain_test::expect_simplex_transform(v5);
}
