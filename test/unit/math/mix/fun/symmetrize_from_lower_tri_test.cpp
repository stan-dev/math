...
void expect_lower_sym_tri(const Eigen::MatrixXd& m) {
  auto f = [](const auto& x) { 
		return stan::math::symmetrize_from_lower_tri(x); 
	};
...
TEST(MathMixMatFun, symmetrize_from_lower_tri) {
  ...
  expect_lower_sym_tri(a);

  Eigen::MatrixXd b(1, 1);
  b << -1.2;
  expect_lower_sym_tri(b);

  Eigen::MatrixXd c(2, 2);
  c << -1, 2, 5, 10;
  expect_lower_sym_tri(c);

  Eigen::MatrixXd d(2, 3);
  d << -1, 2, -3, 5, 10, 100;
  expect_lower_sym_tri(d);

  Eigen::MatrixXd e(3, 1);
  e << 1, 2, 3;
  expect_lower_sym_tri(e);

  Eigen::MatrixXd g(3, 3);
  g << 1, 2, 3, 4, 5, 6, 7, 8, 9;
  expect_lower_sym_tri(g);
}