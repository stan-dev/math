...
inline void test_laplace_marginal_poisson_logit_lpmf_finite_diff(T1&& y,
    T2&& phi_dbl, T3&& x, T4&& n_samples, T5&& theta_0,
    double tolerance, long int max_num_steps,
    const int hessian_block_size, const int solver,
    const int max_steps_line_search,
    const double gp_tol, const double fd_tol) {
    ...
    std::string err_msg = "laplace parameters: \n"
    "\ttolerance(" + std::to_string(tolerance) + ")" +
    "\n\tmax_num_steps(" + std::to_string(max_num_steps) + ")" +
    "\n\thessian_block_size(" + std::to_string(hessian_block_size) + ")" +
    "\n\tsolver(" + std::to_string(solver) + ")" +
    "\n\tmax_steps_line_search(" + std::to_string(max_steps_line_search) + ")";
