...
/**
 * This is the structure for testing mock function atanh (defined in the
...
struct atanh_test {
  ...
  static std::vector<double> valid_inputs() {
    return test::math::vector_builder<double>()
      .add(1.3).add(-2.6).add(0).add(-0.2).add(1.0).add(-1.0).build();
  }
  ...
  static std::vector<int> int_valid_inputs() {
    return test::math::vector_builder<int>()
      .add(1).add(-2).add(0).add(3).add(-1).build();
  }
