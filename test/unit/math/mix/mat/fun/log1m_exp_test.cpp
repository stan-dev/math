...
/**
 * This is the structure for testing mock function log1m_exp (defined in the
 * testing framework).  See README.txt for more instructions.
 */
struct log1m_exp_test {
  ...
  static std::vector<double> valid_inputs() {
    return test::math::vector_builder<double>()
      .add(1.3).add(-12.6).add(0).add(-0.2).build();
  }
  ...
  static std::vector<int> int_valid_inputs() {
    return test::math::vector_builder<int>()
      .add(-14).add(-2).add(0).add(3).build();
  }
...
INSTANTIATE_TYPED_TEST_CASE_P(, prim_scalar_unary_test, log1m_exp_test);
INSTANTIATE_TYPED_TEST_CASE_P(, rev_scalar_unary_test, log1m_exp_test);
INSTANTIATE_TYPED_TEST_CASE_P(, fwd_scalar_unary_test, log1m_exp_test);
INSTANTIATE_TYPED_TEST_CASE_P(, mix_scalar_unary_test, log1m_exp_test);
