...
TEST(AgradFiniteDiff, hessian) {
  ...
  stan::math::finite_diff_hessian(f, f_vec, f_fin_diff_eval,
    ...
    for (int j = 0; j < 3; ++j) {
      for (int k = 0; k < 3; ++k) {
        ...
        EXPECT_NEAR(grad_H_f[i](j, k), fin_diff_grad_H_f[i](j, k), 1e-10)
          << " i: " << i << " j: " << j << " k: " << k;
        EXPECT_NEAR(an_grad_H_norm[i](j, k),
                    fin_diff_grad_H_norm[i](j, k), 1e-10)
          << " i: " << i << " j: " << j << " k: " << k;
...
TEST(AgradFiniteDiff, hessianZeroOneArg) {
  ...
  stan::math::finite_diff_hessian(sum_f, sum_vec, f_eval_sum, f_grad_sum,
                                  f_hess_sum);
  ...
  stan::math::finite_diff_hessian(sum_f, z_sum_vec, f_z_eval_sum, f_z_grad_sum,
                                  f_z_hess_sum);
}
