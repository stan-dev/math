...
template <class F1, class F2>
void compare_grad_hess(const F1& f1, const F2& f2,
                       const Matrix<double, Dynamic, 1>& inp_vec) {
  double f1_eval;
  Matrix<double, Dynamic, Dynamic> H_f1;
  vector<Matrix<double, Dynamic, Dynamic>> grad_H_f1;
  double f2_eval;
  Matrix<double, Dynamic, Dynamic> H_f2;
  vector<Matrix<double, Dynamic, Dynamic>> grad_H_f2;
  stan::math::grad_hessian(f1, inp_vec, f1_eval, H_f1, grad_H_f1);
  stan::math::grad_hessian(f2, inp_vec, f2_eval, H_f2, grad_H_f2);
  EXPECT_FLOAT_EQ(f1_eval, f2_eval);
  for (int i = 0; i < H_f1.rows(); ++i)
    for (int j = 0; j < H_f1.cols(); ++j)
      EXPECT_FLOAT_EQ(H_f1(i, j), H_f2(i, j));

  for (size_t k = 0; k < grad_H_f1.size(); ++k)
    for (int i = 0; i < grad_H_f1[k].rows(); ++i)
      for (int j = 0; j < grad_H_f1[k].cols(); ++j)
        EXPECT_FLOAT_EQ(grad_H_f1[k](i, j), grad_H_f2[k](i, j));
}
...
void test_invalid_args(const vector<Matrix<double, Dynamic, 1>>& invalid_args) {
  for (size_t i = 0; i < invalid_args.size(); ++i) {
    ...
    auto args_V_ff = make_args<fvar<var>, fvar<var>, Dynamic, 1>(inp);
    auto args_V_df = make_args<double, fvar<var>, Dynamic, 1>(inp);
    auto args_V_fd = make_args<fvar<var>, double, Dynamic, 1>(inp);
    auto args_RV_ff = make_args<fvar<var>, fvar<var>, 1, Dynamic>(inp);
    auto args_RV_df = make_args<double, fvar<var>, 1, Dynamic>(inp);
    auto args_RV_fd = make_args<fvar<var>, double, 1, Dynamic>(inp);
    auto args_V_ffff
        = make_args<fvar<fvar<var>>, fvar<fvar<var>>, Dynamic, 1>(inp);
    auto args_V_dff = make_args<double, fvar<fvar<var>>, Dynamic, 1>(inp);
    auto args_V_ffd = make_args<fvar<fvar<var>>, double, Dynamic, 1>(inp);
    auto args_RV_ffff
        = make_args<fvar<fvar<var>>, fvar<fvar<var>>, 1, Dynamic>(inp);
    auto args_RV_dff = make_args<double, fvar<fvar<var>>, 1, Dynamic>(inp);
    auto args_RV_ffd = make_args<fvar<fvar<var>>, double, 1, Dynamic>(inp);
...
TEST(MathFunctions, vec_binormal_integral_val_and_grad_test_fvar_var_fvar_var) {
  ...
  inp_vec << 0.4, 0.7, 0.8;
  compare_grad_hess(dist_fun_V_R, tru_fun, inp_vec);

  compare_grad_hess(dist_fun_RV_R, tru_fun, inp_vec);

  compare_hess(dist_fun_RV_R, tru_fun, inp_vec);
  compare_hess(dist_fun_V_R, tru_fun, inp_vec);
}
TEST(MathFunctions, vec_binormal_integral_val_and_grad_test_fvar_var_doub) {
...
TEST(MathFunctions, vec_binormal_integral_val_and_grad_test_doub_fvar_var) {
