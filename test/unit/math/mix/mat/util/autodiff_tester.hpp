    ...
    namespace test {
      ...
      void expect_near(const std::string& msg, const Eigen::Matrix<T, R, C>& x1,
        ...
        EXPECT_EQ(x1.rows(), x2.rows())
            << "expect_near rows expect_eq(" << x1.rows() << ", " << x2.rows()
            << ")" << std::endl
            << msg << std::endl;
        EXPECT_EQ(x1.cols(), x2.cols())
            << "expect_near cols expect_eq(" << x1.rows() << ", " << x2.rows()
            << ")" << std::endl
            << msg << std::endl;
      ...
      void test_gradient(const F& f, const Eigen::VectorXd& x, double fx,
        ...
        if (!test_derivs || !is_finite(x) || !is_finite(fx)) return;
      ...
      void expect_throw(const F& f, const Eigen::VectorXd& x) {
        ...
        for (int i = 0; i < x.rows(); ++i) x_t(i) = x(i);
        try {
          f(x_t);
        } catch (...) {
          SUCCEED();
