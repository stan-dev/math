...
void expect_matrix_value() {
  ...
  for (int i = 0; i < template_m.size(); ++i) {
    matrix_t y = build_matrix<F>(template_m, i);
    matrix_t z = build_matrix<F>(template_m, i);
  ...
  for (size_t i = 0; i < vector_matrix_size; ++i) {
    for (int j = 0; j < template_m.size(); ++j) {
      ...
      for (size_t k = 0; k < vector_matrix_size; ++k) {
        if (k == i) {
          a.push_back(build_matrix<F>(template_m, j));
          b.push_back(build_matrix<F>(template_m, j));
        } else {
          a.push_back(build_matrix<F>(template_m));
          b.push_back(build_matrix<F>(template_m));
        }
      }
