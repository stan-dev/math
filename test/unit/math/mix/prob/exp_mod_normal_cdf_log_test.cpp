...
TEST_F(AgradRev, mathMixScalFun_exp_mod_normal_lcdf) {
  ...
  for (double y : {-10.0, -3.0, 0.3, 4.0, 60.0}) {
    stan::test::expect_ad(f, Eigen::Vector4d(y, 0.1, 1.3, 0.7));
  }
  // Deep lower tail: log_diff_exp of two ~-480 terms is ill-conditioned for
  // the finite-difference third derivatives.
  stan::test::ad_tolerances tols;
  tols.grad_hessian_grad_hessian_ = 5e-2;
  stan::test::expect_ad(tols, f, Eigen::Vector4d(-40, 0.1, 1.3, 0.7));
  stan::test::expect_ad(
      tols,
