...
TEST_F(AgradRev, mathMixScalFun_normal_lccdf_branch_cutoffs) {
   ...
   * prim/prob/normal_lcdf.hpp. lccdf(y) reflects to lcdf(-y), so y =
   * -scaled_diff * sqrt(2). The offset stays well clear of
...
TEST_F(AgradRev, mathMixScalFun_normal_lccdf_branch_accuracy) {
  ...
  for (const branch_ref& c : cases) {
    stan::math::var y = c.y;
    stan::math::var lp = stan::math::normal_lccdf(y, 0.0, 1.0);
    lp.grad();
...
/** d2/dy2 log(1 - Phi(y)), mu=0 sigma=1 */
double d2_normal_lccdf(double y) {
  stan::math::fvar<stan::math::fvar<double>> yv;
  yv.val_.val_ = y;
  yv.val_.d_ = 1.0;
  yv.d_.val_ = 1.0;
  return stan::math::normal_lccdf(yv, 0.0, 1.0).d_.d_;
}

/** d3/dy3 log(1 - Phi(y)), mu=0 sigma=1 */
double d3_normal_lccdf(double y) {
  stan::math::fvar<stan::math::fvar<stan::math::fvar<double>>> yv;
  yv.val_.val_.val_ = y;
  yv.val_.val_.d_ = 1.0;
  yv.val_.d_.val_ = 1.0;
  yv.d_.val_.val_ = 1.0;
  return stan::math::normal_lccdf(yv, 0.0, 1.0).d_.d_.d_;
}
 ...
 * reference rather than handed to expect_ad: order 2 here was silently zero
 * rather than wrong-by-a-visible-amount, and order 3 uses a type the harness
 * in test/unit/math/test_ad.hpp never instantiates.
 */
TEST_F(AgradRev, mathMixScalFun_normal_lccdf_tail_derivatives) {
  ...
  for (const tail_ref& c : d2_cases) {
    const double got = d2_normal_lccdf(c.y);
  ...
  for (const tail_ref& c : d3_cases) {
    const double got = d3_normal_lccdf(c.y);
...
TEST_F(AgradRev, mathMixScalFun_normal_lccdf_derivatives_are_finite) {
  for (double y = -120.0; y <= 120.0; y += 0.5) {
    EXPECT_FALSE(std::isnan(d2_normal_lccdf(y))) << "d2 NaN at y = " << y;
    EXPECT_FALSE(std::isnan(d3_normal_lccdf(y))) << "d3 NaN at y = " << y;
  }
}
...
TEST_F(AgradRev, mathMixScalFun_normal_lccdf_far_tail_gradient) {
  ...
  for (double y = 42.0; y <= 300.0; y += 0.25) {
    stan::math::var yv = y;
    stan::math::var lp = stan::math::normal_lccdf(yv, 0.0, 1.0);
    lp.grad();
