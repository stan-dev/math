...
TEST_F(AgradRev, mathMixScalFun_std_normal_lcdf) {
  ...
  // P1: exp(x2) overflow in the scaled_y > 2.9 derivative branch
...
TEST_F(AgradRev, mathMixScalFun_std_normal_lcdf_branch_cutoffs) {
  auto f = [](const auto& y) { return stan::math::std_normal_lcdf(y); };
   ...
   * prim/prob/std_normal_lcdf.hpp. y = scaled_y * sqrt(2).
...
TEST_F(AgradRev, mathMixScalFun_std_normal_lcdf_branch_accuracy) {
  ...
  for (const branch_ref& c : cases) {
    stan::math::var y = c.y;
    stan::math::var lp = stan::math::std_normal_lcdf(y);
    lp.grad();
...
/** d2/dy2 log Phi(y) */
double d2_std_normal_lcdf(double y) {
  stan::math::fvar<stan::math::fvar<double>> yv;
  yv.val_.val_ = y;
  yv.val_.d_ = 1.0;
  yv.d_.val_ = 1.0;
  return stan::math::std_normal_lcdf(yv).d_.d_;
}

/** d3/dy3 log Phi(y) */
double d3_std_normal_lcdf(double y) {
  stan::math::fvar<stan::math::fvar<stan::math::fvar<double>>> yv;
  yv.val_.val_.val_ = y;
  yv.val_.val_.d_ = 1.0;
  yv.val_.d_.val_ = 1.0;
  yv.d_.val_.val_ = 1.0;
  return stan::math::std_normal_lcdf(yv).d_.d_.d_;
}
 ...
 * significant digits. std_normal_lcdf(y) is normal_lcdf(y, 0, 1), so the
 * references match mix/prob/normal_cdf_log_test.cpp exactly; see that file for
 * why these are asserted directly rather than handed to expect_ad.
 */
TEST_F(AgradRev, mathMixScalFun_std_normal_lcdf_tail_derivatives) {
  struct tail_ref {
    double y;
    double expected;
  };
  ...
  for (const tail_ref& c : d2_cases) {
    const double got = d2_std_normal_lcdf(c.y);
  ...
  for (const tail_ref& c : d3_cases) {
    const double got = d3_std_normal_lcdf(c.y);
...
TEST_F(AgradRev, mathMixScalFun_std_normal_lcdf_derivatives_are_finite) {
  for (double y = -120.0; y <= 120.0; y += 0.5) {
    EXPECT_FALSE(std::isnan(d2_std_normal_lcdf(y))) << "d2 NaN at y = " << y;
    EXPECT_FALSE(std::isnan(d3_std_normal_lcdf(y))) << "d3 NaN at y = " << y;
  }
}

/**
 * Far lower tail gradient, the inverse Mills ratio, which grows linearly as
 * y -> -infinity (DLMF 7.12.1). The sweep compares against the five-term
 * asymptotic, independent of the four-term form the implementation uses.
 */
TEST_F(AgradRev, mathMixScalFun_std_normal_lcdf_far_tail_gradient) {
  struct far_ref {
    double y;
    double value;
    double grad;
  ...
  for (double y = -300.0; y <= -42.0; y += 0.25) {
    stan::math::var yv = y;
    stan::math::var lp = stan::math::std_normal_lcdf(yv);
    lp.grad();
