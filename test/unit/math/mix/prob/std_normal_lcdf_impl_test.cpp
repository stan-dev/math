...
TEST_F(AgradRev, std_normal_extreme_tail_derivatives) {
  ...
  // The last row uses L'(-a)~a, L''(-a)~-1, L'''(-a)~2/a^3. Larger |z|
  // overflows the squared divisor in nested fvar division.
  for (const auto& row :
...
TEST_F(AgradRev, std_normal_extreme_tail_vector_derivatives) {
  using namespace stan::math;
  const Eigen::Array<double, 5, 1> z(-1e8, -1e10, -1e50, 0, 1e308);
  const Eigen::Array<double, 5, 1> expected(2e-24, 2e-30, 2e-150,
