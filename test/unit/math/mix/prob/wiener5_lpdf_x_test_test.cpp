...
/*TEST(mathMixScalFun_sv, wiener5_lpdf) { // runs successfully
  auto f_sv = [](const auto& y, const auto& a, const auto& t0, const auto& w, const auto& v) {
...
/*TEST(mathMixScalFun_v, wiener5_lpdf) { // runs successfully
  auto f_v = [](const auto& y, const auto& a, const auto& t0, const auto& w, const auto& sv) {
    return [&y, &a, &t0, &w, &sv](const auto& v) {
      return stan::math::wiener5_lpdf(y, a, t0, w, v, sv, 1e-4);
    };
  };
...
TEST(mathMixScalFun_y, wiener5_lpdf) { // error with grad_hessian()
  auto f_y = [](const auto& a, const auto& t0, const auto& w, const auto& v, const auto& sv) {
...
/*TEST(mathMixScalFun_a, wiener5_lpdf) { // error with grad_hessian()
  auto f_a = [](const auto& y, const auto& t0, const auto& w, const auto& v, const auto& sv) {
...
/*TEST(mathMixScalFun_w, wiener5_lpdf) {
  auto f_w = [](const auto& y, const auto& a, const auto& t0, const auto& v, const auto& sv) {
    return [&y, &a, &t0, &v, &sv](const auto& w) {
      return stan::math::wiener5_lpdf(y, a, t0, w, v, sv, 1e-4);
    };
  };
