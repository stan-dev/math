...
TEST_F(Wiener7MixArgs, wiener_lccdf_defective_y_v_sw) {
  auto f_y_v_sw = [](const auto& a, const auto& t0, const auto& w,
                     const auto& sv, const auto& st0) {
    return [&a, &t0, &w, &sv, &st0](const auto& y, const auto& v,
                                    const auto& sw) {
...
TEST_F(Wiener7MixArgs, wiener_lccdf_defective_y_v_st0) {
  auto f_y_v_st0 = [](const auto& a, const auto& t0, const auto& w,
                      const auto& sv, const auto& sw) {
    return [&a, &t0, &w, &sv, &sw](const auto& y, const auto& v,
                                   const auto& st0) {
      return stan::math::wiener_lccdf_defective(y, a, t0, w, v, sv, sw, st0);
    };
  };
...
TEST_F(Wiener7MixArgs, wiener_lccdf_defective_y_sv_sw) {
  auto f_y_sv_sw = [](const auto& a, const auto& t0, const auto& w,
                      const auto& v, const auto& st0) {
    return [&a, &t0, &w, &v, &st0](const auto& y, const auto& sv,
                                   const auto& sw) {
      return stan::math::wiener_lccdf_defective(y, a, t0, w, v, sv, sw, st0);
    };
  };
...
TEST_F(Wiener7MixArgs, wiener_lccdf_defective_y_sv_st0) {
  auto f_y_sv_st0 = [](const auto& a, const auto& t0, const auto& w,
                       const auto& v, const auto& sw) {
    return [&a, &t0, &w, &v, &sw](const auto& y, const auto& sv,
                                  const auto& st0) {
      return stan::math::wiener_lccdf_defective(y, a, t0, w, v, sv, sw, st0);
    };
  };
...
TEST_F(Wiener7MixArgs, wiener_lccdf_defective_y_sw_st0) {
  auto f_y_sw_st0 = [](const auto& a, const auto& t0, const auto& w,
                       const auto& v, const auto& sv) {
    return [&a, &t0, &w, &v, &sv](const auto& y, const auto& sw,
                                  const auto& st0) {
      return stan::math::wiener_lccdf_defective(y, a, t0, w, v, sv, sw, st0);
    };
  };
