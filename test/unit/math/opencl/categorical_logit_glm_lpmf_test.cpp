...
TEST(ProbDistributionsCategoricalLogitGLM, error_checking) {
  ...
  EXPECT_NO_THROW(stan::math::categorical_logit_glm_lpmf(y_cl, x_cl, alpha, beta));

  EXPECT_THROW(stan::math::categorical_logit_glm_lpmf(y_size_cl, x_cl, alpha, beta),
               std::invalid_argument);
  EXPECT_THROW(stan::math::categorical_logit_glm_lpmf(y_cl, x_size1_cl, alpha, beta),
               std::invalid_argument);
  EXPECT_THROW(stan::math::categorical_logit_glm_lpmf(y_cl, x_size2_cl, alpha, beta),
               std::invalid_argument);
  EXPECT_THROW(stan::math::categorical_logit_glm_lpmf(y_cl, x_cl, alpha_size, beta),
               std::invalid_argument);
  EXPECT_THROW(stan::math::categorical_logit_glm_lpmf(y_cl, x_cl, alpha, beta_size1),
               std::invalid_argument);
  EXPECT_THROW(stan::math::categorical_logit_glm_lpmf(y_cl, x_cl, alpha, beta_size2),
               std::invalid_argument);

  EXPECT_THROW(stan::math::categorical_logit_glm_lpmf(y_value_cl, x_cl, alpha, beta),
               std::domain_error);
  EXPECT_THROW(stan::math::categorical_logit_glm_lpmf(y_cl, x_value_cl, alpha, beta),
               std::domain_error);
  EXPECT_THROW(stan::math::categorical_logit_glm_lpmf(y_cl, x_cl, alpha_value, beta),
               std::domain_error);
  EXPECT_THROW(stan::math::categorical_logit_glm_lpmf(y_cl, x_cl, alpha, beta_value),
               std::domain_error);
}
...
TEST(ProbDistributionsCategoricalLogitGLM, gpu_matches_cpu_small_simple) {
  ...
  var res1
      = stan::math::categorical_logit_glm_lpmf(y_cl, x_cl, alpha_var1, beta_var1);
  var res2 = stan::math::categorical_logit_glm_lpmf(y, x, alpha_var2, beta_var2);

  (res1 + res2).grad();

  expect_near_rel("categorical_logit_glm_lpmf (OpenCL)", res1.val(), res2.val());

  expect_near_rel("categorical_logit_glm_lpmf (OpenCL)", alpha_var1.adj().eval(),
                  alpha_var2.adj().eval());
...
TEST(ProbDistributionsCategoricalLogitGLM, gpu_matches_cpu_big) {
  ...
  Matrix<double, Dynamic, Dynamic> beta = Matrix<double, Dynamic, Dynamic>::Random(M, C);
  ...
  var res1 = stan::math::categorical_logit_glm_lpmf<true>(y_cl, x_cl, alpha_var1,
                                                    beta_var1);
  var res2
      = stan::math::categorical_logit_glm_lpmf<true>(y, x, alpha_var2, beta_var2);
