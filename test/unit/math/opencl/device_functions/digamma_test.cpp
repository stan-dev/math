...
static const std::string test_kernel_code = STRINGIFY(
    __kernel void test(__global double *B, __global double *A) {
      const int i = get_global_id(0);
      B[i] = digamma(A[i]);
    }
);

const stan::math::opencl_kernels::kernel_cl<stan::math::opencl_kernels::out_buffer, stan::math::opencl_kernels::in_buffer> digamma("test", {stan::math::opencl_kernels::digamma_device_function, test_kernel_code});
