...
const stan::math::opencl_kernels::kernel_cl<
    ...
    log1m_exp("test", std::vector<std::string>{stan::math::opencl_kernels::log1m_exp_device_function, test_kernel_code});
...
TEST(MathMatrixCL, log1m_exp) {
  ...
  stan::test::expect_near_rel("log1m_exp (OpenCL)", res, stan::math::log1m_exp(a));
}
