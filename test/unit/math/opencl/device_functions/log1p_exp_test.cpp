...
const stan::math::opencl_kernels::kernel_cl<
    ...
    log1p_exp("test", std::vector<std::string>{stan::math::opencl_kernels::log1p_exp_device_function, test_kernel_code});
...
TEST(MathMatrixCL, log1p_exp) {
  ...
  stan::test::expect_near_rel("log1p_exp (OpenCL)", res, stan::math::log1p_exp(a));
}
