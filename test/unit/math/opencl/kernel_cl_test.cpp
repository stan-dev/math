...
TEST(MathGpu, make_kernel) {
  ...
  stan::math::opencl_kernels::fill_strict_tri(cl::NDRange(m_cl.rows(), m_cl.cols()), m_cl,
...
TEST(MathGpu, write_after_write) {
  ...
  for (int j = 0; j < 1000; j++) {
    ...
    for (int i = 0; i < 4; i++) {
      stan::math::opencl_kernels::fill_strict_tri(cl::NDRange(3, 3), m_cl, i, 3, 3,
                                       stan::math::matrix_cl_view::Entire);
    }
...
TEST(MathGpu, read_after_write_and_write_after_read) {
  ...
  for (int j = 0; j < 1000; j++) {
    stan::math::opencl_kernels::fill_strict_tri(cl::NDRange(3, 3), m_cl, j, 3, 3,
                                     stan::math::matrix_cl_view::Entire);
