...
TEST(MathMatrixCL, barebone_buffer_copy) {
  ...
  // a vector of doubles to the OpenCL device and back
  size_t size = 512;
  ...
  // create the OpenCL buffer of the same size
  cl::Buffer opencl_buffer
      = cl::Buffer(ctx, CL_MEM_READ_WRITE, sizeof(double) * size);

  // write the cpu_buffer to the OpenCL buffer
  queue.enqueueWriteBuffer(opencl_buffer, CL_TRUE, 0, sizeof(double) * size,
                           &cpu_buffer[0]);

  // write the OpenCL buffer back to the cpu_dst_buffer
  queue.enqueueReadBuffer(opencl_buffer, CL_TRUE, 0, sizeof(double) * size,
                          &cpu_dst_buffer[0]);
