...
TEST(MathMatrixopencl, matrix_cl_cache) {
  for (int i = 0; i <= 10; i++) {
    test_cache_speed();
  }
}

TEST(MathMatrixopencl, matrix_cl_primitive_creation) {
  ...
  stan::math::matrix_cl d33(d3);
}

TEST(MathMatrixopencl, matrix_cl_var_creation) {
  ...
  EXPECT_NO_THROW(stan::math::matrix_cl d11(m));
