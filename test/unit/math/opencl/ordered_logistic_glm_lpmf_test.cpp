...
TEST(ProbDistributionsOrderedLogisitcGLM, error_checking) {
  ...
  EXPECT_NO_THROW(stan::math::ordered_logistic_glm_lpmf(y_cl, x_cl, beta, cuts));

  EXPECT_THROW(stan::math::ordered_logistic_glm_lpmf(y_size_cl, x_cl, beta, cuts),
               std::invalid_argument);
  EXPECT_THROW(stan::math::ordered_logistic_glm_lpmf(y_cl, x_size1_cl, beta, cuts),
               std::invalid_argument);
  EXPECT_THROW(stan::math::ordered_logistic_glm_lpmf(y_cl, x_size2_cl, beta, cuts),
               std::invalid_argument);
  EXPECT_THROW(stan::math::ordered_logistic_glm_lpmf(y_cl, x_cl, beta_size, cuts),
               std::invalid_argument);

  EXPECT_THROW(stan::math::ordered_logistic_glm_lpmf(y_value1_cl, x_cl, beta, cuts),
               std::domain_error);
  EXPECT_THROW(stan::math::ordered_logistic_glm_lpmf(y_value2_cl, x_cl, beta, cuts),
               std::domain_error);
  EXPECT_THROW(stan::math::ordered_logistic_glm_lpmf(y_cl, x_value_cl, beta, cuts),
               std::domain_error);
  EXPECT_THROW(stan::math::ordered_logistic_glm_lpmf(y_cl, x_cl, beta_value, cuts),
               std::domain_error);
  EXPECT_THROW(stan::math::ordered_logistic_glm_lpmf(y_cl, x_cl, beta, cuts_value1),
               std::domain_error);
  EXPECT_THROW(stan::math::ordered_logistic_glm_lpmf(y_cl, x_cl, beta, cuts_value2),
               std::domain_error);
}
...
TEST(ProbDistributionsOrderedLogisitcGLM, gpu_matches_cpu_big) {
  ...
  Matrix<double, Dynamic, 1> cuts = Array<double, Dynamic, 1>::Random(C-1, 1).abs()+0.00001;
  for (int i = 1; i < C - 1; i++) {
    cuts[i] += cuts[i-1];
  }
  ...
  var res1 = stan::math::ordered_logistic_glm_lpmf<true>(y_cl, x_cl, beta_var1, cuts_var1);
  var res2
