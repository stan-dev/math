...
TEST_F(OpenCLRevTests, prob_distributions_BernoulliLccdf_error_checking) {
  int N = 3;
  ...
  EXPECT_NO_THROW(stan::math::bernoulli_lccdf(n_cl, theta_cl));

  EXPECT_THROW(stan::math::bernoulli_lccdf(n_size_cl, theta_cl),
               std::invalid_argument);
  EXPECT_THROW(stan::math::bernoulli_lccdf(n_cl, theta_size_cl),
               std::invalid_argument);

  EXPECT_THROW(stan::math::bernoulli_lccdf(n_cl, theta_value1_cl),
               std::domain_error);
  EXPECT_THROW(stan::math::bernoulli_lccdf(n_cl, theta_value2_cl),
               std::domain_error);
}

auto bernoulli_lccdf_functor = [](const auto& n, const auto& theta) {
...
TEST_F(OpenCLRevTests, prob_distributions_BernoulliLccdf_opencl_matches_cpu_small) {
  ...
  stan::math::test::compare_cpu_opencl_prim_rev(bernoulli_lccdf_functor, n,
                                                theta);
  stan::math::test::compare_cpu_opencl_prim_rev(bernoulli_lccdf_functor, n,
