...
TEST(ProbDistributionsCauchyLcdf, error_checking) {
  int N = 3;
...
auto cauchy_lccdf_functor = [](const auto& y, const auto& mu, const auto& sigma) {
  return stan::math::cauchy_lccdf(y, mu, sigma);
};

TEST(ProbDistributionsCauchyLcdf, opencl_matches_cpu_small) {
  int N = 3;
  int M = 2;
...
TEST(ProbDistributionsCauchyLcdf, opencl_matches_cpu_small_y_neg_inf) {
  int N = 3;
  int M = 2;
...
TEST(ProbDistributionsCauchyLcdf, opencl_broadcast_y) {
  int N = 3;
...
TEST(ProbDistributionsCauchyLcdf, opencl_broadcast_mu) {
  int N = 3;
  ...
  stan::math::test::test_opencl_broadcasting_prim_rev<1>(cauchy_lccdf_functor, y,
                                                         mu_scal, sigma);
  stan::math::test::test_opencl_broadcasting_prim_rev<1>(
      cauchy_lccdf_functor, y.transpose().eval(), mu_scal, sigma);
}

TEST(ProbDistributionsCauchyLcdf, opencl_broadcast_sigma) {
  int N = 3;
  ...
  stan::math::test::test_opencl_broadcasting_prim_rev<2>(cauchy_lccdf_functor, y,
                                                         mu, sigma_scal);
  stan::math::test::test_opencl_broadcasting_prim_rev<2>(
      cauchy_lccdf_functor, y.transpose().eval(), mu, sigma_scal);
}

TEST(ProbDistributionsCauchyLcdf, opencl_matches_cpu_big) {
  int N = 153;
