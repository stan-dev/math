...
TEST_F(OpenCLRevTests, prob_distributions_Cauchy_error_checking) {
  int N = 3;
  ...
  EXPECT_NO_THROW(stan::math::cauchy_lpdf(y_cl, mu_cl, sigma_cl));

  EXPECT_THROW(stan::math::cauchy_lpdf(y_size_cl, mu_cl, sigma_cl),
               std::invalid_argument);
  EXPECT_THROW(stan::math::cauchy_lpdf(y_cl, mu_size_cl, sigma_cl),
               std::invalid_argument);
  EXPECT_THROW(stan::math::cauchy_lpdf(y_cl, mu_cl, sigma_size_cl),
               std::invalid_argument);

  EXPECT_THROW(stan::math::cauchy_lpdf(y_value_cl, mu_cl, sigma_cl),
               std::domain_error);
  EXPECT_THROW(stan::math::cauchy_lpdf(y_cl, mu_value_cl, sigma_cl),
               std::domain_error);
  EXPECT_THROW(stan::math::cauchy_lpdf(y_cl, mu_cl, sigma_value_cl),
               std::domain_error);
}

auto cauchy_lpdf_functor
    = [](const auto& y, const auto& mu, const auto& sigma) {
        return stan::math::cauchy_lpdf(y, mu, sigma);
      };
auto cauchy_lpdf_functor_propto
    = [](const auto& y, const auto& mu, const auto& sigma) {
        return stan::math::cauchy_lpdf<true>(y, mu, sigma);
      };

TEST_F(OpenCLRevTests, prob_distributions_Cauchy_opencl_matches_cpu_small) {
  int N = 3;
  int M = 2;
  ...
  stan::math::test::compare_cpu_opencl_prim_rev(cauchy_lpdf_functor, y, mu,
                                                sigma);
...
TEST_F(OpenCLRevTests, prob_distributions_Cauchy_opencl_broadcast_y) {
  int N = 3;
  ...
  stan::math::test::test_opencl_broadcasting_prim_rev<0>(cauchy_lpdf_functor,
                                                         y_scal, mu, sigma);
...
TEST_F(OpenCLRevTests, prob_distributions_Cauchy_opencl_broadcast_mu) {
  int N = 3;
  ...
  stan::math::test::test_opencl_broadcasting_prim_rev<1>(cauchy_lpdf_functor, y,
                                                         mu_scal, sigma);
...
TEST_F(OpenCLRevTests, prob_distributions_Cauchy_opencl_broadcast_sigma) {
  int N = 3;
  ...
  stan::math::test::test_opencl_broadcasting_prim_rev<2>(cauchy_lpdf_functor, y,
                                                         mu, sigma_scal);
...
TEST_F(OpenCLRevTests, prob_distributions_Cauchy_opencl_matches_cpu_big) {
  int N = 153;
