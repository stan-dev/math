...
auto ub_constrain_functor = [](const auto& a, const auto& b) {
  return stan::math::ub_constrain(a, b);
};
...
TEST_F(OpenCLRevTests, UbConstrain_prim_rev_values_small) {
  Eigen::VectorXd a(8);
  ...
  stan::math::test::compare_cpu_opencl_prim_rev(ub_constrain_functor, a, b);
...
TEST_F(OpenCLRevTests, UbConstrain_prim_rev_size_0) {
  int N = 0;
...
TEST_F(OpenCLRevTests, UbConstrain_prim_rev_values_large) {
  int N = 71;
