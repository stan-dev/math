...
TEST(MathMatrixGPU, matrix_cl_var_vector_copy) {
  stan::math::vector_v d1_cpu;
  stan::math::vector_v d1_a_cpu;
  stan::math::vector_v d1_b_cpu;
  d1_cpu.resize(3);
  d1_a_cpu.resize(3);
  d1_b_cpu.resize(3);
  d1_cpu << 1, 2, 3;
...
TEST(MathMatrixCL, matrix_cl_matrix_copy) {
  stan::math::matrix_v d2_cpu;
  stan::math::matrix_v d2_a_cpu;
  stan::math::matrix_v d2_b_cpu;
  stan::math::matrix_v d0_cpu;
  d2_cpu.resize(2, 3);
  d2_a_cpu.resize(2, 3);
  d2_b_cpu.resize(2, 3);
  d2_cpu << 1, 2, 3, 4, 5, 6;
...
TEST(MathMatrixCL, matrix_cl_pack_unpack_copy_lower) {
  ...
  stan::math::matrix_v m_flat_cpu(size, size);
  ...
  packed_mat_dst
      = stan::math::packed_copy<stan::math::matrix_cl_view::Lower>(m_cl);
  for (size_t i = 0; i < packed_mat.size(); i++) {
...
TEST(MathMatrixCL, matrix_cl_pack_unpack_copy_upper) {
  ...
  packed_mat_dst
      = stan::math::packed_copy<stan::math::matrix_cl_view::Upper>(m_cl);
  for (size_t i = 0; i < packed_mat.size(); i++) {
    EXPECT_EQ(packed_mat[i], packed_mat_dst[i]);
  }
}
...
TEST(MathMatrixCL, matrix_cl_pack_unpack_copy_exception) {
  ...
  EXPECT_NO_THROW(stan::math::packed_copy<stan::math::matrix_cl_view::Upper>(
      packed_mat, 0));
  EXPECT_NO_THROW(
      stan::math::packed_copy<stan::math::matrix_cl_view::Upper>(m_cl_zero));
  EXPECT_THROW(stan::math::packed_copy<stan::math::matrix_cl_view::Upper>(
                   packed_mat, 1),
               std::invalid_argument);
}
#endif
