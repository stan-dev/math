...
TEST_F(OpenCLRevTests, _log1m_exp_prim_rev_values_small) {
  Eigen::VectorXd a(8);
  a << -0.22, -0.8, 0.5, 1, 0.15, 0.3, 0.34, -0.04;
  stan::math::test::compare_cpu_opencl_prim_rev(log1m_exp_functor, a);
}

TEST_F(OpenCLRevTests, _log1m_exp_prim_rev_size_0) {
  int N = 0;
...
TEST_F(OpenCLRevTests, _log1m_exp_prim_rev_values_large) {
  int N = 71;
