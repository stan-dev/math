...
TEST_F(OpenCLRevTests, prob_distributions_LognormalLccdf_error_checking) {
  int N = 3;
  ...
  EXPECT_NO_THROW(stan::math::lognormal_lccdf(y_cl, mu_cl, sigma_cl));

  EXPECT_THROW(stan::math::lognormal_lccdf(y_size_cl, mu_cl, sigma_cl),
               std::invalid_argument);
  EXPECT_THROW(stan::math::lognormal_lccdf(y_cl, mu_size_cl, sigma_cl),
               std::invalid_argument);
  EXPECT_THROW(stan::math::lognormal_lccdf(y_cl, mu_cl, sigma_size_cl),
               std::invalid_argument);

  EXPECT_THROW(stan::math::lognormal_lccdf(y_value_cl, mu_cl, sigma_cl),
               std::domain_error);
  EXPECT_THROW(stan::math::lognormal_lccdf(y_cl, mu_value_cl, sigma_cl),
               std::domain_error);
  EXPECT_THROW(stan::math::lognormal_lccdf(y_cl, mu_cl, sigma_value_cl),
               std::domain_error);
}
...
TEST_F(OpenCLRevTests, prob_distributions_LognormalLccdf_opencl_matches_cpu_small) {
  ...
  stan::math::test::compare_cpu_opencl_prim_rev(lognormal_lccdf_functor, y, mu,
                                                sigma);
  stan::math::test::compare_cpu_opencl_prim_rev(
      lognormal_lccdf_functor, y.transpose().eval(), mu.transpose().eval(),
      sigma.transpose().eval());
}

TEST_F(OpenCLRevTests, prob_distributions_LognormalLccdf_opencl_matches_cpu_small_y_zero) {
  int N = 3;
  int M = 2;
...
TEST_F(OpenCLRevTests, prob_distributions_LognormalLccdf_opencl_broadcast_y) {
  int N = 3;
...
TEST_F(OpenCLRevTests, prob_distributions_LognormalLccdf_opencl_broadcast_mu) {
  int N = 3;
...
TEST_F(OpenCLRevTests, prob_distributions_LognormalLccdf_opencl_broadcast_sigma) {
  int N = 3;
...
TEST_F(OpenCLRevTests, prob_distributions_LognormalLccdf_opencl_matches_cpu_big) {
  int N = 153;
