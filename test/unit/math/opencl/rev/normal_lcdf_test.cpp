...
TEST_F(OpenCLRevTests, prob_distributions_NormalLcdf_error_checking) {
  int N = 3;
  ...
  EXPECT_NO_THROW(stan::math::normal_lcdf(y_cl, mu_cl, sigma_cl));

  EXPECT_THROW(stan::math::normal_lcdf(y_size_cl, mu_cl, sigma_cl),
               std::invalid_argument);
  EXPECT_THROW(stan::math::normal_lcdf(y_cl, mu_size_cl, sigma_cl),
               std::invalid_argument);
  EXPECT_THROW(stan::math::normal_lcdf(y_cl, mu_cl, sigma_size_cl),
               std::invalid_argument);

  EXPECT_THROW(stan::math::normal_lcdf(y_value_cl, mu_cl, sigma_cl),
               std::domain_error);
  EXPECT_THROW(stan::math::normal_lcdf(y_cl, mu_value_cl, sigma_cl),
               std::domain_error);
  EXPECT_THROW(stan::math::normal_lcdf(y_cl, mu_cl, sigma_value_cl),
               std::domain_error);
}
...
TEST_F(OpenCLRevTests, prob_distributions_NormalLcdf_opencl_matches_cpu_small) {
  int N = 3;
  int M = 2;
...
TEST_F(OpenCLRevTests, prob_distributions_NormalLcdf_opencl_broadcast_y) {
  int N = 3;
...
TEST_F(OpenCLRevTests, prob_distributions_NormalLcdf_opencl_broadcast_sigma) {
  int N = 3;
...
TEST_F(OpenCLRevTests, prob_distributions_NormalLcdf_opencl_matches_cpu_big) {
  int N = 15379;
