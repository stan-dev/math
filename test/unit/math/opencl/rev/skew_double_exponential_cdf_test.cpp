...
TEST_F(OpenCLRevTests,
       prob_distributions_SkewDoubleExponentialCdf_error_checking) {
  int N = 3;
  ...
  EXPECT_NO_THROW(
      stan::math::skew_double_exponential_cdf(y_cl, mu_cl, sigma_cl, tau_cl));

  EXPECT_THROW(stan::math::skew_double_exponential_cdf(y_size_cl, mu_cl,
                                                       sigma_cl, tau_cl),
               std::invalid_argument);
  EXPECT_THROW(stan::math::skew_double_exponential_cdf(y_cl, mu_size_cl,
                                                       sigma_cl, tau_cl),
               std::invalid_argument);
  EXPECT_THROW(stan::math::skew_double_exponential_cdf(y_cl, mu_cl,
                                                       sigma_size_cl, tau_cl),
               std::invalid_argument);
  EXPECT_THROW(stan::math::skew_double_exponential_cdf(y_cl, mu_cl, sigma_cl,
                                                       tau_size_cl),
               std::invalid_argument);

  EXPECT_THROW(stan::math::skew_double_exponential_cdf(y_value_cl, mu_cl,
                                                       sigma_cl, tau_cl),
               std::domain_error);
  EXPECT_THROW(stan::math::skew_double_exponential_cdf(y_cl, mu_value_cl,
                                                       sigma_cl, tau_cl),
               std::domain_error);
  EXPECT_THROW(stan::math::skew_double_exponential_cdf(y_cl, mu_cl,
...
TEST_F(OpenCLRevTests,
       prob_distributions_SkewDoubleExponentialCdf_opencl_matches_cpu_small) {
  int N = 3;
  int M = 2;
  ...
  stan::math::test::compare_cpu_opencl_prim_rev(
      skew_double_exponential_cdf_functor, y, mu, sigma, tau);
  stan::math::test::compare_cpu_opencl_prim_rev(
...
TEST_F(
    OpenCLRevTests,
    prob_distributions_SkewDoubleExponentialCdf_opencl_matches_cpu_small_y_neg_inf) {
  int N = 3;
  int M = 2;
...
TEST_F(OpenCLRevTests,
       prob_distributions_SkewDoubleExponentialCdf_opencl_broadcast_y) {
  int N = 3;
  ...
  stan::math::test::test_opencl_broadcasting_prim_rev<0>(
      skew_double_exponential_cdf_functor, y_scal, mu, sigma, tau);
  stan::math::test::test_opencl_broadcasting_prim_rev<0>(
      skew_double_exponential_cdf_functor, y_scal, mu.transpose().eval(), sigma,
      tau.transpose().eval());
}

TEST_F(OpenCLRevTests,
       prob_distributions_SkewDoubleExponentialCdf_opencl_broadcast_mu) {
  int N = 3;
  ...
  stan::math::test::test_opencl_broadcasting_prim_rev<1>(
      skew_double_exponential_cdf_functor, y, mu_scal, sigma, tau);
  stan::math::test::test_opencl_broadcasting_prim_rev<1>(
      skew_double_exponential_cdf_functor, y.transpose().eval(), mu_scal, sigma,
      tau.transpose().eval());
}

TEST_F(OpenCLRevTests,
       prob_distributions_SkewDoubleExponentialCdf_opencl_matches_cpu_big) {
  int N = 153;
