...
TEST(MathMatrixCL, zero_m_exception_pass) {
  ...
  EXPECT_NO_THROW(m.zeros<stan::math::PartialViewCL::Entire>());
  EXPECT_NO_THROW(m.zeros<stan::math::PartialViewCL::Lower>());
  EXPECT_NO_THROW(m.zeros<stan::math::PartialViewCL::Upper>());

  stan::math::matrix_cl<double> m0;
  EXPECT_NO_THROW(m0.zeros<stan::math::PartialViewCL::Entire>());
  EXPECT_NO_THROW(m0.zeros<stan::math::PartialViewCL::Lower>());
  EXPECT_NO_THROW(m0.zeros<stan::math::PartialViewCL::Upper>());
}
...
TEST(MathMatrixCL, zero_m_value_check) {
  ...
  EXPECT_NO_THROW(m.zeros<stan::math::PartialViewCL::Entire>());
  EXPECT_NO_THROW(m_lower.zeros<stan::math::PartialViewCL::Lower>());
  EXPECT_NO_THROW(m_upper.zeros<stan::math::PartialViewCL::Upper>());
