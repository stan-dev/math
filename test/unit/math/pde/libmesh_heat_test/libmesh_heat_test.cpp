...
#include "libmesh_heat_forward_pde_femparameters.h"
#include "libmesh_heat_forward_pde_heatsystem.h"
...
public:
  ...
  {
    std::ifstream i("libmesh_heat_forward_pde_general.in");
    if (!i)
      libmesh_error_msg('[' << init.comm().rank() << "] Can't find libmesh_heat_forward_pde_general.in; exiting early.");
  }
  GetPot infile("libmesh_heat_forward_pde_general.in");
...
TEST(LibMeshheatAdjointSensitivity, forward_pde_func) {
  char arg0[] = "libmesh_heat_pde_forward";
  char arg1[] = "arg1";
  char arg2[] = "arg2";
  ...
  std::vector<double> res_prm = stan::math::forward_pde(pde, theta, x_r, x_i);
  const double val = 2.472073;
  const double adj = -5.37173;
  ASSERT_FLOAT_EQ(res_prm[0], val);

  {
    std::vector<stan::math::var> theta_v = stan::math::to_var(theta);
    std::vector<stan::math::var> res =
      stan::math::forward_pde(pde, theta_v, x_r, x_i);
    std::vector<double> g;
    stan::math::set_zero_all_adjoints();
    res[0].grad(theta_v, g);
    ASSERT_FLOAT_EQ(res[0].val(), val);
    ASSERT_FLOAT_EQ(g[0], adj);
  }

  // run MPI job again to test MPI is stil active
  {
    std::vector<stan::math::var> theta_v = stan::math::to_var(theta);
    std::vector<stan::math::var> res =
      stan::math::forward_pde(pde, theta_v, x_r, x_i);
