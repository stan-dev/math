...
TEST_F(StanMathOde, observe_states_dd) {
  ...
  stan::math::coupled_ode_observer<harm_osc_ode_fun, double, double, double,
                                   double>
      observer(harm_osc, y0, theta, t0, ts, x, x_int, &msgs, y, false);
...
TEST_F(StanMathOde, observe_states_dd_skip_first) {
  using stan::math::coupled_ode_system;
  ...
  stan::math::coupled_ode_observer<harm_osc_ode_fun, double, double, double,
                                   double>
      observer(harm_osc, y0, theta, t0, ts, x, x_int, &msgs, y, true);

  std::vector<double> ys_coupled_0(coupled_system.size(), 0.0);
  observer(ys_coupled_0, t0);
