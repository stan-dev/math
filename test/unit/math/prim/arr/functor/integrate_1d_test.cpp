 ...
 *   struct f {
 *     inline double operator()(const double& x, const std::vector<double>&
 * theta, const std::vector<double>& x_r, const std::vector<int>& x_i,
...
TEST(StanMath_integrate_1d, TestThrows) {
  ...
  // Inf limits not okay
  EXPECT_THROW(stan::math::integrate_1d(
                   f2{}, 0.0, std::numeric_limits<double>::infinity(),
                   std::vector<double>(), {}, {}, msgs, 1e-6),
               std::domain_error);
  EXPECT_THROW(
