...
TEST(StanMathOde_integrate_ode_rk45, error_conditions) {
  ...
  EXPECT_THROW_MSG(integrate_ode_rk45(harm_osc, y0_bad, t0,
                                      ts, theta, x, x_int),
                   std::invalid_argument,
                   "initial state has size 0");

  double t0_bad = 2.0;
  EXPECT_THROW_MSG(integrate_ode_rk45(harm_osc, y0, t0_bad,
                                      ts, theta, x, x_int),
                   std::domain_error,
                   "initial time is 2, but must be less than 0.1");

  std::vector<double> ts_bad;
  EXPECT_THROW_MSG(integrate_ode_rk45(harm_osc, y0, t0, ts_bad,
                                      theta, x, x_int),
                   std::invalid_argument,
                   "times has size 0");
  ...
  EXPECT_THROW_MSG(integrate_ode_rk45(harm_osc, y0, t0, ts,
                                      theta_bad, x, x_int),
                   std::out_of_range,
                   "vector");
...
TEST(StanMathOde_integrate_ode_rk45, error_conditions_nan) {
  ...
  y0_bad[0] = nan;
  EXPECT_THROW_MSG(integrate_ode_rk45(harm_osc, y0_bad, t0, ts,
                                      theta, x, x_int),
                   std::domain_error,
                   "initial state");
  EXPECT_THROW_MSG(integrate_ode_rk45(harm_osc, y0_bad, t0, ts,
  ...
  double t0_bad = nan;
  EXPECT_THROW_MSG(integrate_ode_rk45(harm_osc, y0, t0_bad, ts,
                                      theta, x, x_int),
                   std::domain_error,
                   "initial time");
  EXPECT_THROW_MSG(integrate_ode_rk45(harm_osc, y0, t0_bad, ts,
