...
TEST(MetaTraits, isConstantStruct) {
  ...
  bool temp =
    is_constant_struct<vector<double>, vector<double>, vector<double>>::value;
  EXPECT_TRUE(temp);
  temp = 
    is_constant_struct<vector<double>, vector<vector<double> >, vector<double>, vector<vector<double>>, vector<vector<vector<double>>>>::value;
  EXPECT_TRUE(temp);
}
