...
TEST(ErrorHandlingScalar_mat, CheckGreaterOrEqualMatrix) {
  const char* function = "check_greater_or_equal";
  double x;
  double low;
...
TEST(ErrorHandlingScalar_mat, CheckGreaterOrEqual_Matrix_one_indexed_message) {
  const char* function = "check_greater_or_equal";
  double x;
  double low;
  Eigen::Matrix<double, Eigen::Dynamic, 1> x_vec;
...
TEST(ErrorHandlingScalar_mat, CheckGreaterOrEqual_nan) {
  const char* function = "check_greater_or_equal";
  double nan = std::numeric_limits<double>::quiet_NaN();
