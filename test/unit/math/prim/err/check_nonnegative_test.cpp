...
TEST(ErrorHandlingScalar_arr, CheckNonnegativeVectorized) {
  int N = 5;
  const char* function = "check_nonnegative";
...
TEST(ErrorHandlingScalar_arr, CheckNonnegativeVectorized_one_indexed_message) {
  int N = 5;
  const char* function = "check_nonnegative";
  std::vector<double> x(N);
  std::string message;
...
TEST(ErrorHandlingScalar_arr, CheckNonnegative_nan) {
  const char* function = "check_nonnegative";
  double nan = std::numeric_limits<double>::quiet_NaN();
