...
TEST(ErrorHandlingMatrix, checkSymmetric_complex) {
  ...
  y << stan::math::complex<double>(1, 1), stan::math::complex<double>(2, 2),
      stan::math::complex<double>(2, 2), stan::math::complex<double>(1, 1);

  EXPECT_NO_THROW(stan::math::check_symmetric("checkSymmetric", "y", y));
  y(0, 1) = stan::math::complex<double>(2, 3);
  EXPECT_THROW(stan::math::check_symmetric("checkSymmetric", "y", y),
               std::domain_error);
}
