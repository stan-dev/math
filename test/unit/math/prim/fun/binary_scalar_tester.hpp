...
void binary_scalar_tester_impl(const F& f, const T1& x, const T2& y) {
  ...
  auto nestnestvec_scal = f(nest_nest_x, y(1));
  auto scal_nestnestvec = f(x(1), nest_nest_y);
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      for (int k = 0; k < x.size(); ++k) {
        EXPECT_FLOAT_EQ(f(nest_nest_x[i][j](k), nest_nest_y[i][j](k)),
                        nestnestvec_nestnestvec[i][j](k));
        EXPECT_FLOAT_EQ(f(nest_nest_x[i][j](k), y(1)),
                        nestnestvec_scal[i][j](k));
        EXPECT_FLOAT_EQ(f(x(1), nest_nest_y[i][j](k)),
