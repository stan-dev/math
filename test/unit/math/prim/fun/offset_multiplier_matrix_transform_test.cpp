...
TEST(ProbTransform, offset_multiplierException) {
  ...
  sigma_zero << 5.0, 0.0;
  EXPECT_THROW(offset_multiplier_constrain(x, mu, sigma_zero), std::domain_error);
  Eigen::VectorXd mu_inf(2);
  mu_inf << 2.0, std::numeric_limits<double>::infinity();
  EXPECT_THROW(offset_multiplier_constrain(x, mu_inf, sigma), std::domain_error);

  Eigen::VectorXd mu_nan(2);
  mu_nan << 2.0, NAN;

  EXPECT_THROW(offset_multiplier_constrain(x, mu_nan, sigma), std::domain_error);
  double lp = 12;
...
TEST(prob_transform, offset_multiplier_f) {
  ...
  EXPECT_MATRIX_EQ(x,
                  offset_multiplier_free(
                      offset_multiplier_constrain(x, mu, sigma), mu, sigma));
}
