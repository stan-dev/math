...
void ternary_scalar_tester_impl(const F& f, const T1& x, const T2& y, const T3& z) {
  ...
  auto nestmat_nestmat_nestmat = math::eval(f(nest_x, nest_y, nest_z));
  auto nestmat_nestmat_scal = math::eval(f(nest_x, nest_y, z(0)));
  auto nestmat_scal_scal = math::eval(f(nest_x, y(0), z(0)));
  auto nestmat_scal_nestmat = math::eval(f(nest_x, y(0), nest_z));
  auto scal_scal_nestmat = math::eval(f(x(0), y(0), nest_z));
  auto scal_nestmat_scal = math::eval(f(x(0), nest_y, z(0)));
  auto scal_nestmat_nestmat = math::eval(f(x(0), nest_y, nest_z));
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < x.size(); ++j) {
      EXPECT_FLOAT_EQ(f(nest_x[i](j), nest_y[i](j), nest_z[i](j)), nestmat_nestmat_nestmat[i](j));
      EXPECT_FLOAT_EQ(f(nest_x[i](j), nest_y[i](j), nest_z[i](0)), nestmat_nestmat_scal[i](j));
      EXPECT_FLOAT_EQ(f(nest_x[i](j), nest_y[i](0), nest_z[i](0)), nestmat_scal_scal[i](j));
      EXPECT_FLOAT_EQ(f(nest_x[i](j), nest_y[i](0), nest_z[i](j)), nestmat_scal_nestmat[i](j));
      EXPECT_FLOAT_EQ(f(nest_x[i](0), nest_y[i](0), nest_z[i](j)), scal_scal_nestmat[i](j));
      EXPECT_FLOAT_EQ(f(nest_x[i](0), nest_y[i](j), nest_z[i](0)), scal_nestmat_scal[i](j));
      EXPECT_FLOAT_EQ(f(nest_x[i](0), nest_y[i](j), nest_z[i](j)), scal_nestmat_nestmat[i](j));
    }
  }
  ...
  auto nestnestmat_nestnestmat_nestnestmat = f(nest_nest_x, nest_nest_y, nest_nest_z);
  auto nestnestmat_nestnestmat_scal = math::eval(f(nest_nest_x, nest_nest_y, z(0)));
  auto nestnestmat_scal_scal = math::eval(f(nest_nest_x, y(0), z(0)));
  auto nestnestmat_scal_nestnestmat = math::eval(f(nest_nest_x, y(0), nest_nest_z));
  auto scal_scal_nestnestmat = math::eval(f(x(0), y(0), nest_nest_z));
  auto scal_nestnestmat_scal = math::eval(f(x(0), nest_nest_y, z(0)));
  auto scal_nestnestmat_nestnestmat = math::eval(f(x(0), nest_nest_y, nest_nest_z));
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < 3; ++j) {
      for (int k = 0; k < x.size(); ++k) {
        EXPECT_FLOAT_EQ(f(nest_nest_x[i][j](k), nest_nest_y[i][j](k), nest_nest_z[i][j](k)),
  ...
  EXPECT_THROW(f(nest_nest_x_small, nest_nest_y, nest_nest_z), std::invalid_argument);
  EXPECT_THROW(f(nest_nest_x, nest_nest_y_small, nest_nest_z), std::invalid_argument);
  EXPECT_THROW(f(nest_nest_x, nest_nest_y, nest_nest_z_small), std::invalid_argument);
}
...
void ternary_scalar_tester_impl(const F& f, const T1& x, const T2& y, const T3& z) {
  ...
  for (int i = 0; i < 3; ++i) {
    for (int j = 0; j < x.size(); ++j) {
      EXPECT_FLOAT_EQ(f(nest_x[i][j], nest_y[i][j], nest_z[i][j]), nestmat_nestmat_nestmat[i][j]);
      EXPECT_FLOAT_EQ(f(nest_x[i][j], nest_y[i][j], z[0]), nestmat_nestmat_scal[i][j]);
      EXPECT_FLOAT_EQ(f(nest_x[i][j], y[0], z[0]), nestmat_scal_scal[i][j]);
      EXPECT_FLOAT_EQ(f(nest_x[i][j], y[0], nest_z[i][j]), nestmat_scal_nestmat[i][j]);
      EXPECT_FLOAT_EQ(f(x[0], y[0], nest_z[i][j]), scal_scal_nestmat[i][j]);
      EXPECT_FLOAT_EQ(f(x[0], nest_y[i][j], z[0]), scal_nestmat_scal[i][j]);
      EXPECT_FLOAT_EQ(f(x[0], nest_y[i][j], nest_z[i][j]), scal_nestmat_nestmat[i][j]);
    }
  }
  std::vector<T1> nest_x_small{x, x};
  std::vector<T2> nest_y_small{y, y};
...
void ternary_scalar_tester(const F& f, const T1& x, const T2& y, const T3& z) {
  ...
  ternary_scalar_tester_impl(f, x.transpose().eval(), y.transpose().eval(), z.transpose().eval());
 ...
 */
template <typename F, typename T1, typename T2, typename T3,
          require_all_std_vector_vt<std::is_integral, T1, T2, T3>* = nullptr>
void ternary_scalar_tester(const F& f, const T1& x, const T2& y, const T3& z) {
  ternary_scalar_tester_impl(f, x, y, z);
}
