...
TEST(MathMatrix, to_ref_matrix_views_tuple) {
  ...
  using x_ref_t = decltype(x_ref);
  using stan::test::is_same_tuple_element_v;
  static_assert(!is_same_tuple_element_v<0, x_ref_t, Eigen::MatrixXd>, "first entry should be a view of an Eigen::MatrixXd!");
  static_assert(!is_same_tuple_element_v<1, x_ref_t, Eigen::MatrixXd>, "second entry should be a view of an Eigen::MatrixXd!");
  static_assert(!is_same_tuple_element_v<2, x_ref_t, Eigen::Array<double, -1, -1, 0, -1, -1>>, "third entry should be a view of an Eigen::ArrayXd!");
}
...
TEST(MathMatrix, to_ref_matrix_views_exprs_tuple) {
  ...
  static_assert(!is_same_tuple_element_v<0, x_ref_t, Eigen::MatrixXd>, "first entry should be a view of an Eigen::MatrixXd!");
  {
    ...
    static_assert(!is_same_tuple_element_v<0, view_inner_tuple, Eigen::MatrixXd>, "tuple<1><0> entry should be a veiw of an Eigen::MatrixXd!");
    ...
    static_assert(!is_same_tuple_element_v<2, view_inner_tuple, Eigen::Array<double, -1, -1, 0, -1, -1>>, "tuple<1><2> entry should be a view of an Eigen::ArrayXd!");

  {
    ...
    static_assert(is_same_tuple_element_v<2, expr_inner_tuple, Eigen::Array<double, -1, -1, 0, -1, -1>>, "tuple<2><2> entry should be Eigen::ArrayXd!");
  }
  static_assert(is_same_tuple_element_v<3, x_ref_t, Eigen::Matrix<double, -1, 3, 0, -1, 3>>, "tuple<3> entry should be Eigen::MatrixXd!");
...
TEST(MathMatrix, to_ref_matrix_views_exprs_moves_tuple) {
  ...
  auto x_ref = stan::math::to_ref(std::forward_as_tuple(
    ...
    std::forward_as_tuple(
      a.block(0, 0, 1, 1),
      a(Eigen::placeholders::all,{Eigen::Index{2}, Eigen::Index{1}, Eigen::Index{1}}),
      a.array()),
    std::forward_as_tuple(
      a * a,
      a,
  ...
  static_assert(is_same_tuple_element_v<0, x_ref_t, Eigen::MatrixXd>, "first entry should be Eigen::MatrixXd!");
  {
    ...
    static_assert(is_same_tuple_element_v<1, view_inner_tuple, Eigen::Matrix<double, -1, 3, 0, -1, 3>>, "tuple<1><1> entry should be Eigen::MatrixXd!");
    static_assert(is_same_tuple_element_v<2, view_inner_tuple, Eigen::Array<double, -1, -1, 0, -1, -1>>, "tuple<1><2> entry should be Eigen::ArrayXd!");
  }
  {
    using expr_inner_tuple = std::tuple_element_t<2, decltype(x_ref)>;
    static_assert(is_same_tuple_element_v<0, expr_inner_tuple, Eigen::MatrixXd>, "tuple<2><0> entry should be Eigen::MatrixXd!");
