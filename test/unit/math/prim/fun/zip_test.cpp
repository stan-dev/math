...
TEST(MathFunctions, zip) {
  ...
  Eigen::VectorXd zipped = stan::math::zip(delta, loc, time);
...
TEST(MathFunctions, zip_errors) {
  ...
  EXPECT_NO_THROW(stan::math::zip(delta, rows, cols));

  // index vectors of different sizes
  std::vector<int> cols_short = {1, 2};
  EXPECT_THROW(stan::math::zip(delta, rows, cols_short), std::invalid_argument);

  // row index too large (matrix has 5 rows)
  std::vector<int> rows_big = {1, 6, 3};
  EXPECT_THROW(stan::math::zip(delta, rows_big, cols), std::out_of_range);

  // row index too small (indices are 1-based)
  std::vector<int> rows_zero = {0, 2, 3};
  EXPECT_THROW(stan::math::zip(delta, rows_zero, cols), std::out_of_range);

  // column index too large (matrix has 4 columns)
  std::vector<int> cols_big = {1, 5, 3};
  EXPECT_THROW(stan::math::zip(delta, rows, cols_big), std::out_of_range);

  // column index too small
  std::vector<int> cols_neg = {1, -1, 3};
  EXPECT_THROW(stan::math::zip(delta, rows, cols_neg), std::out_of_range);
}
