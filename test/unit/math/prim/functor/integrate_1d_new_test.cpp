...
struct f1 {
  ...
  inline T1 operator()(const T1 &x, const T1 &xc,
...
struct rank_density_functor__ {
  double operator()(double x, double xc,
                    std::ostream *pstream__,
...
void test_integration(const F &f, double a, double b,
  ...
  for (auto tolerance : tolerances) {
    ...
    auto flipped =
        [&](double x, double xc,
            std::ostream *msgs, const auto& ...args) {
...
/*TEST(StanMath_integrate_1d_new_prim, TestThrows) {
  ...
  EXPECT_THROW(stan::math::integrate_1d_new(integrate_1d_new_test::f2{}, 1.0, 0.0,
                                        std::vector<double>(), {}, {},
                                        integrate_1d_new_test::msgs, 1e-6),
               std::domain_error);
  // NaN limits not okay
  EXPECT_THROW(
  ...
  EXPECT_THROW(
      stan::math::integrate_1d_new(
          ...
          0.0, std::vector<double>(), {}, {}, integrate_1d_new_test::msgs, 1e-6),
      std::domain_error);
  EXPECT_THROW(
      stan::math::integrate_1d_new(
          integrate_1d_new_test::f2{}, std::numeric_limits<double>::quiet_NaN(),
...
TEST(StanMath_integrate_1d_new_prim, test1) {
  ...
  // Easy integrals
  /*test_integration(integrate_1d_new_test::f4{}, 0.2, 0.7, {0.5}, {}, {},
                   1.0423499493102901);
  test_integration(integrate_1d_new_test::f5{}, -0.2, 0.7, {0.4, 0.4}, {}, {},
                   1.396621954392482);
  test_integration(integrate_1d_new_test::f4{}, 0.0, 0.0, {0.5}, {}, {}, 0.0);
  test_integration(integrate_1d_new_test::f5{}, 1.0, 1.0, {0.4, 0.4}, {}, {}, 0.0);
  // Test x_i
  test_integration(integrate_1d_new_test::f6{}, -0.2, 2.9, {6.0, 5.1}, {}, {4},
                   4131.985414616364);
  // Test x_r
  test_integration(integrate_1d_new_test::f7{}, -0.2, 2.9, {}, {4.0, 6.0, 5.1}, {},
                   24219.985414616367);
  ...
  test_integration(integrate_1d_new_test::f10{}, 0.0, 1.0, {0.1, 0.1}, {}, {},
                   19.71463948905016);
  test_integration(integrate_1d_new_test::f10{}, 0.0, 1.0, {0.1, 0.5}, {}, {},
                   11.32308697521577);
  test_integration(integrate_1d_new_test::f10{}, 0.0, 1.0, {0.5, 0.1}, {}, {},
                   11.32308697521577);
  test_integration(integrate_1d_new_test::f10{}, 0.0, 1.0, {5.0, 3.0}, {}, {},
                   0.00952380952380952);
