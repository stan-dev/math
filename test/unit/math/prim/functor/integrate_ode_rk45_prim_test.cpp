...
void sho_death_test(F harm_osc, std::vector<double>& y0, double t0,
  ...
  EXPECT_DEATH(stan::math::integrate_ode_rk45<Args...>(harm_osc, y0, t0, ts, args...), "");
}
...
TEST(StanMathOde_integrate_ode_rk45, error_conditions) {
  ...
  ASSERT_NO_THROW((integrate_ode_rk45<std::vector<double>, std::vector<double>, std::vector<int>>(harm_osc, y0, t0, ts, theta, x, x_int)));

  std::vector<double> y0_bad;
  EXPECT_THROW_MSG(
		   (integrate_ode_rk45<std::vector<double>, std::vector<double>, std::vector<int>>(harm_osc, y0_bad, t0, ts, theta, x, x_int)),
      std::invalid_argument, "initial state has size 0");

  double t0_bad = 2.0;
  EXPECT_THROW_MSG(
		   (integrate_ode_rk45<std::vector<double>, std::vector<double>, std::vector<int>>(harm_osc, y0, t0_bad, ts, theta, x, x_int)),
      std::domain_error, "initial time is 2, but must be less than 0.1");

  std::vector<double> ts_bad;
  EXPECT_THROW_MSG(
		   (integrate_ode_rk45<std::vector<double>, std::vector<double>, std::vector<int>>(harm_osc, y0, t0, ts_bad, theta, x, x_int)),
  ...
  EXPECT_THROW_MSG(
		   (integrate_ode_rk45<std::vector<double>, std::vector<double>, std::vector<int>>(harm_osc, y0, t0, ts, theta_bad, x, x_int)),
      std::out_of_range, "vector");

  std::vector<double> x_bad;
  EXPECT_THROW_MSG(
		   (integrate_ode_rk45<std::vector<double>, std::vector<double>, std::vector<int>>(harm_osc, y0, t0, ts, theta, x_bad, x_int)),
      std::out_of_range, "vector");

  std::vector<int> x_int_bad;
  EXPECT_THROW_MSG(
		   (integrate_ode_rk45<std::vector<double>, std::vector<double>, std::vector<int>>(harm_osc, y0, t0, ts, theta, x, x_int_bad)),
      std::out_of_range, "vector");

  EXPECT_THROW_MSG((integrate_ode_rk45<std::vector<double>, std::vector<double>, std::vector<int>>(harm_osc, y0, t0, ts, theta, x, x_int, 0,
												   -1, 1e-6, 10)),
