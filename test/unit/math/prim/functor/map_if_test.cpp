...
TEST(MathPrim, map_if_base) {
  ...
  auto x = map_if<is_floating_point_decay>([](auto&& x) noexcept { return x + 1; }, std::forward_as_tuple(1, 2.0, 3, 4.0));
...
TEST(MathPrim, map_if_eigen) {
  ...
  auto x = map_if<stan::is_eigen>([](auto&& x) {
    return (x * x).eval();
  }, std::forward_as_tuple(a, b, a * a, std::vector<int>{1, 2, 3}));
  EXPECT_MATRIX_EQ(std::get<0>(x),(a * a).eval());
  static_assert(is_same_tuple_element_v<0, decltype(x), Eigen::MatrixXd>, "1st should be MatrixXd!");
  static_assert(!is_ref_element_v<0, decltype(x)>, "0th should be an lvalue!");
  for (int i = 0; i < b.size(); i++) {
    EXPECT_EQ(std::get<1>(x)[i], b[i]);
  }
  static_assert(is_ref_element_v<1, decltype(x)>, "1st should be an reference!");
