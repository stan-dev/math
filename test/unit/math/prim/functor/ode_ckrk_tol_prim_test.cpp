...
TEST(ode_ckrk_tol_prim, y0_errors) {
  ...
  EXPECT_NO_THROW(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0, ts,
                                           1e-10, 1e-10, 1e6, nullptr, a));

  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0inf, t0, ts,
                                        1e-10, 1e-10, 1e6, nullptr, a),
               std::domain_error);

  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0NaN, t0, ts,
                                        1e-10, 1e-10, 1e6, nullptr, a),
               std::domain_error);

  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0_empty, t0, ts,
                                        1e-10, 1e-10, 1e6, nullptr, a),
               std::invalid_argument);
}

TEST(ode_ckrk_tol_prim, t0_errors) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  ...
  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0inf, ts,
                                        1e-10, 1e-10, 1e6, nullptr, a),
               std::domain_error);

  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0NaN, ts,
                                        1e-10, 1e-10, 1e6, nullptr, a),
               std::domain_error);
}

TEST(ode_ckrk_tol_prim, ts_errors) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
  ...
  EXPECT_NO_THROW(out
                  = stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0, ts,
                                             1e-10, 1e-10, 1e6, nullptr, a));
  EXPECT_EQ(out.size(), ts.size());

  EXPECT_NO_THROW(out = stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0,
  ...
  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0, ts_empty,
                                        1e-10, 1e-10, 1e6, nullptr, a),
               std::invalid_argument);

  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0, ts_early,
                                        1e-10, 1e-10, 1e6, nullptr, a),
               std::domain_error);

  EXPECT_THROW(
      stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0, ts_decreasing,
                               1e-10, 1e-10, 1e6, nullptr, a),
      std::domain_error);

  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0, tsinf,
                                        1e-10, 1e-10, 1e6, nullptr, a),
               std::domain_error);

  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0, tsNaN,
                                        1e-10, 1e-10, 1e6, nullptr, a),
               std::domain_error);
}

TEST(ode_ckrk_tol_prim, one_arg_errors) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
  ...
  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0, ts,
...
TEST(ode_ckrk_tol_prim, two_arg_errors) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
  ...
  EXPECT_NO_THROW(stan::math::ode_ckrk_tol(stan::test::Cos2Arg(), y0, t0, ts,
                                           1e-10, 1e-10, 1e6, nullptr, a, a));

  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::Cos2Arg(), y0, t0, ts,
...
TEST(ode_ckrk_tol_prim, rtol_errors) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
...
TEST(ode_ckrk_tol_prim, atol_errors) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
  ...
  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0, ts, 1e-6,
...
TEST(ode_ckrk_tol_prim, max_num_steps_errors) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
  ...
  EXPECT_THROW(
      stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0, ts, 1e-6, 1e-6,
...
TEST(ode_ckrk_tol_prim, rhs_wrong_size_errors) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
  ...
  EXPECT_THROW(stan::math::ode_ckrk_tol(stan::test::CosArgWrongSize(), y0, t0,
                                        ts, 1e-6, 1e-6, 100, nullptr, a),
               std::invalid_argument);
}

TEST(ode_ckrk_tol_prim, error_name) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
  ...
  EXPECT_THROW_MSG(stan::math::ode_ckrk_tol(stan::test::CosArg1(), y0, t0, ts,
                   ...
                   std::domain_error, "ode_ckrk_tol");
}

TEST(ode_ckrk_tol_prim, too_much_work) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
