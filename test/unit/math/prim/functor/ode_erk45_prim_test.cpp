...
TEST(ode_erk45_prim, y0_errors) {
  ...
  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0inf, t0, ts, nullptr, a),
      std::domain_error);
...
TEST(ode_erk45_prim, t0_errors) {
  ...
  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0inf, ts, nullptr, a),
      std::domain_error);

  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0NaN, ts, nullptr, a),
      std::domain_error);
}

TEST(ode_erk45_prim, ts_errors) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
  ...
  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, tsinf, nullptr, a),
      std::domain_error);

  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, tsNaN, nullptr, a),
      std::domain_error);
}

TEST(ode_erk45_prim, one_arg_errors) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
  ...
  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, ainf),
      std::domain_error);

  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, aNaN),
      std::domain_error);

  EXPECT_NO_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, va));

  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, vainf),
      std::domain_error);

  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, vaNaN),
      std::domain_error);

  EXPECT_NO_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, ea));

  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, eainf),
      std::domain_error);

  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, eaNaN),
      std::domain_error);

  EXPECT_NO_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, vva));

  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, vvainf),
      std::domain_error);

  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, vvaNaN),
      std::domain_error);

  EXPECT_NO_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, vea));

  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, veainf),
      std::domain_error);

  EXPECT_THROW(
      stan::math::ode_erk45(stan::test::CosArg1(), y0, t0, ts, nullptr, veaNaN),
      std::domain_error);
}

TEST(ode_erk45_prim, two_arg_errors) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
...
TEST(ode_erk45_prim, rhs_wrong_size_errors) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
...
TEST(ode_erk45_prim, error_name) {
  Eigen::VectorXd y0 = Eigen::VectorXd::Zero(1);
  double t0 = 0;
  std::vector<double> ts = {0.45, 1.1};
