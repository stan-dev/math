...
TEST(ode_rk45_prim, ts_errors) {
  ...
  EXPECT_NO_THROW(out = stan::math::ode_rk45(CosArg1(), y0, t0, ts, nullptr, a));
  EXPECT_EQ(out.size(), ts.size());

  EXPECT_NO_THROW(out = stan::math::ode_rk45(CosArg1(), y0, t0, ts_repeat, nullptr, a));
  EXPECT_EQ(out.size(), ts_repeat.size());
  EXPECT_MATRIX_FLOAT_EQ(out[0], out[1]);

  EXPECT_NO_THROW(out = stan::math::ode_rk45(CosArg1(), y0, t0, ts_lots, nullptr, a));
