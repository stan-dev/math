...
TEST(MathPrimMat, vector_correct_size) {
  ...
  Eigen::Matrix<double, Eigen::Dynamic, 1> to_add1(2);
  Eigen::Matrix<double, Eigen::Dynamic, 1> to_add2(3);

  EXPECT_NO_THROW(stan::math::add_diag(mat, to_add1));
  EXPECT_THROW(stan::math::add_diag(mat, to_add2), std::invalid_argument);
}

TEST(MathPrimMat, row_vector_correct_size) {
  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> mat(2, 3);
  Eigen::Matrix<double, 1, Eigen::Dynamic> to_add1(2);
  Eigen::Matrix<double, 1, Eigen::Dynamic> to_add2(3);
