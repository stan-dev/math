...
TEST(ProbTransform, CholeskyCorrelation4) {
  ...
  for (int i = 0; i < yrt.size(); ++i) EXPECT_FLOAT_EQ(y(i), yrt(i));

  for (int m = 0; m < 4; ++m)
    for (int n = 0; n < 4; ++n) EXPECT_FLOAT_EQ(L(m, n), x(m, n));
}
...
void test_cholesky_correlation_values(
  ...
  for (int m = 0; m < K; ++m)
    for (int n = 0; n < K; ++n) EXPECT_FLOAT_EQ(L(m, n), x(m, n));
  ...
  for (int i = 0; i < yrt2.size(); ++i) EXPECT_FLOAT_EQ(y(i), yrt2(i));

  for (int m = 0; m < K; ++m)
    for (int n = 0; n < K; ++n) EXPECT_FLOAT_EQ(L(m, n), x2(m, n));
}
