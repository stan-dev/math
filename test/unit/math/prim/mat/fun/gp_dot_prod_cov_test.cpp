...
TEST(MathPrimMat, scalar_one_x_error) {
  ...
  EXPECT_THROW(cov = stan::math::gp_dot_prod_cov(x, sigma_inf),
               std::domain_error);
  EXPECT_THROW(cov = stan::math::gp_dot_prod_cov(x, sigma_not_positive),
               std::domain_error);
  EXPECT_THROW(cov = stan::math::gp_dot_prod_cov(x, sigma_negative),
               std::domain_error);
}
...
TEST(MathPrimMat, scalar_two_x_error) {
  ...
  EXPECT_THROW(cov = stan::math::gp_dot_prod_cov(x, x, sigma_inf),
               std::domain_error);
  EXPECT_THROW(cov = stan::math::gp_dot_prod_cov(x, x, sigma_not_positive),
               std::domain_error);
  EXPECT_THROW(cov = stan::math::gp_dot_prod_cov(x, x, sigma_negative),
               std::domain_error);
}
