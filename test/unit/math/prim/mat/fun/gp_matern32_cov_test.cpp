...
TEST(MathPrimMat, vec_eigen_ard_gp_matern32_cov1) {
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      EXPECT_FLOAT_EQ(sigma * sigma * (1.0 + pow(3.0, 0.5)
                               * sqrt(stan::math::squared_distance(x_new[i],
                                                               x_new[j]))) *
                      std::exp(-1.0 * pow(3.0, 0.5) *
                      sqrt(stan::math::squared_distance(x_new[i], x_new[j]))),
                      cov(i, j))
          << "index: (" << i << ", " << j << ")";
    }
  }
...
TEST(MathPrimMat, vec_eigen_eigen_ard_gp_matern32_cov1) {
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      EXPECT_FLOAT_EQ(sigma * sigma * (1.0 + pow(3.0, 0.5)
                               * sqrt(stan::math::squared_distance(x1_new[i],
                                                               x2_new[j]))) *
                      std::exp(-1.0 * pow(3.0, 0.5) *
                               sqrt(stan::math::squared_distance(x1_new[i], x2_new[j]))),
                      cov(i, j))
          << "index: (" << i << ", " << j << ")";
    }
  }
...
TEST(MathPrimMat, vec_eigen_rvec_eigen_ard_gp_matern32_cov1) {
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      EXPECT_FLOAT_EQ(sigma * sigma * (1.0 + pow(3.0, 0.5) *
                      sqrt(stan::math::squared_distance(x2_new[i],
                                                        x1_new[j]))) *
                          std::exp(-1.0 * pow(3.0, 0.5) *
            sqrt(stan::math::squared_distance(x2_new[i], x1_new[j]))),
                      cov(i, j))
          << "index: (" << i << ", " << j << ")";
    }
  }
...
TEST(MathPrimMat, vec_eigen_mixed_gp_matern32_cov2) {
  ...
  Eigen::MatrixXd cov;
  EXPECT_NO_THROW(cov = stan::math::gp_matern32_cov(x1_rvec, x2_vec, sigma, l));
...
TEST(MathPrimMat, vec_eigen_mixed_ard_gp_matern32_cov2) {
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      EXPECT_FLOAT_EQ(sigma * sigma * (1.0 + pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x1_rvec_new[i], x2_vec_new[j]))) *
                          std::exp(-1.0 * pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x1_rvec_new[i], x2_vec_new[j]))),
                      cov(i, j))
          << "index: (" << i << ", " << j << ")";
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      EXPECT_FLOAT_EQ(sigma * sigma * (1.0 + pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x2_vec_new[i], x1_rvec_new[j]))) *
        std::exp(-1.0 * pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x2_vec_new[i], x1_rvec_new[j]))),
                      cov7(i, j))
          << "index: (" << i << ", " << j << ")";
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      EXPECT_FLOAT_EQ(sigma * sigma * (1.0 + pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x1_vec_new[i], x2_rvec_new[j]))) *
        std::exp(-1.0 * pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x1_vec_new[i], x2_rvec_new[j]))),
                      cov2(i, j))
          << "index: (" << i << ", " << j << ")";
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      EXPECT_FLOAT_EQ(sigma * sigma * (1.0 + pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x2_rvec_new[i], x1_vec_new[j]))) *
        std::exp(-1.0 * pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x2_rvec_new[i], x1_vec_new[j]))),
                      cov8(i, j))
          << "index: (" << i << ", " << j << ")";
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      EXPECT_FLOAT_EQ(sigma * sigma * (1.0 + pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x2_vec_new[i], x2_rvec_new[j]))) *
                          std::exp(-1.0 * pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x2_vec_new[i], x2_rvec_new[j]))),
                      cov3(i, j))
          << "index: (" << i << ", " << j << ")";
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      EXPECT_FLOAT_EQ(sigma * sigma * (1.0 + pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x2_rvec_new[i], x2_vec_new[j]))) *
        std::exp(-1.0 * pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x2_rvec_new[i], x2_vec_new[j]))),
                      cov4(i, j))
          << "index: (" << i << ", " << j << ")";
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      EXPECT_FLOAT_EQ(sigma * sigma * (1.0 + pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x1_rvec_new[i], x1_vec_new[j]))) *
        std::exp(-1.0 * pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x1_rvec_new[i], x1_vec_new[j]))),
                      cov5(i, j))
          << "index: (" << i << ", " << j << ")";
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      EXPECT_FLOAT_EQ(sigma * sigma * (1.0 + pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x1_vec_new[i], x1_rvec_new[j]))) *
        std::exp(-1.0 * pow(3.0, 0.5) *
        sqrt(stan::math::squared_distance(x1_vec_new[i], x1_rvec_new[j]))),
                      cov6(i, j))
          << "index: (" << i << ", " << j << ")";
