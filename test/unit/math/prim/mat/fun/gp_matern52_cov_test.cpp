...
TEST(MathPrimMat, vec_double_ard_gp_matern52_cov1) {
  double sigma = 0.2;
  double temp;
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      EXPECT_FLOAT_EQ(sigma * sigma * (1 + std::pow(5.0, 0.5) *
                                       stan::math::sqrt(temp) +
                                       (5.0 / 3.0) * temp) *
                      std::exp(-1.0 * pow(5.0, 0.5) * stan::math::sqrt(temp)),
                      cov(i, j))
          << "index: (" << i << ", " << j << ")";
    }
  }
...
TEST(MathPrimMat, vec_double_double_ard_gp_matern52_cov1) {
  double sigma = 0.2;
  double temp;
...
TEST(MathPrimMat, vec_eigen_mixed_gp_matern52_cov2) {
  ...
  Eigen::MatrixXd cov;
  EXPECT_NO_THROW(cov =
                      stan::math::gp_matern52_cov(x1_rvec, x2_vec, sigma, l));
...
TEST(MathPrimMat, vec_eigen_mixed_ard_gp_matern52_cov2) {
  ...
  for (int i = 0; i < 3; i++) {
    for (int j = 0; j < 3; j++) {
      ...
      EXPECT_FLOAT_EQ(sigma * sigma * (1 + std::pow(5.0, 0.5) * stan::math::sqrt(temp) +
                                       (5.0 / 3.0) * temp) *
                      std::exp(-1.0 * pow(5.0, 0.5) * stan::math::sqrt(temp)),
                      cov4(i, j))
          << "index: (" << i << ", " << j << ")";
...
TEST(MathPrimMat, nan_error_training_sig_l_gamma) {
  ...
  EXPECT_THROW(stan::math::gp_matern52_cov(x, sigma, l_bad),
               std::domain_error);
  EXPECT_THROW(stan::math::gp_matern52_cov(x, sigma_bad, l),
               std::domain_error);
  EXPECT_THROW(stan::math::gp_matern52_cov(x, sigma_bad, l_bad),
               std::domain_error);

  EXPECT_THROW(stan::math::gp_matern52_cov(x_bad, l, sigma),
               std::domain_error);
  ...
  EXPECT_THROW(stan::math::gp_matern52_cov(x, sigma, l_vec_bad),
               std::domain_error);
  EXPECT_THROW(stan::math::gp_matern52_cov(x, sigma_bad, l_vec),
               std::domain_error);
  EXPECT_THROW(stan::math::gp_matern52_cov(x, sigma_bad, l_vec_bad),
               std::domain_error);

  EXPECT_THROW(stan::math::gp_matern52_cov(x_bad, sigma, l_vec),
               std::domain_error);
  EXPECT_THROW(stan::math::gp_matern52_cov(x_bad, sigma_bad, l_vec),
               std::domain_error);
  EXPECT_THROW(stan::math::gp_matern52_cov(x_bad, sigma_bad, l_vec_bad),
               std::domain_error);
  EXPECT_THROW(stan::math::gp_matern52_cov(x_bad, sigma, l_vec_bad),
               std::domain_error);
  EXPECT_THROW(stan::math::gp_matern52_cov(x_bad, sigma_bad, l_vec_bad),
               std::domain_error);

  EXPECT_THROW(stan::math::gp_matern52_cov(x_2, sigma, l_vec_bad),
               std::domain_error);
  ...
  EXPECT_THROW(stan::math::gp_matern52_cov(x_3, sigma, l_vec_bad),
               std::domain_error);
  EXPECT_THROW(stan::math::gp_matern52_cov(x_3, sigma_bad, l_vec),
               std::domain_error);
