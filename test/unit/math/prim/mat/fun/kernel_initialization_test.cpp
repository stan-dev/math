...
TEST(MathMatrix, kernel_initialize) {
  ...
  EXPECT_NO_THROW(cl::Kernel kernel_copy =
     stan::math::opencl_context.get_kernel("copy"));
  EXPECT_NO_THROW(cl::Kernel kernel_zeros =
     stan::math::opencl_context.get_kernel("zeros"));
  ...
  EXPECT_NO_THROW(cl::Kernel kernel_scalar_copy_triangular_transposed
                  = stan::math::opencl_context.get_kernel(
                    "copy_triangular_transposed"));
  EXPECT_NO_THROW(cl::Kernel kernel_add = stan::math::opencl_context.get_kernel(
    "add"));
