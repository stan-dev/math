...
struct map_rect : public ::testing::Test {
  ...
  virtual void SetUp() {
   ...
   x_i = std::vector<std::vector<int> >(N, std::vector<int>(0));
  }
...
TEST_F(map_rect, no_job_input_ok_dd) {
  ...
  EXPECT_NO_THROW((stan::math::map_rect<0, hard_work>(shared_params_d, job_params_d, x_r, x_i)));
}

TEST_F(map_rect, size_mismatch_job_params_dd) {
  job_params_d.pop_back();

  EXPECT_THROW((stan::math::map_rect<0, hard_work>(shared_params_d, job_params_d, x_r, x_i)), std::invalid_argument);
}
