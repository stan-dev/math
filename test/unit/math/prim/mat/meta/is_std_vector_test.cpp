...
TEST(MetaTraits, contains_vector_true) {
  ...
  bool r = is_std_vector<std::vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> > >::value;
  EXPECT_TRUE(r);
}
...
TEST(MetaTraits, contains_vector_false) {
  ...
  bool r = is_std_vector<Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> >::value;
  EXPECT_FALSE(r);
}
