...
class WeibullTestRig : public VectorRNGTestRig {
public:
  WeibullTestRig() :
  ...
  auto generate_samples(const T1& alpha, const T2& sigma, const T3&, T_rng& rng)
    const {
    return stan::math::weibull_rng(alpha, sigma, rng);
  }

  std::vector<double> generate_quantiles(double alpha, double sigma, double)
    const {
