...
void expect_binary_vector_value_error() {
  ...
  // int, T
  expect_binary_scalar_std_vector_matrix_err_throw<F, T>(int_invalid_inputs1,
                                                         b2);
  expect_binary_std_vector_matrix_scalar_err_throw<F, T>(b1,
                                                         int_invalid_inputs2);
  // double, T
  expect_binary_scalar_std_vector_matrix_err_throw<F, T>(y1, a2);
  expect_binary_std_vector_matrix_scalar_err_throw<F, T>(a1, y2);
  expect_binary_scalar_std_vector_matrix_err_throw<F, T>(invalid_inputs1, b2);
  expect_binary_std_vector_matrix_scalar_err_throw<F, T>(b1, invalid_inputs2);
  // T, T
  ...
  e2.push_back(b2);
  e2.push_back(b2);
  EXPECT_THROW(F::template apply<vector<vector_t> >(e1, d2), std::domain_error);
  EXPECT_THROW(F::template apply<vector<vector_t> >(d1, e2), std::domain_error);
  EXPECT_THROW(F::template apply<vector<vector_t> >(e1, e2), std::domain_error);
}
