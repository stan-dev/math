...
void expect_int_scalar_error() {
  ...
  for (size_t i = 0; i < int_illegal_inputs.size(); ++i) {
    ...
    EXPECT_THROW(F::apply_base(input), std::domain_error);
  }
}
...
void expect_scalar_error() {
  ...
  for (size_t i = 0; i < illegal_inputs.size(); ++i) {
    V input = illegal_inputs[i];
    EXPECT_THROW(F::apply_base(input), std::domain_error);
    EXPECT_THROW(F::template apply<V>(input), std::domain_error);
  }
}

template <typename F>
void expect_int_std_vectors_error() {
  using std::vector;
