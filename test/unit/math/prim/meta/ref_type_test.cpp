...
TEST(MathMetaPrim, forwarding_ref_type_non_eigen) {
  using stan::forwarding_ref_type_t;
  std::vector<int> a{1, 2, 3};
  std::vector<int> a2{1, 2, 3};
  forwarding_ref_type_t<std::vector<int>> a_ref1 = a;
  forwarding_ref_type_t<std::vector<int>&> a_ref2 = a;
  forwarding_ref_type_t<std::vector<int>&&> a_ref3 = std::move(a2);

  double b = 3;
  forwarding_ref_type_t<double> b_ref1 = b;
  forwarding_ref_type_t<double&> b_ref2 = b;
  forwarding_ref_type_t<double&&> b_ref3 = std::move(b);

  const std::vector<double> c{0.5, 4, 0.7};
  forwarding_ref_type_t<const std::vector<double>> c_ref1 = c;
  forwarding_ref_type_t<const std::vector<double>&> c_ref2 = c;

  EXPECT_STD_VECTOR_FLOAT_EQ(a_ref1, a);
  EXPECT_STD_VECTOR_FLOAT_EQ(a_ref2, a);
  EXPECT_STD_VECTOR_FLOAT_EQ(a_ref3, a);
  EXPECT_STD_VECTOR_FLOAT_EQ(a2, a);
  std::vector<int> a3 = std::move(a_ref3);
  EXPECT_EQ(a2.size(), 0);
  ...
  EXPECT_STD_VECTOR_FLOAT_EQ(c_ref1, c);
  EXPECT_STD_VECTOR_FLOAT_EQ(c_ref2, c);
  EXPECT_TRUE(std::is_lvalue_reference<forwarding_ref_type_t<double>>::value);
  EXPECT_TRUE(std::is_lvalue_reference<forwarding_ref_type_t<double&>>::value);
  EXPECT_TRUE(std::is_rvalue_reference<forwarding_ref_type_t<double&&>>::value);
  EXPECT_TRUE(std::is_lvalue_reference<
              forwarding_ref_type_t<const std::vector<double>>>::value);
  EXPECT_TRUE(std::is_lvalue_reference<
              forwarding_ref_type_t<const std::vector<double>&>>::value);
...
TEST(MathMetaPrim, forwarding_ref_type_eigen_directly_accessible) {
  using stan::forwarding_ref_type_t;
  Eigen::MatrixXd a(3, 3);
  a << 1, 2, 3, 4, 5, 6, 7, 8, 9;
  Eigen::MatrixXd a2 = a;
  forwarding_ref_type_t<Eigen::MatrixXd> a_ref1 = a;
  forwarding_ref_type_t<Eigen::MatrixXd&> a_ref2 = a;
  forwarding_ref_type_t<Eigen::MatrixXd&&> a_ref3 = std::move(a2);

  auto b = a.block(1, 0, 2, 2);
  auto b2 = a.block(1, 0, 2, 2);
  forwarding_ref_type_t<decltype(b)> b_ref1 = b;
  forwarding_ref_type_t<decltype(b)&> b_ref2 = b;
  forwarding_ref_type_t<decltype(b)&&> b_ref3 = std::move(b2);

  Eigen::Ref<Eigen::MatrixXd> c = a;
  Eigen::Ref<Eigen::MatrixXd> c2 = a;
  forwarding_ref_type_t<Eigen::Ref<Eigen::MatrixXd>> c_ref1 = c;
  forwarding_ref_type_t<Eigen::Ref<Eigen::MatrixXd>&> c_ref2 = c;
  forwarding_ref_type_t<Eigen::Ref<Eigen::MatrixXd>&&> c_ref3 = std::move(c2);

  EXPECT_MATRIX_EQ(a_ref1, a);
  EXPECT_MATRIX_EQ(a_ref2, a);
  EXPECT_MATRIX_EQ(a_ref3, a);
  EXPECT_MATRIX_EQ(a2, a);
  Eigen::MatrixXd a3 = std::move(a_ref3);
  EXPECT_EQ(a2.size(), 0);

  EXPECT_MATRIX_EQ(b_ref1, b);
  EXPECT_MATRIX_EQ(b_ref2, b);
  EXPECT_MATRIX_EQ(b_ref3, b);
  EXPECT_MATRIX_EQ(b2, b);
...
TEST(MathMetaPrim, forwarding_ref_type_eigen_expression) {
  ...
  auto a = m * 3;
  auto a2 = m * 3;
  forwarding_ref_type_t<decltype(a)> a_ref1 = a;
  forwarding_ref_type_t<decltype(a)&> a_ref2 = a;
  forwarding_ref_type_t<decltype(a)&&> a_ref3 = std::move(a2);
