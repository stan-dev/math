...
TEST(ProbDistributionsExpModNormal, error_check) {
  ...
  EXPECT_THROW(stan::math::exp_mod_normal_rng(10.0, -2.0, 1.0, rng),
               std::domain_error);
  EXPECT_THROW(stan::math::exp_mod_normal_rng(
                   10.0, 2, stan::math::positive_infinity(), rng),
               std::domain_error);
  EXPECT_THROW(stan::math::exp_mod_normal_rng(stan::math::positive_infinity(),
                                              2, 1.0, rng),
               std::domain_error);
}
...
TEST(ProbDistributionsExpModNormal_mat, errorCheck) {
 ...
 */
TEST(ProbDistributionsExpModNormal_mat, chiSquareGoodnessFitTest) {
  boost::random::mt19937 rng;
  int N = 10000;
