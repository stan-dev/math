...
TEST_F(hmm_marginal_lpdf_test, latent_prob) {
...
TEST_F(hmm_check_test, latent_prob_exceptions) {
  ...
  // Gamma is not square.
  EXPECT_THROW_MSG(
      hmm_latent_marginal_prob(log_omegas, Gamma_rec, rho),
  ...
  EXPECT_THROW_MSG(hmm_latent_marginal_prob(log_omegas, Gamma_bad, rho),
                   std::domain_error,
  ...
  EXPECT_THROW_MSG(
      hmm_latent_marginal_prob(log_omegas, Gamma_wrong_size, rho),
      std::invalid_argument,
  ...
  EXPECT_THROW_MSG(hmm_latent_marginal_prob(log_omegas, Gamma, rho_bad),
                   std::domain_error,
  ...
  EXPECT_THROW_MSG(
      hmm_latent_marginal_prob(log_omegas, Gamma, rho_wrong_size),
      std::invalid_argument,
