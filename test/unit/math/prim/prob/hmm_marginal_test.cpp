...
class hmm_marginal_lpdf_test : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    Eigen::VectorXd rho_(n_states);
    rho_ << p1_init, 1 - p1_init;
    rho = rho_;

    Eigen::MatrixXd Gamma_(n_states, n_states);
    Gamma_ << gamma1, 1 - gamma1, gamma2, 1 - gamma2;
    Gamma = Gamma_;
    ...
    for (int n = 0; n < n_transitions + 1; n++) {
      log_omegas_.col(n)[0] = state_lpdf(obs_data[n], abs_mu, sigma, 0);
      log_omegas_.col(n)[1] = state_lpdf(obs_data[n], abs_mu, sigma, 1);
    }
    log_omegas = log_omegas_;
    log_omegas_zero = log_omegas.block(0, 0, n_states, 1);

    std::vector<double> rho_unconstrained_(n_states - 1);
    for (int i = 0; i < rho.size() - 1; i++)
      rho_unconstrained_[i] = rho(i);
...
class hmm_check_test:public::testing::Test {
protected:
  void SetUp() override {
    n_states = 2;
    n_transitions = 2;
    ...
    rho_(0) = 0.65;
    rho_(1) = 0.35;
    Gamma_ << 0.8, 0.2, 0.6, 0.4;

    log_omegas = log_omegas_;
    rho = rho_;
    Gamma = Gamma_;

    Eigen::MatrixXd Gamma_rec_(n_states, n_states + 1);
    Gamma_rec = Gamma_rec_;

    Gamma_bad = Gamma;
    Gamma_bad(0, 0) = Gamma(0, 0) + 1;

    Eigen::MatrixXd Gamma_wrong_size_(n_states + 1, n_states + 1);
    Gamma_wrong_size = Gamma_wrong_size_;

    rho_bad = rho;
    rho_bad(0) = rho(0) + 1;

    Eigen::VectorXd rho_wrong_size_(n_states + 1);
    rho_wrong_size = rho_wrong_size_;
  }

  int n_states, n_transitions;
  Eigen::VectorXd rho, rho_bad, rho_wrong_size;
  Eigen::MatrixXd Gamma, Gamma_rec, Gamma_bad, Gamma_wrong_size, log_omegas;
};

TEST_F(hmm_check_test, lpdf_exceptions) {
  using stan::math::hmm_marginal_lpdf;
