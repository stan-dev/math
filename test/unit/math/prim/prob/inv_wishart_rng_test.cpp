...
TEST(ProbDistributionsInvWishartRng, rng) {
  ...
  MatrixXd sigma(3, 3);
  sigma << 9.0, -3.0, 2.0, -3.0, 4.0, 1.0, 2.0, 1.0, 3.0;

  MatrixXd sigma_nonsymmetric(3, 3);
  sigma_nonsymmetric << 9.0, -3.0, 0.0, -3.0, 4.0, 1.0, 2.0, 1.0, 3.0;
  ...
  EXPECT_THROW(inv_wishart_rng(3.0, sigma_nonsymmetric, rng),
               std::domain_error);
}
