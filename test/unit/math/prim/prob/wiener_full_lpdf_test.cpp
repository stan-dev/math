...
TEST(mathPrimScalProbWienerFullScal, invalid_rt) {
  ...
  EXPECT_THROW(wiener_lpdf(0, a, t0, w, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(-1, a, t0, w, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(INFTY, a, t0, w, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(-INFTY, a, t0, w, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(NAN, a, t0, w, v, sv, sw, st0),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullScal, invalid_a) {
  ...
  EXPECT_THROW(wiener_lpdf(rt, 0, t0, w, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, -1, t0, w, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, INFTY, t0, w, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, -INFTY, t0, w, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, NAN, t0, w, v, sv, sw, st0),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullScal, invalid_v) {
  ...
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, -INFTY, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, NAN, sv, sw, st0),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullScal, invalid_w) {
  ...
  EXPECT_THROW(wiener_lpdf(rt, a, t0, -0.1, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, 0, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, 1, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, 1.1, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, INFTY, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, -INFTY, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, NAN, v, sv, sw, st0),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullScal, invalid_t0) {
  ...
  EXPECT_THROW(wiener_lpdf(rt, a, -1, w, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, INFTY, w, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, -INFTY, w, v, sv, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, NAN, w, v, sv, sw, st0),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullScal, invalid_sv) {
  ...
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, v, -1, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, v, INFTY, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, v, -INFTY, sw, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, v, NAN, sw, st0),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullScal, invalid_sw) {
  ...
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, v, sv, -1, st0),
               std::domain_error);
  ...
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, v, sv, -INFTY, st0),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, v, sv, NAN, st0),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullScal, invalid_st0) {
  ...
  using stan::math::wiener_lpdf;
  double rt = 1, a = 1, v = -1, w = 0.5, t0 = 0.1, sv = 0.2, sw = 0.2;
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, v, sv, sw, -1),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, v, sv, sw, INFTY),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, v, sv, sw, -INFTY),
               std::domain_error);
  EXPECT_THROW(wiener_lpdf(rt, a, t0, w, v, sv, sw, NAN),
               std::domain_error);
}
