...
TEST(mathPrimScalProbWienerFullPrecScal, invalid_rt) {
  ...
  EXPECT_THROW(wiener_full_prec_lpdf(0, a, v, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(-1, a, v, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(INFTY, a, v, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(-INFTY, a, v, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(NAN, a, v, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullPrecScal, invalid_a) {
  ...
  EXPECT_THROW(wiener_full_prec_lpdf(rt, 0, v, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, -1, v, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, INFTY, v, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, -INFTY, v, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, NAN, v, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullPrecScal, invalid_v) {
  ...
  double rt = 1, a = 1, v = -1, w = 0.5, t0 = 0.1, sv = 0.2, sw = 0.2,
         st0 = 0.1;
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, INFTY, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, -INFTY, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, NAN, w, t0, sv, sw, st0, 1e-4),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullPrecScal, invalid_w) {
  ...
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, -0.1, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, 0, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, 1, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, 1.1, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, INFTY, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, -INFTY, t0, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, NAN, t0, sv, sw, st0, 1e-4),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullPrecScal, invalid_t0) {
  ...
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, 2, sv, sw, st0, 1e-4),
               std::domain_error);  // rt must be greater than t0
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, -1, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, INFTY, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, -INFTY, sv, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, NAN, sv, sw, st0, 1e-4),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullPrecScal, invalid_sv) {
  ...
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, t0, -1, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, t0, INFTY, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, t0, -INFTY, sw, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, t0, NAN, sw, st0, 1e-4),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullPrecScal, invalid_sw) {
  ...
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, t0, sv, -1, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, 0.8, t0, sv, 0.5, st0, 1e-4),
               std::domain_error);  // sw must be smaller than 2*(1-w)
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, 0.3, t0, sv, 0.7, st0, 1e-4),
               std::domain_error);  // sw must be smaller than 2*w
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, t0, sv, INFTY, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, t0, sv, -INFTY, st0, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, t0, sv, NAN, st0, 1e-4),
               std::domain_error);
}
...
TEST(mathPrimScalProbWienerFullPrecScal, invalid_st0) {
  ...
  using stan::math::wiener_full_prec_lpdf;
  double rt = 1, a = 1, v = -1, w = 0.5, t0 = 0.1, sv = 0.2, sw = 0.2;
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, t0, sv, sw, -1, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, t0, sv, sw, INFTY, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, t0, sv, sw, -INFTY, 1e-4),
               std::domain_error);
  EXPECT_THROW(wiener_full_prec_lpdf(rt, a, v, w, t0, sv, sw, NAN, 1e-4),
               std::domain_error);
}
