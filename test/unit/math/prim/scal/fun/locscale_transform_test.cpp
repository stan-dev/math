...
TEST(ProbTransform, locscaleException) {
  ...
  EXPECT_THROW(
      locscale_constrain(5.0, std::numeric_limits<double>::infinity(), 1.0),
      std::domain_error);
  EXPECT_THROW(locscale_constrain(5.0, NAN, 1.0), std::domain_error);
  ...
  EXPECT_THROW(locscale_free(5.0, std::numeric_limits<double>::infinity(), 1.0),
               std::domain_error);
  EXPECT_THROW(locscale_free(5.0, NAN, 1.0), std::domain_error);
  EXPECT_NO_THROW(locscale_free(5.0, 1.0, 0.01));
  double lp = 12;
  EXPECT_THROW(locscale_constrain(5.0, 1.0, 0.0, lp), std::domain_error);
  EXPECT_THROW(
      locscale_constrain(5.0, std::numeric_limits<double>::infinity(), 1.0, lp),
      std::domain_error);
  EXPECT_THROW(locscale_constrain(5.0, NAN, 1.0, lp), std::domain_error);
