...
TEST(prob_transform, lub) {
  ...
  EXPECT_FLOAT_EQ(
    stan::math::lb_constrain(1.8, 3.0),
    stan::math::lub_constrain(1.8, 3.0,
                              +std::numeric_limits<double>::infinity()));
  EXPECT_FLOAT_EQ(
    stan::math::ub_constrain(1.9, -12.5),
...
TEST(prob_transform, lub_j) {
  ...
  EXPECT_FLOAT_EQ(
    -17.0 + log(U - L) + log(stan::math::inv_logit(x))
  ...
  EXPECT_FLOAT_EQ(
    stan::math::lb_constrain(1.8, 3.0, lp2_expected),
    stan::math::lub_constrain(1.8, 3.0,
                              +std::numeric_limits<double>::infinity(), lp2));
...
TEST(prob_transform, lub_f) {
  ...
  EXPECT_FLOAT_EQ(
    ...
    stan::math::lub_free(14.2, -std::numeric_limits<double>::infinity(),
                         std::numeric_limits<double>::infinity()));
  EXPECT_FLOAT_EQ(
    stan::math::ub_free(-18.3, 7.6),
    stan::math::lub_free(-18.3, -std::numeric_limits<double>::infinity(), 7.6));
  EXPECT_FLOAT_EQ(
    stan::math::lb_free(763.9, -3122.2),
    stan::math::lub_free(763.9, -3122.2,
                         std::numeric_limits<double>::infinity()));
}
