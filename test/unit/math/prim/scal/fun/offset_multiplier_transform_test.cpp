...
TEST(prob_transform, offset_multiplier_j) {
  ...
  double U = 5.0;
  double x = -1.0;
  EXPECT_FLOAT_EQ(L + U * x, stan::math::offset_multiplier_constrain(x, L, U, lp));
...
TEST(ProbTransform, offset_multiplierException) {
  ...
  EXPECT_THROW(
      offset_multiplier_constrain(5.0, std::numeric_limits<double>::infinity(), 1.0),
      std::domain_error);
  ...
  EXPECT_THROW(offset_multiplier_free(5.0, std::numeric_limits<double>::infinity(), 1.0),
               std::domain_error);
  EXPECT_THROW(offset_multiplier_free(5.0, NAN, 1.0), std::domain_error);
  EXPECT_NO_THROW(offset_multiplier_free(5.0, 1.0, 0.01));
  double lp = 12;
  EXPECT_THROW(offset_multiplier_constrain(5.0, 1.0, 0.0, lp), std::domain_error);
  EXPECT_THROW(
      offset_multiplier_constrain(5.0, std::numeric_limits<double>::infinity(), 1.0, lp),
      std::domain_error);
  EXPECT_THROW(offset_multiplier_constrain(5.0, NAN, 1.0, lp), std::domain_error);
...
TEST(prob_transform, offset_multiplier_f) {
  ...
  EXPECT_FLOAT_EQ(y, stan::math::offset_multiplier_free(
...
TEST(prob_transform, offset_multiplier_f_exception) {
  double L = -10.0;
  double U = -27.0;
  EXPECT_THROW(stan::math::offset_multiplier_free(L - 0.01, L, U), std::domain_error);
}
