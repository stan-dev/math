...
template <typename T, typename S> void expect_type(S s) {
...
template <typename T, typename S> void expect_same_type() {
  EXPECT_TRUE((boost::is_same<S, T>::value));
}

// E is expected value of promote_scalar_type<T, S>::type
template <typename E, typename T, typename S> void expect_promote_type() {
