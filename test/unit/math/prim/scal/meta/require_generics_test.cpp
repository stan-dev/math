...
TEST(requires, not_all_test) {
  EXPECT_TRUE((require_not_all_tester<std::false_type, std::false_type>::value));
...
TEST(requires, not_any_test) {
  EXPECT_TRUE((require_not_any_tester<std::false_type, std::false_type>::value));
  EXPECT_FALSE((require_not_any_tester<std::true_type, std::true_type>::value));
  EXPECT_FALSE((require_not_any_tester<std::true_type, std::false_type>::value));
}
...
struct require_all_same_tester<T1, T2, T3, stan::require_all_same<T1, T2, T3>> : std::true_type {};
...
template <typename T1, typename T2, typename T3>
struct require_not_all_same_tester<T1, T2, T3, stan::require_not_all_same<T1, T2, T3>>
    : std::true_type {};
...
template <typename T1, typename T2, typename T3>
struct require_all_double_or_int_tester<T1, T2, T3, stan::require_all_double_or_int<T1, T2, T3>> : std::true_type {};

TEST(requires, all_double_or_int_test) {
  EXPECT_FALSE((require_all_double_or_int_tester<double, std::string, double>::value));
  EXPECT_TRUE((require_all_double_or_int_tester<double, double, double>::value));
...
template <typename T1, typename T2, typename T3>
struct require_not_all_double_or_int_tester<T1, T2, T3, stan::require_not_all_double_or_int<T1, T2, T3>>
    : std::true_type {};

TEST(requires, not_all_double_or_int_test) {
  EXPECT_TRUE((require_not_all_double_or_int_tester<double, std::string, double>::value));
  EXPECT_FALSE((require_not_all_double_or_int_tester<double, double, double>::value));
...
template <typename T1, typename T2, typename T3>
struct require_any_double_or_int_tester<T1, T2, T3, stan::require_any_double_or_int<T1, T2, T3>> : std::true_type {};

TEST(requires, any_double_or_int_test) {
  EXPECT_FALSE((require_any_double_or_int_tester<std::string, std::string, std::string>::value));
  EXPECT_TRUE((require_any_double_or_int_tester<double, std::string, double>::value));
...
template <typename T1, typename T2, typename T3>
struct require_not_any_double_or_int_tester<T1, T2, T3, stan::require_not_any_double_or_int<T1, T2, T3>>
    : std::true_type {};

TEST(requires, not_any_double_or_int_test) {
  EXPECT_TRUE((require_not_any_double_or_int_tester<std::string, std::string, std::string>::value));
  EXPECT_FALSE((require_not_any_double_or_int_tester<double, double, double>::value));
...
template <typename T1, typename T2, typename T3>
struct require_all_arithmetic_tester<T1, T2, T3, stan::require_all_arithmetic<T1, T2, T3>> : std::true_type {};

TEST(requires, all_arithmetic_test) {
  EXPECT_FALSE((require_all_arithmetic_tester<double, std::string, double>::value));
...
template <typename T1, typename T2, typename T3>
struct require_not_all_arithmetic_tester<T1, T2, T3, stan::require_not_all_arithmetic<T1, T2, T3>>
    : std::true_type {};

TEST(requires, not_all_arithmetic_test) {
  EXPECT_TRUE((require_not_all_arithmetic_tester<double, std::string, double>::value));
  EXPECT_FALSE((require_not_all_arithmetic_tester<double, double, double>::value));
...
template <typename T1, typename T2, typename T3>
struct require_any_arithmetic_tester<T1, T2, T3, stan::require_any_arithmetic<T1, T2, T3>> : std::true_type {};

TEST(requires, any_arithmetic_test) {
  EXPECT_FALSE((require_any_arithmetic_tester<std::string, std::string, std::string>::value));
  EXPECT_TRUE((require_any_arithmetic_tester<double, std::string, double>::value));
...
template <typename T1, typename T2, typename T3>
struct require_not_any_arithmetic_tester<T1, T2, T3, stan::require_not_any_arithmetic<T1, T2, T3>>
    : std::true_type {};

TEST(requires, not_any_arithmetic_test) {
  EXPECT_TRUE((require_not_any_arithmetic_tester<std::string, std::string, std::string>::value));
  EXPECT_FALSE((require_not_any_arithmetic_tester<double, double, double>::value));
...
template <typename T1>
struct require_not_var_or_arithmetic_tester<T1, stan::require_not_var_or_arithmetic<T1>>
    : std::true_type {};
...
template <typename T1, typename T2, typename T3>
struct require_all_var_or_arithmetic_tester<T1, T2, T3, stan::require_all_var_or_arithmetic<T1, T2, T3>> : std::true_type {};

TEST(requires, all_var_or_arithmetic_test) {
  EXPECT_FALSE((require_all_var_or_arithmetic_tester<double, std::string, double>::value));
  EXPECT_TRUE((require_all_var_or_arithmetic_tester<double, double, double>::value));
...
template <typename T1, typename T2, typename T3>
struct require_not_all_var_or_arithmetic_tester<T1, T2, T3, stan::require_not_all_var_or_arithmetic<T1, T2, T3>>
    : std::true_type {};

TEST(requires, not_all_var_or_arithmetic_test) {
  EXPECT_TRUE((require_not_all_var_or_arithmetic_tester<double, std::string, double>::value));
  EXPECT_FALSE((require_not_all_var_or_arithmetic_tester<double, double, double>::value));
...
template <typename T1, typename T2, typename T3>
struct require_any_var_or_arithmetic_tester<T1, T2, T3, stan::require_any_var_or_arithmetic<T1, T2, T3>> : std::true_type {};

TEST(requires, any_var_or_arithmetic_test) {
  EXPECT_FALSE((require_any_var_or_arithmetic_tester<std::string, std::string, std::string>::value));
  EXPECT_TRUE((require_any_var_or_arithmetic_tester<double, std::string, double>::value));
...
template <typename T1, typename T2, typename T3>
struct require_not_any_var_or_arithmetic_tester<T1, T2, T3, stan::require_not_any_var_or_arithmetic<T1, T2, T3>>
    : std::true_type {};

TEST(requires, not_any_var_or_arithmetic_test) {
  EXPECT_TRUE((require_not_any_var_or_arithmetic_tester<std::string, std::string, std::string>::value));
  EXPECT_FALSE((require_not_any_var_or_arithmetic_tester<double, double, double>::value));
...
struct require_vector_like_tester<TypeCheck, Check, stan::require_vector_like_t<TypeCheck, Check>>
    : std::true_type {};

TEST(requires, vector_like_test) {
  EXPECT_FALSE((require_vector_like_tester<std::is_var_or_arithmetic, double>::value));
  EXPECT_TRUE((require_vector_like_tester<std::is_arithmetic, std::vector<double>>::value));
  EXPECT_TRUE((require_vector_like_tester<std::is_var_or_arithmetic, std::vector<double>>::value));
  EXPECT_FALSE((require_vector_like_tester<std::is_var_or_arithmetic, std::vector<std::string>>::value));
}


/**
 * Require not vector_like
 */
template<template <class> class TypeCheck, class Check, typename = void>
struct require_not_vector_like_tester : std::false_type {};

template<template <class> class TypeCheck, class Check>
struct require_not_vector_like_tester<TypeCheck, Check, stan::require_not_vector_like_t<TypeCheck, Check>>
    : std::true_type {};

TEST(requires, not_vector_like_test) {
  EXPECT_TRUE((require_not_vector_like_tester<std::is_var_or_arithmetic, double>::value));
  EXPECT_FALSE((require_not_vector_like_tester<std::is_arithmetic, std::vector<double>>::value));
  EXPECT_FALSE((require_not_vector_like_tester<std::is_var_or_arithmetic, std::vector<double>>::value));
  EXPECT_TRUE((require_not_vector_like_tester<std::is_var_or_arithmetic, std::vector<std::string>>::value));
}
...
///////
 ...
 struct require_any_vector_like_tester<TypeCheck, Check1, Check2, stan::require_any_vector_like_t<TypeCheck, Check1, Check2>>
     : std::true_type {};

TEST(requires, any_vector_like_test) {
  EXPECT_TRUE((require_any_vector_like_tester<std::is_var_or_arithmetic, std::vector<double>, std::vector<double>>::value));
  EXPECT_TRUE((require_any_vector_like_tester<std::is_var_or_arithmetic, double, std::vector<double>>::value));
  EXPECT_TRUE((require_any_vector_like_tester<std::is_var_or_arithmetic, std::vector<double>, double>::value));
  EXPECT_TRUE((require_any_vector_like_tester<std::is_var_or_arithmetic, std::vector<std::string>, std::vector<double>>::value));
  EXPECT_FALSE((require_any_vector_like_tester<std::is_var_or_arithmetic, int, std::string>::value));
  EXPECT_FALSE((require_any_vector_like_tester<std::is_arithmetic, double, double>::value));
}
...
template<template <class> class TypeCheck, class Check1, class Check2>
struct require_not_any_vector_like_tester<TypeCheck, Check1, Check2, stan::require_not_any_vector_like_t<TypeCheck, Check1, Check2>>
    : std::true_type {};

TEST(requires, not_any_vector_like_test) {
  EXPECT_FALSE((require_not_any_vector_like_tester<std::is_var_or_arithmetic, std::vector<double>, std::vector<double>>::value));
  EXPECT_FALSE((require_not_any_vector_like_tester<std::is_var_or_arithmetic, double, std::vector<double>>::value));
  EXPECT_FALSE((require_not_any_vector_like_tester<std::is_var_or_arithmetic, std::vector<double>, double>::value));
  EXPECT_FALSE((require_not_any_vector_like_tester<std::is_var_or_arithmetic, std::vector<std::string>, std::vector<double>>::value));
  EXPECT_TRUE((require_not_any_vector_like_tester<std::is_var_or_arithmetic, int, std::string>::value));
  EXPECT_TRUE((require_not_any_vector_like_tester<std::is_arithmetic, double, double>::value));
}
...
template<template <class> class TypeCheck, class Check1, class Check2>
struct require_all_vector_like_tester<TypeCheck, Check1, Check2, stan::require_all_vector_like_t<TypeCheck, Check1, Check2>>
    : std::true_type {};

TEST(requires, all_vector_like_test) {
  EXPECT_TRUE((require_all_vector_like_tester<std::is_var_or_arithmetic, std::vector<double>, std::vector<double>>::value));
  EXPECT_FALSE((require_all_vector_like_tester<std::is_var_or_arithmetic, double, std::vector<double>>::value));
  EXPECT_FALSE((require_all_vector_like_tester<std::is_var_or_arithmetic, std::vector<double>, double>::value));
  EXPECT_FALSE((require_all_vector_like_tester<std::is_var_or_arithmetic, std::vector<std::string>, std::vector<double>>::value));
  EXPECT_FALSE((require_all_vector_like_tester<std::is_var_or_arithmetic, int, std::string>::value));
  EXPECT_FALSE((require_all_vector_like_tester<std::is_arithmetic, double, double>::value));
}

/**
 * Require all not vector_like
 */
 template<template <class> class TypeCheck, class Check1, class Check2, typename = void>
 struct require_not_all_vector_like_tester : std::false_type {};

 template<template <class> class TypeCheck, class Check1, class Check2>
 struct require_not_all_vector_like_tester<TypeCheck, Check1, Check2, stan::require_not_all_vector_like_t<TypeCheck, Check1, Check2>>
     : std::true_type {};

TEST(requires, not_all_vector_like_test) {
  EXPECT_FALSE((require_not_all_vector_like_tester<std::is_var_or_arithmetic, std::vector<double>, std::vector<double>>::value));
  EXPECT_TRUE((require_not_all_vector_like_tester<std::is_var_or_arithmetic, double, std::vector<double>>::value));
  EXPECT_TRUE((require_not_all_vector_like_tester<std::is_var_or_arithmetic, std::vector<double>, double>::value));
  EXPECT_TRUE((require_not_all_vector_like_tester<std::is_var_or_arithmetic, std::vector<std::string>, std::vector<double>>::value));
  EXPECT_TRUE((require_not_all_vector_like_tester<std::is_var_or_arithmetic, int, std::string>::value));
  EXPECT_TRUE((require_not_all_vector_like_tester<std::is_arithmetic, double, double>::value));
}
