...
TEST(ProbNegBinomial2Log, log_matches_lpmf) {
  ...
  EXPECT_FLOAT_EQ(
      (stan::math::neg_binomial_2_log_lpmf<true>(y, eta,
                                                                         phi)),
      (stan::math::neg_binomial_2_log_log<true>(y, eta,
                                                                        phi)));
  EXPECT_FLOAT_EQ(
      (stan::math::neg_binomial_2_log_lpmf<false>(
          y, eta, phi)),
      (stan::math::neg_binomial_2_log_log<false>(y, eta,
                                                                         phi)));
  EXPECT_FLOAT_EQ((stan::math::neg_binomial_2_log_lpmf(
                      y, eta, phi)),
                  (stan::math::neg_binomial_2_log_log(
                      y, eta, phi)));
}
