  ...
  namespace math {
    namespace test {
      ...
      void test_slices(T1 result, T2&& vec_value, Args&&... args) {
        ...
        EXPECT_EQ(result, stan::math::reduce_sum_static<sum_lpdf>(data, 1, get_new_msg(), args...)) << "Failed for reduce_sum_static";
        EXPECT_EQ(result, stan::math::reduce_sum<sum_lpdf>(data, 1, get_new_msg(), args...))<< "Failed for reduce_sum";
      }
      ...
      auto reduce_sum_static_sum_lpdf = [](auto&& data, auto&&... args) {
        return stan::math::reduce_sum_static<sum_lpdf>(data, 1, get_new_msg(), args...);
      };
