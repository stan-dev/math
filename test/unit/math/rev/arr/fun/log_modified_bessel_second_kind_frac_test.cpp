...
// std::array<double, 13> z_to_test = {1.48e-7, 3.6e-6, 7.248e-5, 4.32e-4,
//   8.7e-3, 0.04523, 0.17532, 1, 3, 11.32465, 105.6, 1038.4, 105321.6}; 
std::array<double, 9> z_to_test = {
...
TEST(AgradRev, log_modified_bessel_second_kind_frac_recurrence) {    
  ...
  if(output_debug_csv) {
    ...
    *debug_output << "v,z,method,ratio,grad_z,grad_v" << std::endl;
  }
    ...
    for(auto z_iter = z_to_test.begin(); z_iter != z_to_test.end(); ++z_iter) {
      for(int sign = -1; sign <= 1; sign += 2) {
        ...
        try {
          ...
          AVAR log_K_vm1 = log_modified_bessel_second_kind_frac(v_var - 1, z_var);
          AVAR log_K_vm2 = log_modified_bessel_second_kind_frac(v_var - 2, z_var);

          AVAR log_K_vp1 = log_modified_bessel_second_kind_frac(v_var + 1, z_var);
          AVAR log_K_vp2 = log_modified_bessel_second_kind_frac(v_var + 2, z_var);
...
TEST(AgradRev, log_modified_bessel_second_kind_frac_input) {  
  ...
  EXPECT_THROW(log_modified_bessel_second_kind_frac(1.0,-1.0), std::domain_error);
  EXPECT_THROW(log_modified_bessel_second_kind_frac(std::numeric_limits<double>::infinity(),1), std::domain_error);
  EXPECT_THROW(log_modified_bessel_second_kind_frac(1.0,std::numeric_limits<double>::infinity()), std::domain_error);

  std::for_each(v_to_test.begin(), v_to_test.end(), [](double v) {
    EXPECT_TRUE(std::isinf(log_modified_bessel_second_kind_frac(v,0))) << "Infinity for z = 0";
  });
}
