...
std::array<double, 25> v_to_test = {0, 3.15e-7, 2.62e-6, 9.2e-5, 0.0026, 0.0843, 
  0.17345, 1, 1.63, 7.42, 42.42424, 86.5, 113.8, 148.7565, 180.6, 246.3, 300.5, 
  513.6, 712.456, 714.456, 1235.6,8656, 15330.75, 37634.2, 85323
  };

std::array<double, 16> z_to_test = {1.48e-7, 3.6e-6, 7.248e-5, 4.32e-4,
   8.7e-3, 0.04523, 0.17532, 1, 3, 11.32465, 105.6, 1038.4, 4236, 11457.6,
   62384, 105321.6}; 
...
TEST(AgradRev, log_modified_bessel_second_kind_frac_recurrence) {    
  ...
  if(output_debug_csv) {
    ...
    *debug_output << "v,z,method,ratio,grad_z,grad_v,value_m2,value_m1,"
      "value,value_p1,value_p2" << std::endl;
  }
        ...
        try {
          ...
          if(fabs(v - 1) < 1e-4) {
            right_hand = 
              log_diff_exp(
                log_K_vp2,  
                LOG_2 + log(v_var + 1) - log(z_var) + log_K_vp1
              );
          } else if(fabs(v + 1) < 1e-4) {
            right_hand = 
              log_diff_exp(
                log_K_vm2,  
          ...
          if(debug_output != 0) {
            *debug_output << v << "," << z << "," << 
              ...
              << ratio.val() << "," << g[0] << "," << g[1] << "," 
              << log_K_vm2 << "," << log_K_vm1 << "," << left_hand << "," 
              << log_K_vp1 << "," << log_K_vp2 << std::endl;
          }
          recover_memory();
        } catch (...) {
          std::cout << "\nAt v = " << v << ", z = " << z << " method = " << 
...
TEST(AgradRev, log_modified_bessel_second_kind_frac_input) {  
  ...
  EXPECT_THROW(log_modified_bessel_second_kind_frac(
    std::numeric_limits<double>::infinity(),1), std::domain_error);
  EXPECT_THROW(log_modified_bessel_second_kind_frac(
    1.0,std::numeric_limits<double>::infinity()), std::domain_error);
...
std::array<TestValue, 48> testValues = {
  ...
  TestValue(3.e-8,4.e-8,2.842016709795352,2.965102177697192015626573541625171847363e-6,-1.4576989270969522e6),

  TestValue(3.e-8,2.56e-6,2.564290279944925,1.7334431640179288e-6,-30067.88746129086),

  TestValue(3.e-8,1,-0.8650643989067878,2.19330054363531404298968678241041735235e-8,-1.429625398260402),

  TestValue(3.e-8,1.35,-1.345899369014985,1.7261838025937896e-8,-1.3274527125104543),

  TestValue(3.e-8,5.42,-6.060513933302737,5.1040390297908136e-9,-1.0886145077261176),

  TestValue(3.e-8,15.61,-16.76593131015518,1.862584828000698e-9,-1.031547715742656),

  TestValue(8.419999999999999e-6,4.e-8,2.8420167133274603,0.0008389671519545296,-1.4576989372317046e6),

  TestValue(8.419999999999999e-6,2.56e-6,2.5642902819953073,0.00048703094653465367,-30067.887581282954),

  TestValue(8.419999999999999e-6,1,-0.8650643988808719,6.155863540697342e-6,-1.4296253982806608),

  TestValue(8.419999999999999e-6,1.35,-1.3458993689945886,4.8448467223866964e-6,-1.327452712522753),

  TestValue(8.419999999999999e-6,5.42,-6.060513933296705,1.4325944074403853e-6,-1.0886145077271503),

  TestValue(8.419999999999999e-6,15.61,-16.76593131015298,5.232112423113283e-7,-1.0315477157427932),

  TestValue(-6.1,4.e-8,112.40381226433932,-19.451621523830077,-1.5249999999999988e8),

  TestValue(-6.1,2.56e-6,87.034625455845,-15.29273844045875,-2.3828125000002505e6),

  TestValue(-6.1,1,8.44532282091819,-2.4266671542376326,-6.196902250273741),

  TestValue(-6.1,1.35,6.57501751331719,-2.134067641376717,-4.648139310106594),

  TestValue(-6.1,5.42,-3.0846894347387646,-0.9237850302846817,-1.5483090878535395),

  TestValue(-6.1,15.61,-15.622087849008592,-0.3711535955458681,-1.1012821947806557),

  TestValue(2.3,4.e-8,40.234369470202246,18.327573443762898,-5.750000000000001e7),

  TestValue(2.3,2.56e-6,30.668938378473744,14.168690360397694,-898437.5000009845),

  TestValue(2.3,1,0.8839980660421263,1.3959577532638583,-2.6154838303420496),

  TestValue(2.3,1.35,0.06987248240261751,1.1546607398609916,-2.0935960889379617),

  TestValue(2.3,5.42,-5.61499495879317,0.3836097936394976,-1.1635305112552852),

  TestValue(2.3,15.61,-16.6018291360965,0.14247666444981644,-1.041724687228245),

  TestValue(5.68,4.e-8,104.25024571265277,19.373881532593177,-1.42e8),

  TestValue(5.68,2.56e-6,80.62778979916949,15.21499844922185,-2.2187500000002733e6),

  TestValue(5.68,1,7.441890184577832,2.3506506024792277,-5.785344508161015),

  TestValue(5.68,1.35,5.694211980821007,2.0593627203369618,-4.348069083335933),

  TestValue(5.68,5.42,-3.4617329311973237,0.8713616639684476,-1.4945426408404363),

  TestValue(5.68,15.61,-15.772808643097397,0.34653215069805043,-1.0922484982038956),

  TestValue(-5.42,4.e-8,99.21965255139615,-19.322551217399283,-1.3549999999999988e8),

  TestValue(-5.42,2.56e-6,76.67850623958635,-15.16366813402795,-2.1171875000002896e6),

  TestValue(-5.42,1,6.837171294885847,-2.300631355301549,-5.531331879830391),

  TestValue(-5.42,1.35,5.165103157433492,-2.0103284856142833,-4.163267880991594),

  TestValue(-5.42,5.42,-3.68396561124711,-0.838004123015669,-1.4623466919649633),

  TestValue(-5.42,15.61,-15.8609147084551,-0.331194933440333,-1.086946750262522),

  TestValue(14.1,4.e-8,272.0779010168817,20.33782841756488,-3.525e8),

  TestValue(14.1,2.56e-6,213.4376495415102,16.178945334179055,-5.507812500000098e6),

  TestValue(14.1,1,31.87398406459845,3.304895298656512,-14.138107946970608),

  TestValue(14.1,1.35,27.626847510527487,3.0059807652505035,-10.49582397274031),

  TestValue(14.1,5.42,7.514275718863735,1.6533926996855581,-2.7996159235447755),

  TestValue(14.1,15.61,-10.894914609874995,0.7957534956863926,-1.36543716043966),

  TestValue(31.4,4.e-8,631.9793629723308,21.158333378062316,-7.85e8),

  TestValue(31.4,2.56e-6,501.3904341548371,16.999450294647104,-1.2265625000000043e7),

  TestValue(31.4,1,97.0914080168896,4.1242173963614,-31.41644277047241),

  TestValue(31.4,1.35,87.66136250153109,3.824335037153727,-23.281451899330236),

  TestValue(31.4,5.42,43.790021183801784,2.4417007517315192,-5.881782356655926),
...
TEST(AgradRev, log_modified_bessel_second_kind_frac_double_double) {
  std::for_each(
      testValues.begin(), testValues.end(), [](TestValue test) {
        double f1
            = log_modified_bessel_second_kind_frac(test.v, test.z);
        EXPECT_REL_ERROR(test.value, f1) << "ratio at v = " 
          << test.v << ", z = " << test.z;
      });
}
  ...
  std::for_each(
      testValues.begin(), testValues.end(), [](TestValue test) {
        ...
        try {
          ...
          EXPECT_REL_ERROR(test.value, f.val()) << "ratio at v = " << 
            test.v << ", z = " << test.z;

          AVEC x = createAVEC(z);
          VEC g;
          f.grad(x, g);
          EXPECT_REL_ERROR(test.grad_z, g[0]) << "grad_z at v = " << 
            test.v << ", z = " << test.z;;
  ...
  std::for_each(
      testValues.begin(), testValues.end(), [](TestValue test) {
        try {
          ...
          VEC g;
          f.grad(x, g);
          EXPECT_REL_ERROR(test.grad_v, g[0]) << "grad_v at v = " 
            << test.v << ", z = " << test.z;
        } catch (const std::domain_error& err) {
          std::cout << "\nAt v = " << test.v << ", z = " << test.z << ":\n";
          throw;
  ...
  std::for_each(
      testValues.begin(), testValues.end(), [](TestValue test) {
        try {
          ...
          EXPECT_REL_ERROR(test.grad_v, g[0]) << "grad_v at v = " 
            << test.v << ", z = " << test.z;
          EXPECT_REL_ERROR(test.grad_z, g[1]) << "grad_z at v = " 
            << test.v << ", z = " << test.z;
        } catch (const std::domain_error& err) {
          std::cout << "\nAt v = " << test.v << ", z = " << test.z << ":\n";
          throw;
