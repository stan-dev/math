...
TEST_F(StanAgradRevOde, decouple_states_dv) {
  ...
  for (size_t t = 0; t < T; t++) {
    ...
    for (size_t n = 0; n < coupled_system.size(); n++) coupled_state[n] = ++k;
...
TEST_F(StanAgradRevOde, initial_state_dv) {
  ...
  for (size_t n = 0; n < N; n++) y0_d[n] = n + 1;
  for (size_t m = 0; m < M; m++) theta_v[m] = 10 * (m + 1);
  ...
  for (size_t n = N; n < state.size(); n++) EXPECT_FLOAT_EQ(0.0, state[n]);
}
...
TEST_F(StanAgradRevOde, memory_recovery_exception_dv) {
  ...
  for (size_t n = 0; n < N + 1; n++) {
    ...
    EXPECT_TRUE(stan::math::empty_nested());
    EXPECT_THROW_MSG(coupled_system_dv(y, dy_dt, t), std::logic_error, message);
    EXPECT_TRUE(stan::math::empty_nested());
  }
}
...
TEST_F(StanAgradRevOde, initial_state_vd) {
  ...
  for (size_t n = 0; n < N; n++) y0_v[n] = n + 1;
  for (size_t m = 0; m < M; m++) theta_d[m] = 10 * (m + 1);
...
TEST_F(StanAgradRevOde, memory_recovery_exception_vd) {
  ...
  for (size_t n = 0; n < N + 1; n++) {
    ...
    EXPECT_TRUE(stan::math::empty_nested());
    EXPECT_THROW_MSG(coupled_system_vd(y, dy_dt, t), std::logic_error, message);
    EXPECT_TRUE(stan::math::empty_nested());
  }
}
...
TEST_F(StanAgradRevOde, memory_recovery_exception_vv) {
  ...
  for (size_t n = 0; n < N + 1; n++) {
    ...
    EXPECT_TRUE(stan::math::empty_nested());
    EXPECT_THROW_MSG(coupled_system_vv(y, dy_dt, t), std::logic_error, message);
    EXPECT_TRUE(stan::math::empty_nested());
  }
}
