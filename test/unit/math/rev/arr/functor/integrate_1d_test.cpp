...
void test_derivatives(const F& f, double a, double b, std::vector<double> thetas, const std::vector<double>& x_r, const std::vector<int>& x_i, double val, std::vector<double> grad) {
  ...
  for(auto tolerance : tolerances) {
    ...
    EXPECT_FLOAT_EQ(val, integral.val());
    for(size_t i = 0; i < grad.size(); ++i) {
      EXPECT_FLOAT_EQ(grad[i], computed_grad[i]);
    }
  }
}
...
TEST(StanMath_integrate_1d, TestDerivatives) {
  ...
  test_derivatives(f2{}, 0.0, 1.0, {0.5}, {}, {}, 1.563483370741325, {1.257894305390898}) ;
  ...
  test_derivatives(f4{}, 0.0, std::numeric_limits<double>::infinity(), {}, {}, {}, 1.772453850905516, {});
  // Tricky integral from Boost docs + limit at infinity with gradients
  test_derivatives(f5{}, 0.0, std::numeric_limits<double>::infinity(), {0.5, 3.0}, {}, {},
  ...
  test_derivatives(f7{}, -5.0, std::numeric_limits<double>::infinity(), {1.5}, {}, {}, 1205.361609637375, {5223.23364176196});
  // Zero crossing integral + limit at negative infinity
  test_derivatives(f8{}, -std::numeric_limits<double>::infinity(), 5.0, {1.5}, {}, {}, 1205.361609637375, {5223.23364176196});
  // Both limits at infinity + test x_r/x_i + no gradients
  test_derivatives(f9{}, -std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity(), {}, {5.0, 1.7}, {2}, 3.013171546539377, {});
  // Both limits at infinity + test x_r/x_i + no gradients
  test_derivatives(f10{}, -std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity(), {}, {1.7}, {4}, 2.536571480364399, {});
}
