...
TEST_F(AgradRev, count_vars_int_arg) {
  int arg = 5;

  EXPECT_EQ(0, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_double_arg) {
  double arg = 5.0;

  EXPECT_EQ(0, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_std_vector_int_arg) {
  std::vector<int> arg(5, 10);

  EXPECT_EQ(0, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_std_vector_double_arg) {
  std::vector<double> arg(5, 10.0);

  EXPECT_EQ(0, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_eigen_vector_arg) {
  Eigen::VectorXd arg = Eigen::VectorXd::Ones(5);

  EXPECT_EQ(0, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_eigen_row_vector_arg) {
  Eigen::RowVectorXd arg = Eigen::RowVectorXd::Ones(5);

  EXPECT_EQ(0, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_eigen_matrix_arg) {
  Eigen::MatrixXd arg = Eigen::MatrixXd::Ones(5, 5);

  EXPECT_EQ(0, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_std_vector_std_vector_double_arg) {
  std::vector<std::vector<double>> arg(5, std::vector<double>(5, 10.0));

  EXPECT_EQ(0, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_std_vector_eigen_vector_arg) {
  std::vector<Eigen::VectorXd> arg(2, Eigen::VectorXd::Ones(5));

  EXPECT_EQ(0, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_std_vector_eigen_row_vector_arg) {
  std::vector<Eigen::RowVectorXd> arg(2, Eigen::VectorXd::Ones(5));

  EXPECT_EQ(0, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_std_vector_eigen_matrix_arg) {
  std::vector<Eigen::MatrixXd> arg(2, Eigen::MatrixXd::Ones(5, 3));

  EXPECT_EQ(0, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_var_arg) {
  var arg = 5.0;

  EXPECT_EQ(1, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_std_vector_var_arg) {
  std::vector<var> arg(5);

  EXPECT_EQ(5, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_eigen_vector_var_arg) {
  Eigen::Matrix<var, Eigen::Dynamic, 1> arg(5);

  EXPECT_EQ(5, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_eigen_row_vector_var_arg) {
  Eigen::Matrix<var, 1, Eigen::Dynamic> arg(5);

  EXPECT_EQ(5, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_eigen_matrix_var_arg) {
  Eigen::Matrix<var, Eigen::Dynamic, Eigen::Dynamic> arg(5, 5);

  EXPECT_EQ(25, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_std_vector_std_vector_var_arg) {
  std::vector<std::vector<var>> arg(5, std::vector<var>(5));

  EXPECT_EQ(25, stan::math::count_vars(arg));
}

TEST_F(AgradRev, count_vars_std_vector_eigen_vector_var_arg) {
...
TEST_F(AgradRev, count_vars_std_vector_eigen_row_vector_var_arg) {
...
TEST_F(AgradRev, count_vars_std_vector_eigen_matrix_var_arg) {
...
TEST_F(AgradRev, count_vars, zero_args) { EXPECT_EQ(0_stan::math::count_vars()); }

TEST_F(AgradRev, count_vars_sum) {
  int arg1 = 1;
  double arg2 = 1.0;
