...
TEST(MathFunctions, log_sum_exp_identities_rev) {
  ...
  auto rh_multiply
      = [](const var& x, const var& y) { return 1e5 + log_sum_exp(x, y); };
  auto lh_orig = [](const var& x, const var& y) { return log_sum_exp(x, y); };

  auto rh_distribute
      = [](const var& x, const var& y) { return x + log_sum_exp(0, y - x); };
