...
TEST(AgradRevMatrix, multiply_lower_tri_self_transpose_test_large) {
  for(auto N : { 1, 2, 3, 5, 10 }) {
    Eigen::MatrixXd A = Eigen::MatrixXd::Random(N, N).template triangularView<Eigen::Lower>();
    ...
    Eigen::MatrixXd adjoint = (2 * Eigen::MatrixXd::Ones(N, N) * A).template triangularView<Eigen::Lower>();
