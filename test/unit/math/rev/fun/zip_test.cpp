...
TEST_F(AgradRev, zip_values_and_gradients) {
  ...
  const auto y = stan::math::zip(x, rows, cols);
...
TEST_F(AgradRev, zip_index_lifetime) {
  ...
  auto y = [&x]() {
    ...
    auto result = stan::math::zip(x, std::move(rows), std::move(cols));
    rows.assign(3, 2);
    cols.assign(3, 2);
    return result;
  }();
  auto z = stan::math::zip(x, std::vector<int>{2}, std::vector<int>{2});
...
TEST_F(AgradRev, zip_matrix_view) {
  Eigen::MatrixXd values(3, 3);
  values << 11, 12, 13, 21, 22, 23, 31, 32, 33;
  stan::math::var_value<Eigen::MatrixXd> x = values;
  auto y = stan::math::zip(x.block(1, 1, 2, 2), std::vector<int>{2, 1, 2},
...
TEST_F(AgradRev, zip_errors) {
  stan::math::var_value<Eigen::MatrixXd> x = Eigen::MatrixXd::Zero(3, 2);
  const std::vector<int> valid{1, 2};
  const std::vector<int> empty;
  EXPECT_THROW(stan::math::zip(x, valid, empty), std::invalid_argument);
  EXPECT_THROW(stan::math::zip(x, empty, valid), std::invalid_argument);
  for (int row : {-1, 0, 4}) {
    EXPECT_THROW(stan::math::zip(x, std::vector<int>{1, row}, valid),
                 std::out_of_range);
  }
  for (int col : {-1, 0, 3}) {
    EXPECT_THROW(stan::math::zip(x, valid, std::vector<int>{1, col}),
                 std::out_of_range);
  }
  for (int rows : {0, 2}) {
    for (int cols : {0, 3}) {
      ...
      EXPECT_THROW(stan::math::zip(empty_x, valid, valid), std::out_of_range);
    }
  }
}
