...
TEST(AgradRev, setup_adj_arg_types_fill) {
  ...
  arg_eig_mat arg_eig_mat_d
      = setup_adj_arg<eig_mat>(eig_mat_v.rows(), eig_mat_v.cols());
  ...
  arg_std_vec_d
      = adj_arg_t<std_vec>(value_of(std_vec_v).data(), std_vec_v.size());
  for (int i = 0; i < 10; i++) {
    EXPECT_FLOAT_EQ(arg_std_vec_d(i), std_vec_v[i].val());
  }

  eig_row_vec eig_row_vec_v = eig_row_vec::Random(10);
  arg_eig_row_vec arg_eig_row_vec_d
      = setup_adj_arg<eig_row_vec>(eig_row_vec_v.size());
...
TEST(AgradRev, setup_adj_arg_types_fill_override) {
  ...
  eig_mat eig_mat_v = eig_mat::Random(10, 10);
  arg_eig_mat arg_eig_mat_d
      = setup_adj_arg<eig_mat, true>(eig_mat_v.rows(), eig_mat_v.cols());
  arg_eig_mat_d = value_of(eig_mat_v);
  EXPECT_MATRIX_EQ(arg_eig_mat_d, value_of(eig_mat_v));
  ...
  eig_row_vec eig_row_vec_v = eig_row_vec::Random(10);
  arg_eig_row_vec arg_eig_row_vec_d
      = setup_adj_arg<eig_row_vec, true>(eig_row_vec_v.size());
  arg_eig_row_vec_d = value_of(eig_row_vec_v);
  EXPECT_MATRIX_EQ(arg_eig_row_vec_d, value_of(eig_row_vec_v));
}
...
TEST(AgradRev, setup_adj_arg_types_empty) {
  ...
  eig_mat eig_mat_v = eig_mat::Random(10, 10);
  arg_eig_mat arg_eig_mat_d
      = setup_adj_arg<eig_mat, false>(eig_mat_v.rows(), eig_mat_v.cols());
  ...
  eig_row_vec eig_row_vec_v = eig_row_vec::Random(10);
  arg_eig_row_vec arg_eig_row_vec_d
      = setup_adj_arg<eig_row_vec, false>(eig_row_vec_v.size());
