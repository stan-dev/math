...
TEST_F(AgradRev, StanMath_integrate_1d_gk_rev_test_integer_arguments) {
  ...
  EXPECT_NO_THROW(v = stan::math::integrate_1d_gauss_kronrod_tol(
...
TEST_F(AgradRev, StanMath_integrate_1d_gk_rev_TestCauchy) {
  ...
  var I = integrate_1d_gauss_kronrod_tol(pdf, a, b, 1e-8, 0.0, 15, msgs, theta,
                                         std::vector<double>{},
                                         std::vector<int>{});
...
// refinement floor has to hold for that pass too -- and in practice that is
// where it matters most, because a parameter derivative is routinely orders
// of magnitude smaller than the value it differentiates.
