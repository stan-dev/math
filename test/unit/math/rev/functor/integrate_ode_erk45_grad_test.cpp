...
class test_functor_double_var_1 {
 public:
  ...
  inline T operator()(Eigen::Matrix<T, Eigen::Dynamic, 1>& x) const {
    ...
    std::vector<std::vector<T>> ys = stan::math::integrate_ode_erk45(
...
TEST(StanMathOdeIntegrateODEGradArr, double_var) {
  double omega = 0.5;
  double chi = 1.25;
  double t = 5;
...
TEST(StanMathOdeIntegrateODEGradArr, var_double) {
  double omega = 0.5;
  double chi = 1.25;
  double t = 5;
...
TEST(StanMathOdeIntegrateODEGradArr, var_var) {
  double omega = 0.5;
  double chi = 1.25;
  double t = 5;
