...
TEST(StanAgradRevOde_integrate_ode_rk45, time_steps_as_param) {
  ...
  std::vector<std::vector<double>> res_d
      = integrate_ode_rk45(ode, y0, t0, value_of(ts), theta,
                                             x, x_int);
  ...
  test_val();
  res = integrate_ode_rk45(ode, y0v, t0, ts, thetav, x,
                                             x_int);
  test_val();
  res = integrate_ode_rk45(ode, y0, t0v, ts, theta, x, x_int);
  test_val();
  res = integrate_ode_rk45(ode, y0v, t0v, ts, theta, x,
                                             x_int);
  test_val();
  res = integrate_ode_rk45(ode, y0, t0v, ts, thetav, x,
                                             x_int);
  test_val();
  res = integrate_ode_rk45(ode, y0v, t0v, ts, thetav, x,
                                             x_int);
  test_val();
}
