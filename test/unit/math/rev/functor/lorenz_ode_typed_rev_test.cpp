...
TYPED_TEST_P(lorenz_test, param_and_data_finite_diff) {
  if (std::is_same<TypeParam, std::tuple<ode_rk45_functor, ode_rk45_functor>>::value) {
    this -> test_fd_vd(1.e-6, 3e-2);
    this -> test_fd_dv(1.e-6, 3e-2);
    this -> test_fd_vv(1.e-6, 3e-2);
  } else if (std::is_same<TypeParam, std::tuple<ode_ckrk_functor, ode_ckrk_functor>>::value) {
...
TYPED_TEST_P(lorenz_test, param_and_data_finite_diff) {
  ...
  this->test_fd_dv(1.e-6, 5e-2);
  this->test_fd_vv(1.e-6, 5e-2);
>>>>>>> 6a1dcfeb913ff29d77fd88b9021018f279242a38
}
