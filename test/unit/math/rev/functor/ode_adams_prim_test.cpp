...
TEST(ode_adams_prim, ts_errors) {
  ...
  EXPECT_NO_THROW(stan::math::ode_adams(CosArg1(), y0, t0, ts_repeat, nullptr, a));

  EXPECT_THROW(stan::math::ode_adams(CosArg1(), y0, t0, ts_empty,
				   nullptr, a),
               std::invalid_argument);

  EXPECT_THROW(stan::math::ode_adams(CosArg1(), y0, t0, ts_early,
				   nullptr, a),
               std::domain_error);

  EXPECT_THROW(stan::math::ode_adams(CosArg1(), y0, t0, ts_decreasing,
				   nullptr, a),
               std::domain_error);

  EXPECT_THROW(stan::math::ode_adams(CosArg1(), y0, t0, tsinf,
				   nullptr, a),
               std::domain_error);

  EXPECT_THROW(stan::math::ode_adams(CosArg1(), y0, t0, tsNaN,
				   nullptr, a),
               std::domain_error);
}
...
TEST(ode_adams_prim, one_arg_errors) {
  ...
  EXPECT_NO_THROW(stan::math::ode_adams(CosArg1(), y0, t0, ts,
				      nullptr, a));

  EXPECT_THROW(stan::math::ode_adams(CosArg1(), y0, t0, ts,
                                       nullptr, ainf),
               std::domain_error);
...
TEST(ode_adams_prim, two_arg_errors) {
  ...
  EXPECT_NO_THROW(stan::math::ode_adams(Cos2Arg(), y0, t0, ts,
				      nullptr, a, a));

  EXPECT_THROW(stan::math::ode_adams(Cos2Arg(), y0, t0, ts,
                                       nullptr, a, ainf),
               std::domain_error);
