...
struct pkpd_rhs {
  ...
  inline Eigen::Matrix<stan::return_type_t<T1, T2, T3, T4, T5, T6, T7, T8>, Eigen::Dynamic, 1>
  operator()(const T0& t,
             ...
             const T7& kout,
             const T8& ea50) const {
    Eigen::Matrix<stan::return_type_t<T1, T2, T3, T4, T5, T6, T7, T8>, Eigen::Dynamic, 1> dydt(4);
...
TEST(StanMathOdeBench, bdf) {
  ...
  for(std::size_t i = 0; i != 200; i++) {
    ...
    std::vector<Eigen::Matrix<var, Eigen::Dynamic, 1>> y
        = ode_bdf_tol(ode, y0, t0, ts, 1E-8, 1E-8, 10000, nullptr, ka, ke, k12, k21, kin, kout, ea50);
...
TEST(StanMathOdeBench, bdf_adjoint) {
  ...
  for(std::size_t i = 0; i != 200; i++) {
    ...
    std::vector<Eigen::Matrix<var, Eigen::Dynamic, 1>> y
        = ode_bdf_adjoint_tol(ode, y0, t0, ts, 1E-8, 1E-8, 10000, nullptr, ka, ke, k12, k21, kin, kout, ea50);
