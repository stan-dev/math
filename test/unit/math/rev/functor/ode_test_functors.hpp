...
struct solver_name##_functor {\
  ...
  operator()(const F& f, const T_y0& y0, T_t0 t0, const std::vector<T_ts>& ts, std::ostream* msgs, const Args&... args) \
  {\
...
\
  template <typename F, typename T_y0, typename T_t0, typename T_ts,    \
            typename... Args, stan::require_eigen_vector_t<T_y0>* = nullptr> \
  std::vector<Eigen::Matrix<stan::return_type_t<T_y0, T_t0, T_ts, Args...>, Eigen::Dynamic, 1>> \
  operator()(const F& f, const T_y0& y0_arg, T_t0 t0, const std::vector<T_ts>& ts, double rtol, double atol, long int max_num_steps, std::ostream* msgs, const Args&... args) \
  {\
    return solver_func##_tol(f, y0_arg, t0, ts, rtol, atol, max_num_steps, msgs, args...);\
  }\
};
...
struct solver_name##_functor {\
  ...
  {\
    return solver_func(f, y0, t0, ts, theta, x, x_int, msgs, rtol, atol, max_num_step);\
  }\
};
...
STAN_DEF_STD_ODE_SOLVER_FUNCTOR(integrate_ode_adams, stan::math::integrate_ode_adams);
STAN_DEF_STD_ODE_SOLVER_FUNCTOR(integrate_ode_bdf, stan::math::integrate_ode_bdf);
STAN_DEF_STD_ODE_SOLVER_FUNCTOR(integrate_ode_rk45, stan::math::integrate_ode_rk45);
