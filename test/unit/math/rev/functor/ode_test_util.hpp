...
finite_diff_params(solver_functor_t& sol,
    const F& f, const double& t_in, const std::vector<double>& ts,
    const Eigen::VectorXd& y_in, const std::vector<double>& theta,
...
template <typename F, typename solver_functor_t>
std::vector<Eigen::VectorXd>
finite_diff_initial_position(solver_functor_t& sol,
    const F& f, const double& t_in, const std::vector<double>& ts,
    const Eigen::VectorXd& y_in, const std::vector<double>& theta,
    const std::vector<double>& x, const std::vector<int>& x_int,
    const size_t& param_index, const double& diff) {
...
template <typename F, typename solver_functor_t>
void test_ode_finite_diff_dv(solver_functor_t& sol,
                             const F& f, const double& t_in,
  ...
  ode_res = sol(f, y_in, t_in, ts, &msgs, theta_v, x, x_int);
...
template <typename F, typename solver_functor_t>
void test_ode_finite_diff_vd(solver_functor_t& sol,
                             const F& f,
                             const double& t_in,
                             const std::vector<double>& ts,
                             const Eigen::VectorXd& y_in,
  ...
  std::vector<Eigen::Matrix<stan::math::var, -1, 1>>
    ode_res = sol(f, y_in_v, t_in, ts, &msgs, theta, x, x_int);
...
// as vars against finite differences
template <typename F, typename solver_functor_t>
void test_ode_finite_diff_vv(solver_functor_t& sol,
                             const F& f, const double& t_in,
                             const std::vector<double>& ts,
                             const Eigen::VectorXd& y_in,
                             const std::vector<double>& theta,
  ...
  std::vector<Eigen::Matrix<stan::math::var, -1, 1>>
    ode_res = sol(f, yv, t_in, ts, &msgs, theta_v, x, x_int);
...
void test_ode_error_conditions(solver_functor_t& sol,
                               F& f, const double& t0,
                               const std::vector<double>& ts,
  ...
  EXPECT_THROW_MSG(
      (sol(f, y0_bad, t0, ts, &msgs, theta, x, x_int)),
  ...
  expected_msg << "initial time is " << t0_bad << ", but must be less than "
               << ts[0];
  EXPECT_THROW_MSG(
      (sol(f, y0, t0_bad, ts, &msgs, theta, x, x_int)),
  ...
  EXPECT_THROW_MSG(
      (sol(f, y0, t0, ts_bad, &msgs, theta, x, x_int)),
...
template <typename F, typename T1, typename T2, typename solver_functor_t>
void test_ode_error_conditions_nan(solver_functor_t& sol,
                                   F& f, const double& t0,
                                   const std::vector<double>& ts,
                                   const Eigen::Matrix<T1, -1, 1>& y0,
                                   const std::vector<T2>& theta,
  ...
  theta_bad[0] = nan;
  EXPECT_THROW_MSG(
      (sol(f, y0, t0, ts, &msgs, theta_bad, x, x_int)),
  ...
  if (x.size() > 0) {
    ...
    x_bad[0] = nan;
    EXPECT_THROW_MSG(
        (sol(f, y0, t0, ts, &msgs, theta, x_bad, x_int)),
        std::domain_error, "ode parameters and data");
    EXPECT_THROW_MSG(
        (sol(f, y0, t0, ts, &msgs, theta, x_bad, x_int)),
        std::domain_error, expected_is_nan.str());
...
template <typename F, typename T1, typename T2, typename solver_functor_t>
void test_ode_error_conditions_inf(solver_functor_t& sol,
                                   F& f, const double& t0,
                                   const std::vector<double>& ts,
                                   const Eigen::Matrix<T1, -1, 1>& y0,
                                   const std::vector<T2>& theta,
...
template <typename F, typename solver_functor_t>
void test_ode(solver_functor_t& sol,
              const F& f, const double& t_in, const std::vector<double>& ts,
              const Eigen::VectorXd& y_in, const std::vector<double>& theta,
              const std::vector<double>& x, const std::vector<int>& x_int,
