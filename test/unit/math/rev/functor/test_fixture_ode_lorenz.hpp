...
struct lorenz_ode_base {
  ...
  lorenz_ode_base() : f(), y0(3), theta{10.0, 28.0, 8.0/3.0},
                      t0(0.0), ts(100), rtol(1.e-8), atol(1.e-10), max_num_step(100000)
  {
    y0 << 10.0, 1.0, 1.0;
    for (int i = 0; i < 100; i++)
      ts[i] = t0 + (0.1 * (i + 1)); 
...
struct lorenz_test : public lorenz_ode_base,
  ...
  auto apply_solver() {
    ...
    return sol(this -> f, this -> y0, this -> t0, this -> ts,
               nullptr, this -> theta);    
  }
  ...
  auto apply_solver(T1&& init, T2&& theta_in) {
    std::tuple_element_t<0, T> sol;
    return sol(this -> f, init, this -> t0, this -> ts,
               nullptr, theta_in);
  }
