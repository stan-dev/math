#include <stan/math/rev/functor/integrate_ode_erk45.hpp>
...
// calculates finite diffs for integrate_ode_erk45 with varying parameters
template <typename F>
...
// test integrate_ode_erk45 with initial positions as doubles and parameters
    ...
    for (size_t j = 0; j < y_in.size(); j++) {
      ...
      for (size_t k = 0; k < theta.size(); k++)
        EXPECT_NEAR(grads_eff[k], finite_diff_res[k][i][j], diff2)
            << "Gradient of integrate_ode_erk45 failed with initial positions"
...
// test integrate_ode_erk45 with initial positions as vars and parameters
...
void test_ode_error_conditions(F& f, const double& t0,
  ...
  ASSERT_NO_THROW((integrate_ode_erk45(f, y0, t0, ts, theta, x, x_int, 0)));
  ASSERT_EQ("", msgs.str());

  msgs.clear();
  std::vector<T1> y0_bad;
  EXPECT_THROW_MSG(
      (integrate_ode_erk45(f, y0_bad, t0, ts, theta, x, x_int, &msgs)),
  ...
  expected_msg << "initial time is " << t0_bad << ", but must be less than "
               << ts[0];
  EXPECT_THROW_MSG(
      (integrate_ode_erk45(f, y0, t0_bad, ts, theta, x, x_int, &msgs)),
  ...
  EXPECT_THROW_MSG(
      (integrate_ode_erk45(f, y0, t0, ts_bad, theta, x, x_int, &msgs)),
  ...
  EXPECT_THROW_MSG(
      (integrate_ode_erk45(f, y0, t0, ts, theta_bad, x, x_int, &msgs)),
  ...
  if (x.size() > 0) {
    ...
    EXPECT_THROW_MSG(
        (integrate_ode_erk45(f, y0, t0, ts, theta, x_bad, x_int, &msgs)),
        std::out_of_range, "vector");
    EXPECT_EQ("", msgs.str());
  }
  ...
  if (x_int.size() > 0) {
    ...
    EXPECT_THROW_MSG(
        (integrate_ode_erk45(f, y0, t0, ts, theta, x, x_int_bad, &msgs)),
        std::out_of_range, "vector");
    EXPECT_EQ("", msgs.str());
  }
}
