...
TEST(AgradRevMatrix, accumulateDouble) {
  ...
  for (int i = 2; i <= 1000; ++i) a.add(var(i));
  test_sum(a, 1000);
}
TEST(AgradRevMathMatrix, accumulateCollection) {
  ...
  vector<var> v(10);
  for (size_t i = 0; i < 10; ++i) v[i] = var(pos++);
  a.add(v);
  test_sum(a, pos - 1);

  vector<double> d(10);
  for (size_t i = 0; i < 10; ++i) d[i] = pos++;
  a.add(d);
  test_sum(a, pos - 1);
  ...
  vector<int> u(10);
  for (size_t i = 0; i < 10; ++i) a.add(pos++);
  test_sum(a, pos - 1);

  vector<vector<int> > ww(10);
  for (size_t i = 0; i < 10; ++i) {
    vector<int> w(5);
    for (size_t n = 0; n < 5; ++n) w[n] = pos++;
    ww[i] = w;
  }
  a.add(ww);
  test_sum(a, pos - 1);
  ...
  MatrixXd m(5, 6);
  for (int i = 0; i < 5; ++i)
    for (int j = 0; j < 6; ++j) m(i, j) = pos++;
  a.add(m);
  test_sum(a, pos - 1);

  VectorXd mv(7);
  for (int i = 0; i < 7; ++i) mv(i) = pos++;
  a.add(mv);
  test_sum(a, pos - 1);
  ...
  for (int i = 0; i < 5; ++i)
    for (int j = 0; j < 6; ++j) mvar(i, j) = pos++;
  a.add(mvar);
  test_sum(a, pos - 1);

  Matrix<var, 1, Dynamic> mvvar(7);
  for (int i = 0; i < 7; ++i) mvvar(i) = pos++;
  a.add(mvvar);
  test_sum(a, pos - 1);
  ...
  for (size_t i = 0; i < 8; ++i) {
    ...
    for (int j = 0; j < 3; ++j) vx_var(j) = pos++;
    vvx_var[i] = vx_var;
  }
  a.add(vvx_var);
  test_sum(a, pos - 1);
