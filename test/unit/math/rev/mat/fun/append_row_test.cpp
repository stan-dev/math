...
TEST(AgradRevMatrix, append_row_matrix) {
  ...
  size_t idx = 0;
  for (int i = 0; i < 2; i++)
    for (int j = 0; j < 2; j++) EXPECT_FLOAT_EQ(a(i, j).val() * 2.0, g[idx++]);
...
TEST(AgradRevMatrix, append_row_row_vector) {
  ...
  size_t idx = 0;
  for (int i = 0; i < 3; i++) EXPECT_FLOAT_EQ(a(i).val() * 2.0, g[idx++]);
...
TEST(MathMatrix, append_row_different_types) {
  ...
  MatrixXd vm33(3, 3);
  vm33 << 1, 2, 3, 4, 5, 6, 7, 8, 9;

  MatrixXd vm32(3, 2);
  vm32 << 11, 12, 13, 14, 15, 16;
