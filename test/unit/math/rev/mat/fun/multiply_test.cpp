...
class mult_functor_vv {
  int i, j, N, M, K;
  public:
    mult_functor_vv(int i_, int j_, int N_, int M_, int K_) : 
...
template<>
class mult_functor_vv<1, -1, 1> {
  int N, M, K;
  public:
    mult_functor_vv(int N_, int M_, int K_) : 
      N(N_), M(M_), K(K_) { }
    template <typename T>
    T operator()(Eigen::Matrix<T, -1, 1> x) const {
      using stan::math::multiply;
...
template<>
class mult_functor_dv<1, -1, 1> {
  int M, K;
  Eigen::Matrix<double, 1, -1> A_c;
  public:
    mult_functor_dv(int M_, int K_,
...
TEST(AgradRevMatrix, multiply_matrix_matrix_grad_fd) {
  ...
  for (int n = 0; n < N; ++n) {
    for (int k = 0; k < K; ++k) {
      mult_functor_vv<-1, -1, -1> func(n, k, N, M, K);
...
TEST(AgradRevMatrix, multiply_row_vector_vector_grad_fd) {
  ...
  for (int n = 0; n < N; ++n) {
    for (int k = 0; k < K; ++k) {
      mult_functor_vv<1, -1, 1> func(N, M, K);
      VectorXd grad_ad(N * M + M * K);
      VectorXd grad_fd(N * M + M * K);
      double val_ad;
      double val_fd;
...
TEST(AgradRevMatrix, multiply_matrix_matrix_grad_fd_dv) {
  ...
  for (int n = 0; n < N; ++n) {
    for (int k = 0; k < K; ++k) {
      mult_functor_dv<-1, -1, -1> func(n, k, M, K, A);
...
TEST(AgradRevMatrix, multiply_row_vector_vector_grad_fd_dv) {
  ...
  for (int n = 0; n < N; ++n) {
    for (int k = 0; k < K; ++k) {
      mult_functor_dv<1, -1, 1> func(M, K, A);
      VectorXd grad_ad(M * K);
      VectorXd grad_fd(M * K);
      double val_ad;
      double val_fd;
