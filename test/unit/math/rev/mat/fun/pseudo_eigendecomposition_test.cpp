...
struct make_I {
  ...
  Eigen::Matrix<T, Eigen::Dynamic, 1> operator()(
    ...
    matrix_t D = es.pseudoEigenvalueMatrix();
    matrix_t V = es.pseudoEigenvectors();
    matrix_t I = V.inverse() * A * V * D.inverse();
    I.resize(K * K, 1);
    return I;
  }
};

TEST(MathMatrix, pseudo_eigen_decomposition) {
