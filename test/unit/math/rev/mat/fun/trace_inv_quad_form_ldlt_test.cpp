...
TEST(AgradRevMatrix, trace_inv_quad_form_ldlt_mat) {
  ...
  ad << 9.0, 3.0, 3.0, 3.0, 3.0, 10.0, 2.0, 2.0, 3.0, 2.0, 7.0, 1.0, 3.0, 2.0,
      1.0, 112.0;
  av << 9.0, 3.0, 3.0, 3.0, 3.0, 10.0, 2.0, 2.0, 3.0, 2.0, 7.0, 1.0, 3.0, 2.0,
      1.0, 112.0;
...
TEST(AgradRevMatrix, trace_quad_form_ldlt_mat_grad_vd) {
  ...
  vars.clear();
  for (i = 0; i < 4; i++)
    for (j = 0; j < 4; j++) vars.push_back(av(i, j));
  grad = cgradvec(res, vars);
  pos = 0;
  for (i = 0; i < 4; i++)
    for (j = 0; j < 4; j++, pos++) EXPECT_FLOAT_EQ(grad[pos], dqda(i, j));
}
...
TEST(AgradRevMatrix, trace_quad_form_ldlt_mat_grad_dv) {
  ...
  vars.clear();
  for (i = 0; i < 4; i++)
    for (j = 0; j < 2; j++) vars.push_back(bv(i, j));
  grad = cgradvec(res, vars);
  pos = 0;
  for (i = 0; i < 4; i++)
    for (j = 0; j < 2; j++, pos++) EXPECT_FLOAT_EQ(grad[pos], dqdb(i, j));
}
...
TEST(AgradRevMatrix, trace_quad_form_ldlt_vv_basic) {
  ...
  vars.clear();
  for (int n = 0; n < A.size(); n++) vars.push_back(A(n));
  for (int n = 0; n < B.size(); n++) vars.push_back(B(n));
  x.grad(vars, grad);
...
TEST(AgradRevMatrix, trace_quad_form_ldlt_dv_basic) {
  ...
  matrix_v tmp = multiply(transpose(B),
                          multiply(stan::math::to_var(A.inverse().eval()), B));
  x_basic = trace(tmp);
  x_basic_val = x_basic.val();
...
TEST(AgradRevMatrix, check_varis_on_stack) {
  ...
  test::check_varis_on_stack(
      stan::math::trace_inv_quad_form_ldlt(ldlt_av, to_var(b)));
  test::check_varis_on_stack(stan::math::trace_inv_quad_form_ldlt(ldlt_av, b));
  test::check_varis_on_stack(
      stan::math::trace_inv_quad_form_ldlt(ldlt_a, to_var(b)));
}
