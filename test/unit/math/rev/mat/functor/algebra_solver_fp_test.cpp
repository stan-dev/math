...
struct FP_exp_func_test : public ::testing::Test {
  ...
  auto fd_functor() {
    auto f_fd = [this]
      ...
      return fp.solve(x, y_, env, f_tol, max_num_steps, 1)(0);
    };
    return f_fd;
  }
};
...
struct FP_2d_func_test : public ::testing::Test {
  ...
  FP_2d_func_test()
    : f(),
      ...
      y(stan::math::to_vector(std::vector<double>{1.0, 1.0})),
      x_r(),
      x_i(),
      msgs(nullptr),
      u_scale{1.0, 1.0},
...
TEST_F(FP_exp_func_test, solve) {
  ...
  int max_num_steps = 100;
  {
    Eigen::Matrix<double, -1, 1> res = fp.solve(x, y, env, f_tol, max_num_steps, 1);
...
TEST_F(FP_2d_func_test, solve) {
  ...
  double f_tol = 1.e-12;
  int max_num_steps = 100;
  Eigen::Matrix<double, -1, 1> res = fp.solve(x, y, env, f_tol, max_num_steps, 2);
...
TEST_F(FP_exp_func_test, gradient) {
  ...
  x(0) = 0.1;
  y(0) = 0.8;
  Eigen::Matrix<var, -1, 1> x_sol = fp.solve(x, yp, env, f_tol, max_num_steps, 1);
  EXPECT_FLOAT_EQ(value_of(x_sol(0)), 0.612584823501);
  stan::math::set_zero_all_adjoints();
  x_sol(0).grad();
  auto f_fd = fd_functor();
  double fx;
