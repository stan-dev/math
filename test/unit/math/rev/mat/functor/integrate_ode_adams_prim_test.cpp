...
TEST(StanMathOde_integrate_ode_adams, error_conditions) {
  ...
  EXPECT_THROW_MSG(integrate_ode_adams(harm_osc, y0_bad, t0, ts, theta, x, x_int,
                                     0, 1e-8, 1e-10, 1e6),
                   std::invalid_argument, "initial state has size 0");

  double t0_bad = 2.0;
  EXPECT_THROW_MSG(integrate_ode_adams(harm_osc, y0, t0_bad, ts, theta, x, x_int,
                                     0, 1e-8, 1e-10, 1e6),
                   std::domain_error,
                   "initial time is 2, but must be less than 0.1");

  std::vector<double> ts_bad;
  EXPECT_THROW_MSG(integrate_ode_adams(harm_osc, y0, t0, ts_bad, theta, x, x_int,
                                     0, 1e-8, 1e-10, 1e6),
  ...
  EXPECT_THROW_MSG(integrate_ode_adams(harm_osc, y0, t0, ts, theta_bad, x, x_int,
                                     0, 1e-8, 1e-10, 1e6),
                   std::out_of_range, "vector");

  // TODO(carpenter): g++6 failure
  std::vector<double> x_bad;
  EXPECT_THROW_MSG(integrate_ode_adams(harm_osc, y0, t0, ts, theta, x_bad, x_int,
                                     0, 1e-8, 1e-10, 1e6),
                   std::out_of_range, "vector");

  // TODO(carpenter): g++6 failure
  std::vector<int> x_int_bad;
  EXPECT_THROW_MSG(integrate_ode_adams(harm_osc, y0, t0, ts, theta, x, x_int_bad,
                                     0, 1e-8, 1e-10, 1e6),
                   std::out_of_range, "vector");

  EXPECT_THROW_MSG(
      integrate_ode_adams(harm_osc, y0, t0, ts, theta, x, x_int, 0, -1, 1e-6, 10),
      std::invalid_argument, "relative_tolerance");

  EXPECT_THROW_MSG(
      integrate_ode_adams(harm_osc, y0, t0, ts, theta, x, x_int, 0, 1e-6, -1, 10),
