...
TEST(StanMathOde_integrate_ode_adams, error_conditions) {
  ...
  EXPECT_THROW_MSG(integrate_ode_adams(
                                     harm_osc, y0_bad, t0, ts, theta, x, x_int,
                                     0, 1e-8, 1e-10, 1e6),
                   std::invalid_argument, "initial state has size 0");

  double t0_bad = 2.0;
  EXPECT_THROW_MSG(integrate_ode_adams(
                                     harm_osc, y0, t0_bad, ts, theta, x, x_int,
                                     0, 1e-8, 1e-10, 1e6),
                   std::domain_error,
  ...
  EXPECT_THROW_MSG(integrate_ode_adams(
                                     harm_osc, y0, t0, ts, theta_bad, x, x_int,
                                     0, 1e-8, 1e-10, 1e6),
  ...
  EXPECT_THROW_MSG(integrate_ode_adams(
                                     harm_osc, y0, t0, ts, theta, x, x_int_bad,
                                     0, 1e-8, 1e-10, 1e6),
                   std::out_of_range, "vector");
  ...
  EXPECT_THROW_MSG(integrate_ode_adams(
                                     harm_osc, y0, t0, ts, theta, x, x_int, 0,
                                     1e-6, 1e-6, -1),
