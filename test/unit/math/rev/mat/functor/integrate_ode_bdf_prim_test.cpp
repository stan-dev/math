...
TEST(StanMathOde_integrate_ode_bdf, harmonic_oscillator) {
  sho_test(0.0);
  sho_test(1.0);
  sho_test(-1.0);
...
TEST(StanMathOde_integrate_ode_bdf, error_conditions) {
  ...
  ASSERT_NO_THROW(integrate_ode_bdf(harm_osc, y0, t0, ts, theta, x, x_int, 1e-8, 1e-10, 1e6, 0));

  std::vector<double> y0_bad;
  EXPECT_THROW_MSG(integrate_ode_bdf(harm_osc, y0_bad, t0, ts, theta, x, x_int, 1e-8, 1e-10, 1e6, 0),
                   std::invalid_argument,
                   "initial state has size 0");

  double t0_bad = 2.0;
  EXPECT_THROW_MSG(integrate_ode_bdf(harm_osc, y0, t0_bad, ts, theta, x, x_int, 1e-8, 1e-10, 1e6, 0),
                   std::domain_error,
                   "initial time is 2, but must be less than 0.1");

  std::vector<double> ts_bad;
  EXPECT_THROW_MSG(integrate_ode_bdf(harm_osc, y0, t0, ts_bad, theta, x, x_int, 1e-8, 1e-10, 1e6, 0),
                   std::invalid_argument,
                   "times has size 0");
  ...
  EXPECT_THROW_MSG(integrate_ode_bdf(harm_osc, y0, t0, ts, theta_bad, x, x_int, 1e-8, 1e-10, 1e6, 0),
                   std::out_of_range,
                   "vector");

  std::vector<double> x_bad;
  EXPECT_THROW_MSG(integrate_ode_bdf(harm_osc, y0, t0, ts, theta, x_bad, x_int, 1e-8, 1e-10, 1e6, 0),
                   std::out_of_range,
                   "vector");

  std::vector<int> x_int_bad;
  EXPECT_THROW_MSG(integrate_ode_bdf(harm_osc, y0, t0, ts, theta, x, x_int_bad, 1e-8, 1e-10, 1e6, 0),
                   std::out_of_range,
                   "vector");
}
...
TEST(StanMathOde_integrate_ode_bdf, error_conditions_inf) {
  using stan::math::integrate_ode_bdf;
  harm_osc_ode_data_fun harm_osc;
