...
void sho_value_test(F harm_osc, std::vector<double>& y0, double t0,
  ...
  std::vector<std::vector<double> > ode_res_vd = stan::math::integrate_ode_bdf(
...
TEST(StanMathOde_integrate_ode_bdf, error_conditions) {
  ...
  ASSERT_NO_THROW(integrate_ode_bdf(harm_osc, y0, t0, ts, theta, x, x_int, 0,
                                    1e-8, 1e-10, 1e6));

  std::vector<double> y0_bad;
  EXPECT_THROW_MSG(integrate_ode_bdf(harm_osc, y0_bad, t0, ts, theta, x, x_int,
                                     0, 1e-8, 1e-10, 1e6),
                   std::invalid_argument, "initial state has size 0");

  double t0_bad = 2.0;
  EXPECT_THROW_MSG(integrate_ode_bdf(harm_osc, y0, t0_bad, ts, theta, x, x_int,
                                     0, 1e-8, 1e-10, 1e6),
                   std::domain_error,
  ...
  EXPECT_THROW_MSG(integrate_ode_bdf(harm_osc, y0, t0, ts, theta, x_bad, x_int,
                                     0, 1e-8, 1e-10, 1e6),
                   std::out_of_range, "vector");

  // TODO(carpenter): g++6 failure
  std::vector<int> x_int_bad;
  EXPECT_THROW_MSG(integrate_ode_bdf(harm_osc, y0, t0, ts, theta, x, x_int_bad,
                                     0, 1e-8, 1e-10, 1e6),
                   std::out_of_range, "vector");
