...
error_conditions_test(const F& f,
  ...
  EXPECT_THROW_MSG(algebra_solver(non_square_eq_functor(),
                                  x, y, dat, dat_int),
...
void inline unsolvable_test(Eigen::Matrix<T, Eigen::Dynamic, 1>& y) {
  ...
  err_msg << "algebra_solver: the norm of the algebraic function is: "
          ...
          << " max_num_steps.";
  std::string msg = err_msg.str();
  EXPECT_THROW_MSG(algebra_solver(unsolvable_eq_functor(),
                                  x, y, dat, dat_int, 0,
...
degenerate_test(const Eigen::Matrix<T, Eigen::Dynamic, 1>& y,
  ...
  return algebra_solver(degenerate_eq_functor(),
                        x, y, dat, dat_int);
}
