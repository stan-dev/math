...
void test_ode_finite_diff_dv(const F& f, const double& t_in,
  ...
  for (size_t i = 0; i < theta.size(); i++) theta_v.push_back(theta[i]);
...
void test_ode_finite_diff_vd(const F& f, const double& t_in,
  ...
  for (size_t k = 0; k < y_in.size(); k++) y_in_v.push_back(y_in[k]);
...
void test_ode_finite_diff_vv(const F& f, const double& t_in,
  ...
  std::vector<double> grads_eff;
  std::vector<stan::math::var> y_in_v;
  for (size_t i = 0; i < y_in.size(); i++) y_in_v.push_back(y_in[i]);
  ...
  for (size_t i = 0; i < theta_v.size(); i++) vars.push_back(theta_v[i]);
...
void test_ode_error_conditions(F& f, const double& t0,
  ...
  ASSERT_NO_THROW(integrate_ode_bdf(f, y0, t0, ts, theta, x, x_int, &msgs, 1e-8,
                                    1e-10, 1e6));
  ASSERT_EQ("", msgs.str());

  msgs.clear();
  std::vector<T1> y0_bad;
  EXPECT_THROW_MSG(integrate_ode_bdf(f, y0_bad, t0, ts, theta, x, x_int, &msgs,
                                     1e-8, 1e-10, 1e6),
  ...
  expected_msg << "initial time is " << t0_bad << ", but must be less than "
               << ts[0];
  EXPECT_THROW_MSG(integrate_ode_bdf(f, y0, t0_bad, ts, theta, x, x_int, &msgs,
                                     1e-8, 1e-10, 1e6),
...
void test_ode_error_conditions_vd(const F& f, const double& t_in,
  ...
  for (size_t i = 0; i < y_in.size(); i++) y_var.push_back(y_in[i]);
...
void test_ode_error_conditions_dv(const F& f, const double& t_in,
  ...
  for (size_t i = 0; i < theta.size(); i++) theta_var.push_back(theta[i]);
