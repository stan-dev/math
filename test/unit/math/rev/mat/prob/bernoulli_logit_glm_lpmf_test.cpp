...
TEST(ProbDistributionsBernoulliLogitGLM,
  ...
  for (size_t ii = 0; ii < 20000; ii++) {
    ...
    var alpha2 = alphareal[0];
    var lp2 =
      stan::math::bernoulli_logit_glm_lpmf(y, x2, alpha2, beta2);
    lp2.grad();
    EXPECT_FLOAT_EQ(lp_val, lp2.val());
...
TEST(ProbDistributionsBernoulliLogitGLM,
  ...
  value2
    += stan::math::bernoulli_logit_glm_lpmf(vi, mv, evv, evv).val();
  value2
    += stan::math::bernoulli_logit_glm_lpmf(vi, mv, rvv, rvv).val();
