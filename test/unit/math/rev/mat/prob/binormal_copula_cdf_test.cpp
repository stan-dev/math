...
struct binorm_copula_manual {
  ...
  inline T operator()(
    ...
    return 
        stan::math::std_binormal_integral(inv_Phi(inp_vec(0)), inv_Phi(inp_vec(1)), inp_vec(2));
  }
};
...
TEST(MathFunctions, binormal_integral_throw_RV_1_nan) {
  ...
  y << nan, 0.3;
  var rho = 0.3;
  EXPECT_THROW(stan::math::binormal_copula_cdf(y, rho), std::domain_error);
}
...
TEST(MathFunctions, vec_binormal_integral_grad_test_V_R) {
  V_R_binorm_copula_cdf dist_fun;
  binorm_copula_manual tru_fun;
