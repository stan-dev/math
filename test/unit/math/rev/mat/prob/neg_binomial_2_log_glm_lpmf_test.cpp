...
TEST(ProbDistributionsNegBinomial2LogGLM,
  ...
  EXPECT_FLOAT_EQ(
      (stan::math::neg_binomial_2_log_lpmf<false>(n, theta, phi)),
      (stan::math::neg_binomial_2_log_glm_lpmf<false>(n, x, beta, alpha, phi)));
  EXPECT_FLOAT_EQ(
      (stan::math::neg_binomial_2_log_lpmf<true, Matrix<int, Dynamic, 1>>(
          n, theta, phi)),
      (stan::math::neg_binomial_2_log_glm_lpmf<true, Matrix<int, Dynamic, 1>>(
          n, x, beta, alpha, phi)));
  EXPECT_FLOAT_EQ(
      (stan::math::neg_binomial_2_log_lpmf<false, Matrix<int, Dynamic, 1>>(
          n, theta, phi)),
      (stan::math::neg_binomial_2_log_glm_lpmf<false, Matrix<int, Dynamic, 1>>(
          n, x, beta, alpha, phi)));
  EXPECT_FLOAT_EQ(
      (stan::math::neg_binomial_2_log_lpmf<Matrix<int, Dynamic, 1>>(n, theta,
                                                                    phi)),
      (stan::math::neg_binomial_2_log_glm_lpmf<Matrix<int, Dynamic, 1>>(
          n, x, beta, alpha, phi)));
}
  ...
  for (size_t ii = 0; ii < 20000; ii++) {
    ...
    EXPECT_FLOAT_EQ((stan::math::neg_binomial_2_log_lpmf<false>(n, theta, phi)),
                    (stan::math::neg_binomial_2_log_glm_lpmf<false>(n, x, beta,
                                                                    ...
                                                                    phi)));
    EXPECT_FLOAT_EQ(
      (stan::math::neg_binomial_2_log_lpmf<true, Matrix<int, Dynamic, 1>>(n,
                                                                          theta,
                                                                          phi)),
...
TEST(ProbDistributionsNegBinomial2LogGLM,
  ...
  for (size_t ii = 0; ii < 200; ii++) {
    ...
    var lp2 = stan::math::neg_binomial_2_log_glm_lpmf(n, x2, beta2, alpha2,
                                                      phi2);
    lp2.grad();
...
Dynamic, Dynamic>::Random(R, 1)
      + Matrix<double, Dynamic, 1>::Ones(R, 1); // We want phireal to be
positive. Matrix<var, Dynamic, 1> beta = betareal; Matrix<var, Dynamic, 1> phi =
phireal; Matrix<var, Dynamic, 1> theta(R, 1);
