...
std::vector<stan::math::var> get_vvar(std::vector<double> vd) {
  ...
  vv.reserve(size);
  for (size_t i = 0; i < size; i++) vv.push_back(vd[i]);
  return vv;
}
...
std::vector<double> vdouble_from_vvar(std::vector<stan::math::var> vv) {
  ...
  vd.reserve(size);
  for (size_t i = 0; i < size; i++) vd.push_back(vv[i].val());
  return vd;
}
...
std::vector<double> grad_multi_normal(const F& fun,
  ...
  if (!stan::is_constant<T_y>::value) {
    for (size_t i = 0; i < vec_y.size(); i++) vec_vars.push_back(vec_y[i]);
  }
  if (!stan::is_constant<T_mu>::value) {
    for (size_t i = 0; i < vec_mu.size(); i++) vec_vars.push_back(vec_mu[i]);
  }
  if (!stan::is_constant<T_sigma>::value) {
    for (size_t i = 0; i < vec_sigma.size(); i++)
      vec_vars.push_back(vec_sigma[i]);
  }
  if (!stan::is_constant<T_nu>::value) vec_vars.push_back(nu);
