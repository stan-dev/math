...
finite_diffs_multi_normal(const F& fun,
  ...
  if (!stan::is_constant<T_nu>::value) {
      ...
      nu_minus -= epsilon;
      diffs.push_back((fun(vec_y_plus, vec_mu_plus, vec_sigma_plus, nu_plus)
                      - fun(vec_y_minus, vec_mu_minus,
                            vec_sigma_minus, nu_minus))
                      / (2 * epsilon));
  }
  return diffs;
}
...
void test_grad_multi_student_t(const F& fun,
  ...
  using std::fabs;
  std::vector<double>
            diffs_finite = finite_diffs_multi_normal(fun, vec_y, vec_mu,
                                                     vec_sigma, nu);
  std::vector<double>
            diffs_var = grad_multi_normal(fun, vec_y, vec_mu, vec_sigma, nu);
  EXPECT_EQ(diffs_finite.size(), diffs_var.size());
  for (size_t i = 0; i < diffs_finite.size(); ++i) {
    double tolerance = 1e-6 * fmax(fabs(diffs_finite[i]), fabs(diffs_var[i]))
                            + 1e-14;
