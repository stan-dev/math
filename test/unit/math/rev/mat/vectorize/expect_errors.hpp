...
void expect_std_vectors_error() {
  ...
  for (size_t i = 0; i < illegal_inputs.size(); ++i) {
    EXPECT_THROW(F::template apply<var>(illegal_inputs[i]), 
                                                   std::domain_error);
    y.push_back(illegal_inputs[i]);
  }

  EXPECT_THROW(F::template apply<vector<var> >(y), std::domain_error);

  vector<vector<var> > z;
  z.push_back(y);
  z.push_back(y);
  EXPECT_THROW(F::template apply<vector<vector<var> > >(z),
                                                     std::domain_error);
}
