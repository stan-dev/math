...
void expect_matrix_value() {
  ...
  for (int i = 0; i < template_m.size(); ++i) {
    MatrixXvar a = build_matrix<F>(template_m);
    MatrixXvar b = build_matrix<F>(template_m);
  ...
  for (size_t i = 0; i < vector_matrix_size; ++i) {
    for (int j = 0; j < template_m.size(); ++j) {
      ...
      for (size_t k = 0; k < vector_matrix_size; ++k) {
        c.push_back(build_matrix<F>(template_m));
        d.push_back(build_matrix<F>(template_m));
      }
  ...
  MatrixXvar e = build_matrix<F>(template_m).block(1, 1, 1, 1);
  MatrixXvar f = build_matrix<F>(template_m);
