...
const matrix_t2& template_m2) {
  ...
  for (int i = 0; i < template_m1.size(); ++i) {
    matrix_t1 input_ma1 = build_binary_matrix1<F>(template_m1);
    matrix_t1 input_ma2 = build_binary_matrix1<F>(template_m1);
    matrix_t2 input_mb1 = build_binary_matrix2<F>(template_m2);
    matrix_t2 input_mb2 = build_binary_matrix2<F>(template_m2);
  ...
  if (template_m1.rows() > 1 && template_m1.cols() > 1) {
    ...
    input_mb2(1, 1)), input_ma1(1, 1), input_mb2(1, 1), 
