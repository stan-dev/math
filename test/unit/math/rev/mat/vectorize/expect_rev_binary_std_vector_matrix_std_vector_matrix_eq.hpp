...
const matrix_t1& template_m1, const matrix_t2& template_m2) {
  ...
  for (int i = 0; i < 2; ++i) {
    for (int j = 0; j < template_m1.size(); ++j) {
      ...
      input_mva1.push_back(build_binary_matrix1<F>(template_m1));
      input_mva1.push_back(build_binary_matrix1<F>(template_m1));
      vector<matrix_t1> input_mva2;
      input_mva2.push_back(build_binary_matrix1<F>(template_m1));
      input_mva2.push_back(build_binary_matrix1<F>(template_m1));
      vector<matrix_t2> input_mvb1;
      input_mvb1.push_back(build_binary_matrix2<F>(template_m2));
      input_mvb1.push_back(build_binary_matrix2<F>(template_m2));
      vector<matrix_t2> input_mvb2;
      input_mvb2.push_back(build_binary_matrix2<F>(template_m2));
      input_mvb2.push_back(build_binary_matrix2<F>(template_m2));
