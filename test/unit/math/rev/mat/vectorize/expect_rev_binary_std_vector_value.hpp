...
void expect_rev_binary_scalar_std_vector_eq(
std::vector<input_t1> template_v1, std::vector<input_t2> template_v2) {
  ...
  for (size_t i = 0; i < template_v1.size(); ++i) {
    for (size_t j = 0; j < 5; ++j) {
      ...
      vector<var> fa = 
      F::template apply<vector<var> >(input_a2[i], input_v2);
      EXPECT_EQ(input_v2.size(), fa.size());
      expect_binary_val_deriv_eq(F::apply_base(input_a1[i], input_v1[j]), 
      input_a1[i], input_v1[j], fa[j], input_a2[i], input_v2[j]);
    }
  }
}

template <typename F, typename input_t1, typename input_t2>
void expect_rev_binary_std_vector_scalar_eq(
std::vector<input_t1> template_v1, std::vector<input_t2> template_v2) {
  using std::vector;
  using stan::math::var;
  for (size_t i = 0; i < template_v2.size(); ++i) {
    for (size_t j = 0; j < 5; ++j) {
      ...
      vector<var> fa = 
      F::template apply<vector<var> >(input_v2, input_b2[i]);
      EXPECT_EQ(input_v2.size(), fa.size());
      expect_binary_val_deriv_eq(F::apply_base(input_v1[j], input_b1[i]), 
      input_v1[j], input_b1[i], fa[j], input_v2[j], input_b2[i]);
    }
  }
}

template <typename F, typename input_t1, typename input_t2>
void expect_rev_binary_scalar_std_vector_std_vector_eq(
std::vector<input_t1> template_v1, std::vector<input_t2> template_v2) {
  using std::vector;
  using stan::math::var;
  ...
  for (size_t i = 0; i < template_v1.size(); ++i) {
    for (size_t j = 0; j < 2; ++j) {
      for (size_t k = 0; k < 5; ++k) {
        ...
        input_v1[j][k]), input_a1[i], input_v1[j][k], 
...
template <typename F, typename input_t1, typename input_t2>
void expect_rev_binary_std_vector_std_vector_scalar_eq(
std::vector<input_t1> template_v1, std::vector<input_t2> template_v2) {
  using std::vector;
  using stan::math::var;
  ...
  for (size_t i = 0; i < template_v2.size(); ++i) {
    for (size_t j = 0; j < 2; ++j) {
      for (size_t k = 0; k < 5; ++k) {
        ...
        input_b1[i]), input_v1[j][k], input_b1[i], 
...
std::vector<input_t1> template_v1, std::vector<input_t2> template_v2) {
  ...
  for (size_t i = 0; i < template_v1.size(); ++i) {
    ...
    input_va1[i], input_vb1[i], fa[i], input_va2[i], input_vb2[i]);
  }
}
...
std::vector<input_t1> template_v1, std::vector<input_t2> template_v2) {
  ...
  for (size_t i = 0; i < 2; ++i) {
    for (size_t j = 0; j < template_v1.size(); ++j) {
      ...
      input_vb1[i][j]), input_va1[i][j], input_vb1[i][j], 
      fa[i][j], input_va2[i][j], input_vb2[i][j]);
    }
  }
}
