...
TEST(MetaTraitsRevScal, is_arena_matrix_test) {
  ...
  EXPECT_TRUE((is_arena_matrix<arena_matrix<Eigen::Matrix<var, -1, -1>>>::value));
  EXPECT_TRUE((is_arena_matrix<arena_matrix<Eigen::Matrix<var_value<float>, -1, -1>>>::value));
  EXPECT_TRUE((is_arena_matrix<arena_matrix<var_value<Eigen::MatrixXd>>>::value));
  EXPECT_TRUE((is_arena_matrix<arena_matrix<var_value<Eigen::SparseMatrix<double>>>>::value));
