...
TEST_F(agrad_distributions_multi_normal, Propto_prec2) {
  ...
  expect_propto_multi_normal_prec_lpdf(to_var(y), to_var(mu), to_var(Sigma),
...
TEST_F(agrad_distributions_multi_normal, ProptoY_prec2) {
  using stan::math::to_var;
  expect_propto_multi_normal_prec_lpdf(to_var(y), mu, Sigma, to_var(y2), mu,
                                       Sigma, "var: y");
...
TEST_F(agrad_distributions_multi_normal, ProptoYMu_prec2) {
  using stan::math::to_var;
  expect_propto_multi_normal_prec_lpdf(to_var(y), to_var(mu), Sigma, to_var(y2),
...
TEST_F(agrad_distributions_multi_normal, ProptoYSigma_prec2) {
  using stan::math::to_var;
  expect_propto_multi_normal_prec_lpdf(to_var(y), mu, to_var(Sigma), to_var(y2),
...
TEST_F(agrad_distributions_multi_normal, ProptoMu_prec2) {
  using stan::math::to_var;
  expect_propto_multi_normal_prec_lpdf(y, to_var(mu), Sigma, y, to_var(mu2),
                                       Sigma, "var: mu");
...
TEST_F(agrad_distributions_multi_normal, ProptoMuSigma_prec2) {
  using stan::math::to_var;
  expect_propto_multi_normal_prec_lpdf(y, to_var(mu), to_var(Sigma), y,
...
TEST_F(agrad_distributions_multi_normal, ProptoSigma_prec2) {
  using stan::math::to_var;
  expect_propto_multi_normal_prec_lpdf(y, mu, to_var(Sigma), y, mu,
...
struct multi_normal_prec_fun {
  ...
  T operator()(const std::vector<T>& x) const {
    ...
    return stan::math::multi_normal_prec_lpdf<false>(y, mu, Sigma);
  }
};
  ...
  stan::return_type_t<T_y, T_mu, T_sigma> operator()(
    ...
    if (dont_vectorize_y) {
      if (dont_vectorize_mu)
        return stan::math::multi_normal_prec_lpdf<false>(y[0], mu[0], Sigma);
      else
        return stan::math::multi_normal_prec_lpdf<false>(y[0], mu, Sigma);
    } else {
      if (dont_vectorize_mu)
        return stan::math::multi_normal_prec_lpdf<false>(y, mu[0], Sigma);
      else
        return stan::math::multi_normal_prec_lpdf<false>(y, mu, Sigma);
    }
  }
