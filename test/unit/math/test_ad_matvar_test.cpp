...
auto one_arg_bad_vals(const stan::math::var_value<T>& x) {
  return stan::math::make_callback_var(x.val() * 0, [x](const auto& vi) mutable {
      x.adj() += vi.adj();
    });
}
...
TEST(test_unit_math_test_ad_matvar, one_arg_bad_vals) {
  ...
  EXPECT_NONFATAL_FAILURE(EXPECT_NONFATAL_FAILURE(stan::test::expect_ad_matvar(f, x), ""), "");
}
...
auto one_arg_bad_grads(const stan::math::var_value<T>& x) {
  return stan::math::make_callback_var(x.val(), [x](const auto& vi) mutable {
      x.adj() -= vi.adj();
    });
}
...
auto two_arg_bad_vals(const T1& x1, const stan::math::var_value<T2>& x2) {
  return stan::math::make_callback_var(x2.val() * 0, [x2](const auto& vi) mutable {
      x2.adj() += vi.adj();
    });
}
...
TEST(test_unit_math_test_ad_matvar, two_arg_bad_vals) {
  auto f = [](const auto& x1, const auto& x2) {
    return two_arg_bad_vals(x1, x2);
  };

  Eigen::VectorXd x = Eigen::VectorXd::Ones(2);
  double y = 1.0;

  EXPECT_NONFATAL_FAILURE(EXPECT_NONFATAL_FAILURE(stan::test::expect_ad_matvar(f, y, x), ""), "");
  EXPECT_NONFATAL_FAILURE(EXPECT_NONFATAL_FAILURE(stan::test::expect_ad_matvar(f, x, y), ""), "");
  EXPECT_NONFATAL_FAILURE(EXPECT_NONFATAL_FAILURE(stan::test::expect_ad_matvar(f, x, x), ""), "");
}
...
auto two_arg_bad_grads(const T1& x1, const stan::math::var_value<T2>& x2) {
  return stan::math::make_callback_var(x2.val(), [x2](const auto& vi) mutable {
      x2.adj() -= vi.adj();
    });
}
...
template <typename T1, typename T2, typename T3>
auto three_arg_bad_vals(const T1& x1, const T2& x2, const stan::math::var_value<T3>& x3) {
  return stan::math::make_callback_var(x3.val() * 0, [x3](const auto& vi) mutable {
      x3.adj() += vi.adj();
    });
}
...
TEST(test_unit_math_test_ad_matvar, three_arg_bad_vals) {
  ...
  EXPECT_NONFATAL_FAILURE(EXPECT_NONFATAL_FAILURE(stan::test::expect_ad_matvar(f, y, y, x), ""), "");
  EXPECT_NONFATAL_FAILURE(EXPECT_NONFATAL_FAILURE(stan::test::expect_ad_matvar(f, y, x, y), ""), "");
  EXPECT_NONFATAL_FAILURE(EXPECT_NONFATAL_FAILURE(stan::test::expect_ad_matvar(f, y, x, x), ""), "");
  EXPECT_NONFATAL_FAILURE(EXPECT_NONFATAL_FAILURE(stan::test::expect_ad_matvar(f, x, y, y), ""), "");
  EXPECT_NONFATAL_FAILURE(EXPECT_NONFATAL_FAILURE(stan::test::expect_ad_matvar(f, x, y, x), ""), "");
  EXPECT_NONFATAL_FAILURE(EXPECT_NONFATAL_FAILURE(stan::test::expect_ad_matvar(f, x, x, y), ""), "");
  EXPECT_NONFATAL_FAILURE(EXPECT_NONFATAL_FAILURE(stan::test::expect_ad_matvar(f, x, x, x), ""), "");
}
...
template <typename T1, typename T2, typename T3>
auto three_arg_bad_grads(const T1& x1, const T2& x2, const stan::math::var_value<T3>& x3) {
  return stan::math::make_callback_var(x3.val(), [x3](const auto& vi) mutable {
      x3.adj() -= vi.adj();
    });
}
...
template <typename T>
auto one_arg_bad_vals_std_vector(const std::vector<stan::math::var_value<T>>& x) {
...
template <typename T>
auto one_arg_bad_grads_std_vector(const std::vector<stan::math::var_value<T>>& x) {
  using ret_type = std::vector<stan::math::var_value<T>>;
  stan::arena_t<ret_type> arena_x(x.size());
...
auto bad_return_type(const stan::math::var_value<T>& x) {
  ...
  stan::math::reverse_pass_callback([x, out]() mutable {
      x.adj() += out.adj();
    });
...
template <typename T>
auto bad_return_type_std_vector(const std::vector<stan::math::var_value<T>>& x) {
